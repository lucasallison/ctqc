OPENQASM 3.0;
include "stdgates.inc";
bit[81] c;
qubit[81] q;
qubit[1] psi;
h q[0];
h q[1];
h q[2];
h q[3];
h q[4];
h q[5];
h q[6];
h q[7];
h q[8];
h q[9];
h q[10];
h q[11];
h q[12];
h q[13];
h q[14];
h q[15];
h q[16];
h q[17];
h q[18];
h q[19];
h q[20];
h q[21];
h q[22];
h q[23];
h q[24];
h q[25];
h q[26];
h q[27];
h q[28];
h q[29];
h q[30];
h q[31];
h q[32];
h q[33];
h q[34];
h q[35];
h q[36];
h q[37];
h q[38];
h q[39];
h q[40];
h q[41];
h q[42];
h q[43];
h q[44];
h q[45];
h q[46];
h q[47];
h q[48];
h q[49];
h q[50];
h q[51];
h q[52];
h q[53];
h q[54];
h q[55];
h q[56];
h q[57];
h q[58];
h q[59];
h q[60];
h q[61];
h q[62];
h q[63];
h q[64];
h q[65];
h q[66];
h q[67];
h q[68];
h q[69];
h q[70];
h q[71];
h q[72];
h q[73];
h q[74];
h q[75];
h q[76];
h q[77];
h q[78];
h q[79];
h q[80];
h psi[0];
s psi[0];
s psi[0];
h psi[0];
rz(1.3473871871227625) psi[0];
cx psi[0], q[0];
rz(-1.3473871871227625) q[0];
cx psi[0], q[0];
rz(1.3473871871227625) q[0];
rz(-0.44681827934426793) psi[0];
cx psi[0], q[1];
rz(0.44681827934426793) q[1];
cx psi[0], q[1];
rz(-0.44681827934426793) q[1];
rz(-0.8936365586885359) psi[0];
cx psi[0], q[2];
rz(0.8936365586885359) q[2];
cx psi[0], q[2];
rz(-0.8936365586885359) q[2];
rz(1.3543195362127214) psi[0];
cx psi[0], q[3];
rz(-1.3543195362127214) q[3];
cx psi[0], q[3];
rz(1.3543195362127214) q[3];
rz(-0.43295358116435007) psi[0];
cx psi[0], q[4];
rz(0.43295358116435007) q[4];
cx psi[0], q[4];
rz(-0.43295358116435007) q[4];
rz(-0.8659071623287001) psi[0];
cx psi[0], q[5];
rz(0.8659071623287001) q[5];
cx psi[0], q[5];
rz(-0.8659071623287001) q[5];
rz(1.4097783289323929) psi[0];
cx psi[0], q[6];
rz(-1.4097783289323929) q[6];
cx psi[0], q[6];
rz(1.4097783289323929) q[6];
rz(-0.32203599572500735) psi[0];
cx psi[0], q[7];
rz(0.32203599572500735) q[7];
cx psi[0], q[7];
rz(-0.32203599572500735) q[7];
rz(-0.6440719914500147) psi[0];
cx psi[0], q[8];
rz(0.6440719914500147) q[8];
cx psi[0], q[8];
rz(-0.6440719914500147) q[8];
rz(-1.2881439829000294) psi[0];
cx psi[0], q[9];
rz(1.2881439829000294) q[9];
cx psi[0], q[9];
rz(-1.2881439829000294) q[9];
rz(0.5653046877897342) psi[0];
cx psi[0], q[10];
rz(-0.5653046877897342) q[10];
cx psi[0], q[10];
rz(0.5653046877897342) q[10];
rz(1.1306093755794684) psi[0];
cx psi[0], q[11];
rz(-1.1306093755794684) q[11];
cx psi[0], q[11];
rz(1.1306093755794684) q[11];
rz(-0.8803739024308563) psi[0];
cx psi[0], q[12];
rz(0.8803739024308563) q[12];
cx psi[0], q[12];
rz(-0.8803739024308563) q[12];
rz(1.3808448487280804) psi[0];
cx psi[0], q[13];
rz(-1.3808448487280804) q[13];
cx psi[0], q[13];
rz(1.3808448487280804) q[13];
rz(-0.37990295613363223) psi[0];
cx psi[0], q[14];
rz(0.37990295613363223) q[14];
cx psi[0], q[14];
rz(-0.37990295613363223) q[14];
rz(-0.7598059122672645) psi[0];
cx psi[0], q[15];
rz(0.7598059122672645) q[15];
cx psi[0], q[15];
rz(-0.7598059122672645) q[15];
rz(-1.519611824534529) psi[0];
cx psi[0], q[16];
rz(1.519611824534529) q[16];
cx psi[0], q[16];
rz(-1.519611824534529) q[16];
rz(0.10236900452073516) psi[0];
cx psi[0], q[17];
rz(-0.10236900452073516) q[17];
cx psi[0], q[17];
rz(0.10236900452073516) q[17];
rz(0.20473800904147033) psi[0];
cx psi[0], q[18];
rz(-0.20473800904147033) q[18];
cx psi[0], q[18];
rz(0.20473800904147033) q[18];
rz(0.40947601808294065) psi[0];
cx psi[0], q[19];
rz(-0.40947601808294065) q[19];
cx psi[0], q[19];
rz(0.40947601808294065) q[19];
rz(0.8189520361658813) psi[0];
cx psi[0], q[20];
rz(-0.8189520361658813) q[20];
cx psi[0], q[20];
rz(0.8189520361658813) q[20];
rz(-1.5036885812580307) psi[0];
cx psi[0], q[21];
rz(1.5036885812580307) q[21];
cx psi[0], q[21];
rz(-1.5036885812580307) q[21];
rz(0.13421549107373182) psi[0];
cx psi[0], q[22];
rz(-0.13421549107373182) q[22];
cx psi[0], q[22];
rz(0.13421549107373182) q[22];
rz(0.26843098214746364) psi[0];
cx psi[0], q[23];
rz(-0.26843098214746364) q[23];
cx psi[0], q[23];
rz(0.26843098214746364) q[23];
rz(0.5368619642949273) psi[0];
cx psi[0], q[24];
rz(-0.5368619642949273) q[24];
cx psi[0], q[24];
rz(0.5368619642949273) q[24];
rz(1.0737239285898545) psi[0];
cx psi[0], q[25];
rz(-1.0737239285898545) q[25];
cx psi[0], q[25];
rz(1.0737239285898545) q[25];
rz(-0.9941447964100841) psi[0];
cx psi[0], q[26];
rz(0.9941447964100841) q[26];
cx psi[0], q[26];
rz(-0.9941447964100841) q[26];
rz(1.1533030607696249) psi[0];
cx psi[0], q[27];
rz(-1.1533030607696249) q[27];
cx psi[0], q[27];
rz(1.1533030607696249) q[27];
rz(-0.8349865320505435) psi[0];
cx psi[0], q[28];
rz(0.8349865320505435) q[28];
cx psi[0], q[28];
rz(-0.8349865320505435) q[28];
rz(1.471619589488706) psi[0];
cx psi[0], q[29];
rz(-1.471619589488706) q[29];
cx psi[0], q[29];
rz(1.471619589488706) q[29];
rz(-0.19835347461238112) psi[0];
cx psi[0], q[30];
rz(0.19835347461238112) q[30];
cx psi[0], q[30];
rz(-0.19835347461238112) q[30];
rz(-0.39670694922476224) psi[0];
cx psi[0], q[31];
rz(0.39670694922476224) q[31];
cx psi[0], q[31];
rz(-0.39670694922476224) q[31];
rz(-0.7934138984495245) psi[0];
cx psi[0], q[32];
rz(0.7934138984495245) q[32];
cx psi[0], q[32];
rz(-0.7934138984495245) q[32];
rz(1.5547648566907442) psi[0];
cx psi[0], q[33];
rz(-1.5547648566907442) q[33];
cx psi[0], q[33];
rz(1.5547648566907442) q[33];
rz(-0.032062940208304615) psi[0];
cx psi[0], q[34];
rz(0.032062940208304615) q[34];
cx psi[0], q[34];
rz(-0.032062940208304615) q[34];
rz(-0.06412588041660923) psi[0];
cx psi[0], q[35];
rz(0.06412588041660923) q[35];
cx psi[0], q[35];
rz(-0.06412588041660923) q[35];
rz(-0.12825176083321846) psi[0];
cx psi[0], q[36];
rz(0.12825176083321846) q[36];
cx psi[0], q[36];
rz(-0.12825176083321846) q[36];
rz(-0.2565035216664369) psi[0];
cx psi[0], q[37];
rz(0.2565035216664369) q[37];
cx psi[0], q[37];
rz(-0.2565035216664369) q[37];
rz(-0.5130070433328738) psi[0];
cx psi[0], q[38];
rz(0.5130070433328738) q[38];
cx psi[0], q[38];
rz(-0.5130070433328738) q[38];
rz(-1.0260140866657477) psi[0];
cx psi[0], q[39];
rz(1.0260140866657477) q[39];
cx psi[0], q[39];
rz(-1.0260140866657477) q[39];
rz(1.0895644802582976) psi[0];
cx psi[0], q[40];
rz(-1.0895644802582976) q[40];
cx psi[0], q[40];
rz(1.0895644802582976) q[40];
rz(0) q[40];
rz(-0.9624636930731979) psi[0];
cx psi[0], q[41];
rz(0.9624636930731979) q[41];
cx psi[0], q[41];
rz(-0.9624636930731979) q[41];
cx q[39], q[41];
cx q[41], q[39];
cx q[39], q[41];
rz(0) q[39];
rz(0) q[41];
rz(1.2166652674433973) psi[0];
cx psi[0], q[42];
rz(-1.2166652674433973) q[42];
cx psi[0], q[42];
rz(1.2166652674433973) q[42];
cx q[38], q[42];
cx q[42], q[38];
cx q[38], q[42];
rz(0) q[38];
rz(0) q[42];
rz(-0.7082621187029985) psi[0];
cx psi[0], q[43];
rz(0.7082621187029985) q[43];
cx psi[0], q[43];
rz(-0.7082621187029985) q[43];
cx q[37], q[43];
cx q[43], q[37];
cx q[37], q[43];
rz(0) q[37];
rz(0) q[43];
rz(-1.416524237405997) psi[0];
cx psi[0], q[44];
rz(1.416524237405997) q[44];
cx psi[0], q[44];
rz(-1.416524237405997) q[44];
cx q[36], q[44];
cx q[44], q[36];
cx q[36], q[44];
rz(0) q[36];
rz(0) q[44];
rz(0.30854417877779905) psi[0];
cx psi[0], q[45];
rz(-0.30854417877779905) q[45];
cx psi[0], q[45];
rz(0.30854417877779905) q[45];
cx q[35], q[45];
cx q[45], q[35];
cx q[35], q[45];
rz(0) q[35];
rz(0) q[45];
rz(0.6170883575555981) psi[0];
cx psi[0], q[46];
rz(-0.6170883575555981) q[46];
cx psi[0], q[46];
rz(0.6170883575555981) q[46];
cx q[34], q[46];
cx q[46], q[34];
cx q[34], q[46];
rz(0) q[34];
rz(0) q[46];
rz(1.2341767151111962) psi[0];
cx psi[0], q[47];
rz(-1.2341767151111962) q[47];
cx psi[0], q[47];
rz(1.2341767151111962) q[47];
cx q[33], q[47];
cx q[47], q[33];
cx q[33], q[47];
rz(0) q[33];
rz(0) q[47];
rz(-0.6732392233674006) psi[0];
cx psi[0], q[48];
rz(0.6732392233674006) q[48];
cx psi[0], q[48];
rz(-0.6732392233674006) q[48];
cx q[32], q[48];
cx q[48], q[32];
cx q[32], q[48];
rz(0) q[32];
rz(0) q[48];
rz(-1.3464784467348012) psi[0];
cx psi[0], q[49];
rz(1.3464784467348012) q[49];
cx psi[0], q[49];
rz(-1.3464784467348012) q[49];
cx q[31], q[49];
cx q[49], q[31];
cx q[31], q[49];
rz(0) q[31];
rz(0) q[49];
rz(0.4486357601201907) psi[0];
cx psi[0], q[50];
rz(-0.4486357601201907) q[50];
cx psi[0], q[50];
rz(0.4486357601201907) q[50];
cx q[30], q[50];
cx q[50], q[30];
cx q[30], q[50];
rz(-pi/2147483648) q[30];
rz(0) q[50];
rz(0.8972715202403814) psi[0];
cx psi[0], q[51];
rz(-0.8972715202403814) q[51];
cx psi[0], q[51];
rz(0.8972715202403814) q[51];
cx q[29], q[51];
cx q[51], q[29];
cx q[29], q[51];
rz(-pi/1073741824) q[29];
rz(0) q[51];
rz(-1.3470496131090304) psi[0];
cx psi[0], q[52];
rz(1.3470496131090304) q[52];
cx psi[0], q[52];
rz(-1.3470496131090304) q[52];
cx q[28], q[52];
cx q[52], q[28];
cx q[28], q[52];
rz(-pi/536870912) q[28];
rz(0) q[52];
rz(0.4474934273717322) psi[0];
cx psi[0], q[53];
rz(-0.4474934273717322) q[53];
cx psi[0], q[53];
rz(0.4474934273717322) q[53];
cx q[27], q[53];
cx q[53], q[27];
cx q[27], q[53];
rz(-pi/268435456) q[27];
rz(0) q[53];
rz(0.8949868547434644) psi[0];
cx psi[0], q[54];
rz(-0.8949868547434644) q[54];
cx psi[0], q[54];
rz(0.8949868547434644) q[54];
cx q[26], q[54];
cx q[54], q[26];
cx q[26], q[54];
rz(-pi/134217728) q[26];
rz(0) q[54];
rz(-1.3516189441028643) psi[0];
cx psi[0], q[55];
rz(1.3516189441028643) q[55];
cx psi[0], q[55];
rz(-1.3516189441028643) q[55];
cx q[25], q[55];
cx q[55], q[25];
cx q[25], q[55];
rz(-pi/67108864) q[25];
rz(0) q[55];
rz(0.43835476538406437) psi[0];
cx psi[0], q[56];
rz(-0.43835476538406437) q[56];
cx psi[0], q[56];
rz(0.43835476538406437) q[56];
cx q[24], q[56];
cx q[56], q[24];
cx q[24], q[56];
rz(-pi/33554432) q[24];
rz(0) q[56];
rz(0.8767095307681287) psi[0];
cx psi[0], q[57];
rz(-0.8767095307681287) q[57];
cx psi[0], q[57];
rz(0.8767095307681287) q[57];
cx q[23], q[57];
cx q[57], q[23];
cx q[23], q[57];
rz(-pi/16777216) q[23];
rz(0) q[57];
rz(-1.3881735920535356) psi[0];
cx psi[0], q[58];
rz(1.3881735920535356) q[58];
cx psi[0], q[58];
rz(-1.3881735920535356) q[58];
cx q[22], q[58];
cx q[58], q[22];
cx q[22], q[58];
rz(-pi/8388608) q[22];
rz(0) q[58];
rz(0.36524546948272196) psi[0];
cx psi[0], q[59];
rz(-0.36524546948272196) q[59];
cx psi[0], q[59];
rz(0.36524546948272196) q[59];
cx q[21], q[59];
cx q[59], q[21];
cx q[21], q[59];
rz(-pi/4194304) q[21];
rz(0) q[59];
rz(0.7304909389654439) psi[0];
cx psi[0], q[60];
rz(-0.7304909389654439) q[60];
cx psi[0], q[60];
rz(0.7304909389654439) q[60];
cx q[20], q[60];
cx q[60], q[20];
cx q[20], q[60];
rz(-pi/2097152) q[20];
rz(0) q[60];
rz(1.4609818779308879) psi[0];
cx psi[0], q[61];
rz(-1.4609818779308879) q[61];
cx psi[0], q[61];
rz(1.4609818779308879) q[61];
cx q[19], q[61];
cx q[61], q[19];
cx q[19], q[61];
rz(-pi/1048576) q[19];
rz(0) q[61];
rz(-0.21962889772801722) psi[0];
cx psi[0], q[62];
rz(0.21962889772801722) q[62];
cx psi[0], q[62];
rz(-0.21962889772801722) q[62];
cx q[18], q[62];
cx q[62], q[18];
cx q[18], q[62];
rz(-pi/524288) q[18];
rz(0) q[62];
rz(-0.43925779545603444) psi[0];
cx psi[0], q[63];
rz(0.43925779545603444) q[63];
cx psi[0], q[63];
rz(-0.43925779545603444) q[63];
cx q[17], q[63];
cx q[63], q[17];
cx q[17], q[63];
rz(-pi/262144) q[17];
rz(0) q[63];
rz(-0.8785155909120689) psi[0];
cx psi[0], q[64];
rz(0.8785155909120689) q[64];
cx psi[0], q[64];
rz(-0.8785155909120689) q[64];
cx q[16], q[64];
cx q[64], q[16];
cx q[16], q[64];
rz(-pi/131072) q[16];
rz(0) q[64];
rz(1.3845614717656554) psi[0];
cx psi[0], q[65];
rz(-1.3845614717656554) q[65];
cx psi[0], q[65];
rz(1.3845614717656554) q[65];
cx q[15], q[65];
cx q[65], q[15];
cx q[15], q[65];
rz(-pi/65536) q[15];
rz(0) q[65];
rz(-0.37246971005848223) psi[0];
cx psi[0], q[66];
rz(0.37246971005848223) q[66];
cx psi[0], q[66];
rz(-0.37246971005848223) q[66];
cx q[14], q[66];
cx q[66], q[14];
cx q[14], q[66];
rz(-pi/32768) q[14];
rz(0) q[66];
rz(-0.7449394201169645) psi[0];
cx psi[0], q[67];
rz(0.7449394201169645) q[67];
cx psi[0], q[67];
rz(-0.7449394201169645) q[67];
cx q[13], q[67];
cx q[67], q[13];
cx q[13], q[67];
rz(-pi/16384) q[13];
rz(0) q[67];
rz(-1.489878840233929) psi[0];
cx psi[0], q[68];
rz(1.489878840233929) q[68];
cx psi[0], q[68];
rz(-1.489878840233929) q[68];
cx q[12], q[68];
cx q[68], q[12];
cx q[12], q[68];
rz(-pi/8192) q[12];
rz(0) q[68];
rz(0.16183497312193515) psi[0];
cx psi[0], q[69];
rz(-0.16183497312193515) q[69];
cx psi[0], q[69];
rz(0.16183497312193515) q[69];
cx q[11], q[69];
cx q[69], q[11];
cx q[11], q[69];
rz(-pi/4096) q[11];
rz(0) q[69];
rz(0.3236699462438703) psi[0];
cx psi[0], q[70];
rz(-0.3236699462438703) q[70];
cx psi[0], q[70];
rz(0.3236699462438703) q[70];
cx q[10], q[70];
cx q[70], q[10];
cx q[10], q[70];
rz(-pi/2048) q[10];
rz(0) q[70];
rz(0.6473398924877406) psi[0];
cx psi[0], q[71];
rz(-0.6473398924877406) q[71];
cx psi[0], q[71];
rz(0.6473398924877406) q[71];
cx q[9], q[71];
cx q[71], q[9];
cx q[9], q[71];
rz(-pi/1024) q[9];
rz(0) q[71];
rz(1.2946797849754812) psi[0];
cx psi[0], q[72];
rz(-1.2946797849754812) q[72];
cx psi[0], q[72];
rz(1.2946797849754812) q[72];
cx q[8], q[72];
cx q[72], q[8];
cx q[8], q[72];
rz(-pi/512) q[8];
rz(0) q[72];
rz(-0.5522330836388308) psi[0];
cx psi[0], q[73];
rz(0.5522330836388308) q[73];
cx psi[0], q[73];
rz(-0.5522330836388308) q[73];
cx q[7], q[73];
cx q[73], q[7];
cx q[7], q[73];
rz(-pi/256) q[7];
rz(0) q[73];
rz(-1.1044661672776617) psi[0];
cx psi[0], q[74];
rz(1.1044661672776617) q[74];
cx psi[0], q[74];
rz(-1.1044661672776617) q[74];
cx q[6], q[74];
cx q[74], q[6];
cx q[6], q[74];
rz(-pi/128) q[6];
rz(0) q[74];
rz(0.9326603190344698) psi[0];
cx psi[0], q[75];
rz(-0.9326603190344698) q[75];
cx psi[0], q[75];
rz(0.9326603190344698) q[75];
cx q[5], q[75];
cx q[75], q[5];
cx q[5], q[75];
rz(-pi/64) q[5];
rz(0) q[75];
rz(-1.2762720155208536) psi[0];
cx psi[0], q[76];
rz(1.2762720155208536) q[76];
cx psi[0], q[76];
rz(-1.2762720155208536) q[76];
cx q[4], q[76];
cx q[76], q[4];
cx q[4], q[76];
rz(-pi/32) q[4];
rz(0) q[76];
rz(3*pi/16) psi[0];
cx psi[0], q[77];
rz(-3*pi/16) q[77];
cx psi[0], q[77];
rz(3*pi/16) q[77];
cx q[3], q[77];
cx q[77], q[3];
cx q[3], q[77];
rz(-pi/16) q[3];
rz(0) q[77];
rz(3*pi/8) psi[0];
cx psi[0], q[78];
rz(-3*pi/8) q[78];
cx psi[0], q[78];
rz(3*pi/8) q[78];
cx q[2], q[78];
cx q[78], q[2];
cx q[2], q[78];
rz(-pi/8) q[2];
rz(0) q[78];
rz(-pi/4) psi[0];
cx psi[0], q[79];
rz(pi/4) q[79];
cx psi[0], q[79];
rz(-pi/4) q[79];
cx q[1], q[79];
cx q[79], q[1];
cx q[1], q[79];
rz(-pi/4) q[1];
rz(0) q[79];
rz(pi/2) psi[0];
cx psi[0], q[80];
rz(-pi/2) q[80];
cx psi[0], q[80];
rz(pi/2) q[80];
cx q[0], q[80];
cx q[80], q[0];
cx q[0], q[80];
h q[0];
cx q[1], q[0];
rz(pi/4) q[0];
cx q[1], q[0];
rz(-pi/4) q[0];
h q[1];
cx q[2], q[0];
rz(pi/8) q[0];
cx q[2], q[0];
rz(-pi/8) q[0];
rz(-pi/4) q[2];
cx q[2], q[1];
rz(pi/4) q[1];
cx q[2], q[1];
rz(-pi/4) q[1];
h q[2];
cx q[3], q[0];
rz(pi/16) q[0];
cx q[3], q[0];
rz(-pi/16) q[0];
rz(-pi/8) q[3];
cx q[3], q[1];
rz(pi/8) q[1];
cx q[3], q[1];
rz(-pi/8) q[1];
rz(-pi/4) q[3];
cx q[3], q[2];
rz(pi/4) q[2];
cx q[3], q[2];
rz(-pi/4) q[2];
h q[3];
cx q[4], q[0];
rz(pi/32) q[0];
cx q[4], q[0];
rz(-pi/32) q[0];
rz(-pi/16) q[4];
cx q[4], q[1];
rz(pi/16) q[1];
cx q[4], q[1];
rz(-pi/16) q[1];
rz(-pi/8) q[4];
cx q[4], q[2];
rz(pi/8) q[2];
cx q[4], q[2];
rz(-pi/8) q[2];
rz(-pi/4) q[4];
cx q[4], q[3];
rz(pi/4) q[3];
cx q[4], q[3];
rz(-pi/4) q[3];
h q[4];
cx q[5], q[0];
rz(pi/64) q[0];
cx q[5], q[0];
rz(-pi/64) q[0];
rz(-pi/32) q[5];
cx q[5], q[1];
rz(pi/32) q[1];
cx q[5], q[1];
rz(-pi/32) q[1];
rz(-pi/16) q[5];
cx q[5], q[2];
rz(pi/16) q[2];
cx q[5], q[2];
rz(-pi/16) q[2];
rz(-pi/8) q[5];
cx q[5], q[3];
rz(pi/8) q[3];
cx q[5], q[3];
rz(-pi/8) q[3];
rz(-pi/4) q[5];
cx q[5], q[4];
rz(pi/4) q[4];
cx q[5], q[4];
rz(-pi/4) q[4];
h q[5];
cx q[6], q[0];
rz(pi/128) q[0];
cx q[6], q[0];
rz(-pi/128) q[0];
rz(-pi/64) q[6];
cx q[6], q[1];
rz(pi/64) q[1];
cx q[6], q[1];
rz(-pi/64) q[1];
rz(-pi/32) q[6];
cx q[6], q[2];
rz(pi/32) q[2];
cx q[6], q[2];
rz(-pi/32) q[2];
rz(-pi/16) q[6];
cx q[6], q[3];
rz(pi/16) q[3];
cx q[6], q[3];
rz(-pi/16) q[3];
rz(-pi/8) q[6];
cx q[6], q[4];
rz(pi/8) q[4];
cx q[6], q[4];
rz(-pi/8) q[4];
rz(-pi/4) q[6];
cx q[6], q[5];
rz(pi/4) q[5];
cx q[6], q[5];
rz(-pi/4) q[5];
h q[6];
cx q[7], q[0];
rz(pi/256) q[0];
cx q[7], q[0];
rz(-pi/256) q[0];
rz(-pi/128) q[7];
cx q[7], q[1];
rz(pi/128) q[1];
cx q[7], q[1];
rz(-pi/128) q[1];
rz(-pi/64) q[7];
cx q[7], q[2];
rz(pi/64) q[2];
cx q[7], q[2];
rz(-pi/64) q[2];
rz(-pi/32) q[7];
cx q[7], q[3];
rz(pi/32) q[3];
cx q[7], q[3];
rz(-pi/32) q[3];
rz(-pi/16) q[7];
cx q[7], q[4];
rz(pi/16) q[4];
cx q[7], q[4];
rz(-pi/16) q[4];
rz(-pi/8) q[7];
cx q[7], q[5];
rz(pi/8) q[5];
cx q[7], q[5];
rz(-pi/8) q[5];
rz(-pi/4) q[7];
cx q[7], q[6];
rz(pi/4) q[6];
cx q[7], q[6];
rz(-pi/4) q[6];
h q[7];
cx q[8], q[0];
rz(pi/512) q[0];
cx q[8], q[0];
rz(-pi/512) q[0];
rz(-pi/256) q[8];
cx q[8], q[1];
rz(pi/256) q[1];
cx q[8], q[1];
rz(-pi/256) q[1];
rz(-pi/128) q[8];
cx q[8], q[2];
rz(pi/128) q[2];
cx q[8], q[2];
rz(-pi/128) q[2];
rz(-pi/64) q[8];
cx q[8], q[3];
rz(pi/64) q[3];
cx q[8], q[3];
rz(-pi/64) q[3];
rz(-pi/32) q[8];
cx q[8], q[4];
rz(pi/32) q[4];
cx q[8], q[4];
rz(-pi/32) q[4];
rz(-pi/16) q[8];
cx q[8], q[5];
rz(pi/16) q[5];
cx q[8], q[5];
rz(-pi/16) q[5];
rz(-pi/8) q[8];
cx q[8], q[6];
rz(pi/8) q[6];
cx q[8], q[6];
rz(-pi/8) q[6];
rz(-pi/4) q[8];
cx q[8], q[7];
rz(pi/4) q[7];
cx q[8], q[7];
rz(-pi/4) q[7];
h q[8];
cx q[9], q[0];
rz(pi/1024) q[0];
cx q[9], q[0];
rz(-pi/1024) q[0];
rz(-pi/512) q[9];
cx q[9], q[1];
rz(pi/512) q[1];
cx q[9], q[1];
rz(-pi/512) q[1];
rz(-pi/256) q[9];
cx q[9], q[2];
rz(pi/256) q[2];
cx q[9], q[2];
rz(-pi/256) q[2];
rz(-pi/128) q[9];
cx q[9], q[3];
rz(pi/128) q[3];
cx q[9], q[3];
rz(-pi/128) q[3];
rz(-pi/64) q[9];
cx q[9], q[4];
rz(pi/64) q[4];
cx q[9], q[4];
rz(-pi/64) q[4];
rz(-pi/32) q[9];
cx q[9], q[5];
rz(pi/32) q[5];
cx q[9], q[5];
rz(-pi/32) q[5];
rz(-pi/16) q[9];
cx q[9], q[6];
rz(pi/16) q[6];
cx q[9], q[6];
rz(-pi/16) q[6];
rz(-pi/8) q[9];
cx q[9], q[7];
rz(pi/8) q[7];
cx q[9], q[7];
rz(-pi/8) q[7];
rz(-pi/4) q[9];
cx q[9], q[8];
rz(pi/4) q[8];
cx q[9], q[8];
rz(-pi/4) q[8];
h q[9];
cx q[10], q[0];
rz(pi/2048) q[0];
cx q[10], q[0];
rz(-pi/2048) q[0];
rz(-pi/1024) q[10];
cx q[10], q[1];
rz(pi/1024) q[1];
cx q[10], q[1];
rz(-pi/1024) q[1];
rz(-pi/512) q[10];
cx q[10], q[2];
rz(pi/512) q[2];
cx q[10], q[2];
rz(-pi/512) q[2];
rz(-pi/256) q[10];
cx q[10], q[3];
rz(pi/256) q[3];
cx q[10], q[3];
rz(-pi/256) q[3];
rz(-pi/128) q[10];
cx q[10], q[4];
rz(pi/128) q[4];
cx q[10], q[4];
rz(-pi/128) q[4];
rz(-pi/64) q[10];
cx q[10], q[5];
rz(pi/64) q[5];
cx q[10], q[5];
rz(-pi/64) q[5];
rz(-pi/32) q[10];
cx q[10], q[6];
rz(pi/32) q[6];
cx q[10], q[6];
rz(-pi/32) q[6];
rz(-pi/16) q[10];
cx q[10], q[7];
rz(pi/16) q[7];
cx q[10], q[7];
rz(-pi/16) q[7];
rz(-pi/8) q[10];
cx q[10], q[8];
rz(pi/8) q[8];
cx q[10], q[8];
rz(-pi/8) q[8];
rz(-pi/4) q[10];
cx q[10], q[9];
rz(pi/4) q[9];
cx q[10], q[9];
rz(-pi/4) q[9];
h q[10];
cx q[11], q[0];
rz(pi/4096) q[0];
cx q[11], q[0];
rz(-pi/4096) q[0];
rz(-pi/2048) q[11];
cx q[11], q[1];
rz(pi/2048) q[1];
cx q[11], q[1];
rz(-pi/2048) q[1];
rz(-pi/1024) q[11];
cx q[11], q[2];
rz(pi/1024) q[2];
cx q[11], q[2];
rz(-pi/1024) q[2];
rz(-pi/512) q[11];
cx q[11], q[3];
rz(pi/512) q[3];
cx q[11], q[3];
rz(-pi/512) q[3];
rz(-pi/256) q[11];
cx q[11], q[4];
rz(pi/256) q[4];
cx q[11], q[4];
rz(-pi/256) q[4];
rz(-pi/128) q[11];
cx q[11], q[5];
rz(pi/128) q[5];
cx q[11], q[5];
rz(-pi/128) q[5];
rz(-pi/64) q[11];
cx q[11], q[6];
rz(pi/64) q[6];
cx q[11], q[6];
rz(-pi/64) q[6];
rz(-pi/32) q[11];
cx q[11], q[7];
rz(pi/32) q[7];
cx q[11], q[7];
rz(-pi/32) q[7];
rz(-pi/16) q[11];
cx q[11], q[8];
rz(pi/16) q[8];
cx q[11], q[8];
rz(-pi/16) q[8];
rz(-pi/8) q[11];
cx q[11], q[9];
rz(pi/8) q[9];
cx q[11], q[9];
rz(-pi/8) q[9];
rz(-pi/4) q[11];
cx q[11], q[10];
rz(pi/4) q[10];
cx q[11], q[10];
rz(-pi/4) q[10];
h q[11];
cx q[12], q[0];
rz(pi/8192) q[0];
cx q[12], q[0];
rz(-pi/8192) q[0];
rz(-pi/4096) q[12];
cx q[12], q[1];
rz(pi/4096) q[1];
cx q[12], q[1];
rz(-pi/4096) q[1];
rz(-pi/2048) q[12];
cx q[12], q[2];
rz(pi/2048) q[2];
cx q[12], q[2];
rz(-pi/2048) q[2];
rz(-pi/1024) q[12];
cx q[12], q[3];
rz(pi/1024) q[3];
cx q[12], q[3];
rz(-pi/1024) q[3];
rz(-pi/512) q[12];
cx q[12], q[4];
rz(pi/512) q[4];
cx q[12], q[4];
rz(-pi/512) q[4];
rz(-pi/256) q[12];
cx q[12], q[5];
rz(pi/256) q[5];
cx q[12], q[5];
rz(-pi/256) q[5];
rz(-pi/128) q[12];
cx q[12], q[6];
rz(pi/128) q[6];
cx q[12], q[6];
rz(-pi/128) q[6];
rz(-pi/64) q[12];
cx q[12], q[7];
rz(pi/64) q[7];
cx q[12], q[7];
rz(-pi/64) q[7];
rz(-pi/32) q[12];
cx q[12], q[8];
rz(pi/32) q[8];
cx q[12], q[8];
rz(-pi/32) q[8];
rz(-pi/16) q[12];
cx q[12], q[9];
rz(pi/16) q[9];
cx q[12], q[9];
rz(-pi/16) q[9];
rz(-pi/8) q[12];
cx q[12], q[10];
rz(pi/8) q[10];
cx q[12], q[10];
rz(-pi/8) q[10];
rz(-pi/4) q[12];
cx q[12], q[11];
rz(pi/4) q[11];
cx q[12], q[11];
rz(-pi/4) q[11];
h q[12];
cx q[13], q[0];
rz(pi/16384) q[0];
cx q[13], q[0];
rz(-pi/16384) q[0];
rz(-pi/8192) q[13];
cx q[13], q[1];
rz(pi/8192) q[1];
cx q[13], q[1];
rz(-pi/8192) q[1];
rz(-pi/4096) q[13];
cx q[13], q[2];
rz(pi/4096) q[2];
cx q[13], q[2];
rz(-pi/4096) q[2];
rz(-pi/2048) q[13];
cx q[13], q[3];
rz(pi/2048) q[3];
cx q[13], q[3];
rz(-pi/2048) q[3];
rz(-pi/1024) q[13];
cx q[13], q[4];
rz(pi/1024) q[4];
cx q[13], q[4];
rz(-pi/1024) q[4];
rz(-pi/512) q[13];
cx q[13], q[5];
rz(pi/512) q[5];
cx q[13], q[5];
rz(-pi/512) q[5];
rz(-pi/256) q[13];
cx q[13], q[6];
rz(pi/256) q[6];
cx q[13], q[6];
rz(-pi/256) q[6];
rz(-pi/128) q[13];
cx q[13], q[7];
rz(pi/128) q[7];
cx q[13], q[7];
rz(-pi/128) q[7];
rz(-pi/64) q[13];
cx q[13], q[8];
rz(pi/64) q[8];
cx q[13], q[8];
rz(-pi/64) q[8];
rz(-pi/32) q[13];
cx q[13], q[9];
rz(pi/32) q[9];
cx q[13], q[9];
rz(-pi/32) q[9];
rz(-pi/16) q[13];
cx q[13], q[10];
rz(pi/16) q[10];
cx q[13], q[10];
rz(-pi/16) q[10];
rz(-pi/8) q[13];
cx q[13], q[11];
rz(pi/8) q[11];
cx q[13], q[11];
rz(-pi/8) q[11];
rz(-pi/4) q[13];
cx q[13], q[12];
rz(pi/4) q[12];
cx q[13], q[12];
rz(-pi/4) q[12];
h q[13];
cx q[14], q[0];
rz(pi/32768) q[0];
cx q[14], q[0];
rz(-pi/32768) q[0];
rz(-pi/16384) q[14];
cx q[14], q[1];
rz(pi/16384) q[1];
cx q[14], q[1];
rz(-pi/16384) q[1];
rz(-pi/8192) q[14];
cx q[14], q[2];
rz(pi/8192) q[2];
cx q[14], q[2];
rz(-pi/8192) q[2];
rz(-pi/4096) q[14];
cx q[14], q[3];
rz(pi/4096) q[3];
cx q[14], q[3];
rz(-pi/4096) q[3];
rz(-pi/2048) q[14];
cx q[14], q[4];
rz(pi/2048) q[4];
cx q[14], q[4];
rz(-pi/2048) q[4];
rz(-pi/1024) q[14];
cx q[14], q[5];
rz(pi/1024) q[5];
cx q[14], q[5];
rz(-pi/1024) q[5];
rz(-pi/512) q[14];
cx q[14], q[6];
rz(pi/512) q[6];
cx q[14], q[6];
rz(-pi/512) q[6];
rz(-pi/256) q[14];
cx q[14], q[7];
rz(pi/256) q[7];
cx q[14], q[7];
rz(-pi/256) q[7];
rz(-pi/128) q[14];
cx q[14], q[8];
rz(pi/128) q[8];
cx q[14], q[8];
rz(-pi/128) q[8];
rz(-pi/64) q[14];
cx q[14], q[9];
rz(pi/64) q[9];
cx q[14], q[9];
rz(-pi/64) q[9];
rz(-pi/32) q[14];
cx q[14], q[10];
rz(pi/32) q[10];
cx q[14], q[10];
rz(-pi/32) q[10];
rz(-pi/16) q[14];
cx q[14], q[11];
rz(pi/16) q[11];
cx q[14], q[11];
rz(-pi/16) q[11];
rz(-pi/8) q[14];
cx q[14], q[12];
rz(pi/8) q[12];
cx q[14], q[12];
rz(-pi/8) q[12];
rz(-pi/4) q[14];
cx q[14], q[13];
rz(pi/4) q[13];
cx q[14], q[13];
rz(-pi/4) q[13];
h q[14];
cx q[15], q[0];
rz(pi/65536) q[0];
cx q[15], q[0];
rz(-pi/65536) q[0];
rz(-pi/32768) q[15];
cx q[15], q[1];
rz(pi/32768) q[1];
cx q[15], q[1];
rz(-pi/32768) q[1];
rz(-pi/16384) q[15];
cx q[15], q[2];
rz(pi/16384) q[2];
cx q[15], q[2];
rz(-pi/16384) q[2];
rz(-pi/8192) q[15];
cx q[15], q[3];
rz(pi/8192) q[3];
cx q[15], q[3];
rz(-pi/8192) q[3];
rz(-pi/4096) q[15];
cx q[15], q[4];
rz(pi/4096) q[4];
cx q[15], q[4];
rz(-pi/4096) q[4];
rz(-pi/2048) q[15];
cx q[15], q[5];
rz(pi/2048) q[5];
cx q[15], q[5];
rz(-pi/2048) q[5];
rz(-pi/1024) q[15];
cx q[15], q[6];
rz(pi/1024) q[6];
cx q[15], q[6];
rz(-pi/1024) q[6];
rz(-pi/512) q[15];
cx q[15], q[7];
rz(pi/512) q[7];
cx q[15], q[7];
rz(-pi/512) q[7];
rz(-pi/256) q[15];
cx q[15], q[8];
rz(pi/256) q[8];
cx q[15], q[8];
rz(-pi/256) q[8];
rz(-pi/128) q[15];
cx q[15], q[9];
rz(pi/128) q[9];
cx q[15], q[9];
rz(-pi/128) q[9];
rz(-pi/64) q[15];
cx q[15], q[10];
rz(pi/64) q[10];
cx q[15], q[10];
rz(-pi/64) q[10];
rz(-pi/32) q[15];
cx q[15], q[11];
rz(pi/32) q[11];
cx q[15], q[11];
rz(-pi/32) q[11];
rz(-pi/16) q[15];
cx q[15], q[12];
rz(pi/16) q[12];
cx q[15], q[12];
rz(-pi/16) q[12];
rz(-pi/8) q[15];
cx q[15], q[13];
rz(pi/8) q[13];
cx q[15], q[13];
rz(-pi/8) q[13];
rz(-pi/4) q[15];
cx q[15], q[14];
rz(pi/4) q[14];
cx q[15], q[14];
rz(-pi/4) q[14];
h q[15];
cx q[16], q[0];
rz(pi/131072) q[0];
cx q[16], q[0];
rz(-pi/131072) q[0];
rz(-pi/65536) q[16];
cx q[16], q[1];
rz(pi/65536) q[1];
cx q[16], q[1];
rz(-pi/65536) q[1];
rz(-pi/32768) q[16];
cx q[16], q[2];
rz(pi/32768) q[2];
cx q[16], q[2];
rz(-pi/32768) q[2];
rz(-pi/16384) q[16];
cx q[16], q[3];
rz(pi/16384) q[3];
cx q[16], q[3];
rz(-pi/16384) q[3];
rz(-pi/8192) q[16];
cx q[16], q[4];
rz(pi/8192) q[4];
cx q[16], q[4];
rz(-pi/8192) q[4];
rz(-pi/4096) q[16];
cx q[16], q[5];
rz(pi/4096) q[5];
cx q[16], q[5];
rz(-pi/4096) q[5];
rz(-pi/2048) q[16];
cx q[16], q[6];
rz(pi/2048) q[6];
cx q[16], q[6];
rz(-pi/2048) q[6];
rz(-pi/1024) q[16];
cx q[16], q[7];
rz(pi/1024) q[7];
cx q[16], q[7];
rz(-pi/1024) q[7];
rz(-pi/512) q[16];
cx q[16], q[8];
rz(pi/512) q[8];
cx q[16], q[8];
rz(-pi/512) q[8];
rz(-pi/256) q[16];
cx q[16], q[9];
rz(pi/256) q[9];
cx q[16], q[9];
rz(-pi/256) q[9];
rz(-pi/128) q[16];
cx q[16], q[10];
rz(pi/128) q[10];
cx q[16], q[10];
rz(-pi/128) q[10];
rz(-pi/64) q[16];
cx q[16], q[11];
rz(pi/64) q[11];
cx q[16], q[11];
rz(-pi/64) q[11];
rz(-pi/32) q[16];
cx q[16], q[12];
rz(pi/32) q[12];
cx q[16], q[12];
rz(-pi/32) q[12];
rz(-pi/16) q[16];
cx q[16], q[13];
rz(pi/16) q[13];
cx q[16], q[13];
rz(-pi/16) q[13];
rz(-pi/8) q[16];
cx q[16], q[14];
rz(pi/8) q[14];
cx q[16], q[14];
rz(-pi/8) q[14];
rz(-pi/4) q[16];
cx q[16], q[15];
rz(pi/4) q[15];
cx q[16], q[15];
rz(-pi/4) q[15];
h q[16];
cx q[17], q[0];
rz(pi/262144) q[0];
cx q[17], q[0];
rz(-pi/262144) q[0];
rz(-pi/131072) q[17];
cx q[17], q[1];
rz(pi/131072) q[1];
cx q[17], q[1];
rz(-pi/131072) q[1];
rz(-pi/65536) q[17];
cx q[17], q[2];
rz(pi/65536) q[2];
cx q[17], q[2];
rz(-pi/65536) q[2];
rz(-pi/32768) q[17];
cx q[17], q[3];
rz(pi/32768) q[3];
cx q[17], q[3];
rz(-pi/32768) q[3];
rz(-pi/16384) q[17];
cx q[17], q[4];
rz(pi/16384) q[4];
cx q[17], q[4];
rz(-pi/16384) q[4];
rz(-pi/8192) q[17];
cx q[17], q[5];
rz(pi/8192) q[5];
cx q[17], q[5];
rz(-pi/8192) q[5];
rz(-pi/4096) q[17];
cx q[17], q[6];
rz(pi/4096) q[6];
cx q[17], q[6];
rz(-pi/4096) q[6];
rz(-pi/2048) q[17];
cx q[17], q[7];
rz(pi/2048) q[7];
cx q[17], q[7];
rz(-pi/2048) q[7];
rz(-pi/1024) q[17];
cx q[17], q[8];
rz(pi/1024) q[8];
cx q[17], q[8];
rz(-pi/1024) q[8];
rz(-pi/512) q[17];
cx q[17], q[9];
rz(pi/512) q[9];
cx q[17], q[9];
rz(-pi/512) q[9];
rz(-pi/256) q[17];
cx q[17], q[10];
rz(pi/256) q[10];
cx q[17], q[10];
rz(-pi/256) q[10];
rz(-pi/128) q[17];
cx q[17], q[11];
rz(pi/128) q[11];
cx q[17], q[11];
rz(-pi/128) q[11];
rz(-pi/64) q[17];
cx q[17], q[12];
rz(pi/64) q[12];
cx q[17], q[12];
rz(-pi/64) q[12];
rz(-pi/32) q[17];
cx q[17], q[13];
rz(pi/32) q[13];
cx q[17], q[13];
rz(-pi/32) q[13];
rz(-pi/16) q[17];
cx q[17], q[14];
rz(pi/16) q[14];
cx q[17], q[14];
rz(-pi/16) q[14];
rz(-pi/8) q[17];
cx q[17], q[15];
rz(pi/8) q[15];
cx q[17], q[15];
rz(-pi/8) q[15];
rz(-pi/4) q[17];
cx q[17], q[16];
rz(pi/4) q[16];
cx q[17], q[16];
rz(-pi/4) q[16];
h q[17];
cx q[18], q[0];
rz(pi/524288) q[0];
cx q[18], q[0];
rz(-pi/524288) q[0];
rz(-pi/262144) q[18];
cx q[18], q[1];
rz(pi/262144) q[1];
cx q[18], q[1];
rz(-pi/262144) q[1];
rz(-pi/131072) q[18];
cx q[18], q[2];
rz(pi/131072) q[2];
cx q[18], q[2];
rz(-pi/131072) q[2];
rz(-pi/65536) q[18];
cx q[18], q[3];
rz(pi/65536) q[3];
cx q[18], q[3];
rz(-pi/65536) q[3];
rz(-pi/32768) q[18];
cx q[18], q[4];
rz(pi/32768) q[4];
cx q[18], q[4];
rz(-pi/32768) q[4];
rz(-pi/16384) q[18];
cx q[18], q[5];
rz(pi/16384) q[5];
cx q[18], q[5];
rz(-pi/16384) q[5];
rz(-pi/8192) q[18];
cx q[18], q[6];
rz(pi/8192) q[6];
cx q[18], q[6];
rz(-pi/8192) q[6];
rz(-pi/4096) q[18];
cx q[18], q[7];
rz(pi/4096) q[7];
cx q[18], q[7];
rz(-pi/4096) q[7];
rz(-pi/2048) q[18];
cx q[18], q[8];
rz(pi/2048) q[8];
cx q[18], q[8];
rz(-pi/2048) q[8];
rz(-pi/1024) q[18];
cx q[18], q[9];
rz(pi/1024) q[9];
cx q[18], q[9];
rz(-pi/1024) q[9];
rz(-pi/512) q[18];
cx q[18], q[10];
rz(pi/512) q[10];
cx q[18], q[10];
rz(-pi/512) q[10];
rz(-pi/256) q[18];
cx q[18], q[11];
rz(pi/256) q[11];
cx q[18], q[11];
rz(-pi/256) q[11];
rz(-pi/128) q[18];
cx q[18], q[12];
rz(pi/128) q[12];
cx q[18], q[12];
rz(-pi/128) q[12];
rz(-pi/64) q[18];
cx q[18], q[13];
rz(pi/64) q[13];
cx q[18], q[13];
rz(-pi/64) q[13];
rz(-pi/32) q[18];
cx q[18], q[14];
rz(pi/32) q[14];
cx q[18], q[14];
rz(-pi/32) q[14];
rz(-pi/16) q[18];
cx q[18], q[15];
rz(pi/16) q[15];
cx q[18], q[15];
rz(-pi/16) q[15];
rz(-pi/8) q[18];
cx q[18], q[16];
rz(pi/8) q[16];
cx q[18], q[16];
rz(-pi/8) q[16];
rz(-pi/4) q[18];
cx q[18], q[17];
rz(pi/4) q[17];
cx q[18], q[17];
rz(-pi/4) q[17];
h q[18];
cx q[19], q[0];
rz(pi/1048576) q[0];
cx q[19], q[0];
rz(-pi/1048576) q[0];
rz(-pi/524288) q[19];
cx q[19], q[1];
rz(pi/524288) q[1];
cx q[19], q[1];
rz(-pi/524288) q[1];
rz(-pi/262144) q[19];
cx q[19], q[2];
rz(pi/262144) q[2];
cx q[19], q[2];
rz(-pi/262144) q[2];
rz(-pi/131072) q[19];
cx q[19], q[3];
rz(pi/131072) q[3];
cx q[19], q[3];
rz(-pi/131072) q[3];
rz(-pi/65536) q[19];
cx q[19], q[4];
rz(pi/65536) q[4];
cx q[19], q[4];
rz(-pi/65536) q[4];
rz(-pi/32768) q[19];
cx q[19], q[5];
rz(pi/32768) q[5];
cx q[19], q[5];
rz(-pi/32768) q[5];
rz(-pi/16384) q[19];
cx q[19], q[6];
rz(pi/16384) q[6];
cx q[19], q[6];
rz(-pi/16384) q[6];
rz(-pi/8192) q[19];
cx q[19], q[7];
rz(pi/8192) q[7];
cx q[19], q[7];
rz(-pi/8192) q[7];
rz(-pi/4096) q[19];
cx q[19], q[8];
rz(pi/4096) q[8];
cx q[19], q[8];
rz(-pi/4096) q[8];
rz(-pi/2048) q[19];
cx q[19], q[9];
rz(pi/2048) q[9];
cx q[19], q[9];
rz(-pi/2048) q[9];
rz(-pi/1024) q[19];
cx q[19], q[10];
rz(pi/1024) q[10];
cx q[19], q[10];
rz(-pi/1024) q[10];
rz(-pi/512) q[19];
cx q[19], q[11];
rz(pi/512) q[11];
cx q[19], q[11];
rz(-pi/512) q[11];
rz(-pi/256) q[19];
cx q[19], q[12];
rz(pi/256) q[12];
cx q[19], q[12];
rz(-pi/256) q[12];
rz(-pi/128) q[19];
cx q[19], q[13];
rz(pi/128) q[13];
cx q[19], q[13];
rz(-pi/128) q[13];
rz(-pi/64) q[19];
cx q[19], q[14];
rz(pi/64) q[14];
cx q[19], q[14];
rz(-pi/64) q[14];
rz(-pi/32) q[19];
cx q[19], q[15];
rz(pi/32) q[15];
cx q[19], q[15];
rz(-pi/32) q[15];
rz(-pi/16) q[19];
cx q[19], q[16];
rz(pi/16) q[16];
cx q[19], q[16];
rz(-pi/16) q[16];
rz(-pi/8) q[19];
cx q[19], q[17];
rz(pi/8) q[17];
cx q[19], q[17];
rz(-pi/8) q[17];
rz(-pi/4) q[19];
cx q[19], q[18];
rz(pi/4) q[18];
cx q[19], q[18];
rz(-pi/4) q[18];
h q[19];
cx q[20], q[0];
rz(pi/2097152) q[0];
cx q[20], q[0];
rz(-pi/2097152) q[0];
rz(-pi/1048576) q[20];
cx q[20], q[1];
rz(pi/1048576) q[1];
cx q[20], q[1];
rz(-pi/1048576) q[1];
rz(-pi/524288) q[20];
cx q[20], q[2];
rz(pi/524288) q[2];
cx q[20], q[2];
rz(-pi/524288) q[2];
rz(-pi/262144) q[20];
cx q[20], q[3];
rz(pi/262144) q[3];
cx q[20], q[3];
rz(-pi/262144) q[3];
rz(-pi/131072) q[20];
cx q[20], q[4];
rz(pi/131072) q[4];
cx q[20], q[4];
rz(-pi/131072) q[4];
rz(-pi/65536) q[20];
cx q[20], q[5];
rz(pi/65536) q[5];
cx q[20], q[5];
rz(-pi/65536) q[5];
rz(-pi/32768) q[20];
cx q[20], q[6];
rz(pi/32768) q[6];
cx q[20], q[6];
rz(-pi/32768) q[6];
rz(-pi/16384) q[20];
cx q[20], q[7];
rz(pi/16384) q[7];
cx q[20], q[7];
rz(-pi/16384) q[7];
rz(-pi/8192) q[20];
cx q[20], q[8];
rz(pi/8192) q[8];
cx q[20], q[8];
rz(-pi/8192) q[8];
rz(-pi/4096) q[20];
cx q[20], q[9];
rz(pi/4096) q[9];
cx q[20], q[9];
rz(-pi/4096) q[9];
rz(-pi/2048) q[20];
cx q[20], q[10];
rz(pi/2048) q[10];
cx q[20], q[10];
rz(-pi/2048) q[10];
rz(-pi/1024) q[20];
cx q[20], q[11];
rz(pi/1024) q[11];
cx q[20], q[11];
rz(-pi/1024) q[11];
rz(-pi/512) q[20];
cx q[20], q[12];
rz(pi/512) q[12];
cx q[20], q[12];
rz(-pi/512) q[12];
rz(-pi/256) q[20];
cx q[20], q[13];
rz(pi/256) q[13];
cx q[20], q[13];
rz(-pi/256) q[13];
rz(-pi/128) q[20];
cx q[20], q[14];
rz(pi/128) q[14];
cx q[20], q[14];
rz(-pi/128) q[14];
rz(-pi/64) q[20];
cx q[20], q[15];
rz(pi/64) q[15];
cx q[20], q[15];
rz(-pi/64) q[15];
rz(-pi/32) q[20];
cx q[20], q[16];
rz(pi/32) q[16];
cx q[20], q[16];
rz(-pi/32) q[16];
rz(-pi/16) q[20];
cx q[20], q[17];
rz(pi/16) q[17];
cx q[20], q[17];
rz(-pi/16) q[17];
rz(-pi/8) q[20];
cx q[20], q[18];
rz(pi/8) q[18];
cx q[20], q[18];
rz(-pi/8) q[18];
rz(-pi/4) q[20];
cx q[20], q[19];
rz(pi/4) q[19];
cx q[20], q[19];
rz(-pi/4) q[19];
h q[20];
cx q[21], q[0];
rz(pi/4194304) q[0];
cx q[21], q[0];
rz(-pi/4194304) q[0];
rz(-pi/2097152) q[21];
cx q[21], q[1];
rz(pi/2097152) q[1];
cx q[21], q[1];
rz(-pi/2097152) q[1];
rz(-pi/1048576) q[21];
cx q[21], q[2];
rz(pi/1048576) q[2];
cx q[21], q[2];
rz(-pi/1048576) q[2];
rz(-pi/524288) q[21];
cx q[21], q[3];
rz(pi/524288) q[3];
cx q[21], q[3];
rz(-pi/524288) q[3];
rz(-pi/262144) q[21];
cx q[21], q[4];
rz(pi/262144) q[4];
cx q[21], q[4];
rz(-pi/262144) q[4];
rz(-pi/131072) q[21];
cx q[21], q[5];
rz(pi/131072) q[5];
cx q[21], q[5];
rz(-pi/131072) q[5];
rz(-pi/65536) q[21];
cx q[21], q[6];
rz(pi/65536) q[6];
cx q[21], q[6];
rz(-pi/65536) q[6];
rz(-pi/32768) q[21];
cx q[21], q[7];
rz(pi/32768) q[7];
cx q[21], q[7];
rz(-pi/32768) q[7];
rz(-pi/16384) q[21];
cx q[21], q[8];
rz(pi/16384) q[8];
cx q[21], q[8];
rz(-pi/16384) q[8];
rz(-pi/8192) q[21];
cx q[21], q[9];
rz(pi/8192) q[9];
cx q[21], q[9];
rz(-pi/8192) q[9];
rz(-pi/4096) q[21];
cx q[21], q[10];
rz(pi/4096) q[10];
cx q[21], q[10];
rz(-pi/4096) q[10];
rz(-pi/2048) q[21];
cx q[21], q[11];
rz(pi/2048) q[11];
cx q[21], q[11];
rz(-pi/2048) q[11];
rz(-pi/1024) q[21];
cx q[21], q[12];
rz(pi/1024) q[12];
cx q[21], q[12];
rz(-pi/1024) q[12];
rz(-pi/512) q[21];
cx q[21], q[13];
rz(pi/512) q[13];
cx q[21], q[13];
rz(-pi/512) q[13];
rz(-pi/256) q[21];
cx q[21], q[14];
rz(pi/256) q[14];
cx q[21], q[14];
rz(-pi/256) q[14];
rz(-pi/128) q[21];
cx q[21], q[15];
rz(pi/128) q[15];
cx q[21], q[15];
rz(-pi/128) q[15];
rz(-pi/64) q[21];
cx q[21], q[16];
rz(pi/64) q[16];
cx q[21], q[16];
rz(-pi/64) q[16];
rz(-pi/32) q[21];
cx q[21], q[17];
rz(pi/32) q[17];
cx q[21], q[17];
rz(-pi/32) q[17];
rz(-pi/16) q[21];
cx q[21], q[18];
rz(pi/16) q[18];
cx q[21], q[18];
rz(-pi/16) q[18];
rz(-pi/8) q[21];
cx q[21], q[19];
rz(pi/8) q[19];
cx q[21], q[19];
rz(-pi/8) q[19];
rz(-pi/4) q[21];
cx q[21], q[20];
rz(pi/4) q[20];
cx q[21], q[20];
rz(-pi/4) q[20];
h q[21];
cx q[22], q[0];
rz(pi/8388608) q[0];
cx q[22], q[0];
rz(-pi/8388608) q[0];
rz(-pi/4194304) q[22];
cx q[22], q[1];
rz(pi/4194304) q[1];
cx q[22], q[1];
rz(-pi/4194304) q[1];
rz(-pi/2097152) q[22];
cx q[22], q[2];
rz(pi/2097152) q[2];
cx q[22], q[2];
rz(-pi/2097152) q[2];
rz(-pi/1048576) q[22];
cx q[22], q[3];
rz(pi/1048576) q[3];
cx q[22], q[3];
rz(-pi/1048576) q[3];
rz(-pi/524288) q[22];
cx q[22], q[4];
rz(pi/524288) q[4];
cx q[22], q[4];
rz(-pi/524288) q[4];
rz(-pi/262144) q[22];
cx q[22], q[5];
rz(pi/262144) q[5];
cx q[22], q[5];
rz(-pi/262144) q[5];
rz(-pi/131072) q[22];
cx q[22], q[6];
rz(pi/131072) q[6];
cx q[22], q[6];
rz(-pi/131072) q[6];
rz(-pi/65536) q[22];
cx q[22], q[7];
rz(pi/65536) q[7];
cx q[22], q[7];
rz(-pi/65536) q[7];
rz(-pi/32768) q[22];
cx q[22], q[8];
rz(pi/32768) q[8];
cx q[22], q[8];
rz(-pi/32768) q[8];
rz(-pi/16384) q[22];
cx q[22], q[9];
rz(pi/16384) q[9];
cx q[22], q[9];
rz(-pi/16384) q[9];
rz(-pi/8192) q[22];
cx q[22], q[10];
rz(pi/8192) q[10];
cx q[22], q[10];
rz(-pi/8192) q[10];
rz(-pi/4096) q[22];
cx q[22], q[11];
rz(pi/4096) q[11];
cx q[22], q[11];
rz(-pi/4096) q[11];
rz(-pi/2048) q[22];
cx q[22], q[12];
rz(pi/2048) q[12];
cx q[22], q[12];
rz(-pi/2048) q[12];
rz(-pi/1024) q[22];
cx q[22], q[13];
rz(pi/1024) q[13];
cx q[22], q[13];
rz(-pi/1024) q[13];
rz(-pi/512) q[22];
cx q[22], q[14];
rz(pi/512) q[14];
cx q[22], q[14];
rz(-pi/512) q[14];
rz(-pi/256) q[22];
cx q[22], q[15];
rz(pi/256) q[15];
cx q[22], q[15];
rz(-pi/256) q[15];
rz(-pi/128) q[22];
cx q[22], q[16];
rz(pi/128) q[16];
cx q[22], q[16];
rz(-pi/128) q[16];
rz(-pi/64) q[22];
cx q[22], q[17];
rz(pi/64) q[17];
cx q[22], q[17];
rz(-pi/64) q[17];
rz(-pi/32) q[22];
cx q[22], q[18];
rz(pi/32) q[18];
cx q[22], q[18];
rz(-pi/32) q[18];
rz(-pi/16) q[22];
cx q[22], q[19];
rz(pi/16) q[19];
cx q[22], q[19];
rz(-pi/16) q[19];
rz(-pi/8) q[22];
cx q[22], q[20];
rz(pi/8) q[20];
cx q[22], q[20];
rz(-pi/8) q[20];
rz(-pi/4) q[22];
cx q[22], q[21];
rz(pi/4) q[21];
cx q[22], q[21];
rz(-pi/4) q[21];
h q[22];
cx q[23], q[0];
rz(pi/16777216) q[0];
cx q[23], q[0];
rz(-pi/16777216) q[0];
rz(-pi/8388608) q[23];
cx q[23], q[1];
rz(pi/8388608) q[1];
cx q[23], q[1];
rz(-pi/8388608) q[1];
rz(-pi/4194304) q[23];
cx q[23], q[2];
rz(pi/4194304) q[2];
cx q[23], q[2];
rz(-pi/4194304) q[2];
rz(-pi/2097152) q[23];
cx q[23], q[3];
rz(pi/2097152) q[3];
cx q[23], q[3];
rz(-pi/2097152) q[3];
rz(-pi/1048576) q[23];
cx q[23], q[4];
rz(pi/1048576) q[4];
cx q[23], q[4];
rz(-pi/1048576) q[4];
rz(-pi/524288) q[23];
cx q[23], q[5];
rz(pi/524288) q[5];
cx q[23], q[5];
rz(-pi/524288) q[5];
rz(-pi/262144) q[23];
cx q[23], q[6];
rz(pi/262144) q[6];
cx q[23], q[6];
rz(-pi/262144) q[6];
rz(-pi/131072) q[23];
cx q[23], q[7];
rz(pi/131072) q[7];
cx q[23], q[7];
rz(-pi/131072) q[7];
rz(-pi/65536) q[23];
cx q[23], q[8];
rz(pi/65536) q[8];
cx q[23], q[8];
rz(-pi/65536) q[8];
rz(-pi/32768) q[23];
cx q[23], q[9];
rz(pi/32768) q[9];
cx q[23], q[9];
rz(-pi/32768) q[9];
rz(-pi/16384) q[23];
cx q[23], q[10];
rz(pi/16384) q[10];
cx q[23], q[10];
rz(-pi/16384) q[10];
rz(-pi/8192) q[23];
cx q[23], q[11];
rz(pi/8192) q[11];
cx q[23], q[11];
rz(-pi/8192) q[11];
rz(-pi/4096) q[23];
cx q[23], q[12];
rz(pi/4096) q[12];
cx q[23], q[12];
rz(-pi/4096) q[12];
rz(-pi/2048) q[23];
cx q[23], q[13];
rz(pi/2048) q[13];
cx q[23], q[13];
rz(-pi/2048) q[13];
rz(-pi/1024) q[23];
cx q[23], q[14];
rz(pi/1024) q[14];
cx q[23], q[14];
rz(-pi/1024) q[14];
rz(-pi/512) q[23];
cx q[23], q[15];
rz(pi/512) q[15];
cx q[23], q[15];
rz(-pi/512) q[15];
rz(-pi/256) q[23];
cx q[23], q[16];
rz(pi/256) q[16];
cx q[23], q[16];
rz(-pi/256) q[16];
rz(-pi/128) q[23];
cx q[23], q[17];
rz(pi/128) q[17];
cx q[23], q[17];
rz(-pi/128) q[17];
rz(-pi/64) q[23];
cx q[23], q[18];
rz(pi/64) q[18];
cx q[23], q[18];
rz(-pi/64) q[18];
rz(-pi/32) q[23];
cx q[23], q[19];
rz(pi/32) q[19];
cx q[23], q[19];
rz(-pi/32) q[19];
rz(-pi/16) q[23];
cx q[23], q[20];
rz(pi/16) q[20];
cx q[23], q[20];
rz(-pi/16) q[20];
rz(-pi/8) q[23];
cx q[23], q[21];
rz(pi/8) q[21];
cx q[23], q[21];
rz(-pi/8) q[21];
rz(-pi/4) q[23];
cx q[23], q[22];
rz(pi/4) q[22];
cx q[23], q[22];
rz(-pi/4) q[22];
h q[23];
cx q[24], q[0];
rz(pi/33554432) q[0];
cx q[24], q[0];
rz(-pi/33554432) q[0];
rz(-pi/16777216) q[24];
cx q[24], q[1];
rz(pi/16777216) q[1];
cx q[24], q[1];
rz(-pi/16777216) q[1];
rz(-pi/8388608) q[24];
cx q[24], q[2];
rz(pi/8388608) q[2];
cx q[24], q[2];
rz(-pi/8388608) q[2];
rz(-pi/4194304) q[24];
cx q[24], q[3];
rz(pi/4194304) q[3];
cx q[24], q[3];
rz(-pi/4194304) q[3];
rz(-pi/2097152) q[24];
cx q[24], q[4];
rz(pi/2097152) q[4];
cx q[24], q[4];
rz(-pi/2097152) q[4];
rz(-pi/1048576) q[24];
cx q[24], q[5];
rz(pi/1048576) q[5];
cx q[24], q[5];
rz(-pi/1048576) q[5];
rz(-pi/524288) q[24];
cx q[24], q[6];
rz(pi/524288) q[6];
cx q[24], q[6];
rz(-pi/524288) q[6];
rz(-pi/262144) q[24];
cx q[24], q[7];
rz(pi/262144) q[7];
cx q[24], q[7];
rz(-pi/262144) q[7];
rz(-pi/131072) q[24];
cx q[24], q[8];
rz(pi/131072) q[8];
cx q[24], q[8];
rz(-pi/131072) q[8];
rz(-pi/65536) q[24];
cx q[24], q[9];
rz(pi/65536) q[9];
cx q[24], q[9];
rz(-pi/65536) q[9];
rz(-pi/32768) q[24];
cx q[24], q[10];
rz(pi/32768) q[10];
cx q[24], q[10];
rz(-pi/32768) q[10];
rz(-pi/16384) q[24];
cx q[24], q[11];
rz(pi/16384) q[11];
cx q[24], q[11];
rz(-pi/16384) q[11];
rz(-pi/8192) q[24];
cx q[24], q[12];
rz(pi/8192) q[12];
cx q[24], q[12];
rz(-pi/8192) q[12];
rz(-pi/4096) q[24];
cx q[24], q[13];
rz(pi/4096) q[13];
cx q[24], q[13];
rz(-pi/4096) q[13];
rz(-pi/2048) q[24];
cx q[24], q[14];
rz(pi/2048) q[14];
cx q[24], q[14];
rz(-pi/2048) q[14];
rz(-pi/1024) q[24];
cx q[24], q[15];
rz(pi/1024) q[15];
cx q[24], q[15];
rz(-pi/1024) q[15];
rz(-pi/512) q[24];
cx q[24], q[16];
rz(pi/512) q[16];
cx q[24], q[16];
rz(-pi/512) q[16];
rz(-pi/256) q[24];
cx q[24], q[17];
rz(pi/256) q[17];
cx q[24], q[17];
rz(-pi/256) q[17];
rz(-pi/128) q[24];
cx q[24], q[18];
rz(pi/128) q[18];
cx q[24], q[18];
rz(-pi/128) q[18];
rz(-pi/64) q[24];
cx q[24], q[19];
rz(pi/64) q[19];
cx q[24], q[19];
rz(-pi/64) q[19];
rz(-pi/32) q[24];
cx q[24], q[20];
rz(pi/32) q[20];
cx q[24], q[20];
rz(-pi/32) q[20];
rz(-pi/16) q[24];
cx q[24], q[21];
rz(pi/16) q[21];
cx q[24], q[21];
rz(-pi/16) q[21];
rz(-pi/8) q[24];
cx q[24], q[22];
rz(pi/8) q[22];
cx q[24], q[22];
rz(-pi/8) q[22];
rz(-pi/4) q[24];
cx q[24], q[23];
rz(pi/4) q[23];
cx q[24], q[23];
rz(-pi/4) q[23];
h q[24];
cx q[25], q[0];
rz(pi/67108864) q[0];
cx q[25], q[0];
rz(-pi/67108864) q[0];
rz(-pi/33554432) q[25];
cx q[25], q[1];
rz(pi/33554432) q[1];
cx q[25], q[1];
rz(-pi/33554432) q[1];
rz(-pi/16777216) q[25];
cx q[25], q[2];
rz(pi/16777216) q[2];
cx q[25], q[2];
rz(-pi/16777216) q[2];
rz(-pi/8388608) q[25];
cx q[25], q[3];
rz(pi/8388608) q[3];
cx q[25], q[3];
rz(-pi/8388608) q[3];
rz(-pi/4194304) q[25];
cx q[25], q[4];
rz(pi/4194304) q[4];
cx q[25], q[4];
rz(-pi/4194304) q[4];
rz(-pi/2097152) q[25];
cx q[25], q[5];
rz(pi/2097152) q[5];
cx q[25], q[5];
rz(-pi/2097152) q[5];
rz(-pi/1048576) q[25];
cx q[25], q[6];
rz(pi/1048576) q[6];
cx q[25], q[6];
rz(-pi/1048576) q[6];
rz(-pi/524288) q[25];
cx q[25], q[7];
rz(pi/524288) q[7];
cx q[25], q[7];
rz(-pi/524288) q[7];
rz(-pi/262144) q[25];
cx q[25], q[8];
rz(pi/262144) q[8];
cx q[25], q[8];
rz(-pi/262144) q[8];
rz(-pi/131072) q[25];
cx q[25], q[9];
rz(pi/131072) q[9];
cx q[25], q[9];
rz(-pi/131072) q[9];
rz(-pi/65536) q[25];
cx q[25], q[10];
rz(pi/65536) q[10];
cx q[25], q[10];
rz(-pi/65536) q[10];
rz(-pi/32768) q[25];
cx q[25], q[11];
rz(pi/32768) q[11];
cx q[25], q[11];
rz(-pi/32768) q[11];
rz(-pi/16384) q[25];
cx q[25], q[12];
rz(pi/16384) q[12];
cx q[25], q[12];
rz(-pi/16384) q[12];
rz(-pi/8192) q[25];
cx q[25], q[13];
rz(pi/8192) q[13];
cx q[25], q[13];
rz(-pi/8192) q[13];
rz(-pi/4096) q[25];
cx q[25], q[14];
rz(pi/4096) q[14];
cx q[25], q[14];
rz(-pi/4096) q[14];
rz(-pi/2048) q[25];
cx q[25], q[15];
rz(pi/2048) q[15];
cx q[25], q[15];
rz(-pi/2048) q[15];
rz(-pi/1024) q[25];
cx q[25], q[16];
rz(pi/1024) q[16];
cx q[25], q[16];
rz(-pi/1024) q[16];
rz(-pi/512) q[25];
cx q[25], q[17];
rz(pi/512) q[17];
cx q[25], q[17];
rz(-pi/512) q[17];
rz(-pi/256) q[25];
cx q[25], q[18];
rz(pi/256) q[18];
cx q[25], q[18];
rz(-pi/256) q[18];
rz(-pi/128) q[25];
cx q[25], q[19];
rz(pi/128) q[19];
cx q[25], q[19];
rz(-pi/128) q[19];
rz(-pi/64) q[25];
cx q[25], q[20];
rz(pi/64) q[20];
cx q[25], q[20];
rz(-pi/64) q[20];
rz(-pi/32) q[25];
cx q[25], q[21];
rz(pi/32) q[21];
cx q[25], q[21];
rz(-pi/32) q[21];
rz(-pi/16) q[25];
cx q[25], q[22];
rz(pi/16) q[22];
cx q[25], q[22];
rz(-pi/16) q[22];
rz(-pi/8) q[25];
cx q[25], q[23];
rz(pi/8) q[23];
cx q[25], q[23];
rz(-pi/8) q[23];
rz(-pi/4) q[25];
cx q[25], q[24];
rz(pi/4) q[24];
cx q[25], q[24];
rz(-pi/4) q[24];
h q[25];
cx q[26], q[0];
rz(pi/134217728) q[0];
cx q[26], q[0];
rz(-pi/134217728) q[0];
rz(-pi/67108864) q[26];
cx q[26], q[1];
rz(pi/67108864) q[1];
cx q[26], q[1];
rz(-pi/67108864) q[1];
rz(-pi/33554432) q[26];
cx q[26], q[2];
rz(pi/33554432) q[2];
cx q[26], q[2];
rz(-pi/33554432) q[2];
rz(-pi/16777216) q[26];
cx q[26], q[3];
rz(pi/16777216) q[3];
cx q[26], q[3];
rz(-pi/16777216) q[3];
rz(-pi/8388608) q[26];
cx q[26], q[4];
rz(pi/8388608) q[4];
cx q[26], q[4];
rz(-pi/8388608) q[4];
rz(-pi/4194304) q[26];
cx q[26], q[5];
rz(pi/4194304) q[5];
cx q[26], q[5];
rz(-pi/4194304) q[5];
rz(-pi/2097152) q[26];
cx q[26], q[6];
rz(pi/2097152) q[6];
cx q[26], q[6];
rz(-pi/2097152) q[6];
rz(-pi/1048576) q[26];
cx q[26], q[7];
rz(pi/1048576) q[7];
cx q[26], q[7];
rz(-pi/1048576) q[7];
rz(-pi/524288) q[26];
cx q[26], q[8];
rz(pi/524288) q[8];
cx q[26], q[8];
rz(-pi/524288) q[8];
rz(-pi/262144) q[26];
cx q[26], q[9];
rz(pi/262144) q[9];
cx q[26], q[9];
rz(-pi/262144) q[9];
rz(-pi/131072) q[26];
cx q[26], q[10];
rz(pi/131072) q[10];
cx q[26], q[10];
rz(-pi/131072) q[10];
rz(-pi/65536) q[26];
cx q[26], q[11];
rz(pi/65536) q[11];
cx q[26], q[11];
rz(-pi/65536) q[11];
rz(-pi/32768) q[26];
cx q[26], q[12];
rz(pi/32768) q[12];
cx q[26], q[12];
rz(-pi/32768) q[12];
rz(-pi/16384) q[26];
cx q[26], q[13];
rz(pi/16384) q[13];
cx q[26], q[13];
rz(-pi/16384) q[13];
rz(-pi/8192) q[26];
cx q[26], q[14];
rz(pi/8192) q[14];
cx q[26], q[14];
rz(-pi/8192) q[14];
rz(-pi/4096) q[26];
cx q[26], q[15];
rz(pi/4096) q[15];
cx q[26], q[15];
rz(-pi/4096) q[15];
rz(-pi/2048) q[26];
cx q[26], q[16];
rz(pi/2048) q[16];
cx q[26], q[16];
rz(-pi/2048) q[16];
rz(-pi/1024) q[26];
cx q[26], q[17];
rz(pi/1024) q[17];
cx q[26], q[17];
rz(-pi/1024) q[17];
rz(-pi/512) q[26];
cx q[26], q[18];
rz(pi/512) q[18];
cx q[26], q[18];
rz(-pi/512) q[18];
rz(-pi/256) q[26];
cx q[26], q[19];
rz(pi/256) q[19];
cx q[26], q[19];
rz(-pi/256) q[19];
rz(-pi/128) q[26];
cx q[26], q[20];
rz(pi/128) q[20];
cx q[26], q[20];
rz(-pi/128) q[20];
rz(-pi/64) q[26];
cx q[26], q[21];
rz(pi/64) q[21];
cx q[26], q[21];
rz(-pi/64) q[21];
rz(-pi/32) q[26];
cx q[26], q[22];
rz(pi/32) q[22];
cx q[26], q[22];
rz(-pi/32) q[22];
rz(-pi/16) q[26];
cx q[26], q[23];
rz(pi/16) q[23];
cx q[26], q[23];
rz(-pi/16) q[23];
rz(-pi/8) q[26];
cx q[26], q[24];
rz(pi/8) q[24];
cx q[26], q[24];
rz(-pi/8) q[24];
rz(-pi/4) q[26];
cx q[26], q[25];
rz(pi/4) q[25];
cx q[26], q[25];
rz(-pi/4) q[25];
h q[26];
cx q[27], q[0];
rz(pi/268435456) q[0];
cx q[27], q[0];
rz(-pi/268435456) q[0];
rz(-pi/134217728) q[27];
cx q[27], q[1];
rz(pi/134217728) q[1];
cx q[27], q[1];
rz(-pi/134217728) q[1];
rz(-pi/67108864) q[27];
cx q[27], q[2];
rz(pi/67108864) q[2];
cx q[27], q[2];
rz(-pi/67108864) q[2];
rz(-pi/33554432) q[27];
cx q[27], q[3];
rz(pi/33554432) q[3];
cx q[27], q[3];
rz(-pi/33554432) q[3];
rz(-pi/16777216) q[27];
cx q[27], q[4];
rz(pi/16777216) q[4];
cx q[27], q[4];
rz(-pi/16777216) q[4];
rz(-pi/8388608) q[27];
cx q[27], q[5];
rz(pi/8388608) q[5];
cx q[27], q[5];
rz(-pi/8388608) q[5];
rz(-pi/4194304) q[27];
cx q[27], q[6];
rz(pi/4194304) q[6];
cx q[27], q[6];
rz(-pi/4194304) q[6];
rz(-pi/2097152) q[27];
cx q[27], q[7];
rz(pi/2097152) q[7];
cx q[27], q[7];
rz(-pi/2097152) q[7];
rz(-pi/1048576) q[27];
cx q[27], q[8];
rz(pi/1048576) q[8];
cx q[27], q[8];
rz(-pi/1048576) q[8];
rz(-pi/524288) q[27];
cx q[27], q[9];
rz(pi/524288) q[9];
cx q[27], q[9];
rz(-pi/524288) q[9];
rz(-pi/262144) q[27];
cx q[27], q[10];
rz(pi/262144) q[10];
cx q[27], q[10];
rz(-pi/262144) q[10];
rz(-pi/131072) q[27];
cx q[27], q[11];
rz(pi/131072) q[11];
cx q[27], q[11];
rz(-pi/131072) q[11];
rz(-pi/65536) q[27];
cx q[27], q[12];
rz(pi/65536) q[12];
cx q[27], q[12];
rz(-pi/65536) q[12];
rz(-pi/32768) q[27];
cx q[27], q[13];
rz(pi/32768) q[13];
cx q[27], q[13];
rz(-pi/32768) q[13];
rz(-pi/16384) q[27];
cx q[27], q[14];
rz(pi/16384) q[14];
cx q[27], q[14];
rz(-pi/16384) q[14];
rz(-pi/8192) q[27];
cx q[27], q[15];
rz(pi/8192) q[15];
cx q[27], q[15];
rz(-pi/8192) q[15];
rz(-pi/4096) q[27];
cx q[27], q[16];
rz(pi/4096) q[16];
cx q[27], q[16];
rz(-pi/4096) q[16];
rz(-pi/2048) q[27];
cx q[27], q[17];
rz(pi/2048) q[17];
cx q[27], q[17];
rz(-pi/2048) q[17];
rz(-pi/1024) q[27];
cx q[27], q[18];
rz(pi/1024) q[18];
cx q[27], q[18];
rz(-pi/1024) q[18];
rz(-pi/512) q[27];
cx q[27], q[19];
rz(pi/512) q[19];
cx q[27], q[19];
rz(-pi/512) q[19];
rz(-pi/256) q[27];
cx q[27], q[20];
rz(pi/256) q[20];
cx q[27], q[20];
rz(-pi/256) q[20];
rz(-pi/128) q[27];
cx q[27], q[21];
rz(pi/128) q[21];
cx q[27], q[21];
rz(-pi/128) q[21];
rz(-pi/64) q[27];
cx q[27], q[22];
rz(pi/64) q[22];
cx q[27], q[22];
rz(-pi/64) q[22];
rz(-pi/32) q[27];
cx q[27], q[23];
rz(pi/32) q[23];
cx q[27], q[23];
rz(-pi/32) q[23];
rz(-pi/16) q[27];
cx q[27], q[24];
rz(pi/16) q[24];
cx q[27], q[24];
rz(-pi/16) q[24];
rz(-pi/8) q[27];
cx q[27], q[25];
rz(pi/8) q[25];
cx q[27], q[25];
rz(-pi/8) q[25];
rz(-pi/4) q[27];
cx q[27], q[26];
rz(pi/4) q[26];
cx q[27], q[26];
rz(-pi/4) q[26];
h q[27];
cx q[28], q[0];
rz(pi/536870912) q[0];
cx q[28], q[0];
rz(-pi/536870912) q[0];
rz(-pi/268435456) q[28];
cx q[28], q[1];
rz(pi/268435456) q[1];
cx q[28], q[1];
rz(-pi/268435456) q[1];
rz(-pi/134217728) q[28];
cx q[28], q[2];
rz(pi/134217728) q[2];
cx q[28], q[2];
rz(-pi/134217728) q[2];
rz(-pi/67108864) q[28];
cx q[28], q[3];
rz(pi/67108864) q[3];
cx q[28], q[3];
rz(-pi/67108864) q[3];
rz(-pi/33554432) q[28];
cx q[28], q[4];
rz(pi/33554432) q[4];
cx q[28], q[4];
rz(-pi/33554432) q[4];
rz(-pi/16777216) q[28];
cx q[28], q[5];
rz(pi/16777216) q[5];
cx q[28], q[5];
rz(-pi/16777216) q[5];
rz(-pi/8388608) q[28];
cx q[28], q[6];
rz(pi/8388608) q[6];
cx q[28], q[6];
rz(-pi/8388608) q[6];
rz(-pi/4194304) q[28];
cx q[28], q[7];
rz(pi/4194304) q[7];
cx q[28], q[7];
rz(-pi/4194304) q[7];
rz(-pi/2097152) q[28];
cx q[28], q[8];
rz(pi/2097152) q[8];
cx q[28], q[8];
rz(-pi/2097152) q[8];
rz(-pi/1048576) q[28];
cx q[28], q[9];
rz(pi/1048576) q[9];
cx q[28], q[9];
rz(-pi/1048576) q[9];
rz(-pi/524288) q[28];
cx q[28], q[10];
rz(pi/524288) q[10];
cx q[28], q[10];
rz(-pi/524288) q[10];
rz(-pi/262144) q[28];
cx q[28], q[11];
rz(pi/262144) q[11];
cx q[28], q[11];
rz(-pi/262144) q[11];
rz(-pi/131072) q[28];
cx q[28], q[12];
rz(pi/131072) q[12];
cx q[28], q[12];
rz(-pi/131072) q[12];
rz(-pi/65536) q[28];
cx q[28], q[13];
rz(pi/65536) q[13];
cx q[28], q[13];
rz(-pi/65536) q[13];
rz(-pi/32768) q[28];
cx q[28], q[14];
rz(pi/32768) q[14];
cx q[28], q[14];
rz(-pi/32768) q[14];
rz(-pi/16384) q[28];
cx q[28], q[15];
rz(pi/16384) q[15];
cx q[28], q[15];
rz(-pi/16384) q[15];
rz(-pi/8192) q[28];
cx q[28], q[16];
rz(pi/8192) q[16];
cx q[28], q[16];
rz(-pi/8192) q[16];
rz(-pi/4096) q[28];
cx q[28], q[17];
rz(pi/4096) q[17];
cx q[28], q[17];
rz(-pi/4096) q[17];
rz(-pi/2048) q[28];
cx q[28], q[18];
rz(pi/2048) q[18];
cx q[28], q[18];
rz(-pi/2048) q[18];
rz(-pi/1024) q[28];
cx q[28], q[19];
rz(pi/1024) q[19];
cx q[28], q[19];
rz(-pi/1024) q[19];
rz(-pi/512) q[28];
cx q[28], q[20];
rz(pi/512) q[20];
cx q[28], q[20];
rz(-pi/512) q[20];
rz(-pi/256) q[28];
cx q[28], q[21];
rz(pi/256) q[21];
cx q[28], q[21];
rz(-pi/256) q[21];
rz(-pi/128) q[28];
cx q[28], q[22];
rz(pi/128) q[22];
cx q[28], q[22];
rz(-pi/128) q[22];
rz(-pi/64) q[28];
cx q[28], q[23];
rz(pi/64) q[23];
cx q[28], q[23];
rz(-pi/64) q[23];
rz(-pi/32) q[28];
cx q[28], q[24];
rz(pi/32) q[24];
cx q[28], q[24];
rz(-pi/32) q[24];
rz(-pi/16) q[28];
cx q[28], q[25];
rz(pi/16) q[25];
cx q[28], q[25];
rz(-pi/16) q[25];
rz(-pi/8) q[28];
cx q[28], q[26];
rz(pi/8) q[26];
cx q[28], q[26];
rz(-pi/8) q[26];
rz(-pi/4) q[28];
cx q[28], q[27];
rz(pi/4) q[27];
cx q[28], q[27];
rz(-pi/4) q[27];
h q[28];
cx q[29], q[0];
rz(pi/1073741824) q[0];
cx q[29], q[0];
rz(-pi/1073741824) q[0];
rz(-pi/536870912) q[29];
cx q[29], q[1];
rz(pi/536870912) q[1];
cx q[29], q[1];
rz(-pi/536870912) q[1];
rz(-pi/268435456) q[29];
cx q[29], q[2];
rz(pi/268435456) q[2];
cx q[29], q[2];
rz(-pi/268435456) q[2];
rz(-pi/134217728) q[29];
cx q[29], q[3];
rz(pi/134217728) q[3];
cx q[29], q[3];
rz(-pi/134217728) q[3];
rz(-pi/67108864) q[29];
cx q[29], q[4];
rz(pi/67108864) q[4];
cx q[29], q[4];
rz(-pi/67108864) q[4];
rz(-pi/33554432) q[29];
cx q[29], q[5];
rz(pi/33554432) q[5];
cx q[29], q[5];
rz(-pi/33554432) q[5];
rz(-pi/16777216) q[29];
cx q[29], q[6];
rz(pi/16777216) q[6];
cx q[29], q[6];
rz(-pi/16777216) q[6];
rz(-pi/8388608) q[29];
cx q[29], q[7];
rz(pi/8388608) q[7];
cx q[29], q[7];
rz(-pi/8388608) q[7];
rz(-pi/4194304) q[29];
cx q[29], q[8];
rz(pi/4194304) q[8];
cx q[29], q[8];
rz(-pi/4194304) q[8];
rz(-pi/2097152) q[29];
cx q[29], q[9];
rz(pi/2097152) q[9];
cx q[29], q[9];
rz(-pi/2097152) q[9];
rz(-pi/1048576) q[29];
cx q[29], q[10];
rz(pi/1048576) q[10];
cx q[29], q[10];
rz(-pi/1048576) q[10];
rz(-pi/524288) q[29];
cx q[29], q[11];
rz(pi/524288) q[11];
cx q[29], q[11];
rz(-pi/524288) q[11];
rz(-pi/262144) q[29];
cx q[29], q[12];
rz(pi/262144) q[12];
cx q[29], q[12];
rz(-pi/262144) q[12];
rz(-pi/131072) q[29];
cx q[29], q[13];
rz(pi/131072) q[13];
cx q[29], q[13];
rz(-pi/131072) q[13];
rz(-pi/65536) q[29];
cx q[29], q[14];
rz(pi/65536) q[14];
cx q[29], q[14];
rz(-pi/65536) q[14];
rz(-pi/32768) q[29];
cx q[29], q[15];
rz(pi/32768) q[15];
cx q[29], q[15];
rz(-pi/32768) q[15];
rz(-pi/16384) q[29];
cx q[29], q[16];
rz(pi/16384) q[16];
cx q[29], q[16];
rz(-pi/16384) q[16];
rz(-pi/8192) q[29];
cx q[29], q[17];
rz(pi/8192) q[17];
cx q[29], q[17];
rz(-pi/8192) q[17];
rz(-pi/4096) q[29];
cx q[29], q[18];
rz(pi/4096) q[18];
cx q[29], q[18];
rz(-pi/4096) q[18];
rz(-pi/2048) q[29];
cx q[29], q[19];
rz(pi/2048) q[19];
cx q[29], q[19];
rz(-pi/2048) q[19];
rz(-pi/1024) q[29];
cx q[29], q[20];
rz(pi/1024) q[20];
cx q[29], q[20];
rz(-pi/1024) q[20];
rz(-pi/512) q[29];
cx q[29], q[21];
rz(pi/512) q[21];
cx q[29], q[21];
rz(-pi/512) q[21];
rz(-pi/256) q[29];
cx q[29], q[22];
rz(pi/256) q[22];
cx q[29], q[22];
rz(-pi/256) q[22];
rz(-pi/128) q[29];
cx q[29], q[23];
rz(pi/128) q[23];
cx q[29], q[23];
rz(-pi/128) q[23];
rz(-pi/64) q[29];
cx q[29], q[24];
rz(pi/64) q[24];
cx q[29], q[24];
rz(-pi/64) q[24];
rz(-pi/32) q[29];
cx q[29], q[25];
rz(pi/32) q[25];
cx q[29], q[25];
rz(-pi/32) q[25];
rz(-pi/16) q[29];
cx q[29], q[26];
rz(pi/16) q[26];
cx q[29], q[26];
rz(-pi/16) q[26];
rz(-pi/8) q[29];
cx q[29], q[27];
rz(pi/8) q[27];
cx q[29], q[27];
rz(-pi/8) q[27];
rz(-pi/4) q[29];
cx q[29], q[28];
rz(pi/4) q[28];
cx q[29], q[28];
rz(-pi/4) q[28];
h q[29];
cx q[30], q[0];
rz(pi/2147483648) q[0];
cx q[30], q[0];
rz(-pi/2147483648) q[0];
rz(-pi/1073741824) q[30];
cx q[30], q[1];
rz(pi/1073741824) q[1];
cx q[30], q[1];
rz(-pi/1073741824) q[1];
rz(-pi/536870912) q[30];
cx q[30], q[2];
rz(pi/536870912) q[2];
cx q[30], q[2];
rz(-pi/536870912) q[2];
rz(-pi/268435456) q[30];
cx q[30], q[3];
rz(pi/268435456) q[3];
cx q[30], q[3];
rz(-pi/268435456) q[3];
rz(-pi/134217728) q[30];
cx q[30], q[4];
rz(pi/134217728) q[4];
cx q[30], q[4];
rz(-pi/134217728) q[4];
rz(-pi/67108864) q[30];
cx q[30], q[5];
rz(pi/67108864) q[5];
cx q[30], q[5];
rz(-pi/67108864) q[5];
rz(-pi/33554432) q[30];
cx q[30], q[6];
rz(pi/33554432) q[6];
cx q[30], q[6];
rz(-pi/33554432) q[6];
rz(-pi/16777216) q[30];
cx q[30], q[7];
rz(pi/16777216) q[7];
cx q[30], q[7];
rz(-pi/16777216) q[7];
rz(-pi/8388608) q[30];
cx q[30], q[8];
rz(pi/8388608) q[8];
cx q[30], q[8];
rz(-pi/8388608) q[8];
rz(-pi/4194304) q[30];
cx q[30], q[9];
rz(pi/4194304) q[9];
cx q[30], q[9];
rz(-pi/4194304) q[9];
rz(-pi/2097152) q[30];
cx q[30], q[10];
rz(pi/2097152) q[10];
cx q[30], q[10];
rz(-pi/2097152) q[10];
rz(-pi/1048576) q[30];
cx q[30], q[11];
rz(pi/1048576) q[11];
cx q[30], q[11];
rz(-pi/1048576) q[11];
rz(-pi/524288) q[30];
cx q[30], q[12];
rz(pi/524288) q[12];
cx q[30], q[12];
rz(-pi/524288) q[12];
rz(-pi/262144) q[30];
cx q[30], q[13];
rz(pi/262144) q[13];
cx q[30], q[13];
rz(-pi/262144) q[13];
rz(-pi/131072) q[30];
cx q[30], q[14];
rz(pi/131072) q[14];
cx q[30], q[14];
rz(-pi/131072) q[14];
rz(-pi/65536) q[30];
cx q[30], q[15];
rz(pi/65536) q[15];
cx q[30], q[15];
rz(-pi/65536) q[15];
rz(-pi/32768) q[30];
cx q[30], q[16];
rz(pi/32768) q[16];
cx q[30], q[16];
rz(-pi/32768) q[16];
rz(-pi/16384) q[30];
cx q[30], q[17];
rz(pi/16384) q[17];
cx q[30], q[17];
rz(-pi/16384) q[17];
rz(-pi/8192) q[30];
cx q[30], q[18];
rz(pi/8192) q[18];
cx q[30], q[18];
rz(-pi/8192) q[18];
rz(-pi/4096) q[30];
cx q[30], q[19];
rz(pi/4096) q[19];
cx q[30], q[19];
rz(-pi/4096) q[19];
rz(-pi/2048) q[30];
cx q[30], q[20];
rz(pi/2048) q[20];
cx q[30], q[20];
rz(-pi/2048) q[20];
rz(-pi/1024) q[30];
cx q[30], q[21];
rz(pi/1024) q[21];
cx q[30], q[21];
rz(-pi/1024) q[21];
rz(-pi/512) q[30];
cx q[30], q[22];
rz(pi/512) q[22];
cx q[30], q[22];
rz(-pi/512) q[22];
rz(-pi/256) q[30];
cx q[30], q[23];
rz(pi/256) q[23];
cx q[30], q[23];
rz(-pi/256) q[23];
rz(-pi/128) q[30];
cx q[30], q[24];
rz(pi/128) q[24];
cx q[30], q[24];
rz(-pi/128) q[24];
rz(-pi/64) q[30];
cx q[30], q[25];
rz(pi/64) q[25];
cx q[30], q[25];
rz(-pi/64) q[25];
rz(-pi/32) q[30];
cx q[30], q[26];
rz(pi/32) q[26];
cx q[30], q[26];
rz(-pi/32) q[26];
rz(-pi/16) q[30];
cx q[30], q[27];
rz(pi/16) q[27];
cx q[30], q[27];
rz(-pi/16) q[27];
rz(-pi/8) q[30];
cx q[30], q[28];
rz(pi/8) q[28];
cx q[30], q[28];
rz(-pi/8) q[28];
rz(-pi/4) q[30];
cx q[30], q[29];
rz(pi/4) q[29];
cx q[30], q[29];
rz(-pi/4) q[29];
h q[30];
cx q[31], q[0];
rz(0) q[0];
cx q[31], q[0];
rz(0) q[0];
rz(-pi/2147483648) q[31];
cx q[31], q[1];
rz(pi/2147483648) q[1];
cx q[31], q[1];
rz(-pi/2147483648) q[1];
rz(-pi/1073741824) q[31];
cx q[31], q[2];
rz(pi/1073741824) q[2];
cx q[31], q[2];
rz(-pi/1073741824) q[2];
rz(-pi/536870912) q[31];
cx q[31], q[3];
rz(pi/536870912) q[3];
cx q[31], q[3];
rz(-pi/536870912) q[3];
rz(-pi/268435456) q[31];
cx q[31], q[4];
rz(pi/268435456) q[4];
cx q[31], q[4];
rz(-pi/268435456) q[4];
rz(-pi/134217728) q[31];
cx q[31], q[5];
rz(pi/134217728) q[5];
cx q[31], q[5];
rz(-pi/134217728) q[5];
rz(-pi/67108864) q[31];
cx q[31], q[6];
rz(pi/67108864) q[6];
cx q[31], q[6];
rz(-pi/67108864) q[6];
rz(-pi/33554432) q[31];
cx q[31], q[7];
rz(pi/33554432) q[7];
cx q[31], q[7];
rz(-pi/33554432) q[7];
rz(-pi/16777216) q[31];
cx q[31], q[8];
rz(pi/16777216) q[8];
cx q[31], q[8];
rz(-pi/16777216) q[8];
rz(-pi/8388608) q[31];
cx q[31], q[9];
rz(pi/8388608) q[9];
cx q[31], q[9];
rz(-pi/8388608) q[9];
rz(-pi/4194304) q[31];
cx q[31], q[10];
rz(pi/4194304) q[10];
cx q[31], q[10];
rz(-pi/4194304) q[10];
rz(-pi/2097152) q[31];
cx q[31], q[11];
rz(pi/2097152) q[11];
cx q[31], q[11];
rz(-pi/2097152) q[11];
rz(-pi/1048576) q[31];
cx q[31], q[12];
rz(pi/1048576) q[12];
cx q[31], q[12];
rz(-pi/1048576) q[12];
rz(-pi/524288) q[31];
cx q[31], q[13];
rz(pi/524288) q[13];
cx q[31], q[13];
rz(-pi/524288) q[13];
rz(-pi/262144) q[31];
cx q[31], q[14];
rz(pi/262144) q[14];
cx q[31], q[14];
rz(-pi/262144) q[14];
rz(-pi/131072) q[31];
cx q[31], q[15];
rz(pi/131072) q[15];
cx q[31], q[15];
rz(-pi/131072) q[15];
rz(-pi/65536) q[31];
cx q[31], q[16];
rz(pi/65536) q[16];
cx q[31], q[16];
rz(-pi/65536) q[16];
rz(-pi/32768) q[31];
cx q[31], q[17];
rz(pi/32768) q[17];
cx q[31], q[17];
rz(-pi/32768) q[17];
rz(-pi/16384) q[31];
cx q[31], q[18];
rz(pi/16384) q[18];
cx q[31], q[18];
rz(-pi/16384) q[18];
rz(-pi/8192) q[31];
cx q[31], q[19];
rz(pi/8192) q[19];
cx q[31], q[19];
rz(-pi/8192) q[19];
rz(-pi/4096) q[31];
cx q[31], q[20];
rz(pi/4096) q[20];
cx q[31], q[20];
rz(-pi/4096) q[20];
rz(-pi/2048) q[31];
cx q[31], q[21];
rz(pi/2048) q[21];
cx q[31], q[21];
rz(-pi/2048) q[21];
rz(-pi/1024) q[31];
cx q[31], q[22];
rz(pi/1024) q[22];
cx q[31], q[22];
rz(-pi/1024) q[22];
rz(-pi/512) q[31];
cx q[31], q[23];
rz(pi/512) q[23];
cx q[31], q[23];
rz(-pi/512) q[23];
rz(-pi/256) q[31];
cx q[31], q[24];
rz(pi/256) q[24];
cx q[31], q[24];
rz(-pi/256) q[24];
rz(-pi/128) q[31];
cx q[31], q[25];
rz(pi/128) q[25];
cx q[31], q[25];
rz(-pi/128) q[25];
rz(-pi/64) q[31];
cx q[31], q[26];
rz(pi/64) q[26];
cx q[31], q[26];
rz(-pi/64) q[26];
rz(-pi/32) q[31];
cx q[31], q[27];
rz(pi/32) q[27];
cx q[31], q[27];
rz(-pi/32) q[27];
rz(-pi/16) q[31];
cx q[31], q[28];
rz(pi/16) q[28];
cx q[31], q[28];
rz(-pi/16) q[28];
rz(-pi/8) q[31];
cx q[31], q[29];
rz(pi/8) q[29];
cx q[31], q[29];
rz(-pi/8) q[29];
rz(-pi/4) q[31];
cx q[31], q[30];
rz(pi/4) q[30];
cx q[31], q[30];
rz(-pi/4) q[30];
h q[31];
cx q[32], q[0];
rz(0) q[0];
cx q[32], q[0];
rz(0) q[0];
rz(0) q[32];
cx q[32], q[1];
rz(0) q[1];
cx q[32], q[1];
rz(0) q[1];
rz(-pi/2147483648) q[32];
cx q[32], q[2];
rz(pi/2147483648) q[2];
cx q[32], q[2];
rz(-pi/2147483648) q[2];
rz(-pi/1073741824) q[32];
cx q[32], q[3];
rz(pi/1073741824) q[3];
cx q[32], q[3];
rz(-pi/1073741824) q[3];
rz(-pi/536870912) q[32];
cx q[32], q[4];
rz(pi/536870912) q[4];
cx q[32], q[4];
rz(-pi/536870912) q[4];
rz(-pi/268435456) q[32];
cx q[32], q[5];
rz(pi/268435456) q[5];
cx q[32], q[5];
rz(-pi/268435456) q[5];
rz(-pi/134217728) q[32];
cx q[32], q[6];
rz(pi/134217728) q[6];
cx q[32], q[6];
rz(-pi/134217728) q[6];
rz(-pi/67108864) q[32];
cx q[32], q[7];
rz(pi/67108864) q[7];
cx q[32], q[7];
rz(-pi/67108864) q[7];
rz(-pi/33554432) q[32];
cx q[32], q[8];
rz(pi/33554432) q[8];
cx q[32], q[8];
rz(-pi/33554432) q[8];
rz(-pi/16777216) q[32];
cx q[32], q[9];
rz(pi/16777216) q[9];
cx q[32], q[9];
rz(-pi/16777216) q[9];
rz(-pi/8388608) q[32];
cx q[32], q[10];
rz(pi/8388608) q[10];
cx q[32], q[10];
rz(-pi/8388608) q[10];
rz(-pi/4194304) q[32];
cx q[32], q[11];
rz(pi/4194304) q[11];
cx q[32], q[11];
rz(-pi/4194304) q[11];
rz(-pi/2097152) q[32];
cx q[32], q[12];
rz(pi/2097152) q[12];
cx q[32], q[12];
rz(-pi/2097152) q[12];
rz(-pi/1048576) q[32];
cx q[32], q[13];
rz(pi/1048576) q[13];
cx q[32], q[13];
rz(-pi/1048576) q[13];
rz(-pi/524288) q[32];
cx q[32], q[14];
rz(pi/524288) q[14];
cx q[32], q[14];
rz(-pi/524288) q[14];
rz(-pi/262144) q[32];
cx q[32], q[15];
rz(pi/262144) q[15];
cx q[32], q[15];
rz(-pi/262144) q[15];
rz(-pi/131072) q[32];
cx q[32], q[16];
rz(pi/131072) q[16];
cx q[32], q[16];
rz(-pi/131072) q[16];
rz(-pi/65536) q[32];
cx q[32], q[17];
rz(pi/65536) q[17];
cx q[32], q[17];
rz(-pi/65536) q[17];
rz(-pi/32768) q[32];
cx q[32], q[18];
rz(pi/32768) q[18];
cx q[32], q[18];
rz(-pi/32768) q[18];
rz(-pi/16384) q[32];
cx q[32], q[19];
rz(pi/16384) q[19];
cx q[32], q[19];
rz(-pi/16384) q[19];
rz(-pi/8192) q[32];
cx q[32], q[20];
rz(pi/8192) q[20];
cx q[32], q[20];
rz(-pi/8192) q[20];
rz(-pi/4096) q[32];
cx q[32], q[21];
rz(pi/4096) q[21];
cx q[32], q[21];
rz(-pi/4096) q[21];
rz(-pi/2048) q[32];
cx q[32], q[22];
rz(pi/2048) q[22];
cx q[32], q[22];
rz(-pi/2048) q[22];
rz(-pi/1024) q[32];
cx q[32], q[23];
rz(pi/1024) q[23];
cx q[32], q[23];
rz(-pi/1024) q[23];
rz(-pi/512) q[32];
cx q[32], q[24];
rz(pi/512) q[24];
cx q[32], q[24];
rz(-pi/512) q[24];
rz(-pi/256) q[32];
cx q[32], q[25];
rz(pi/256) q[25];
cx q[32], q[25];
rz(-pi/256) q[25];
rz(-pi/128) q[32];
cx q[32], q[26];
rz(pi/128) q[26];
cx q[32], q[26];
rz(-pi/128) q[26];
rz(-pi/64) q[32];
cx q[32], q[27];
rz(pi/64) q[27];
cx q[32], q[27];
rz(-pi/64) q[27];
rz(-pi/32) q[32];
cx q[32], q[28];
rz(pi/32) q[28];
cx q[32], q[28];
rz(-pi/32) q[28];
rz(-pi/16) q[32];
cx q[32], q[29];
rz(pi/16) q[29];
cx q[32], q[29];
rz(-pi/16) q[29];
rz(-pi/8) q[32];
cx q[32], q[30];
rz(pi/8) q[30];
cx q[32], q[30];
rz(-pi/8) q[30];
rz(-pi/4) q[32];
cx q[32], q[31];
rz(pi/4) q[31];
cx q[32], q[31];
rz(-pi/4) q[31];
h q[32];
cx q[33], q[0];
rz(0) q[0];
cx q[33], q[0];
rz(0) q[0];
rz(0) q[33];
cx q[33], q[1];
rz(0) q[1];
cx q[33], q[1];
rz(0) q[1];
rz(0) q[33];
cx q[33], q[2];
rz(0) q[2];
cx q[33], q[2];
rz(0) q[2];
rz(-pi/2147483648) q[33];
cx q[33], q[3];
rz(pi/2147483648) q[3];
cx q[33], q[3];
rz(-pi/2147483648) q[3];
rz(-pi/1073741824) q[33];
cx q[33], q[4];
rz(pi/1073741824) q[4];
cx q[33], q[4];
rz(-pi/1073741824) q[4];
rz(-pi/536870912) q[33];
cx q[33], q[5];
rz(pi/536870912) q[5];
cx q[33], q[5];
rz(-pi/536870912) q[5];
rz(-pi/268435456) q[33];
cx q[33], q[6];
rz(pi/268435456) q[6];
cx q[33], q[6];
rz(-pi/268435456) q[6];
rz(-pi/134217728) q[33];
cx q[33], q[7];
rz(pi/134217728) q[7];
cx q[33], q[7];
rz(-pi/134217728) q[7];
rz(-pi/67108864) q[33];
cx q[33], q[8];
rz(pi/67108864) q[8];
cx q[33], q[8];
rz(-pi/67108864) q[8];
rz(-pi/33554432) q[33];
cx q[33], q[9];
rz(pi/33554432) q[9];
cx q[33], q[9];
rz(-pi/33554432) q[9];
rz(-pi/16777216) q[33];
cx q[33], q[10];
rz(pi/16777216) q[10];
cx q[33], q[10];
rz(-pi/16777216) q[10];
rz(-pi/8388608) q[33];
cx q[33], q[11];
rz(pi/8388608) q[11];
cx q[33], q[11];
rz(-pi/8388608) q[11];
rz(-pi/4194304) q[33];
cx q[33], q[12];
rz(pi/4194304) q[12];
cx q[33], q[12];
rz(-pi/4194304) q[12];
rz(-pi/2097152) q[33];
cx q[33], q[13];
rz(pi/2097152) q[13];
cx q[33], q[13];
rz(-pi/2097152) q[13];
rz(-pi/1048576) q[33];
cx q[33], q[14];
rz(pi/1048576) q[14];
cx q[33], q[14];
rz(-pi/1048576) q[14];
rz(-pi/524288) q[33];
cx q[33], q[15];
rz(pi/524288) q[15];
cx q[33], q[15];
rz(-pi/524288) q[15];
rz(-pi/262144) q[33];
cx q[33], q[16];
rz(pi/262144) q[16];
cx q[33], q[16];
rz(-pi/262144) q[16];
rz(-pi/131072) q[33];
cx q[33], q[17];
rz(pi/131072) q[17];
cx q[33], q[17];
rz(-pi/131072) q[17];
rz(-pi/65536) q[33];
cx q[33], q[18];
rz(pi/65536) q[18];
cx q[33], q[18];
rz(-pi/65536) q[18];
rz(-pi/32768) q[33];
cx q[33], q[19];
rz(pi/32768) q[19];
cx q[33], q[19];
rz(-pi/32768) q[19];
rz(-pi/16384) q[33];
cx q[33], q[20];
rz(pi/16384) q[20];
cx q[33], q[20];
rz(-pi/16384) q[20];
rz(-pi/8192) q[33];
cx q[33], q[21];
rz(pi/8192) q[21];
cx q[33], q[21];
rz(-pi/8192) q[21];
rz(-pi/4096) q[33];
cx q[33], q[22];
rz(pi/4096) q[22];
cx q[33], q[22];
rz(-pi/4096) q[22];
rz(-pi/2048) q[33];
cx q[33], q[23];
rz(pi/2048) q[23];
cx q[33], q[23];
rz(-pi/2048) q[23];
rz(-pi/1024) q[33];
cx q[33], q[24];
rz(pi/1024) q[24];
cx q[33], q[24];
rz(-pi/1024) q[24];
rz(-pi/512) q[33];
cx q[33], q[25];
rz(pi/512) q[25];
cx q[33], q[25];
rz(-pi/512) q[25];
rz(-pi/256) q[33];
cx q[33], q[26];
rz(pi/256) q[26];
cx q[33], q[26];
rz(-pi/256) q[26];
rz(-pi/128) q[33];
cx q[33], q[27];
rz(pi/128) q[27];
cx q[33], q[27];
rz(-pi/128) q[27];
rz(-pi/64) q[33];
cx q[33], q[28];
rz(pi/64) q[28];
cx q[33], q[28];
rz(-pi/64) q[28];
rz(-pi/32) q[33];
cx q[33], q[29];
rz(pi/32) q[29];
cx q[33], q[29];
rz(-pi/32) q[29];
rz(-pi/16) q[33];
cx q[33], q[30];
rz(pi/16) q[30];
cx q[33], q[30];
rz(-pi/16) q[30];
rz(-pi/8) q[33];
cx q[33], q[31];
rz(pi/8) q[31];
cx q[33], q[31];
rz(-pi/8) q[31];
rz(-pi/4) q[33];
cx q[33], q[32];
rz(pi/4) q[32];
cx q[33], q[32];
rz(-pi/4) q[32];
h q[33];
cx q[34], q[0];
rz(0) q[0];
cx q[34], q[0];
rz(0) q[0];
rz(0) q[34];
cx q[34], q[1];
rz(0) q[1];
cx q[34], q[1];
rz(0) q[1];
rz(0) q[34];
cx q[34], q[2];
rz(0) q[2];
cx q[34], q[2];
rz(0) q[2];
rz(0) q[34];
cx q[34], q[3];
rz(0) q[3];
cx q[34], q[3];
rz(0) q[3];
rz(-pi/2147483648) q[34];
cx q[34], q[4];
rz(pi/2147483648) q[4];
cx q[34], q[4];
rz(-pi/2147483648) q[4];
rz(-pi/1073741824) q[34];
cx q[34], q[5];
rz(pi/1073741824) q[5];
cx q[34], q[5];
rz(-pi/1073741824) q[5];
rz(-pi/536870912) q[34];
cx q[34], q[6];
rz(pi/536870912) q[6];
cx q[34], q[6];
rz(-pi/536870912) q[6];
rz(-pi/268435456) q[34];
cx q[34], q[7];
rz(pi/268435456) q[7];
cx q[34], q[7];
rz(-pi/268435456) q[7];
rz(-pi/134217728) q[34];
cx q[34], q[8];
rz(pi/134217728) q[8];
cx q[34], q[8];
rz(-pi/134217728) q[8];
rz(-pi/67108864) q[34];
cx q[34], q[9];
rz(pi/67108864) q[9];
cx q[34], q[9];
rz(-pi/67108864) q[9];
rz(-pi/33554432) q[34];
cx q[34], q[10];
rz(pi/33554432) q[10];
cx q[34], q[10];
rz(-pi/33554432) q[10];
rz(-pi/16777216) q[34];
cx q[34], q[11];
rz(pi/16777216) q[11];
cx q[34], q[11];
rz(-pi/16777216) q[11];
rz(-pi/8388608) q[34];
cx q[34], q[12];
rz(pi/8388608) q[12];
cx q[34], q[12];
rz(-pi/8388608) q[12];
rz(-pi/4194304) q[34];
cx q[34], q[13];
rz(pi/4194304) q[13];
cx q[34], q[13];
rz(-pi/4194304) q[13];
rz(-pi/2097152) q[34];
cx q[34], q[14];
rz(pi/2097152) q[14];
cx q[34], q[14];
rz(-pi/2097152) q[14];
rz(-pi/1048576) q[34];
cx q[34], q[15];
rz(pi/1048576) q[15];
cx q[34], q[15];
rz(-pi/1048576) q[15];
rz(-pi/524288) q[34];
cx q[34], q[16];
rz(pi/524288) q[16];
cx q[34], q[16];
rz(-pi/524288) q[16];
rz(-pi/262144) q[34];
cx q[34], q[17];
rz(pi/262144) q[17];
cx q[34], q[17];
rz(-pi/262144) q[17];
rz(-pi/131072) q[34];
cx q[34], q[18];
rz(pi/131072) q[18];
cx q[34], q[18];
rz(-pi/131072) q[18];
rz(-pi/65536) q[34];
cx q[34], q[19];
rz(pi/65536) q[19];
cx q[34], q[19];
rz(-pi/65536) q[19];
rz(-pi/32768) q[34];
cx q[34], q[20];
rz(pi/32768) q[20];
cx q[34], q[20];
rz(-pi/32768) q[20];
rz(-pi/16384) q[34];
cx q[34], q[21];
rz(pi/16384) q[21];
cx q[34], q[21];
rz(-pi/16384) q[21];
rz(-pi/8192) q[34];
cx q[34], q[22];
rz(pi/8192) q[22];
cx q[34], q[22];
rz(-pi/8192) q[22];
rz(-pi/4096) q[34];
cx q[34], q[23];
rz(pi/4096) q[23];
cx q[34], q[23];
rz(-pi/4096) q[23];
rz(-pi/2048) q[34];
cx q[34], q[24];
rz(pi/2048) q[24];
cx q[34], q[24];
rz(-pi/2048) q[24];
rz(-pi/1024) q[34];
cx q[34], q[25];
rz(pi/1024) q[25];
cx q[34], q[25];
rz(-pi/1024) q[25];
rz(-pi/512) q[34];
cx q[34], q[26];
rz(pi/512) q[26];
cx q[34], q[26];
rz(-pi/512) q[26];
rz(-pi/256) q[34];
cx q[34], q[27];
rz(pi/256) q[27];
cx q[34], q[27];
rz(-pi/256) q[27];
rz(-pi/128) q[34];
cx q[34], q[28];
rz(pi/128) q[28];
cx q[34], q[28];
rz(-pi/128) q[28];
rz(-pi/64) q[34];
cx q[34], q[29];
rz(pi/64) q[29];
cx q[34], q[29];
rz(-pi/64) q[29];
rz(-pi/32) q[34];
cx q[34], q[30];
rz(pi/32) q[30];
cx q[34], q[30];
rz(-pi/32) q[30];
rz(-pi/16) q[34];
cx q[34], q[31];
rz(pi/16) q[31];
cx q[34], q[31];
rz(-pi/16) q[31];
rz(-pi/8) q[34];
cx q[34], q[32];
rz(pi/8) q[32];
cx q[34], q[32];
rz(-pi/8) q[32];
rz(-pi/4) q[34];
cx q[34], q[33];
rz(pi/4) q[33];
cx q[34], q[33];
rz(-pi/4) q[33];
h q[34];
cx q[35], q[0];
rz(0) q[0];
cx q[35], q[0];
rz(0) q[0];
rz(0) q[35];
cx q[35], q[1];
rz(0) q[1];
cx q[35], q[1];
rz(0) q[1];
rz(0) q[35];
cx q[35], q[2];
rz(0) q[2];
cx q[35], q[2];
rz(0) q[2];
rz(0) q[35];
cx q[35], q[3];
rz(0) q[3];
cx q[35], q[3];
rz(0) q[3];
rz(0) q[35];
cx q[35], q[4];
rz(0) q[4];
cx q[35], q[4];
rz(0) q[4];
rz(-pi/2147483648) q[35];
cx q[35], q[5];
rz(pi/2147483648) q[5];
cx q[35], q[5];
rz(-pi/2147483648) q[5];
rz(-pi/1073741824) q[35];
cx q[35], q[6];
rz(pi/1073741824) q[6];
cx q[35], q[6];
rz(-pi/1073741824) q[6];
rz(-pi/536870912) q[35];
cx q[35], q[7];
rz(pi/536870912) q[7];
cx q[35], q[7];
rz(-pi/536870912) q[7];
rz(-pi/268435456) q[35];
cx q[35], q[8];
rz(pi/268435456) q[8];
cx q[35], q[8];
rz(-pi/268435456) q[8];
rz(-pi/134217728) q[35];
cx q[35], q[9];
rz(pi/134217728) q[9];
cx q[35], q[9];
rz(-pi/134217728) q[9];
rz(-pi/67108864) q[35];
cx q[35], q[10];
rz(pi/67108864) q[10];
cx q[35], q[10];
rz(-pi/67108864) q[10];
rz(-pi/33554432) q[35];
cx q[35], q[11];
rz(pi/33554432) q[11];
cx q[35], q[11];
rz(-pi/33554432) q[11];
rz(-pi/16777216) q[35];
cx q[35], q[12];
rz(pi/16777216) q[12];
cx q[35], q[12];
rz(-pi/16777216) q[12];
rz(-pi/8388608) q[35];
cx q[35], q[13];
rz(pi/8388608) q[13];
cx q[35], q[13];
rz(-pi/8388608) q[13];
rz(-pi/4194304) q[35];
cx q[35], q[14];
rz(pi/4194304) q[14];
cx q[35], q[14];
rz(-pi/4194304) q[14];
rz(-pi/2097152) q[35];
cx q[35], q[15];
rz(pi/2097152) q[15];
cx q[35], q[15];
rz(-pi/2097152) q[15];
rz(-pi/1048576) q[35];
cx q[35], q[16];
rz(pi/1048576) q[16];
cx q[35], q[16];
rz(-pi/1048576) q[16];
rz(-pi/524288) q[35];
cx q[35], q[17];
rz(pi/524288) q[17];
cx q[35], q[17];
rz(-pi/524288) q[17];
rz(-pi/262144) q[35];
cx q[35], q[18];
rz(pi/262144) q[18];
cx q[35], q[18];
rz(-pi/262144) q[18];
rz(-pi/131072) q[35];
cx q[35], q[19];
rz(pi/131072) q[19];
cx q[35], q[19];
rz(-pi/131072) q[19];
rz(-pi/65536) q[35];
cx q[35], q[20];
rz(pi/65536) q[20];
cx q[35], q[20];
rz(-pi/65536) q[20];
rz(-pi/32768) q[35];
cx q[35], q[21];
rz(pi/32768) q[21];
cx q[35], q[21];
rz(-pi/32768) q[21];
rz(-pi/16384) q[35];
cx q[35], q[22];
rz(pi/16384) q[22];
cx q[35], q[22];
rz(-pi/16384) q[22];
rz(-pi/8192) q[35];
cx q[35], q[23];
rz(pi/8192) q[23];
cx q[35], q[23];
rz(-pi/8192) q[23];
rz(-pi/4096) q[35];
cx q[35], q[24];
rz(pi/4096) q[24];
cx q[35], q[24];
rz(-pi/4096) q[24];
rz(-pi/2048) q[35];
cx q[35], q[25];
rz(pi/2048) q[25];
cx q[35], q[25];
rz(-pi/2048) q[25];
rz(-pi/1024) q[35];
cx q[35], q[26];
rz(pi/1024) q[26];
cx q[35], q[26];
rz(-pi/1024) q[26];
rz(-pi/512) q[35];
cx q[35], q[27];
rz(pi/512) q[27];
cx q[35], q[27];
rz(-pi/512) q[27];
rz(-pi/256) q[35];
cx q[35], q[28];
rz(pi/256) q[28];
cx q[35], q[28];
rz(-pi/256) q[28];
rz(-pi/128) q[35];
cx q[35], q[29];
rz(pi/128) q[29];
cx q[35], q[29];
rz(-pi/128) q[29];
rz(-pi/64) q[35];
cx q[35], q[30];
rz(pi/64) q[30];
cx q[35], q[30];
rz(-pi/64) q[30];
rz(-pi/32) q[35];
cx q[35], q[31];
rz(pi/32) q[31];
cx q[35], q[31];
rz(-pi/32) q[31];
rz(-pi/16) q[35];
cx q[35], q[32];
rz(pi/16) q[32];
cx q[35], q[32];
rz(-pi/16) q[32];
rz(-pi/8) q[35];
cx q[35], q[33];
rz(pi/8) q[33];
cx q[35], q[33];
rz(-pi/8) q[33];
rz(-pi/4) q[35];
cx q[35], q[34];
rz(pi/4) q[34];
cx q[35], q[34];
rz(-pi/4) q[34];
h q[35];
cx q[36], q[0];
rz(0) q[0];
cx q[36], q[0];
rz(0) q[0];
rz(0) q[36];
cx q[36], q[1];
rz(0) q[1];
cx q[36], q[1];
rz(0) q[1];
rz(0) q[36];
cx q[36], q[2];
rz(0) q[2];
cx q[36], q[2];
rz(0) q[2];
rz(0) q[36];
cx q[36], q[3];
rz(0) q[3];
cx q[36], q[3];
rz(0) q[3];
rz(0) q[36];
cx q[36], q[4];
rz(0) q[4];
cx q[36], q[4];
rz(0) q[4];
rz(0) q[36];
cx q[36], q[5];
rz(0) q[5];
cx q[36], q[5];
rz(0) q[5];
rz(-pi/2147483648) q[36];
cx q[36], q[6];
rz(pi/2147483648) q[6];
cx q[36], q[6];
rz(-pi/2147483648) q[6];
rz(-pi/1073741824) q[36];
cx q[36], q[7];
rz(pi/1073741824) q[7];
cx q[36], q[7];
rz(-pi/1073741824) q[7];
rz(-pi/536870912) q[36];
cx q[36], q[8];
rz(pi/536870912) q[8];
cx q[36], q[8];
rz(-pi/536870912) q[8];
rz(-pi/268435456) q[36];
cx q[36], q[9];
rz(pi/268435456) q[9];
cx q[36], q[9];
rz(-pi/268435456) q[9];
rz(-pi/134217728) q[36];
cx q[36], q[10];
rz(pi/134217728) q[10];
cx q[36], q[10];
rz(-pi/134217728) q[10];
rz(-pi/67108864) q[36];
cx q[36], q[11];
rz(pi/67108864) q[11];
cx q[36], q[11];
rz(-pi/67108864) q[11];
rz(-pi/33554432) q[36];
cx q[36], q[12];
rz(pi/33554432) q[12];
cx q[36], q[12];
rz(-pi/33554432) q[12];
rz(-pi/16777216) q[36];
cx q[36], q[13];
rz(pi/16777216) q[13];
cx q[36], q[13];
rz(-pi/16777216) q[13];
rz(-pi/8388608) q[36];
cx q[36], q[14];
rz(pi/8388608) q[14];
cx q[36], q[14];
rz(-pi/8388608) q[14];
rz(-pi/4194304) q[36];
cx q[36], q[15];
rz(pi/4194304) q[15];
cx q[36], q[15];
rz(-pi/4194304) q[15];
rz(-pi/2097152) q[36];
cx q[36], q[16];
rz(pi/2097152) q[16];
cx q[36], q[16];
rz(-pi/2097152) q[16];
rz(-pi/1048576) q[36];
cx q[36], q[17];
rz(pi/1048576) q[17];
cx q[36], q[17];
rz(-pi/1048576) q[17];
rz(-pi/524288) q[36];
cx q[36], q[18];
rz(pi/524288) q[18];
cx q[36], q[18];
rz(-pi/524288) q[18];
rz(-pi/262144) q[36];
cx q[36], q[19];
rz(pi/262144) q[19];
cx q[36], q[19];
rz(-pi/262144) q[19];
rz(-pi/131072) q[36];
cx q[36], q[20];
rz(pi/131072) q[20];
cx q[36], q[20];
rz(-pi/131072) q[20];
rz(-pi/65536) q[36];
cx q[36], q[21];
rz(pi/65536) q[21];
cx q[36], q[21];
rz(-pi/65536) q[21];
rz(-pi/32768) q[36];
cx q[36], q[22];
rz(pi/32768) q[22];
cx q[36], q[22];
rz(-pi/32768) q[22];
rz(-pi/16384) q[36];
cx q[36], q[23];
rz(pi/16384) q[23];
cx q[36], q[23];
rz(-pi/16384) q[23];
rz(-pi/8192) q[36];
cx q[36], q[24];
rz(pi/8192) q[24];
cx q[36], q[24];
rz(-pi/8192) q[24];
rz(-pi/4096) q[36];
cx q[36], q[25];
rz(pi/4096) q[25];
cx q[36], q[25];
rz(-pi/4096) q[25];
rz(-pi/2048) q[36];
cx q[36], q[26];
rz(pi/2048) q[26];
cx q[36], q[26];
rz(-pi/2048) q[26];
rz(-pi/1024) q[36];
cx q[36], q[27];
rz(pi/1024) q[27];
cx q[36], q[27];
rz(-pi/1024) q[27];
rz(-pi/512) q[36];
cx q[36], q[28];
rz(pi/512) q[28];
cx q[36], q[28];
rz(-pi/512) q[28];
rz(-pi/256) q[36];
cx q[36], q[29];
rz(pi/256) q[29];
cx q[36], q[29];
rz(-pi/256) q[29];
rz(-pi/128) q[36];
cx q[36], q[30];
rz(pi/128) q[30];
cx q[36], q[30];
rz(-pi/128) q[30];
rz(-pi/64) q[36];
cx q[36], q[31];
rz(pi/64) q[31];
cx q[36], q[31];
rz(-pi/64) q[31];
rz(-pi/32) q[36];
cx q[36], q[32];
rz(pi/32) q[32];
cx q[36], q[32];
rz(-pi/32) q[32];
rz(-pi/16) q[36];
cx q[36], q[33];
rz(pi/16) q[33];
cx q[36], q[33];
rz(-pi/16) q[33];
rz(-pi/8) q[36];
cx q[36], q[34];
rz(pi/8) q[34];
cx q[36], q[34];
rz(-pi/8) q[34];
rz(-pi/4) q[36];
cx q[36], q[35];
rz(pi/4) q[35];
cx q[36], q[35];
rz(-pi/4) q[35];
h q[36];
cx q[37], q[0];
rz(0) q[0];
cx q[37], q[0];
rz(0) q[0];
rz(0) q[37];
cx q[37], q[1];
rz(0) q[1];
cx q[37], q[1];
rz(0) q[1];
rz(0) q[37];
cx q[37], q[2];
rz(0) q[2];
cx q[37], q[2];
rz(0) q[2];
rz(0) q[37];
cx q[37], q[3];
rz(0) q[3];
cx q[37], q[3];
rz(0) q[3];
rz(0) q[37];
cx q[37], q[4];
rz(0) q[4];
cx q[37], q[4];
rz(0) q[4];
rz(0) q[37];
cx q[37], q[5];
rz(0) q[5];
cx q[37], q[5];
rz(0) q[5];
rz(0) q[37];
cx q[37], q[6];
rz(0) q[6];
cx q[37], q[6];
rz(0) q[6];
rz(-pi/2147483648) q[37];
cx q[37], q[7];
rz(pi/2147483648) q[7];
cx q[37], q[7];
rz(-pi/2147483648) q[7];
rz(-pi/1073741824) q[37];
cx q[37], q[8];
rz(pi/1073741824) q[8];
cx q[37], q[8];
rz(-pi/1073741824) q[8];
rz(-pi/536870912) q[37];
cx q[37], q[9];
rz(pi/536870912) q[9];
cx q[37], q[9];
rz(-pi/536870912) q[9];
rz(-pi/268435456) q[37];
cx q[37], q[10];
rz(pi/268435456) q[10];
cx q[37], q[10];
rz(-pi/268435456) q[10];
rz(-pi/134217728) q[37];
cx q[37], q[11];
rz(pi/134217728) q[11];
cx q[37], q[11];
rz(-pi/134217728) q[11];
rz(-pi/67108864) q[37];
cx q[37], q[12];
rz(pi/67108864) q[12];
cx q[37], q[12];
rz(-pi/67108864) q[12];
rz(-pi/33554432) q[37];
cx q[37], q[13];
rz(pi/33554432) q[13];
cx q[37], q[13];
rz(-pi/33554432) q[13];
rz(-pi/16777216) q[37];
cx q[37], q[14];
rz(pi/16777216) q[14];
cx q[37], q[14];
rz(-pi/16777216) q[14];
rz(-pi/8388608) q[37];
cx q[37], q[15];
rz(pi/8388608) q[15];
cx q[37], q[15];
rz(-pi/8388608) q[15];
rz(-pi/4194304) q[37];
cx q[37], q[16];
rz(pi/4194304) q[16];
cx q[37], q[16];
rz(-pi/4194304) q[16];
rz(-pi/2097152) q[37];
cx q[37], q[17];
rz(pi/2097152) q[17];
cx q[37], q[17];
rz(-pi/2097152) q[17];
rz(-pi/1048576) q[37];
cx q[37], q[18];
rz(pi/1048576) q[18];
cx q[37], q[18];
rz(-pi/1048576) q[18];
rz(-pi/524288) q[37];
cx q[37], q[19];
rz(pi/524288) q[19];
cx q[37], q[19];
rz(-pi/524288) q[19];
rz(-pi/262144) q[37];
cx q[37], q[20];
rz(pi/262144) q[20];
cx q[37], q[20];
rz(-pi/262144) q[20];
rz(-pi/131072) q[37];
cx q[37], q[21];
rz(pi/131072) q[21];
cx q[37], q[21];
rz(-pi/131072) q[21];
rz(-pi/65536) q[37];
cx q[37], q[22];
rz(pi/65536) q[22];
cx q[37], q[22];
rz(-pi/65536) q[22];
rz(-pi/32768) q[37];
cx q[37], q[23];
rz(pi/32768) q[23];
cx q[37], q[23];
rz(-pi/32768) q[23];
rz(-pi/16384) q[37];
cx q[37], q[24];
rz(pi/16384) q[24];
cx q[37], q[24];
rz(-pi/16384) q[24];
rz(-pi/8192) q[37];
cx q[37], q[25];
rz(pi/8192) q[25];
cx q[37], q[25];
rz(-pi/8192) q[25];
rz(-pi/4096) q[37];
cx q[37], q[26];
rz(pi/4096) q[26];
cx q[37], q[26];
rz(-pi/4096) q[26];
rz(-pi/2048) q[37];
cx q[37], q[27];
rz(pi/2048) q[27];
cx q[37], q[27];
rz(-pi/2048) q[27];
rz(-pi/1024) q[37];
cx q[37], q[28];
rz(pi/1024) q[28];
cx q[37], q[28];
rz(-pi/1024) q[28];
rz(-pi/512) q[37];
cx q[37], q[29];
rz(pi/512) q[29];
cx q[37], q[29];
rz(-pi/512) q[29];
rz(-pi/256) q[37];
cx q[37], q[30];
rz(pi/256) q[30];
cx q[37], q[30];
rz(-pi/256) q[30];
rz(-pi/128) q[37];
cx q[37], q[31];
rz(pi/128) q[31];
cx q[37], q[31];
rz(-pi/128) q[31];
rz(-pi/64) q[37];
cx q[37], q[32];
rz(pi/64) q[32];
cx q[37], q[32];
rz(-pi/64) q[32];
rz(-pi/32) q[37];
cx q[37], q[33];
rz(pi/32) q[33];
cx q[37], q[33];
rz(-pi/32) q[33];
rz(-pi/16) q[37];
cx q[37], q[34];
rz(pi/16) q[34];
cx q[37], q[34];
rz(-pi/16) q[34];
rz(-pi/8) q[37];
cx q[37], q[35];
rz(pi/8) q[35];
cx q[37], q[35];
rz(-pi/8) q[35];
rz(-pi/4) q[37];
cx q[37], q[36];
rz(pi/4) q[36];
cx q[37], q[36];
rz(-pi/4) q[36];
h q[37];
cx q[38], q[0];
rz(0) q[0];
cx q[38], q[0];
rz(0) q[0];
rz(0) q[38];
cx q[38], q[1];
rz(0) q[1];
cx q[38], q[1];
rz(0) q[1];
rz(0) q[38];
cx q[38], q[2];
rz(0) q[2];
cx q[38], q[2];
rz(0) q[2];
rz(0) q[38];
cx q[38], q[3];
rz(0) q[3];
cx q[38], q[3];
rz(0) q[3];
rz(0) q[38];
cx q[38], q[4];
rz(0) q[4];
cx q[38], q[4];
rz(0) q[4];
rz(0) q[38];
cx q[38], q[5];
rz(0) q[5];
cx q[38], q[5];
rz(0) q[5];
rz(0) q[38];
cx q[38], q[6];
rz(0) q[6];
cx q[38], q[6];
rz(0) q[6];
rz(0) q[38];
cx q[38], q[7];
rz(0) q[7];
cx q[38], q[7];
rz(0) q[7];
rz(-pi/2147483648) q[38];
cx q[38], q[8];
rz(pi/2147483648) q[8];
cx q[38], q[8];
rz(-pi/2147483648) q[8];
rz(-pi/1073741824) q[38];
cx q[38], q[9];
rz(pi/1073741824) q[9];
cx q[38], q[9];
rz(-pi/1073741824) q[9];
rz(-pi/536870912) q[38];
cx q[38], q[10];
rz(pi/536870912) q[10];
cx q[38], q[10];
rz(-pi/536870912) q[10];
rz(-pi/268435456) q[38];
cx q[38], q[11];
rz(pi/268435456) q[11];
cx q[38], q[11];
rz(-pi/268435456) q[11];
rz(-pi/134217728) q[38];
cx q[38], q[12];
rz(pi/134217728) q[12];
cx q[38], q[12];
rz(-pi/134217728) q[12];
rz(-pi/67108864) q[38];
cx q[38], q[13];
rz(pi/67108864) q[13];
cx q[38], q[13];
rz(-pi/67108864) q[13];
rz(-pi/33554432) q[38];
cx q[38], q[14];
rz(pi/33554432) q[14];
cx q[38], q[14];
rz(-pi/33554432) q[14];
rz(-pi/16777216) q[38];
cx q[38], q[15];
rz(pi/16777216) q[15];
cx q[38], q[15];
rz(-pi/16777216) q[15];
rz(-pi/8388608) q[38];
cx q[38], q[16];
rz(pi/8388608) q[16];
cx q[38], q[16];
rz(-pi/8388608) q[16];
rz(-pi/4194304) q[38];
cx q[38], q[17];
rz(pi/4194304) q[17];
cx q[38], q[17];
rz(-pi/4194304) q[17];
rz(-pi/2097152) q[38];
cx q[38], q[18];
rz(pi/2097152) q[18];
cx q[38], q[18];
rz(-pi/2097152) q[18];
rz(-pi/1048576) q[38];
cx q[38], q[19];
rz(pi/1048576) q[19];
cx q[38], q[19];
rz(-pi/1048576) q[19];
rz(-pi/524288) q[38];
cx q[38], q[20];
rz(pi/524288) q[20];
cx q[38], q[20];
rz(-pi/524288) q[20];
rz(-pi/262144) q[38];
cx q[38], q[21];
rz(pi/262144) q[21];
cx q[38], q[21];
rz(-pi/262144) q[21];
rz(-pi/131072) q[38];
cx q[38], q[22];
rz(pi/131072) q[22];
cx q[38], q[22];
rz(-pi/131072) q[22];
rz(-pi/65536) q[38];
cx q[38], q[23];
rz(pi/65536) q[23];
cx q[38], q[23];
rz(-pi/65536) q[23];
rz(-pi/32768) q[38];
cx q[38], q[24];
rz(pi/32768) q[24];
cx q[38], q[24];
rz(-pi/32768) q[24];
rz(-pi/16384) q[38];
cx q[38], q[25];
rz(pi/16384) q[25];
cx q[38], q[25];
rz(-pi/16384) q[25];
rz(-pi/8192) q[38];
cx q[38], q[26];
rz(pi/8192) q[26];
cx q[38], q[26];
rz(-pi/8192) q[26];
rz(-pi/4096) q[38];
cx q[38], q[27];
rz(pi/4096) q[27];
cx q[38], q[27];
rz(-pi/4096) q[27];
rz(-pi/2048) q[38];
cx q[38], q[28];
rz(pi/2048) q[28];
cx q[38], q[28];
rz(-pi/2048) q[28];
rz(-pi/1024) q[38];
cx q[38], q[29];
rz(pi/1024) q[29];
cx q[38], q[29];
rz(-pi/1024) q[29];
rz(-pi/512) q[38];
cx q[38], q[30];
rz(pi/512) q[30];
cx q[38], q[30];
rz(-pi/512) q[30];
rz(-pi/256) q[38];
cx q[38], q[31];
rz(pi/256) q[31];
cx q[38], q[31];
rz(-pi/256) q[31];
rz(-pi/128) q[38];
cx q[38], q[32];
rz(pi/128) q[32];
cx q[38], q[32];
rz(-pi/128) q[32];
rz(-pi/64) q[38];
cx q[38], q[33];
rz(pi/64) q[33];
cx q[38], q[33];
rz(-pi/64) q[33];
rz(-pi/32) q[38];
cx q[38], q[34];
rz(pi/32) q[34];
cx q[38], q[34];
rz(-pi/32) q[34];
rz(-pi/16) q[38];
cx q[38], q[35];
rz(pi/16) q[35];
cx q[38], q[35];
rz(-pi/16) q[35];
rz(-pi/8) q[38];
cx q[38], q[36];
rz(pi/8) q[36];
cx q[38], q[36];
rz(-pi/8) q[36];
rz(-pi/4) q[38];
cx q[38], q[37];
rz(pi/4) q[37];
cx q[38], q[37];
rz(-pi/4) q[37];
h q[38];
cx q[39], q[0];
rz(0) q[0];
cx q[39], q[0];
rz(0) q[0];
rz(0) q[39];
cx q[39], q[1];
rz(0) q[1];
cx q[39], q[1];
rz(0) q[1];
rz(0) q[39];
cx q[39], q[2];
rz(0) q[2];
cx q[39], q[2];
rz(0) q[2];
rz(0) q[39];
cx q[39], q[3];
rz(0) q[3];
cx q[39], q[3];
rz(0) q[3];
rz(0) q[39];
cx q[39], q[4];
rz(0) q[4];
cx q[39], q[4];
rz(0) q[4];
rz(0) q[39];
cx q[39], q[5];
rz(0) q[5];
cx q[39], q[5];
rz(0) q[5];
rz(0) q[39];
cx q[39], q[6];
rz(0) q[6];
cx q[39], q[6];
rz(0) q[6];
rz(0) q[39];
cx q[39], q[7];
rz(0) q[7];
cx q[39], q[7];
rz(0) q[7];
rz(0) q[39];
cx q[39], q[8];
rz(0) q[8];
cx q[39], q[8];
rz(0) q[8];
rz(-pi/2147483648) q[39];
cx q[39], q[9];
rz(pi/2147483648) q[9];
cx q[39], q[9];
rz(-pi/2147483648) q[9];
rz(-pi/1073741824) q[39];
cx q[39], q[10];
rz(pi/1073741824) q[10];
cx q[39], q[10];
rz(-pi/1073741824) q[10];
rz(-pi/536870912) q[39];
cx q[39], q[11];
rz(pi/536870912) q[11];
cx q[39], q[11];
rz(-pi/536870912) q[11];
rz(-pi/268435456) q[39];
cx q[39], q[12];
rz(pi/268435456) q[12];
cx q[39], q[12];
rz(-pi/268435456) q[12];
rz(-pi/134217728) q[39];
cx q[39], q[13];
rz(pi/134217728) q[13];
cx q[39], q[13];
rz(-pi/134217728) q[13];
rz(-pi/67108864) q[39];
cx q[39], q[14];
rz(pi/67108864) q[14];
cx q[39], q[14];
rz(-pi/67108864) q[14];
rz(-pi/33554432) q[39];
cx q[39], q[15];
rz(pi/33554432) q[15];
cx q[39], q[15];
rz(-pi/33554432) q[15];
rz(-pi/16777216) q[39];
cx q[39], q[16];
rz(pi/16777216) q[16];
cx q[39], q[16];
rz(-pi/16777216) q[16];
rz(-pi/8388608) q[39];
cx q[39], q[17];
rz(pi/8388608) q[17];
cx q[39], q[17];
rz(-pi/8388608) q[17];
rz(-pi/4194304) q[39];
cx q[39], q[18];
rz(pi/4194304) q[18];
cx q[39], q[18];
rz(-pi/4194304) q[18];
rz(-pi/2097152) q[39];
cx q[39], q[19];
rz(pi/2097152) q[19];
cx q[39], q[19];
rz(-pi/2097152) q[19];
rz(-pi/1048576) q[39];
cx q[39], q[20];
rz(pi/1048576) q[20];
cx q[39], q[20];
rz(-pi/1048576) q[20];
rz(-pi/524288) q[39];
cx q[39], q[21];
rz(pi/524288) q[21];
cx q[39], q[21];
rz(-pi/524288) q[21];
rz(-pi/262144) q[39];
cx q[39], q[22];
rz(pi/262144) q[22];
cx q[39], q[22];
rz(-pi/262144) q[22];
rz(-pi/131072) q[39];
cx q[39], q[23];
rz(pi/131072) q[23];
cx q[39], q[23];
rz(-pi/131072) q[23];
rz(-pi/65536) q[39];
cx q[39], q[24];
rz(pi/65536) q[24];
cx q[39], q[24];
rz(-pi/65536) q[24];
rz(-pi/32768) q[39];
cx q[39], q[25];
rz(pi/32768) q[25];
cx q[39], q[25];
rz(-pi/32768) q[25];
rz(-pi/16384) q[39];
cx q[39], q[26];
rz(pi/16384) q[26];
cx q[39], q[26];
rz(-pi/16384) q[26];
rz(-pi/8192) q[39];
cx q[39], q[27];
rz(pi/8192) q[27];
cx q[39], q[27];
rz(-pi/8192) q[27];
rz(-pi/4096) q[39];
cx q[39], q[28];
rz(pi/4096) q[28];
cx q[39], q[28];
rz(-pi/4096) q[28];
rz(-pi/2048) q[39];
cx q[39], q[29];
rz(pi/2048) q[29];
cx q[39], q[29];
rz(-pi/2048) q[29];
rz(-pi/1024) q[39];
cx q[39], q[30];
rz(pi/1024) q[30];
cx q[39], q[30];
rz(-pi/1024) q[30];
rz(-pi/512) q[39];
cx q[39], q[31];
rz(pi/512) q[31];
cx q[39], q[31];
rz(-pi/512) q[31];
rz(-pi/256) q[39];
cx q[39], q[32];
rz(pi/256) q[32];
cx q[39], q[32];
rz(-pi/256) q[32];
rz(-pi/128) q[39];
cx q[39], q[33];
rz(pi/128) q[33];
cx q[39], q[33];
rz(-pi/128) q[33];
rz(-pi/64) q[39];
cx q[39], q[34];
rz(pi/64) q[34];
cx q[39], q[34];
rz(-pi/64) q[34];
rz(-pi/32) q[39];
cx q[39], q[35];
rz(pi/32) q[35];
cx q[39], q[35];
rz(-pi/32) q[35];
rz(-pi/16) q[39];
cx q[39], q[36];
rz(pi/16) q[36];
cx q[39], q[36];
rz(-pi/16) q[36];
rz(-pi/8) q[39];
cx q[39], q[37];
rz(pi/8) q[37];
cx q[39], q[37];
rz(-pi/8) q[37];
rz(-pi/4) q[39];
cx q[39], q[38];
rz(pi/4) q[38];
cx q[39], q[38];
rz(-pi/4) q[38];
h q[39];
cx q[40], q[0];
rz(0) q[0];
cx q[40], q[0];
rz(0) q[0];
rz(0) q[40];
cx q[40], q[1];
rz(0) q[1];
cx q[40], q[1];
rz(0) q[1];
rz(0) q[40];
cx q[40], q[2];
rz(0) q[2];
cx q[40], q[2];
rz(0) q[2];
rz(0) q[40];
cx q[40], q[3];
rz(0) q[3];
cx q[40], q[3];
rz(0) q[3];
rz(0) q[40];
cx q[40], q[4];
rz(0) q[4];
cx q[40], q[4];
rz(0) q[4];
rz(0) q[40];
cx q[40], q[5];
rz(0) q[5];
cx q[40], q[5];
rz(0) q[5];
rz(0) q[40];
cx q[40], q[6];
rz(0) q[6];
cx q[40], q[6];
rz(0) q[6];
rz(0) q[40];
cx q[40], q[7];
rz(0) q[7];
cx q[40], q[7];
rz(0) q[7];
rz(0) q[40];
cx q[40], q[8];
rz(0) q[8];
cx q[40], q[8];
rz(0) q[8];
rz(0) q[40];
cx q[40], q[9];
rz(0) q[9];
cx q[40], q[9];
rz(0) q[9];
rz(-pi/2147483648) q[40];
cx q[40], q[10];
rz(pi/2147483648) q[10];
cx q[40], q[10];
rz(-pi/2147483648) q[10];
rz(-pi/1073741824) q[40];
cx q[40], q[11];
rz(pi/1073741824) q[11];
cx q[40], q[11];
rz(-pi/1073741824) q[11];
rz(-pi/536870912) q[40];
cx q[40], q[12];
rz(pi/536870912) q[12];
cx q[40], q[12];
rz(-pi/536870912) q[12];
rz(-pi/268435456) q[40];
cx q[40], q[13];
rz(pi/268435456) q[13];
cx q[40], q[13];
rz(-pi/268435456) q[13];
rz(-pi/134217728) q[40];
cx q[40], q[14];
rz(pi/134217728) q[14];
cx q[40], q[14];
rz(-pi/134217728) q[14];
rz(-pi/67108864) q[40];
cx q[40], q[15];
rz(pi/67108864) q[15];
cx q[40], q[15];
rz(-pi/67108864) q[15];
rz(-pi/33554432) q[40];
cx q[40], q[16];
rz(pi/33554432) q[16];
cx q[40], q[16];
rz(-pi/33554432) q[16];
rz(-pi/16777216) q[40];
cx q[40], q[17];
rz(pi/16777216) q[17];
cx q[40], q[17];
rz(-pi/16777216) q[17];
rz(-pi/8388608) q[40];
cx q[40], q[18];
rz(pi/8388608) q[18];
cx q[40], q[18];
rz(-pi/8388608) q[18];
rz(-pi/4194304) q[40];
cx q[40], q[19];
rz(pi/4194304) q[19];
cx q[40], q[19];
rz(-pi/4194304) q[19];
rz(-pi/2097152) q[40];
cx q[40], q[20];
rz(pi/2097152) q[20];
cx q[40], q[20];
rz(-pi/2097152) q[20];
rz(-pi/1048576) q[40];
cx q[40], q[21];
rz(pi/1048576) q[21];
cx q[40], q[21];
rz(-pi/1048576) q[21];
rz(-pi/524288) q[40];
cx q[40], q[22];
rz(pi/524288) q[22];
cx q[40], q[22];
rz(-pi/524288) q[22];
rz(-pi/262144) q[40];
cx q[40], q[23];
rz(pi/262144) q[23];
cx q[40], q[23];
rz(-pi/262144) q[23];
rz(-pi/131072) q[40];
cx q[40], q[24];
rz(pi/131072) q[24];
cx q[40], q[24];
rz(-pi/131072) q[24];
rz(-pi/65536) q[40];
cx q[40], q[25];
rz(pi/65536) q[25];
cx q[40], q[25];
rz(-pi/65536) q[25];
rz(-pi/32768) q[40];
cx q[40], q[26];
rz(pi/32768) q[26];
cx q[40], q[26];
rz(-pi/32768) q[26];
rz(-pi/16384) q[40];
cx q[40], q[27];
rz(pi/16384) q[27];
cx q[40], q[27];
rz(-pi/16384) q[27];
rz(-pi/8192) q[40];
cx q[40], q[28];
rz(pi/8192) q[28];
cx q[40], q[28];
rz(-pi/8192) q[28];
rz(-pi/4096) q[40];
cx q[40], q[29];
rz(pi/4096) q[29];
cx q[40], q[29];
rz(-pi/4096) q[29];
rz(-pi/2048) q[40];
cx q[40], q[30];
rz(pi/2048) q[30];
cx q[40], q[30];
rz(-pi/2048) q[30];
rz(-pi/1024) q[40];
cx q[40], q[31];
rz(pi/1024) q[31];
cx q[40], q[31];
rz(-pi/1024) q[31];
rz(-pi/512) q[40];
cx q[40], q[32];
rz(pi/512) q[32];
cx q[40], q[32];
rz(-pi/512) q[32];
rz(-pi/256) q[40];
cx q[40], q[33];
rz(pi/256) q[33];
cx q[40], q[33];
rz(-pi/256) q[33];
rz(-pi/128) q[40];
cx q[40], q[34];
rz(pi/128) q[34];
cx q[40], q[34];
rz(-pi/128) q[34];
rz(-pi/64) q[40];
cx q[40], q[35];
rz(pi/64) q[35];
cx q[40], q[35];
rz(-pi/64) q[35];
rz(-pi/32) q[40];
cx q[40], q[36];
rz(pi/32) q[36];
cx q[40], q[36];
rz(-pi/32) q[36];
rz(-pi/16) q[40];
cx q[40], q[37];
rz(pi/16) q[37];
cx q[40], q[37];
rz(-pi/16) q[37];
rz(-pi/8) q[40];
cx q[40], q[38];
rz(pi/8) q[38];
cx q[40], q[38];
rz(-pi/8) q[38];
rz(-pi/4) q[40];
cx q[40], q[39];
rz(pi/4) q[39];
cx q[40], q[39];
rz(-pi/4) q[39];
h q[40];
cx q[41], q[0];
rz(0) q[0];
cx q[41], q[0];
rz(0) q[0];
rz(0) q[41];
cx q[41], q[1];
rz(0) q[1];
cx q[41], q[1];
rz(0) q[1];
rz(0) q[41];
cx q[41], q[2];
rz(0) q[2];
cx q[41], q[2];
rz(0) q[2];
rz(0) q[41];
cx q[41], q[3];
rz(0) q[3];
cx q[41], q[3];
rz(0) q[3];
rz(0) q[41];
cx q[41], q[4];
rz(0) q[4];
cx q[41], q[4];
rz(0) q[4];
rz(0) q[41];
cx q[41], q[5];
rz(0) q[5];
cx q[41], q[5];
rz(0) q[5];
rz(0) q[41];
cx q[41], q[6];
rz(0) q[6];
cx q[41], q[6];
rz(0) q[6];
rz(0) q[41];
cx q[41], q[7];
rz(0) q[7];
cx q[41], q[7];
rz(0) q[7];
rz(0) q[41];
cx q[41], q[8];
rz(0) q[8];
cx q[41], q[8];
rz(0) q[8];
rz(0) q[41];
cx q[41], q[9];
rz(0) q[9];
cx q[41], q[9];
rz(0) q[9];
rz(0) q[41];
cx q[41], q[10];
rz(0) q[10];
cx q[41], q[10];
rz(0) q[10];
rz(-pi/2147483648) q[41];
cx q[41], q[11];
rz(pi/2147483648) q[11];
cx q[41], q[11];
rz(-pi/2147483648) q[11];
rz(-pi/1073741824) q[41];
cx q[41], q[12];
rz(pi/1073741824) q[12];
cx q[41], q[12];
rz(-pi/1073741824) q[12];
rz(-pi/536870912) q[41];
cx q[41], q[13];
rz(pi/536870912) q[13];
cx q[41], q[13];
rz(-pi/536870912) q[13];
rz(-pi/268435456) q[41];
cx q[41], q[14];
rz(pi/268435456) q[14];
cx q[41], q[14];
rz(-pi/268435456) q[14];
rz(-pi/134217728) q[41];
cx q[41], q[15];
rz(pi/134217728) q[15];
cx q[41], q[15];
rz(-pi/134217728) q[15];
rz(-pi/67108864) q[41];
cx q[41], q[16];
rz(pi/67108864) q[16];
cx q[41], q[16];
rz(-pi/67108864) q[16];
rz(-pi/33554432) q[41];
cx q[41], q[17];
rz(pi/33554432) q[17];
cx q[41], q[17];
rz(-pi/33554432) q[17];
rz(-pi/16777216) q[41];
cx q[41], q[18];
rz(pi/16777216) q[18];
cx q[41], q[18];
rz(-pi/16777216) q[18];
rz(-pi/8388608) q[41];
cx q[41], q[19];
rz(pi/8388608) q[19];
cx q[41], q[19];
rz(-pi/8388608) q[19];
rz(-pi/4194304) q[41];
cx q[41], q[20];
rz(pi/4194304) q[20];
cx q[41], q[20];
rz(-pi/4194304) q[20];
rz(-pi/2097152) q[41];
cx q[41], q[21];
rz(pi/2097152) q[21];
cx q[41], q[21];
rz(-pi/2097152) q[21];
rz(-pi/1048576) q[41];
cx q[41], q[22];
rz(pi/1048576) q[22];
cx q[41], q[22];
rz(-pi/1048576) q[22];
rz(-pi/524288) q[41];
cx q[41], q[23];
rz(pi/524288) q[23];
cx q[41], q[23];
rz(-pi/524288) q[23];
rz(-pi/262144) q[41];
cx q[41], q[24];
rz(pi/262144) q[24];
cx q[41], q[24];
rz(-pi/262144) q[24];
rz(-pi/131072) q[41];
cx q[41], q[25];
rz(pi/131072) q[25];
cx q[41], q[25];
rz(-pi/131072) q[25];
rz(-pi/65536) q[41];
cx q[41], q[26];
rz(pi/65536) q[26];
cx q[41], q[26];
rz(-pi/65536) q[26];
rz(-pi/32768) q[41];
cx q[41], q[27];
rz(pi/32768) q[27];
cx q[41], q[27];
rz(-pi/32768) q[27];
rz(-pi/16384) q[41];
cx q[41], q[28];
rz(pi/16384) q[28];
cx q[41], q[28];
rz(-pi/16384) q[28];
rz(-pi/8192) q[41];
cx q[41], q[29];
rz(pi/8192) q[29];
cx q[41], q[29];
rz(-pi/8192) q[29];
rz(-pi/4096) q[41];
cx q[41], q[30];
rz(pi/4096) q[30];
cx q[41], q[30];
rz(-pi/4096) q[30];
rz(-pi/2048) q[41];
cx q[41], q[31];
rz(pi/2048) q[31];
cx q[41], q[31];
rz(-pi/2048) q[31];
rz(-pi/1024) q[41];
cx q[41], q[32];
rz(pi/1024) q[32];
cx q[41], q[32];
rz(-pi/1024) q[32];
rz(-pi/512) q[41];
cx q[41], q[33];
rz(pi/512) q[33];
cx q[41], q[33];
rz(-pi/512) q[33];
rz(-pi/256) q[41];
cx q[41], q[34];
rz(pi/256) q[34];
cx q[41], q[34];
rz(-pi/256) q[34];
rz(-pi/128) q[41];
cx q[41], q[35];
rz(pi/128) q[35];
cx q[41], q[35];
rz(-pi/128) q[35];
rz(-pi/64) q[41];
cx q[41], q[36];
rz(pi/64) q[36];
cx q[41], q[36];
rz(-pi/64) q[36];
rz(-pi/32) q[41];
cx q[41], q[37];
rz(pi/32) q[37];
cx q[41], q[37];
rz(-pi/32) q[37];
rz(-pi/16) q[41];
cx q[41], q[38];
rz(pi/16) q[38];
cx q[41], q[38];
rz(-pi/16) q[38];
rz(-pi/8) q[41];
cx q[41], q[39];
rz(pi/8) q[39];
cx q[41], q[39];
rz(-pi/8) q[39];
rz(-pi/4) q[41];
cx q[41], q[40];
rz(pi/4) q[40];
cx q[41], q[40];
rz(-pi/4) q[40];
h q[41];
cx q[42], q[0];
rz(0) q[0];
cx q[42], q[0];
rz(0) q[0];
rz(0) q[42];
cx q[42], q[1];
rz(0) q[1];
cx q[42], q[1];
rz(0) q[1];
rz(0) q[42];
cx q[42], q[2];
rz(0) q[2];
cx q[42], q[2];
rz(0) q[2];
rz(0) q[42];
cx q[42], q[3];
rz(0) q[3];
cx q[42], q[3];
rz(0) q[3];
rz(0) q[42];
cx q[42], q[4];
rz(0) q[4];
cx q[42], q[4];
rz(0) q[4];
rz(0) q[42];
cx q[42], q[5];
rz(0) q[5];
cx q[42], q[5];
rz(0) q[5];
rz(0) q[42];
cx q[42], q[6];
rz(0) q[6];
cx q[42], q[6];
rz(0) q[6];
rz(0) q[42];
cx q[42], q[7];
rz(0) q[7];
cx q[42], q[7];
rz(0) q[7];
rz(0) q[42];
cx q[42], q[8];
rz(0) q[8];
cx q[42], q[8];
rz(0) q[8];
rz(0) q[42];
cx q[42], q[9];
rz(0) q[9];
cx q[42], q[9];
rz(0) q[9];
rz(0) q[42];
cx q[42], q[10];
rz(0) q[10];
cx q[42], q[10];
rz(0) q[10];
rz(0) q[42];
cx q[42], q[11];
rz(0) q[11];
cx q[42], q[11];
rz(0) q[11];
rz(-pi/2147483648) q[42];
cx q[42], q[12];
rz(pi/2147483648) q[12];
cx q[42], q[12];
rz(-pi/2147483648) q[12];
rz(-pi/1073741824) q[42];
cx q[42], q[13];
rz(pi/1073741824) q[13];
cx q[42], q[13];
rz(-pi/1073741824) q[13];
rz(-pi/536870912) q[42];
cx q[42], q[14];
rz(pi/536870912) q[14];
cx q[42], q[14];
rz(-pi/536870912) q[14];
rz(-pi/268435456) q[42];
cx q[42], q[15];
rz(pi/268435456) q[15];
cx q[42], q[15];
rz(-pi/268435456) q[15];
rz(-pi/134217728) q[42];
cx q[42], q[16];
rz(pi/134217728) q[16];
cx q[42], q[16];
rz(-pi/134217728) q[16];
rz(-pi/67108864) q[42];
cx q[42], q[17];
rz(pi/67108864) q[17];
cx q[42], q[17];
rz(-pi/67108864) q[17];
rz(-pi/33554432) q[42];
cx q[42], q[18];
rz(pi/33554432) q[18];
cx q[42], q[18];
rz(-pi/33554432) q[18];
rz(-pi/16777216) q[42];
cx q[42], q[19];
rz(pi/16777216) q[19];
cx q[42], q[19];
rz(-pi/16777216) q[19];
rz(-pi/8388608) q[42];
cx q[42], q[20];
rz(pi/8388608) q[20];
cx q[42], q[20];
rz(-pi/8388608) q[20];
rz(-pi/4194304) q[42];
cx q[42], q[21];
rz(pi/4194304) q[21];
cx q[42], q[21];
rz(-pi/4194304) q[21];
rz(-pi/2097152) q[42];
cx q[42], q[22];
rz(pi/2097152) q[22];
cx q[42], q[22];
rz(-pi/2097152) q[22];
rz(-pi/1048576) q[42];
cx q[42], q[23];
rz(pi/1048576) q[23];
cx q[42], q[23];
rz(-pi/1048576) q[23];
rz(-pi/524288) q[42];
cx q[42], q[24];
rz(pi/524288) q[24];
cx q[42], q[24];
rz(-pi/524288) q[24];
rz(-pi/262144) q[42];
cx q[42], q[25];
rz(pi/262144) q[25];
cx q[42], q[25];
rz(-pi/262144) q[25];
rz(-pi/131072) q[42];
cx q[42], q[26];
rz(pi/131072) q[26];
cx q[42], q[26];
rz(-pi/131072) q[26];
rz(-pi/65536) q[42];
cx q[42], q[27];
rz(pi/65536) q[27];
cx q[42], q[27];
rz(-pi/65536) q[27];
rz(-pi/32768) q[42];
cx q[42], q[28];
rz(pi/32768) q[28];
cx q[42], q[28];
rz(-pi/32768) q[28];
rz(-pi/16384) q[42];
cx q[42], q[29];
rz(pi/16384) q[29];
cx q[42], q[29];
rz(-pi/16384) q[29];
rz(-pi/8192) q[42];
cx q[42], q[30];
rz(pi/8192) q[30];
cx q[42], q[30];
rz(-pi/8192) q[30];
rz(-pi/4096) q[42];
cx q[42], q[31];
rz(pi/4096) q[31];
cx q[42], q[31];
rz(-pi/4096) q[31];
rz(-pi/2048) q[42];
cx q[42], q[32];
rz(pi/2048) q[32];
cx q[42], q[32];
rz(-pi/2048) q[32];
rz(-pi/1024) q[42];
cx q[42], q[33];
rz(pi/1024) q[33];
cx q[42], q[33];
rz(-pi/1024) q[33];
rz(-pi/512) q[42];
cx q[42], q[34];
rz(pi/512) q[34];
cx q[42], q[34];
rz(-pi/512) q[34];
rz(-pi/256) q[42];
cx q[42], q[35];
rz(pi/256) q[35];
cx q[42], q[35];
rz(-pi/256) q[35];
rz(-pi/128) q[42];
cx q[42], q[36];
rz(pi/128) q[36];
cx q[42], q[36];
rz(-pi/128) q[36];
rz(-pi/64) q[42];
cx q[42], q[37];
rz(pi/64) q[37];
cx q[42], q[37];
rz(-pi/64) q[37];
rz(-pi/32) q[42];
cx q[42], q[38];
rz(pi/32) q[38];
cx q[42], q[38];
rz(-pi/32) q[38];
rz(-pi/16) q[42];
cx q[42], q[39];
rz(pi/16) q[39];
cx q[42], q[39];
rz(-pi/16) q[39];
rz(-pi/8) q[42];
cx q[42], q[40];
rz(pi/8) q[40];
cx q[42], q[40];
rz(-pi/8) q[40];
rz(-pi/4) q[42];
cx q[42], q[41];
rz(pi/4) q[41];
cx q[42], q[41];
rz(-pi/4) q[41];
h q[42];
cx q[43], q[0];
rz(0) q[0];
cx q[43], q[0];
rz(0) q[0];
rz(0) q[43];
cx q[43], q[1];
rz(0) q[1];
cx q[43], q[1];
rz(0) q[1];
rz(0) q[43];
cx q[43], q[2];
rz(0) q[2];
cx q[43], q[2];
rz(0) q[2];
rz(0) q[43];
cx q[43], q[3];
rz(0) q[3];
cx q[43], q[3];
rz(0) q[3];
rz(0) q[43];
cx q[43], q[4];
rz(0) q[4];
cx q[43], q[4];
rz(0) q[4];
rz(0) q[43];
cx q[43], q[5];
rz(0) q[5];
cx q[43], q[5];
rz(0) q[5];
rz(0) q[43];
cx q[43], q[6];
rz(0) q[6];
cx q[43], q[6];
rz(0) q[6];
rz(0) q[43];
cx q[43], q[7];
rz(0) q[7];
cx q[43], q[7];
rz(0) q[7];
rz(0) q[43];
cx q[43], q[8];
rz(0) q[8];
cx q[43], q[8];
rz(0) q[8];
rz(0) q[43];
cx q[43], q[9];
rz(0) q[9];
cx q[43], q[9];
rz(0) q[9];
rz(0) q[43];
cx q[43], q[10];
rz(0) q[10];
cx q[43], q[10];
rz(0) q[10];
rz(0) q[43];
cx q[43], q[11];
rz(0) q[11];
cx q[43], q[11];
rz(0) q[11];
rz(0) q[43];
cx q[43], q[12];
rz(0) q[12];
cx q[43], q[12];
rz(0) q[12];
rz(-pi/2147483648) q[43];
cx q[43], q[13];
rz(pi/2147483648) q[13];
cx q[43], q[13];
rz(-pi/2147483648) q[13];
rz(-pi/1073741824) q[43];
cx q[43], q[14];
rz(pi/1073741824) q[14];
cx q[43], q[14];
rz(-pi/1073741824) q[14];
rz(-pi/536870912) q[43];
cx q[43], q[15];
rz(pi/536870912) q[15];
cx q[43], q[15];
rz(-pi/536870912) q[15];
rz(-pi/268435456) q[43];
cx q[43], q[16];
rz(pi/268435456) q[16];
cx q[43], q[16];
rz(-pi/268435456) q[16];
rz(-pi/134217728) q[43];
cx q[43], q[17];
rz(pi/134217728) q[17];
cx q[43], q[17];
rz(-pi/134217728) q[17];
rz(-pi/67108864) q[43];
cx q[43], q[18];
rz(pi/67108864) q[18];
cx q[43], q[18];
rz(-pi/67108864) q[18];
rz(-pi/33554432) q[43];
cx q[43], q[19];
rz(pi/33554432) q[19];
cx q[43], q[19];
rz(-pi/33554432) q[19];
rz(-pi/16777216) q[43];
cx q[43], q[20];
rz(pi/16777216) q[20];
cx q[43], q[20];
rz(-pi/16777216) q[20];
rz(-pi/8388608) q[43];
cx q[43], q[21];
rz(pi/8388608) q[21];
cx q[43], q[21];
rz(-pi/8388608) q[21];
rz(-pi/4194304) q[43];
cx q[43], q[22];
rz(pi/4194304) q[22];
cx q[43], q[22];
rz(-pi/4194304) q[22];
rz(-pi/2097152) q[43];
cx q[43], q[23];
rz(pi/2097152) q[23];
cx q[43], q[23];
rz(-pi/2097152) q[23];
rz(-pi/1048576) q[43];
cx q[43], q[24];
rz(pi/1048576) q[24];
cx q[43], q[24];
rz(-pi/1048576) q[24];
rz(-pi/524288) q[43];
cx q[43], q[25];
rz(pi/524288) q[25];
cx q[43], q[25];
rz(-pi/524288) q[25];
rz(-pi/262144) q[43];
cx q[43], q[26];
rz(pi/262144) q[26];
cx q[43], q[26];
rz(-pi/262144) q[26];
rz(-pi/131072) q[43];
cx q[43], q[27];
rz(pi/131072) q[27];
cx q[43], q[27];
rz(-pi/131072) q[27];
rz(-pi/65536) q[43];
cx q[43], q[28];
rz(pi/65536) q[28];
cx q[43], q[28];
rz(-pi/65536) q[28];
rz(-pi/32768) q[43];
cx q[43], q[29];
rz(pi/32768) q[29];
cx q[43], q[29];
rz(-pi/32768) q[29];
rz(-pi/16384) q[43];
cx q[43], q[30];
rz(pi/16384) q[30];
cx q[43], q[30];
rz(-pi/16384) q[30];
rz(-pi/8192) q[43];
cx q[43], q[31];
rz(pi/8192) q[31];
cx q[43], q[31];
rz(-pi/8192) q[31];
rz(-pi/4096) q[43];
cx q[43], q[32];
rz(pi/4096) q[32];
cx q[43], q[32];
rz(-pi/4096) q[32];
rz(-pi/2048) q[43];
cx q[43], q[33];
rz(pi/2048) q[33];
cx q[43], q[33];
rz(-pi/2048) q[33];
rz(-pi/1024) q[43];
cx q[43], q[34];
rz(pi/1024) q[34];
cx q[43], q[34];
rz(-pi/1024) q[34];
rz(-pi/512) q[43];
cx q[43], q[35];
rz(pi/512) q[35];
cx q[43], q[35];
rz(-pi/512) q[35];
rz(-pi/256) q[43];
cx q[43], q[36];
rz(pi/256) q[36];
cx q[43], q[36];
rz(-pi/256) q[36];
rz(-pi/128) q[43];
cx q[43], q[37];
rz(pi/128) q[37];
cx q[43], q[37];
rz(-pi/128) q[37];
rz(-pi/64) q[43];
cx q[43], q[38];
rz(pi/64) q[38];
cx q[43], q[38];
rz(-pi/64) q[38];
rz(-pi/32) q[43];
cx q[43], q[39];
rz(pi/32) q[39];
cx q[43], q[39];
rz(-pi/32) q[39];
rz(-pi/16) q[43];
cx q[43], q[40];
rz(pi/16) q[40];
cx q[43], q[40];
rz(-pi/16) q[40];
rz(-pi/8) q[43];
cx q[43], q[41];
rz(pi/8) q[41];
cx q[43], q[41];
rz(-pi/8) q[41];
rz(-pi/4) q[43];
cx q[43], q[42];
rz(pi/4) q[42];
cx q[43], q[42];
rz(-pi/4) q[42];
h q[43];
cx q[44], q[0];
rz(0) q[0];
cx q[44], q[0];
rz(0) q[0];
rz(0) q[44];
cx q[44], q[1];
rz(0) q[1];
cx q[44], q[1];
rz(0) q[1];
rz(0) q[44];
cx q[44], q[2];
rz(0) q[2];
cx q[44], q[2];
rz(0) q[2];
rz(0) q[44];
cx q[44], q[3];
rz(0) q[3];
cx q[44], q[3];
rz(0) q[3];
rz(0) q[44];
cx q[44], q[4];
rz(0) q[4];
cx q[44], q[4];
rz(0) q[4];
rz(0) q[44];
cx q[44], q[5];
rz(0) q[5];
cx q[44], q[5];
rz(0) q[5];
rz(0) q[44];
cx q[44], q[6];
rz(0) q[6];
cx q[44], q[6];
rz(0) q[6];
rz(0) q[44];
cx q[44], q[7];
rz(0) q[7];
cx q[44], q[7];
rz(0) q[7];
rz(0) q[44];
cx q[44], q[8];
rz(0) q[8];
cx q[44], q[8];
rz(0) q[8];
rz(0) q[44];
cx q[44], q[9];
rz(0) q[9];
cx q[44], q[9];
rz(0) q[9];
rz(0) q[44];
cx q[44], q[10];
rz(0) q[10];
cx q[44], q[10];
rz(0) q[10];
rz(0) q[44];
cx q[44], q[11];
rz(0) q[11];
cx q[44], q[11];
rz(0) q[11];
rz(0) q[44];
cx q[44], q[12];
rz(0) q[12];
cx q[44], q[12];
rz(0) q[12];
rz(0) q[44];
cx q[44], q[13];
rz(0) q[13];
cx q[44], q[13];
rz(0) q[13];
rz(-pi/2147483648) q[44];
cx q[44], q[14];
rz(pi/2147483648) q[14];
cx q[44], q[14];
rz(-pi/2147483648) q[14];
rz(-pi/1073741824) q[44];
cx q[44], q[15];
rz(pi/1073741824) q[15];
cx q[44], q[15];
rz(-pi/1073741824) q[15];
rz(-pi/536870912) q[44];
cx q[44], q[16];
rz(pi/536870912) q[16];
cx q[44], q[16];
rz(-pi/536870912) q[16];
rz(-pi/268435456) q[44];
cx q[44], q[17];
rz(pi/268435456) q[17];
cx q[44], q[17];
rz(-pi/268435456) q[17];
rz(-pi/134217728) q[44];
cx q[44], q[18];
rz(pi/134217728) q[18];
cx q[44], q[18];
rz(-pi/134217728) q[18];
rz(-pi/67108864) q[44];
cx q[44], q[19];
rz(pi/67108864) q[19];
cx q[44], q[19];
rz(-pi/67108864) q[19];
rz(-pi/33554432) q[44];
cx q[44], q[20];
rz(pi/33554432) q[20];
cx q[44], q[20];
rz(-pi/33554432) q[20];
rz(-pi/16777216) q[44];
cx q[44], q[21];
rz(pi/16777216) q[21];
cx q[44], q[21];
rz(-pi/16777216) q[21];
rz(-pi/8388608) q[44];
cx q[44], q[22];
rz(pi/8388608) q[22];
cx q[44], q[22];
rz(-pi/8388608) q[22];
rz(-pi/4194304) q[44];
cx q[44], q[23];
rz(pi/4194304) q[23];
cx q[44], q[23];
rz(-pi/4194304) q[23];
rz(-pi/2097152) q[44];
cx q[44], q[24];
rz(pi/2097152) q[24];
cx q[44], q[24];
rz(-pi/2097152) q[24];
rz(-pi/1048576) q[44];
cx q[44], q[25];
rz(pi/1048576) q[25];
cx q[44], q[25];
rz(-pi/1048576) q[25];
rz(-pi/524288) q[44];
cx q[44], q[26];
rz(pi/524288) q[26];
cx q[44], q[26];
rz(-pi/524288) q[26];
rz(-pi/262144) q[44];
cx q[44], q[27];
rz(pi/262144) q[27];
cx q[44], q[27];
rz(-pi/262144) q[27];
rz(-pi/131072) q[44];
cx q[44], q[28];
rz(pi/131072) q[28];
cx q[44], q[28];
rz(-pi/131072) q[28];
rz(-pi/65536) q[44];
cx q[44], q[29];
rz(pi/65536) q[29];
cx q[44], q[29];
rz(-pi/65536) q[29];
rz(-pi/32768) q[44];
cx q[44], q[30];
rz(pi/32768) q[30];
cx q[44], q[30];
rz(-pi/32768) q[30];
rz(-pi/16384) q[44];
cx q[44], q[31];
rz(pi/16384) q[31];
cx q[44], q[31];
rz(-pi/16384) q[31];
rz(-pi/8192) q[44];
cx q[44], q[32];
rz(pi/8192) q[32];
cx q[44], q[32];
rz(-pi/8192) q[32];
rz(-pi/4096) q[44];
cx q[44], q[33];
rz(pi/4096) q[33];
cx q[44], q[33];
rz(-pi/4096) q[33];
rz(-pi/2048) q[44];
cx q[44], q[34];
rz(pi/2048) q[34];
cx q[44], q[34];
rz(-pi/2048) q[34];
rz(-pi/1024) q[44];
cx q[44], q[35];
rz(pi/1024) q[35];
cx q[44], q[35];
rz(-pi/1024) q[35];
rz(-pi/512) q[44];
cx q[44], q[36];
rz(pi/512) q[36];
cx q[44], q[36];
rz(-pi/512) q[36];
rz(-pi/256) q[44];
cx q[44], q[37];
rz(pi/256) q[37];
cx q[44], q[37];
rz(-pi/256) q[37];
rz(-pi/128) q[44];
cx q[44], q[38];
rz(pi/128) q[38];
cx q[44], q[38];
rz(-pi/128) q[38];
rz(-pi/64) q[44];
cx q[44], q[39];
rz(pi/64) q[39];
cx q[44], q[39];
rz(-pi/64) q[39];
rz(-pi/32) q[44];
cx q[44], q[40];
rz(pi/32) q[40];
cx q[44], q[40];
rz(-pi/32) q[40];
rz(-pi/16) q[44];
cx q[44], q[41];
rz(pi/16) q[41];
cx q[44], q[41];
rz(-pi/16) q[41];
rz(-pi/8) q[44];
cx q[44], q[42];
rz(pi/8) q[42];
cx q[44], q[42];
rz(-pi/8) q[42];
rz(-pi/4) q[44];
cx q[44], q[43];
rz(pi/4) q[43];
cx q[44], q[43];
rz(-pi/4) q[43];
h q[44];
cx q[45], q[0];
rz(0) q[0];
cx q[45], q[0];
rz(0) q[0];
rz(0) q[45];
cx q[45], q[1];
rz(0) q[1];
cx q[45], q[1];
rz(0) q[1];
rz(0) q[45];
cx q[45], q[2];
rz(0) q[2];
cx q[45], q[2];
rz(0) q[2];
rz(0) q[45];
cx q[45], q[3];
rz(0) q[3];
cx q[45], q[3];
rz(0) q[3];
rz(0) q[45];
cx q[45], q[4];
rz(0) q[4];
cx q[45], q[4];
rz(0) q[4];
rz(0) q[45];
cx q[45], q[5];
rz(0) q[5];
cx q[45], q[5];
rz(0) q[5];
rz(0) q[45];
cx q[45], q[6];
rz(0) q[6];
cx q[45], q[6];
rz(0) q[6];
rz(0) q[45];
cx q[45], q[7];
rz(0) q[7];
cx q[45], q[7];
rz(0) q[7];
rz(0) q[45];
cx q[45], q[8];
rz(0) q[8];
cx q[45], q[8];
rz(0) q[8];
rz(0) q[45];
cx q[45], q[9];
rz(0) q[9];
cx q[45], q[9];
rz(0) q[9];
rz(0) q[45];
cx q[45], q[10];
rz(0) q[10];
cx q[45], q[10];
rz(0) q[10];
rz(0) q[45];
cx q[45], q[11];
rz(0) q[11];
cx q[45], q[11];
rz(0) q[11];
rz(0) q[45];
cx q[45], q[12];
rz(0) q[12];
cx q[45], q[12];
rz(0) q[12];
rz(0) q[45];
cx q[45], q[13];
rz(0) q[13];
cx q[45], q[13];
rz(0) q[13];
rz(0) q[45];
cx q[45], q[14];
rz(0) q[14];
cx q[45], q[14];
rz(0) q[14];
rz(-pi/2147483648) q[45];
cx q[45], q[15];
rz(pi/2147483648) q[15];
cx q[45], q[15];
rz(-pi/2147483648) q[15];
rz(-pi/1073741824) q[45];
cx q[45], q[16];
rz(pi/1073741824) q[16];
cx q[45], q[16];
rz(-pi/1073741824) q[16];
rz(-pi/536870912) q[45];
cx q[45], q[17];
rz(pi/536870912) q[17];
cx q[45], q[17];
rz(-pi/536870912) q[17];
rz(-pi/268435456) q[45];
cx q[45], q[18];
rz(pi/268435456) q[18];
cx q[45], q[18];
rz(-pi/268435456) q[18];
rz(-pi/134217728) q[45];
cx q[45], q[19];
rz(pi/134217728) q[19];
cx q[45], q[19];
rz(-pi/134217728) q[19];
rz(-pi/67108864) q[45];
cx q[45], q[20];
rz(pi/67108864) q[20];
cx q[45], q[20];
rz(-pi/67108864) q[20];
rz(-pi/33554432) q[45];
cx q[45], q[21];
rz(pi/33554432) q[21];
cx q[45], q[21];
rz(-pi/33554432) q[21];
rz(-pi/16777216) q[45];
cx q[45], q[22];
rz(pi/16777216) q[22];
cx q[45], q[22];
rz(-pi/16777216) q[22];
rz(-pi/8388608) q[45];
cx q[45], q[23];
rz(pi/8388608) q[23];
cx q[45], q[23];
rz(-pi/8388608) q[23];
rz(-pi/4194304) q[45];
cx q[45], q[24];
rz(pi/4194304) q[24];
cx q[45], q[24];
rz(-pi/4194304) q[24];
rz(-pi/2097152) q[45];
cx q[45], q[25];
rz(pi/2097152) q[25];
cx q[45], q[25];
rz(-pi/2097152) q[25];
rz(-pi/1048576) q[45];
cx q[45], q[26];
rz(pi/1048576) q[26];
cx q[45], q[26];
rz(-pi/1048576) q[26];
rz(-pi/524288) q[45];
cx q[45], q[27];
rz(pi/524288) q[27];
cx q[45], q[27];
rz(-pi/524288) q[27];
rz(-pi/262144) q[45];
cx q[45], q[28];
rz(pi/262144) q[28];
cx q[45], q[28];
rz(-pi/262144) q[28];
rz(-pi/131072) q[45];
cx q[45], q[29];
rz(pi/131072) q[29];
cx q[45], q[29];
rz(-pi/131072) q[29];
rz(-pi/65536) q[45];
cx q[45], q[30];
rz(pi/65536) q[30];
cx q[45], q[30];
rz(-pi/65536) q[30];
rz(-pi/32768) q[45];
cx q[45], q[31];
rz(pi/32768) q[31];
cx q[45], q[31];
rz(-pi/32768) q[31];
rz(-pi/16384) q[45];
cx q[45], q[32];
rz(pi/16384) q[32];
cx q[45], q[32];
rz(-pi/16384) q[32];
rz(-pi/8192) q[45];
cx q[45], q[33];
rz(pi/8192) q[33];
cx q[45], q[33];
rz(-pi/8192) q[33];
rz(-pi/4096) q[45];
cx q[45], q[34];
rz(pi/4096) q[34];
cx q[45], q[34];
rz(-pi/4096) q[34];
rz(-pi/2048) q[45];
cx q[45], q[35];
rz(pi/2048) q[35];
cx q[45], q[35];
rz(-pi/2048) q[35];
rz(-pi/1024) q[45];
cx q[45], q[36];
rz(pi/1024) q[36];
cx q[45], q[36];
rz(-pi/1024) q[36];
rz(-pi/512) q[45];
cx q[45], q[37];
rz(pi/512) q[37];
cx q[45], q[37];
rz(-pi/512) q[37];
rz(-pi/256) q[45];
cx q[45], q[38];
rz(pi/256) q[38];
cx q[45], q[38];
rz(-pi/256) q[38];
rz(-pi/128) q[45];
cx q[45], q[39];
rz(pi/128) q[39];
cx q[45], q[39];
rz(-pi/128) q[39];
rz(-pi/64) q[45];
cx q[45], q[40];
rz(pi/64) q[40];
cx q[45], q[40];
rz(-pi/64) q[40];
rz(-pi/32) q[45];
cx q[45], q[41];
rz(pi/32) q[41];
cx q[45], q[41];
rz(-pi/32) q[41];
rz(-pi/16) q[45];
cx q[45], q[42];
rz(pi/16) q[42];
cx q[45], q[42];
rz(-pi/16) q[42];
rz(-pi/8) q[45];
cx q[45], q[43];
rz(pi/8) q[43];
cx q[45], q[43];
rz(-pi/8) q[43];
rz(-pi/4) q[45];
cx q[45], q[44];
rz(pi/4) q[44];
cx q[45], q[44];
rz(-pi/4) q[44];
h q[45];
cx q[46], q[0];
rz(0) q[0];
cx q[46], q[0];
rz(0) q[0];
rz(0) q[46];
cx q[46], q[1];
rz(0) q[1];
cx q[46], q[1];
rz(0) q[1];
rz(0) q[46];
cx q[46], q[2];
rz(0) q[2];
cx q[46], q[2];
rz(0) q[2];
rz(0) q[46];
cx q[46], q[3];
rz(0) q[3];
cx q[46], q[3];
rz(0) q[3];
rz(0) q[46];
cx q[46], q[4];
rz(0) q[4];
cx q[46], q[4];
rz(0) q[4];
rz(0) q[46];
cx q[46], q[5];
rz(0) q[5];
cx q[46], q[5];
rz(0) q[5];
rz(0) q[46];
cx q[46], q[6];
rz(0) q[6];
cx q[46], q[6];
rz(0) q[6];
rz(0) q[46];
cx q[46], q[7];
rz(0) q[7];
cx q[46], q[7];
rz(0) q[7];
rz(0) q[46];
cx q[46], q[8];
rz(0) q[8];
cx q[46], q[8];
rz(0) q[8];
rz(0) q[46];
cx q[46], q[9];
rz(0) q[9];
cx q[46], q[9];
rz(0) q[9];
rz(0) q[46];
cx q[46], q[10];
rz(0) q[10];
cx q[46], q[10];
rz(0) q[10];
rz(0) q[46];
cx q[46], q[11];
rz(0) q[11];
cx q[46], q[11];
rz(0) q[11];
rz(0) q[46];
cx q[46], q[12];
rz(0) q[12];
cx q[46], q[12];
rz(0) q[12];
rz(0) q[46];
cx q[46], q[13];
rz(0) q[13];
cx q[46], q[13];
rz(0) q[13];
rz(0) q[46];
cx q[46], q[14];
rz(0) q[14];
cx q[46], q[14];
rz(0) q[14];
rz(0) q[46];
cx q[46], q[15];
rz(0) q[15];
cx q[46], q[15];
rz(0) q[15];
rz(-pi/2147483648) q[46];
cx q[46], q[16];
rz(pi/2147483648) q[16];
cx q[46], q[16];
rz(-pi/2147483648) q[16];
rz(-pi/1073741824) q[46];
cx q[46], q[17];
rz(pi/1073741824) q[17];
cx q[46], q[17];
rz(-pi/1073741824) q[17];
rz(-pi/536870912) q[46];
cx q[46], q[18];
rz(pi/536870912) q[18];
cx q[46], q[18];
rz(-pi/536870912) q[18];
rz(-pi/268435456) q[46];
cx q[46], q[19];
rz(pi/268435456) q[19];
cx q[46], q[19];
rz(-pi/268435456) q[19];
rz(-pi/134217728) q[46];
cx q[46], q[20];
rz(pi/134217728) q[20];
cx q[46], q[20];
rz(-pi/134217728) q[20];
rz(-pi/67108864) q[46];
cx q[46], q[21];
rz(pi/67108864) q[21];
cx q[46], q[21];
rz(-pi/67108864) q[21];
rz(-pi/33554432) q[46];
cx q[46], q[22];
rz(pi/33554432) q[22];
cx q[46], q[22];
rz(-pi/33554432) q[22];
rz(-pi/16777216) q[46];
cx q[46], q[23];
rz(pi/16777216) q[23];
cx q[46], q[23];
rz(-pi/16777216) q[23];
rz(-pi/8388608) q[46];
cx q[46], q[24];
rz(pi/8388608) q[24];
cx q[46], q[24];
rz(-pi/8388608) q[24];
rz(-pi/4194304) q[46];
cx q[46], q[25];
rz(pi/4194304) q[25];
cx q[46], q[25];
rz(-pi/4194304) q[25];
rz(-pi/2097152) q[46];
cx q[46], q[26];
rz(pi/2097152) q[26];
cx q[46], q[26];
rz(-pi/2097152) q[26];
rz(-pi/1048576) q[46];
cx q[46], q[27];
rz(pi/1048576) q[27];
cx q[46], q[27];
rz(-pi/1048576) q[27];
rz(-pi/524288) q[46];
cx q[46], q[28];
rz(pi/524288) q[28];
cx q[46], q[28];
rz(-pi/524288) q[28];
rz(-pi/262144) q[46];
cx q[46], q[29];
rz(pi/262144) q[29];
cx q[46], q[29];
rz(-pi/262144) q[29];
rz(-pi/131072) q[46];
cx q[46], q[30];
rz(pi/131072) q[30];
cx q[46], q[30];
rz(-pi/131072) q[30];
rz(-pi/65536) q[46];
cx q[46], q[31];
rz(pi/65536) q[31];
cx q[46], q[31];
rz(-pi/65536) q[31];
rz(-pi/32768) q[46];
cx q[46], q[32];
rz(pi/32768) q[32];
cx q[46], q[32];
rz(-pi/32768) q[32];
rz(-pi/16384) q[46];
cx q[46], q[33];
rz(pi/16384) q[33];
cx q[46], q[33];
rz(-pi/16384) q[33];
rz(-pi/8192) q[46];
cx q[46], q[34];
rz(pi/8192) q[34];
cx q[46], q[34];
rz(-pi/8192) q[34];
rz(-pi/4096) q[46];
cx q[46], q[35];
rz(pi/4096) q[35];
cx q[46], q[35];
rz(-pi/4096) q[35];
rz(-pi/2048) q[46];
cx q[46], q[36];
rz(pi/2048) q[36];
cx q[46], q[36];
rz(-pi/2048) q[36];
rz(-pi/1024) q[46];
cx q[46], q[37];
rz(pi/1024) q[37];
cx q[46], q[37];
rz(-pi/1024) q[37];
rz(-pi/512) q[46];
cx q[46], q[38];
rz(pi/512) q[38];
cx q[46], q[38];
rz(-pi/512) q[38];
rz(-pi/256) q[46];
cx q[46], q[39];
rz(pi/256) q[39];
cx q[46], q[39];
rz(-pi/256) q[39];
rz(-pi/128) q[46];
cx q[46], q[40];
rz(pi/128) q[40];
cx q[46], q[40];
rz(-pi/128) q[40];
rz(-pi/64) q[46];
cx q[46], q[41];
rz(pi/64) q[41];
cx q[46], q[41];
rz(-pi/64) q[41];
rz(-pi/32) q[46];
cx q[46], q[42];
rz(pi/32) q[42];
cx q[46], q[42];
rz(-pi/32) q[42];
rz(-pi/16) q[46];
cx q[46], q[43];
rz(pi/16) q[43];
cx q[46], q[43];
rz(-pi/16) q[43];
rz(-pi/8) q[46];
cx q[46], q[44];
rz(pi/8) q[44];
cx q[46], q[44];
rz(-pi/8) q[44];
rz(-pi/4) q[46];
cx q[46], q[45];
rz(pi/4) q[45];
cx q[46], q[45];
rz(-pi/4) q[45];
h q[46];
cx q[47], q[0];
rz(0) q[0];
cx q[47], q[0];
rz(0) q[0];
rz(0) q[47];
cx q[47], q[1];
rz(0) q[1];
cx q[47], q[1];
rz(0) q[1];
rz(0) q[47];
cx q[47], q[2];
rz(0) q[2];
cx q[47], q[2];
rz(0) q[2];
rz(0) q[47];
cx q[47], q[3];
rz(0) q[3];
cx q[47], q[3];
rz(0) q[3];
rz(0) q[47];
cx q[47], q[4];
rz(0) q[4];
cx q[47], q[4];
rz(0) q[4];
rz(0) q[47];
cx q[47], q[5];
rz(0) q[5];
cx q[47], q[5];
rz(0) q[5];
rz(0) q[47];
cx q[47], q[6];
rz(0) q[6];
cx q[47], q[6];
rz(0) q[6];
rz(0) q[47];
cx q[47], q[7];
rz(0) q[7];
cx q[47], q[7];
rz(0) q[7];
rz(0) q[47];
cx q[47], q[8];
rz(0) q[8];
cx q[47], q[8];
rz(0) q[8];
rz(0) q[47];
cx q[47], q[9];
rz(0) q[9];
cx q[47], q[9];
rz(0) q[9];
rz(0) q[47];
cx q[47], q[10];
rz(0) q[10];
cx q[47], q[10];
rz(0) q[10];
rz(0) q[47];
cx q[47], q[11];
rz(0) q[11];
cx q[47], q[11];
rz(0) q[11];
rz(0) q[47];
cx q[47], q[12];
rz(0) q[12];
cx q[47], q[12];
rz(0) q[12];
rz(0) q[47];
cx q[47], q[13];
rz(0) q[13];
cx q[47], q[13];
rz(0) q[13];
rz(0) q[47];
cx q[47], q[14];
rz(0) q[14];
cx q[47], q[14];
rz(0) q[14];
rz(0) q[47];
cx q[47], q[15];
rz(0) q[15];
cx q[47], q[15];
rz(0) q[15];
rz(0) q[47];
cx q[47], q[16];
rz(0) q[16];
cx q[47], q[16];
rz(0) q[16];
rz(-pi/2147483648) q[47];
cx q[47], q[17];
rz(pi/2147483648) q[17];
cx q[47], q[17];
rz(-pi/2147483648) q[17];
rz(-pi/1073741824) q[47];
cx q[47], q[18];
rz(pi/1073741824) q[18];
cx q[47], q[18];
rz(-pi/1073741824) q[18];
rz(-pi/536870912) q[47];
cx q[47], q[19];
rz(pi/536870912) q[19];
cx q[47], q[19];
rz(-pi/536870912) q[19];
rz(-pi/268435456) q[47];
cx q[47], q[20];
rz(pi/268435456) q[20];
cx q[47], q[20];
rz(-pi/268435456) q[20];
rz(-pi/134217728) q[47];
cx q[47], q[21];
rz(pi/134217728) q[21];
cx q[47], q[21];
rz(-pi/134217728) q[21];
rz(-pi/67108864) q[47];
cx q[47], q[22];
rz(pi/67108864) q[22];
cx q[47], q[22];
rz(-pi/67108864) q[22];
rz(-pi/33554432) q[47];
cx q[47], q[23];
rz(pi/33554432) q[23];
cx q[47], q[23];
rz(-pi/33554432) q[23];
rz(-pi/16777216) q[47];
cx q[47], q[24];
rz(pi/16777216) q[24];
cx q[47], q[24];
rz(-pi/16777216) q[24];
rz(-pi/8388608) q[47];
cx q[47], q[25];
rz(pi/8388608) q[25];
cx q[47], q[25];
rz(-pi/8388608) q[25];
rz(-pi/4194304) q[47];
cx q[47], q[26];
rz(pi/4194304) q[26];
cx q[47], q[26];
rz(-pi/4194304) q[26];
rz(-pi/2097152) q[47];
cx q[47], q[27];
rz(pi/2097152) q[27];
cx q[47], q[27];
rz(-pi/2097152) q[27];
rz(-pi/1048576) q[47];
cx q[47], q[28];
rz(pi/1048576) q[28];
cx q[47], q[28];
rz(-pi/1048576) q[28];
rz(-pi/524288) q[47];
cx q[47], q[29];
rz(pi/524288) q[29];
cx q[47], q[29];
rz(-pi/524288) q[29];
rz(-pi/262144) q[47];
cx q[47], q[30];
rz(pi/262144) q[30];
cx q[47], q[30];
rz(-pi/262144) q[30];
rz(-pi/131072) q[47];
cx q[47], q[31];
rz(pi/131072) q[31];
cx q[47], q[31];
rz(-pi/131072) q[31];
rz(-pi/65536) q[47];
cx q[47], q[32];
rz(pi/65536) q[32];
cx q[47], q[32];
rz(-pi/65536) q[32];
rz(-pi/32768) q[47];
cx q[47], q[33];
rz(pi/32768) q[33];
cx q[47], q[33];
rz(-pi/32768) q[33];
rz(-pi/16384) q[47];
cx q[47], q[34];
rz(pi/16384) q[34];
cx q[47], q[34];
rz(-pi/16384) q[34];
rz(-pi/8192) q[47];
cx q[47], q[35];
rz(pi/8192) q[35];
cx q[47], q[35];
rz(-pi/8192) q[35];
rz(-pi/4096) q[47];
cx q[47], q[36];
rz(pi/4096) q[36];
cx q[47], q[36];
rz(-pi/4096) q[36];
rz(-pi/2048) q[47];
cx q[47], q[37];
rz(pi/2048) q[37];
cx q[47], q[37];
rz(-pi/2048) q[37];
rz(-pi/1024) q[47];
cx q[47], q[38];
rz(pi/1024) q[38];
cx q[47], q[38];
rz(-pi/1024) q[38];
rz(-pi/512) q[47];
cx q[47], q[39];
rz(pi/512) q[39];
cx q[47], q[39];
rz(-pi/512) q[39];
rz(-pi/256) q[47];
cx q[47], q[40];
rz(pi/256) q[40];
cx q[47], q[40];
rz(-pi/256) q[40];
rz(-pi/128) q[47];
cx q[47], q[41];
rz(pi/128) q[41];
cx q[47], q[41];
rz(-pi/128) q[41];
rz(-pi/64) q[47];
cx q[47], q[42];
rz(pi/64) q[42];
cx q[47], q[42];
rz(-pi/64) q[42];
rz(-pi/32) q[47];
cx q[47], q[43];
rz(pi/32) q[43];
cx q[47], q[43];
rz(-pi/32) q[43];
rz(-pi/16) q[47];
cx q[47], q[44];
rz(pi/16) q[44];
cx q[47], q[44];
rz(-pi/16) q[44];
rz(-pi/8) q[47];
cx q[47], q[45];
rz(pi/8) q[45];
cx q[47], q[45];
rz(-pi/8) q[45];
rz(-pi/4) q[47];
cx q[47], q[46];
rz(pi/4) q[46];
cx q[47], q[46];
rz(-pi/4) q[46];
h q[47];
cx q[48], q[0];
rz(0) q[0];
cx q[48], q[0];
rz(0) q[0];
rz(0) q[48];
cx q[48], q[1];
rz(0) q[1];
cx q[48], q[1];
rz(0) q[1];
rz(0) q[48];
cx q[48], q[2];
rz(0) q[2];
cx q[48], q[2];
rz(0) q[2];
rz(0) q[48];
cx q[48], q[3];
rz(0) q[3];
cx q[48], q[3];
rz(0) q[3];
rz(0) q[48];
cx q[48], q[4];
rz(0) q[4];
cx q[48], q[4];
rz(0) q[4];
rz(0) q[48];
cx q[48], q[5];
rz(0) q[5];
cx q[48], q[5];
rz(0) q[5];
rz(0) q[48];
cx q[48], q[6];
rz(0) q[6];
cx q[48], q[6];
rz(0) q[6];
rz(0) q[48];
cx q[48], q[7];
rz(0) q[7];
cx q[48], q[7];
rz(0) q[7];
rz(0) q[48];
cx q[48], q[8];
rz(0) q[8];
cx q[48], q[8];
rz(0) q[8];
rz(0) q[48];
cx q[48], q[9];
rz(0) q[9];
cx q[48], q[9];
rz(0) q[9];
rz(0) q[48];
cx q[48], q[10];
rz(0) q[10];
cx q[48], q[10];
rz(0) q[10];
rz(0) q[48];
cx q[48], q[11];
rz(0) q[11];
cx q[48], q[11];
rz(0) q[11];
rz(0) q[48];
cx q[48], q[12];
rz(0) q[12];
cx q[48], q[12];
rz(0) q[12];
rz(0) q[48];
cx q[48], q[13];
rz(0) q[13];
cx q[48], q[13];
rz(0) q[13];
rz(0) q[48];
cx q[48], q[14];
rz(0) q[14];
cx q[48], q[14];
rz(0) q[14];
rz(0) q[48];
cx q[48], q[15];
rz(0) q[15];
cx q[48], q[15];
rz(0) q[15];
rz(0) q[48];
cx q[48], q[16];
rz(0) q[16];
cx q[48], q[16];
rz(0) q[16];
rz(0) q[48];
cx q[48], q[17];
rz(0) q[17];
cx q[48], q[17];
rz(0) q[17];
rz(-pi/2147483648) q[48];
cx q[48], q[18];
rz(pi/2147483648) q[18];
cx q[48], q[18];
rz(-pi/2147483648) q[18];
rz(-pi/1073741824) q[48];
cx q[48], q[19];
rz(pi/1073741824) q[19];
cx q[48], q[19];
rz(-pi/1073741824) q[19];
rz(-pi/536870912) q[48];
cx q[48], q[20];
rz(pi/536870912) q[20];
cx q[48], q[20];
rz(-pi/536870912) q[20];
rz(-pi/268435456) q[48];
cx q[48], q[21];
rz(pi/268435456) q[21];
cx q[48], q[21];
rz(-pi/268435456) q[21];
rz(-pi/134217728) q[48];
cx q[48], q[22];
rz(pi/134217728) q[22];
cx q[48], q[22];
rz(-pi/134217728) q[22];
rz(-pi/67108864) q[48];
cx q[48], q[23];
rz(pi/67108864) q[23];
cx q[48], q[23];
rz(-pi/67108864) q[23];
rz(-pi/33554432) q[48];
cx q[48], q[24];
rz(pi/33554432) q[24];
cx q[48], q[24];
rz(-pi/33554432) q[24];
rz(-pi/16777216) q[48];
cx q[48], q[25];
rz(pi/16777216) q[25];
cx q[48], q[25];
rz(-pi/16777216) q[25];
rz(-pi/8388608) q[48];
cx q[48], q[26];
rz(pi/8388608) q[26];
cx q[48], q[26];
rz(-pi/8388608) q[26];
rz(-pi/4194304) q[48];
cx q[48], q[27];
rz(pi/4194304) q[27];
cx q[48], q[27];
rz(-pi/4194304) q[27];
rz(-pi/2097152) q[48];
cx q[48], q[28];
rz(pi/2097152) q[28];
cx q[48], q[28];
rz(-pi/2097152) q[28];
rz(-pi/1048576) q[48];
cx q[48], q[29];
rz(pi/1048576) q[29];
cx q[48], q[29];
rz(-pi/1048576) q[29];
rz(-pi/524288) q[48];
cx q[48], q[30];
rz(pi/524288) q[30];
cx q[48], q[30];
rz(-pi/524288) q[30];
rz(-pi/262144) q[48];
cx q[48], q[31];
rz(pi/262144) q[31];
cx q[48], q[31];
rz(-pi/262144) q[31];
rz(-pi/131072) q[48];
cx q[48], q[32];
rz(pi/131072) q[32];
cx q[48], q[32];
rz(-pi/131072) q[32];
rz(-pi/65536) q[48];
cx q[48], q[33];
rz(pi/65536) q[33];
cx q[48], q[33];
rz(-pi/65536) q[33];
rz(-pi/32768) q[48];
cx q[48], q[34];
rz(pi/32768) q[34];
cx q[48], q[34];
rz(-pi/32768) q[34];
rz(-pi/16384) q[48];
cx q[48], q[35];
rz(pi/16384) q[35];
cx q[48], q[35];
rz(-pi/16384) q[35];
rz(-pi/8192) q[48];
cx q[48], q[36];
rz(pi/8192) q[36];
cx q[48], q[36];
rz(-pi/8192) q[36];
rz(-pi/4096) q[48];
cx q[48], q[37];
rz(pi/4096) q[37];
cx q[48], q[37];
rz(-pi/4096) q[37];
rz(-pi/2048) q[48];
cx q[48], q[38];
rz(pi/2048) q[38];
cx q[48], q[38];
rz(-pi/2048) q[38];
rz(-pi/1024) q[48];
cx q[48], q[39];
rz(pi/1024) q[39];
cx q[48], q[39];
rz(-pi/1024) q[39];
rz(-pi/512) q[48];
cx q[48], q[40];
rz(pi/512) q[40];
cx q[48], q[40];
rz(-pi/512) q[40];
rz(-pi/256) q[48];
cx q[48], q[41];
rz(pi/256) q[41];
cx q[48], q[41];
rz(-pi/256) q[41];
rz(-pi/128) q[48];
cx q[48], q[42];
rz(pi/128) q[42];
cx q[48], q[42];
rz(-pi/128) q[42];
rz(-pi/64) q[48];
cx q[48], q[43];
rz(pi/64) q[43];
cx q[48], q[43];
rz(-pi/64) q[43];
rz(-pi/32) q[48];
cx q[48], q[44];
rz(pi/32) q[44];
cx q[48], q[44];
rz(-pi/32) q[44];
rz(-pi/16) q[48];
cx q[48], q[45];
rz(pi/16) q[45];
cx q[48], q[45];
rz(-pi/16) q[45];
rz(-pi/8) q[48];
cx q[48], q[46];
rz(pi/8) q[46];
cx q[48], q[46];
rz(-pi/8) q[46];
rz(-pi/4) q[48];
cx q[48], q[47];
rz(pi/4) q[47];
cx q[48], q[47];
rz(-pi/4) q[47];
h q[48];
cx q[49], q[0];
rz(0) q[0];
cx q[49], q[0];
rz(0) q[0];
rz(0) q[49];
cx q[49], q[1];
rz(0) q[1];
cx q[49], q[1];
rz(0) q[1];
rz(0) q[49];
cx q[49], q[2];
rz(0) q[2];
cx q[49], q[2];
rz(0) q[2];
rz(0) q[49];
cx q[49], q[3];
rz(0) q[3];
cx q[49], q[3];
rz(0) q[3];
rz(0) q[49];
cx q[49], q[4];
rz(0) q[4];
cx q[49], q[4];
rz(0) q[4];
rz(0) q[49];
cx q[49], q[5];
rz(0) q[5];
cx q[49], q[5];
rz(0) q[5];
rz(0) q[49];
cx q[49], q[6];
rz(0) q[6];
cx q[49], q[6];
rz(0) q[6];
rz(0) q[49];
cx q[49], q[7];
rz(0) q[7];
cx q[49], q[7];
rz(0) q[7];
rz(0) q[49];
cx q[49], q[8];
rz(0) q[8];
cx q[49], q[8];
rz(0) q[8];
rz(0) q[49];
cx q[49], q[9];
rz(0) q[9];
cx q[49], q[9];
rz(0) q[9];
rz(0) q[49];
cx q[49], q[10];
rz(0) q[10];
cx q[49], q[10];
rz(0) q[10];
rz(0) q[49];
cx q[49], q[11];
rz(0) q[11];
cx q[49], q[11];
rz(0) q[11];
rz(0) q[49];
cx q[49], q[12];
rz(0) q[12];
cx q[49], q[12];
rz(0) q[12];
rz(0) q[49];
cx q[49], q[13];
rz(0) q[13];
cx q[49], q[13];
rz(0) q[13];
rz(0) q[49];
cx q[49], q[14];
rz(0) q[14];
cx q[49], q[14];
rz(0) q[14];
rz(0) q[49];
cx q[49], q[15];
rz(0) q[15];
cx q[49], q[15];
rz(0) q[15];
rz(0) q[49];
cx q[49], q[16];
rz(0) q[16];
cx q[49], q[16];
rz(0) q[16];
rz(0) q[49];
cx q[49], q[17];
rz(0) q[17];
cx q[49], q[17];
rz(0) q[17];
rz(0) q[49];
cx q[49], q[18];
rz(0) q[18];
cx q[49], q[18];
rz(0) q[18];
rz(-pi/2147483648) q[49];
cx q[49], q[19];
rz(pi/2147483648) q[19];
cx q[49], q[19];
rz(-pi/2147483648) q[19];
rz(-pi/1073741824) q[49];
cx q[49], q[20];
rz(pi/1073741824) q[20];
cx q[49], q[20];
rz(-pi/1073741824) q[20];
rz(-pi/536870912) q[49];
cx q[49], q[21];
rz(pi/536870912) q[21];
cx q[49], q[21];
rz(-pi/536870912) q[21];
rz(-pi/268435456) q[49];
cx q[49], q[22];
rz(pi/268435456) q[22];
cx q[49], q[22];
rz(-pi/268435456) q[22];
rz(-pi/134217728) q[49];
cx q[49], q[23];
rz(pi/134217728) q[23];
cx q[49], q[23];
rz(-pi/134217728) q[23];
rz(-pi/67108864) q[49];
cx q[49], q[24];
rz(pi/67108864) q[24];
cx q[49], q[24];
rz(-pi/67108864) q[24];
rz(-pi/33554432) q[49];
cx q[49], q[25];
rz(pi/33554432) q[25];
cx q[49], q[25];
rz(-pi/33554432) q[25];
rz(-pi/16777216) q[49];
cx q[49], q[26];
rz(pi/16777216) q[26];
cx q[49], q[26];
rz(-pi/16777216) q[26];
rz(-pi/8388608) q[49];
cx q[49], q[27];
rz(pi/8388608) q[27];
cx q[49], q[27];
rz(-pi/8388608) q[27];
rz(-pi/4194304) q[49];
cx q[49], q[28];
rz(pi/4194304) q[28];
cx q[49], q[28];
rz(-pi/4194304) q[28];
rz(-pi/2097152) q[49];
cx q[49], q[29];
rz(pi/2097152) q[29];
cx q[49], q[29];
rz(-pi/2097152) q[29];
rz(-pi/1048576) q[49];
cx q[49], q[30];
rz(pi/1048576) q[30];
cx q[49], q[30];
rz(-pi/1048576) q[30];
rz(-pi/524288) q[49];
cx q[49], q[31];
rz(pi/524288) q[31];
cx q[49], q[31];
rz(-pi/524288) q[31];
rz(-pi/262144) q[49];
cx q[49], q[32];
rz(pi/262144) q[32];
cx q[49], q[32];
rz(-pi/262144) q[32];
rz(-pi/131072) q[49];
cx q[49], q[33];
rz(pi/131072) q[33];
cx q[49], q[33];
rz(-pi/131072) q[33];
rz(-pi/65536) q[49];
cx q[49], q[34];
rz(pi/65536) q[34];
cx q[49], q[34];
rz(-pi/65536) q[34];
rz(-pi/32768) q[49];
cx q[49], q[35];
rz(pi/32768) q[35];
cx q[49], q[35];
rz(-pi/32768) q[35];
rz(-pi/16384) q[49];
cx q[49], q[36];
rz(pi/16384) q[36];
cx q[49], q[36];
rz(-pi/16384) q[36];
rz(-pi/8192) q[49];
cx q[49], q[37];
rz(pi/8192) q[37];
cx q[49], q[37];
rz(-pi/8192) q[37];
rz(-pi/4096) q[49];
cx q[49], q[38];
rz(pi/4096) q[38];
cx q[49], q[38];
rz(-pi/4096) q[38];
rz(-pi/2048) q[49];
cx q[49], q[39];
rz(pi/2048) q[39];
cx q[49], q[39];
rz(-pi/2048) q[39];
rz(-pi/1024) q[49];
cx q[49], q[40];
rz(pi/1024) q[40];
cx q[49], q[40];
rz(-pi/1024) q[40];
rz(-pi/512) q[49];
cx q[49], q[41];
rz(pi/512) q[41];
cx q[49], q[41];
rz(-pi/512) q[41];
rz(-pi/256) q[49];
cx q[49], q[42];
rz(pi/256) q[42];
cx q[49], q[42];
rz(-pi/256) q[42];
rz(-pi/128) q[49];
cx q[49], q[43];
rz(pi/128) q[43];
cx q[49], q[43];
rz(-pi/128) q[43];
rz(-pi/64) q[49];
cx q[49], q[44];
rz(pi/64) q[44];
cx q[49], q[44];
rz(-pi/64) q[44];
rz(-pi/32) q[49];
cx q[49], q[45];
rz(pi/32) q[45];
cx q[49], q[45];
rz(-pi/32) q[45];
rz(-pi/16) q[49];
cx q[49], q[46];
rz(pi/16) q[46];
cx q[49], q[46];
rz(-pi/16) q[46];
rz(-pi/8) q[49];
cx q[49], q[47];
rz(pi/8) q[47];
cx q[49], q[47];
rz(-pi/8) q[47];
rz(-pi/4) q[49];
cx q[49], q[48];
rz(pi/4) q[48];
cx q[49], q[48];
rz(-pi/4) q[48];
h q[49];
cx q[50], q[0];
rz(0) q[0];
cx q[50], q[0];
rz(0) q[0];
rz(0) q[50];
cx q[50], q[1];
rz(0) q[1];
cx q[50], q[1];
rz(0) q[1];
rz(0) q[50];
cx q[50], q[2];
rz(0) q[2];
cx q[50], q[2];
rz(0) q[2];
rz(0) q[50];
cx q[50], q[3];
rz(0) q[3];
cx q[50], q[3];
rz(0) q[3];
rz(0) q[50];
cx q[50], q[4];
rz(0) q[4];
cx q[50], q[4];
rz(0) q[4];
rz(0) q[50];
cx q[50], q[5];
rz(0) q[5];
cx q[50], q[5];
rz(0) q[5];
rz(0) q[50];
cx q[50], q[6];
rz(0) q[6];
cx q[50], q[6];
rz(0) q[6];
rz(0) q[50];
cx q[50], q[7];
rz(0) q[7];
cx q[50], q[7];
rz(0) q[7];
rz(0) q[50];
cx q[50], q[8];
rz(0) q[8];
cx q[50], q[8];
rz(0) q[8];
rz(0) q[50];
cx q[50], q[9];
rz(0) q[9];
cx q[50], q[9];
rz(0) q[9];
rz(0) q[50];
cx q[50], q[10];
rz(0) q[10];
cx q[50], q[10];
rz(0) q[10];
rz(0) q[50];
cx q[50], q[11];
rz(0) q[11];
cx q[50], q[11];
rz(0) q[11];
rz(0) q[50];
cx q[50], q[12];
rz(0) q[12];
cx q[50], q[12];
rz(0) q[12];
rz(0) q[50];
cx q[50], q[13];
rz(0) q[13];
cx q[50], q[13];
rz(0) q[13];
rz(0) q[50];
cx q[50], q[14];
rz(0) q[14];
cx q[50], q[14];
rz(0) q[14];
rz(0) q[50];
cx q[50], q[15];
rz(0) q[15];
cx q[50], q[15];
rz(0) q[15];
rz(0) q[50];
cx q[50], q[16];
rz(0) q[16];
cx q[50], q[16];
rz(0) q[16];
rz(0) q[50];
cx q[50], q[17];
rz(0) q[17];
cx q[50], q[17];
rz(0) q[17];
rz(0) q[50];
cx q[50], q[18];
rz(0) q[18];
cx q[50], q[18];
rz(0) q[18];
rz(0) q[50];
cx q[50], q[19];
rz(0) q[19];
cx q[50], q[19];
rz(0) q[19];
rz(-pi/2147483648) q[50];
cx q[50], q[20];
rz(pi/2147483648) q[20];
cx q[50], q[20];
rz(-pi/2147483648) q[20];
rz(-pi/1073741824) q[50];
cx q[50], q[21];
rz(pi/1073741824) q[21];
cx q[50], q[21];
rz(-pi/1073741824) q[21];
rz(-pi/536870912) q[50];
cx q[50], q[22];
rz(pi/536870912) q[22];
cx q[50], q[22];
rz(-pi/536870912) q[22];
rz(-pi/268435456) q[50];
cx q[50], q[23];
rz(pi/268435456) q[23];
cx q[50], q[23];
rz(-pi/268435456) q[23];
rz(-pi/134217728) q[50];
cx q[50], q[24];
rz(pi/134217728) q[24];
cx q[50], q[24];
rz(-pi/134217728) q[24];
rz(-pi/67108864) q[50];
cx q[50], q[25];
rz(pi/67108864) q[25];
cx q[50], q[25];
rz(-pi/67108864) q[25];
rz(-pi/33554432) q[50];
cx q[50], q[26];
rz(pi/33554432) q[26];
cx q[50], q[26];
rz(-pi/33554432) q[26];
rz(-pi/16777216) q[50];
cx q[50], q[27];
rz(pi/16777216) q[27];
cx q[50], q[27];
rz(-pi/16777216) q[27];
rz(-pi/8388608) q[50];
cx q[50], q[28];
rz(pi/8388608) q[28];
cx q[50], q[28];
rz(-pi/8388608) q[28];
rz(-pi/4194304) q[50];
cx q[50], q[29];
rz(pi/4194304) q[29];
cx q[50], q[29];
rz(-pi/4194304) q[29];
rz(-pi/2097152) q[50];
cx q[50], q[30];
rz(pi/2097152) q[30];
cx q[50], q[30];
rz(-pi/2097152) q[30];
rz(-pi/1048576) q[50];
cx q[50], q[31];
rz(pi/1048576) q[31];
cx q[50], q[31];
rz(-pi/1048576) q[31];
rz(-pi/524288) q[50];
cx q[50], q[32];
rz(pi/524288) q[32];
cx q[50], q[32];
rz(-pi/524288) q[32];
rz(-pi/262144) q[50];
cx q[50], q[33];
rz(pi/262144) q[33];
cx q[50], q[33];
rz(-pi/262144) q[33];
rz(-pi/131072) q[50];
cx q[50], q[34];
rz(pi/131072) q[34];
cx q[50], q[34];
rz(-pi/131072) q[34];
rz(-pi/65536) q[50];
cx q[50], q[35];
rz(pi/65536) q[35];
cx q[50], q[35];
rz(-pi/65536) q[35];
rz(-pi/32768) q[50];
cx q[50], q[36];
rz(pi/32768) q[36];
cx q[50], q[36];
rz(-pi/32768) q[36];
rz(-pi/16384) q[50];
cx q[50], q[37];
rz(pi/16384) q[37];
cx q[50], q[37];
rz(-pi/16384) q[37];
rz(-pi/8192) q[50];
cx q[50], q[38];
rz(pi/8192) q[38];
cx q[50], q[38];
rz(-pi/8192) q[38];
rz(-pi/4096) q[50];
cx q[50], q[39];
rz(pi/4096) q[39];
cx q[50], q[39];
rz(-pi/4096) q[39];
rz(-pi/2048) q[50];
cx q[50], q[40];
rz(pi/2048) q[40];
cx q[50], q[40];
rz(-pi/2048) q[40];
rz(-pi/1024) q[50];
cx q[50], q[41];
rz(pi/1024) q[41];
cx q[50], q[41];
rz(-pi/1024) q[41];
rz(-pi/512) q[50];
cx q[50], q[42];
rz(pi/512) q[42];
cx q[50], q[42];
rz(-pi/512) q[42];
rz(-pi/256) q[50];
cx q[50], q[43];
rz(pi/256) q[43];
cx q[50], q[43];
rz(-pi/256) q[43];
rz(-pi/128) q[50];
cx q[50], q[44];
rz(pi/128) q[44];
cx q[50], q[44];
rz(-pi/128) q[44];
rz(-pi/64) q[50];
cx q[50], q[45];
rz(pi/64) q[45];
cx q[50], q[45];
rz(-pi/64) q[45];
rz(-pi/32) q[50];
cx q[50], q[46];
rz(pi/32) q[46];
cx q[50], q[46];
rz(-pi/32) q[46];
rz(-pi/16) q[50];
cx q[50], q[47];
rz(pi/16) q[47];
cx q[50], q[47];
rz(-pi/16) q[47];
rz(-pi/8) q[50];
cx q[50], q[48];
rz(pi/8) q[48];
cx q[50], q[48];
rz(-pi/8) q[48];
rz(-pi/4) q[50];
cx q[50], q[49];
rz(pi/4) q[49];
cx q[50], q[49];
rz(-pi/4) q[49];
h q[50];
cx q[51], q[0];
rz(0) q[0];
cx q[51], q[0];
rz(0) q[0];
rz(0) q[51];
cx q[51], q[1];
rz(0) q[1];
cx q[51], q[1];
rz(0) q[1];
rz(0) q[51];
cx q[51], q[2];
rz(0) q[2];
cx q[51], q[2];
rz(0) q[2];
rz(0) q[51];
cx q[51], q[3];
rz(0) q[3];
cx q[51], q[3];
rz(0) q[3];
rz(0) q[51];
cx q[51], q[4];
rz(0) q[4];
cx q[51], q[4];
rz(0) q[4];
rz(0) q[51];
cx q[51], q[5];
rz(0) q[5];
cx q[51], q[5];
rz(0) q[5];
rz(0) q[51];
cx q[51], q[6];
rz(0) q[6];
cx q[51], q[6];
rz(0) q[6];
rz(0) q[51];
cx q[51], q[7];
rz(0) q[7];
cx q[51], q[7];
rz(0) q[7];
rz(0) q[51];
cx q[51], q[8];
rz(0) q[8];
cx q[51], q[8];
rz(0) q[8];
rz(0) q[51];
cx q[51], q[9];
rz(0) q[9];
cx q[51], q[9];
rz(0) q[9];
rz(0) q[51];
cx q[51], q[10];
rz(0) q[10];
cx q[51], q[10];
rz(0) q[10];
rz(0) q[51];
cx q[51], q[11];
rz(0) q[11];
cx q[51], q[11];
rz(0) q[11];
rz(0) q[51];
cx q[51], q[12];
rz(0) q[12];
cx q[51], q[12];
rz(0) q[12];
rz(0) q[51];
cx q[51], q[13];
rz(0) q[13];
cx q[51], q[13];
rz(0) q[13];
rz(0) q[51];
cx q[51], q[14];
rz(0) q[14];
cx q[51], q[14];
rz(0) q[14];
rz(0) q[51];
cx q[51], q[15];
rz(0) q[15];
cx q[51], q[15];
rz(0) q[15];
rz(0) q[51];
cx q[51], q[16];
rz(0) q[16];
cx q[51], q[16];
rz(0) q[16];
rz(0) q[51];
cx q[51], q[17];
rz(0) q[17];
cx q[51], q[17];
rz(0) q[17];
rz(0) q[51];
cx q[51], q[18];
rz(0) q[18];
cx q[51], q[18];
rz(0) q[18];
rz(0) q[51];
cx q[51], q[19];
rz(0) q[19];
cx q[51], q[19];
rz(0) q[19];
rz(0) q[51];
cx q[51], q[20];
rz(0) q[20];
cx q[51], q[20];
rz(0) q[20];
rz(-pi/2147483648) q[51];
cx q[51], q[21];
rz(pi/2147483648) q[21];
cx q[51], q[21];
rz(-pi/2147483648) q[21];
rz(-pi/1073741824) q[51];
cx q[51], q[22];
rz(pi/1073741824) q[22];
cx q[51], q[22];
rz(-pi/1073741824) q[22];
rz(-pi/536870912) q[51];
cx q[51], q[23];
rz(pi/536870912) q[23];
cx q[51], q[23];
rz(-pi/536870912) q[23];
rz(-pi/268435456) q[51];
cx q[51], q[24];
rz(pi/268435456) q[24];
cx q[51], q[24];
rz(-pi/268435456) q[24];
rz(-pi/134217728) q[51];
cx q[51], q[25];
rz(pi/134217728) q[25];
cx q[51], q[25];
rz(-pi/134217728) q[25];
rz(-pi/67108864) q[51];
cx q[51], q[26];
rz(pi/67108864) q[26];
cx q[51], q[26];
rz(-pi/67108864) q[26];
rz(-pi/33554432) q[51];
cx q[51], q[27];
rz(pi/33554432) q[27];
cx q[51], q[27];
rz(-pi/33554432) q[27];
rz(-pi/16777216) q[51];
cx q[51], q[28];
rz(pi/16777216) q[28];
cx q[51], q[28];
rz(-pi/16777216) q[28];
rz(-pi/8388608) q[51];
cx q[51], q[29];
rz(pi/8388608) q[29];
cx q[51], q[29];
rz(-pi/8388608) q[29];
rz(-pi/4194304) q[51];
cx q[51], q[30];
rz(pi/4194304) q[30];
cx q[51], q[30];
rz(-pi/4194304) q[30];
rz(-pi/2097152) q[51];
cx q[51], q[31];
rz(pi/2097152) q[31];
cx q[51], q[31];
rz(-pi/2097152) q[31];
rz(-pi/1048576) q[51];
cx q[51], q[32];
rz(pi/1048576) q[32];
cx q[51], q[32];
rz(-pi/1048576) q[32];
rz(-pi/524288) q[51];
cx q[51], q[33];
rz(pi/524288) q[33];
cx q[51], q[33];
rz(-pi/524288) q[33];
rz(-pi/262144) q[51];
cx q[51], q[34];
rz(pi/262144) q[34];
cx q[51], q[34];
rz(-pi/262144) q[34];
rz(-pi/131072) q[51];
cx q[51], q[35];
rz(pi/131072) q[35];
cx q[51], q[35];
rz(-pi/131072) q[35];
rz(-pi/65536) q[51];
cx q[51], q[36];
rz(pi/65536) q[36];
cx q[51], q[36];
rz(-pi/65536) q[36];
rz(-pi/32768) q[51];
cx q[51], q[37];
rz(pi/32768) q[37];
cx q[51], q[37];
rz(-pi/32768) q[37];
rz(-pi/16384) q[51];
cx q[51], q[38];
rz(pi/16384) q[38];
cx q[51], q[38];
rz(-pi/16384) q[38];
rz(-pi/8192) q[51];
cx q[51], q[39];
rz(pi/8192) q[39];
cx q[51], q[39];
rz(-pi/8192) q[39];
rz(-pi/4096) q[51];
cx q[51], q[40];
rz(pi/4096) q[40];
cx q[51], q[40];
rz(-pi/4096) q[40];
rz(-pi/2048) q[51];
cx q[51], q[41];
rz(pi/2048) q[41];
cx q[51], q[41];
rz(-pi/2048) q[41];
rz(-pi/1024) q[51];
cx q[51], q[42];
rz(pi/1024) q[42];
cx q[51], q[42];
rz(-pi/1024) q[42];
rz(-pi/512) q[51];
cx q[51], q[43];
rz(pi/512) q[43];
cx q[51], q[43];
rz(-pi/512) q[43];
rz(-pi/256) q[51];
cx q[51], q[44];
rz(pi/256) q[44];
cx q[51], q[44];
rz(-pi/256) q[44];
rz(-pi/128) q[51];
cx q[51], q[45];
rz(pi/128) q[45];
cx q[51], q[45];
rz(-pi/128) q[45];
rz(-pi/64) q[51];
cx q[51], q[46];
rz(pi/64) q[46];
cx q[51], q[46];
rz(-pi/64) q[46];
rz(-pi/32) q[51];
cx q[51], q[47];
rz(pi/32) q[47];
cx q[51], q[47];
rz(-pi/32) q[47];
rz(-pi/16) q[51];
cx q[51], q[48];
rz(pi/16) q[48];
cx q[51], q[48];
rz(-pi/16) q[48];
rz(-pi/8) q[51];
cx q[51], q[49];
rz(pi/8) q[49];
cx q[51], q[49];
rz(-pi/8) q[49];
rz(-pi/4) q[51];
cx q[51], q[50];
rz(pi/4) q[50];
cx q[51], q[50];
rz(-pi/4) q[50];
h q[51];
cx q[52], q[0];
rz(0) q[0];
cx q[52], q[0];
rz(0) q[0];
rz(0) q[52];
cx q[52], q[1];
rz(0) q[1];
cx q[52], q[1];
rz(0) q[1];
rz(0) q[52];
cx q[52], q[2];
rz(0) q[2];
cx q[52], q[2];
rz(0) q[2];
rz(0) q[52];
cx q[52], q[3];
rz(0) q[3];
cx q[52], q[3];
rz(0) q[3];
rz(0) q[52];
cx q[52], q[4];
rz(0) q[4];
cx q[52], q[4];
rz(0) q[4];
rz(0) q[52];
cx q[52], q[5];
rz(0) q[5];
cx q[52], q[5];
rz(0) q[5];
rz(0) q[52];
cx q[52], q[6];
rz(0) q[6];
cx q[52], q[6];
rz(0) q[6];
rz(0) q[52];
cx q[52], q[7];
rz(0) q[7];
cx q[52], q[7];
rz(0) q[7];
rz(0) q[52];
cx q[52], q[8];
rz(0) q[8];
cx q[52], q[8];
rz(0) q[8];
rz(0) q[52];
cx q[52], q[9];
rz(0) q[9];
cx q[52], q[9];
rz(0) q[9];
rz(0) q[52];
cx q[52], q[10];
rz(0) q[10];
cx q[52], q[10];
rz(0) q[10];
rz(0) q[52];
cx q[52], q[11];
rz(0) q[11];
cx q[52], q[11];
rz(0) q[11];
rz(0) q[52];
cx q[52], q[12];
rz(0) q[12];
cx q[52], q[12];
rz(0) q[12];
rz(0) q[52];
cx q[52], q[13];
rz(0) q[13];
cx q[52], q[13];
rz(0) q[13];
rz(0) q[52];
cx q[52], q[14];
rz(0) q[14];
cx q[52], q[14];
rz(0) q[14];
rz(0) q[52];
cx q[52], q[15];
rz(0) q[15];
cx q[52], q[15];
rz(0) q[15];
rz(0) q[52];
cx q[52], q[16];
rz(0) q[16];
cx q[52], q[16];
rz(0) q[16];
rz(0) q[52];
cx q[52], q[17];
rz(0) q[17];
cx q[52], q[17];
rz(0) q[17];
rz(0) q[52];
cx q[52], q[18];
rz(0) q[18];
cx q[52], q[18];
rz(0) q[18];
rz(0) q[52];
cx q[52], q[19];
rz(0) q[19];
cx q[52], q[19];
rz(0) q[19];
rz(0) q[52];
cx q[52], q[20];
rz(0) q[20];
cx q[52], q[20];
rz(0) q[20];
rz(0) q[52];
cx q[52], q[21];
rz(0) q[21];
cx q[52], q[21];
rz(0) q[21];
rz(-pi/2147483648) q[52];
cx q[52], q[22];
rz(pi/2147483648) q[22];
cx q[52], q[22];
rz(-pi/2147483648) q[22];
rz(-pi/1073741824) q[52];
cx q[52], q[23];
rz(pi/1073741824) q[23];
cx q[52], q[23];
rz(-pi/1073741824) q[23];
rz(-pi/536870912) q[52];
cx q[52], q[24];
rz(pi/536870912) q[24];
cx q[52], q[24];
rz(-pi/536870912) q[24];
rz(-pi/268435456) q[52];
cx q[52], q[25];
rz(pi/268435456) q[25];
cx q[52], q[25];
rz(-pi/268435456) q[25];
rz(-pi/134217728) q[52];
cx q[52], q[26];
rz(pi/134217728) q[26];
cx q[52], q[26];
rz(-pi/134217728) q[26];
rz(-pi/67108864) q[52];
cx q[52], q[27];
rz(pi/67108864) q[27];
cx q[52], q[27];
rz(-pi/67108864) q[27];
rz(-pi/33554432) q[52];
cx q[52], q[28];
rz(pi/33554432) q[28];
cx q[52], q[28];
rz(-pi/33554432) q[28];
rz(-pi/16777216) q[52];
cx q[52], q[29];
rz(pi/16777216) q[29];
cx q[52], q[29];
rz(-pi/16777216) q[29];
rz(-pi/8388608) q[52];
cx q[52], q[30];
rz(pi/8388608) q[30];
cx q[52], q[30];
rz(-pi/8388608) q[30];
rz(-pi/4194304) q[52];
cx q[52], q[31];
rz(pi/4194304) q[31];
cx q[52], q[31];
rz(-pi/4194304) q[31];
rz(-pi/2097152) q[52];
cx q[52], q[32];
rz(pi/2097152) q[32];
cx q[52], q[32];
rz(-pi/2097152) q[32];
rz(-pi/1048576) q[52];
cx q[52], q[33];
rz(pi/1048576) q[33];
cx q[52], q[33];
rz(-pi/1048576) q[33];
rz(-pi/524288) q[52];
cx q[52], q[34];
rz(pi/524288) q[34];
cx q[52], q[34];
rz(-pi/524288) q[34];
rz(-pi/262144) q[52];
cx q[52], q[35];
rz(pi/262144) q[35];
cx q[52], q[35];
rz(-pi/262144) q[35];
rz(-pi/131072) q[52];
cx q[52], q[36];
rz(pi/131072) q[36];
cx q[52], q[36];
rz(-pi/131072) q[36];
rz(-pi/65536) q[52];
cx q[52], q[37];
rz(pi/65536) q[37];
cx q[52], q[37];
rz(-pi/65536) q[37];
rz(-pi/32768) q[52];
cx q[52], q[38];
rz(pi/32768) q[38];
cx q[52], q[38];
rz(-pi/32768) q[38];
rz(-pi/16384) q[52];
cx q[52], q[39];
rz(pi/16384) q[39];
cx q[52], q[39];
rz(-pi/16384) q[39];
rz(-pi/8192) q[52];
cx q[52], q[40];
rz(pi/8192) q[40];
cx q[52], q[40];
rz(-pi/8192) q[40];
rz(-pi/4096) q[52];
cx q[52], q[41];
rz(pi/4096) q[41];
cx q[52], q[41];
rz(-pi/4096) q[41];
rz(-pi/2048) q[52];
cx q[52], q[42];
rz(pi/2048) q[42];
cx q[52], q[42];
rz(-pi/2048) q[42];
rz(-pi/1024) q[52];
cx q[52], q[43];
rz(pi/1024) q[43];
cx q[52], q[43];
rz(-pi/1024) q[43];
rz(-pi/512) q[52];
cx q[52], q[44];
rz(pi/512) q[44];
cx q[52], q[44];
rz(-pi/512) q[44];
rz(-pi/256) q[52];
cx q[52], q[45];
rz(pi/256) q[45];
cx q[52], q[45];
rz(-pi/256) q[45];
rz(-pi/128) q[52];
cx q[52], q[46];
rz(pi/128) q[46];
cx q[52], q[46];
rz(-pi/128) q[46];
rz(-pi/64) q[52];
cx q[52], q[47];
rz(pi/64) q[47];
cx q[52], q[47];
rz(-pi/64) q[47];
rz(-pi/32) q[52];
cx q[52], q[48];
rz(pi/32) q[48];
cx q[52], q[48];
rz(-pi/32) q[48];
rz(-pi/16) q[52];
cx q[52], q[49];
rz(pi/16) q[49];
cx q[52], q[49];
rz(-pi/16) q[49];
rz(-pi/8) q[52];
cx q[52], q[50];
rz(pi/8) q[50];
cx q[52], q[50];
rz(-pi/8) q[50];
rz(-pi/4) q[52];
cx q[52], q[51];
rz(pi/4) q[51];
cx q[52], q[51];
rz(-pi/4) q[51];
h q[52];
cx q[53], q[0];
rz(0) q[0];
cx q[53], q[0];
rz(0) q[0];
rz(0) q[53];
cx q[53], q[1];
rz(0) q[1];
cx q[53], q[1];
rz(0) q[1];
rz(0) q[53];
cx q[53], q[2];
rz(0) q[2];
cx q[53], q[2];
rz(0) q[2];
rz(0) q[53];
cx q[53], q[3];
rz(0) q[3];
cx q[53], q[3];
rz(0) q[3];
rz(0) q[53];
cx q[53], q[4];
rz(0) q[4];
cx q[53], q[4];
rz(0) q[4];
rz(0) q[53];
cx q[53], q[5];
rz(0) q[5];
cx q[53], q[5];
rz(0) q[5];
rz(0) q[53];
cx q[53], q[6];
rz(0) q[6];
cx q[53], q[6];
rz(0) q[6];
rz(0) q[53];
cx q[53], q[7];
rz(0) q[7];
cx q[53], q[7];
rz(0) q[7];
rz(0) q[53];
cx q[53], q[8];
rz(0) q[8];
cx q[53], q[8];
rz(0) q[8];
rz(0) q[53];
cx q[53], q[9];
rz(0) q[9];
cx q[53], q[9];
rz(0) q[9];
rz(0) q[53];
cx q[53], q[10];
rz(0) q[10];
cx q[53], q[10];
rz(0) q[10];
rz(0) q[53];
cx q[53], q[11];
rz(0) q[11];
cx q[53], q[11];
rz(0) q[11];
rz(0) q[53];
cx q[53], q[12];
rz(0) q[12];
cx q[53], q[12];
rz(0) q[12];
rz(0) q[53];
cx q[53], q[13];
rz(0) q[13];
cx q[53], q[13];
rz(0) q[13];
rz(0) q[53];
cx q[53], q[14];
rz(0) q[14];
cx q[53], q[14];
rz(0) q[14];
rz(0) q[53];
cx q[53], q[15];
rz(0) q[15];
cx q[53], q[15];
rz(0) q[15];
rz(0) q[53];
cx q[53], q[16];
rz(0) q[16];
cx q[53], q[16];
rz(0) q[16];
rz(0) q[53];
cx q[53], q[17];
rz(0) q[17];
cx q[53], q[17];
rz(0) q[17];
rz(0) q[53];
cx q[53], q[18];
rz(0) q[18];
cx q[53], q[18];
rz(0) q[18];
rz(0) q[53];
cx q[53], q[19];
rz(0) q[19];
cx q[53], q[19];
rz(0) q[19];
rz(0) q[53];
cx q[53], q[20];
rz(0) q[20];
cx q[53], q[20];
rz(0) q[20];
rz(0) q[53];
cx q[53], q[21];
rz(0) q[21];
cx q[53], q[21];
rz(0) q[21];
rz(0) q[53];
cx q[53], q[22];
rz(0) q[22];
cx q[53], q[22];
rz(0) q[22];
rz(-pi/2147483648) q[53];
cx q[53], q[23];
rz(pi/2147483648) q[23];
cx q[53], q[23];
rz(-pi/2147483648) q[23];
rz(-pi/1073741824) q[53];
cx q[53], q[24];
rz(pi/1073741824) q[24];
cx q[53], q[24];
rz(-pi/1073741824) q[24];
rz(-pi/536870912) q[53];
cx q[53], q[25];
rz(pi/536870912) q[25];
cx q[53], q[25];
rz(-pi/536870912) q[25];
rz(-pi/268435456) q[53];
cx q[53], q[26];
rz(pi/268435456) q[26];
cx q[53], q[26];
rz(-pi/268435456) q[26];
rz(-pi/134217728) q[53];
cx q[53], q[27];
rz(pi/134217728) q[27];
cx q[53], q[27];
rz(-pi/134217728) q[27];
rz(-pi/67108864) q[53];
cx q[53], q[28];
rz(pi/67108864) q[28];
cx q[53], q[28];
rz(-pi/67108864) q[28];
rz(-pi/33554432) q[53];
cx q[53], q[29];
rz(pi/33554432) q[29];
cx q[53], q[29];
rz(-pi/33554432) q[29];
rz(-pi/16777216) q[53];
cx q[53], q[30];
rz(pi/16777216) q[30];
cx q[53], q[30];
rz(-pi/16777216) q[30];
rz(-pi/8388608) q[53];
cx q[53], q[31];
rz(pi/8388608) q[31];
cx q[53], q[31];
rz(-pi/8388608) q[31];
rz(-pi/4194304) q[53];
cx q[53], q[32];
rz(pi/4194304) q[32];
cx q[53], q[32];
rz(-pi/4194304) q[32];
rz(-pi/2097152) q[53];
cx q[53], q[33];
rz(pi/2097152) q[33];
cx q[53], q[33];
rz(-pi/2097152) q[33];
rz(-pi/1048576) q[53];
cx q[53], q[34];
rz(pi/1048576) q[34];
cx q[53], q[34];
rz(-pi/1048576) q[34];
rz(-pi/524288) q[53];
cx q[53], q[35];
rz(pi/524288) q[35];
cx q[53], q[35];
rz(-pi/524288) q[35];
rz(-pi/262144) q[53];
cx q[53], q[36];
rz(pi/262144) q[36];
cx q[53], q[36];
rz(-pi/262144) q[36];
rz(-pi/131072) q[53];
cx q[53], q[37];
rz(pi/131072) q[37];
cx q[53], q[37];
rz(-pi/131072) q[37];
rz(-pi/65536) q[53];
cx q[53], q[38];
rz(pi/65536) q[38];
cx q[53], q[38];
rz(-pi/65536) q[38];
rz(-pi/32768) q[53];
cx q[53], q[39];
rz(pi/32768) q[39];
cx q[53], q[39];
rz(-pi/32768) q[39];
rz(-pi/16384) q[53];
cx q[53], q[40];
rz(pi/16384) q[40];
cx q[53], q[40];
rz(-pi/16384) q[40];
rz(-pi/8192) q[53];
cx q[53], q[41];
rz(pi/8192) q[41];
cx q[53], q[41];
rz(-pi/8192) q[41];
rz(-pi/4096) q[53];
cx q[53], q[42];
rz(pi/4096) q[42];
cx q[53], q[42];
rz(-pi/4096) q[42];
rz(-pi/2048) q[53];
cx q[53], q[43];
rz(pi/2048) q[43];
cx q[53], q[43];
rz(-pi/2048) q[43];
rz(-pi/1024) q[53];
cx q[53], q[44];
rz(pi/1024) q[44];
cx q[53], q[44];
rz(-pi/1024) q[44];
rz(-pi/512) q[53];
cx q[53], q[45];
rz(pi/512) q[45];
cx q[53], q[45];
rz(-pi/512) q[45];
rz(-pi/256) q[53];
cx q[53], q[46];
rz(pi/256) q[46];
cx q[53], q[46];
rz(-pi/256) q[46];
rz(-pi/128) q[53];
cx q[53], q[47];
rz(pi/128) q[47];
cx q[53], q[47];
rz(-pi/128) q[47];
rz(-pi/64) q[53];
cx q[53], q[48];
rz(pi/64) q[48];
cx q[53], q[48];
rz(-pi/64) q[48];
rz(-pi/32) q[53];
cx q[53], q[49];
rz(pi/32) q[49];
cx q[53], q[49];
rz(-pi/32) q[49];
rz(-pi/16) q[53];
cx q[53], q[50];
rz(pi/16) q[50];
cx q[53], q[50];
rz(-pi/16) q[50];
rz(-pi/8) q[53];
cx q[53], q[51];
rz(pi/8) q[51];
cx q[53], q[51];
rz(-pi/8) q[51];
rz(-pi/4) q[53];
cx q[53], q[52];
rz(pi/4) q[52];
cx q[53], q[52];
rz(-pi/4) q[52];
h q[53];
cx q[54], q[0];
rz(0) q[0];
cx q[54], q[0];
rz(0) q[0];
rz(0) q[54];
cx q[54], q[1];
rz(0) q[1];
cx q[54], q[1];
rz(0) q[1];
rz(0) q[54];
cx q[54], q[2];
rz(0) q[2];
cx q[54], q[2];
rz(0) q[2];
rz(0) q[54];
cx q[54], q[3];
rz(0) q[3];
cx q[54], q[3];
rz(0) q[3];
rz(0) q[54];
cx q[54], q[4];
rz(0) q[4];
cx q[54], q[4];
rz(0) q[4];
rz(0) q[54];
cx q[54], q[5];
rz(0) q[5];
cx q[54], q[5];
rz(0) q[5];
rz(0) q[54];
cx q[54], q[6];
rz(0) q[6];
cx q[54], q[6];
rz(0) q[6];
rz(0) q[54];
cx q[54], q[7];
rz(0) q[7];
cx q[54], q[7];
rz(0) q[7];
rz(0) q[54];
cx q[54], q[8];
rz(0) q[8];
cx q[54], q[8];
rz(0) q[8];
rz(0) q[54];
cx q[54], q[9];
rz(0) q[9];
cx q[54], q[9];
rz(0) q[9];
rz(0) q[54];
cx q[54], q[10];
rz(0) q[10];
cx q[54], q[10];
rz(0) q[10];
rz(0) q[54];
cx q[54], q[11];
rz(0) q[11];
cx q[54], q[11];
rz(0) q[11];
rz(0) q[54];
cx q[54], q[12];
rz(0) q[12];
cx q[54], q[12];
rz(0) q[12];
rz(0) q[54];
cx q[54], q[13];
rz(0) q[13];
cx q[54], q[13];
rz(0) q[13];
rz(0) q[54];
cx q[54], q[14];
rz(0) q[14];
cx q[54], q[14];
rz(0) q[14];
rz(0) q[54];
cx q[54], q[15];
rz(0) q[15];
cx q[54], q[15];
rz(0) q[15];
rz(0) q[54];
cx q[54], q[16];
rz(0) q[16];
cx q[54], q[16];
rz(0) q[16];
rz(0) q[54];
cx q[54], q[17];
rz(0) q[17];
cx q[54], q[17];
rz(0) q[17];
rz(0) q[54];
cx q[54], q[18];
rz(0) q[18];
cx q[54], q[18];
rz(0) q[18];
rz(0) q[54];
cx q[54], q[19];
rz(0) q[19];
cx q[54], q[19];
rz(0) q[19];
rz(0) q[54];
cx q[54], q[20];
rz(0) q[20];
cx q[54], q[20];
rz(0) q[20];
rz(0) q[54];
cx q[54], q[21];
rz(0) q[21];
cx q[54], q[21];
rz(0) q[21];
rz(0) q[54];
cx q[54], q[22];
rz(0) q[22];
cx q[54], q[22];
rz(0) q[22];
rz(0) q[54];
cx q[54], q[23];
rz(0) q[23];
cx q[54], q[23];
rz(0) q[23];
rz(-pi/2147483648) q[54];
cx q[54], q[24];
rz(pi/2147483648) q[24];
cx q[54], q[24];
rz(-pi/2147483648) q[24];
rz(-pi/1073741824) q[54];
cx q[54], q[25];
rz(pi/1073741824) q[25];
cx q[54], q[25];
rz(-pi/1073741824) q[25];
rz(-pi/536870912) q[54];
cx q[54], q[26];
rz(pi/536870912) q[26];
cx q[54], q[26];
rz(-pi/536870912) q[26];
rz(-pi/268435456) q[54];
cx q[54], q[27];
rz(pi/268435456) q[27];
cx q[54], q[27];
rz(-pi/268435456) q[27];
rz(-pi/134217728) q[54];
cx q[54], q[28];
rz(pi/134217728) q[28];
cx q[54], q[28];
rz(-pi/134217728) q[28];
rz(-pi/67108864) q[54];
cx q[54], q[29];
rz(pi/67108864) q[29];
cx q[54], q[29];
rz(-pi/67108864) q[29];
rz(-pi/33554432) q[54];
cx q[54], q[30];
rz(pi/33554432) q[30];
cx q[54], q[30];
rz(-pi/33554432) q[30];
rz(-pi/16777216) q[54];
cx q[54], q[31];
rz(pi/16777216) q[31];
cx q[54], q[31];
rz(-pi/16777216) q[31];
rz(-pi/8388608) q[54];
cx q[54], q[32];
rz(pi/8388608) q[32];
cx q[54], q[32];
rz(-pi/8388608) q[32];
rz(-pi/4194304) q[54];
cx q[54], q[33];
rz(pi/4194304) q[33];
cx q[54], q[33];
rz(-pi/4194304) q[33];
rz(-pi/2097152) q[54];
cx q[54], q[34];
rz(pi/2097152) q[34];
cx q[54], q[34];
rz(-pi/2097152) q[34];
rz(-pi/1048576) q[54];
cx q[54], q[35];
rz(pi/1048576) q[35];
cx q[54], q[35];
rz(-pi/1048576) q[35];
rz(-pi/524288) q[54];
cx q[54], q[36];
rz(pi/524288) q[36];
cx q[54], q[36];
rz(-pi/524288) q[36];
rz(-pi/262144) q[54];
cx q[54], q[37];
rz(pi/262144) q[37];
cx q[54], q[37];
rz(-pi/262144) q[37];
rz(-pi/131072) q[54];
cx q[54], q[38];
rz(pi/131072) q[38];
cx q[54], q[38];
rz(-pi/131072) q[38];
rz(-pi/65536) q[54];
cx q[54], q[39];
rz(pi/65536) q[39];
cx q[54], q[39];
rz(-pi/65536) q[39];
rz(-pi/32768) q[54];
cx q[54], q[40];
rz(pi/32768) q[40];
cx q[54], q[40];
rz(-pi/32768) q[40];
rz(-pi/16384) q[54];
cx q[54], q[41];
rz(pi/16384) q[41];
cx q[54], q[41];
rz(-pi/16384) q[41];
rz(-pi/8192) q[54];
cx q[54], q[42];
rz(pi/8192) q[42];
cx q[54], q[42];
rz(-pi/8192) q[42];
rz(-pi/4096) q[54];
cx q[54], q[43];
rz(pi/4096) q[43];
cx q[54], q[43];
rz(-pi/4096) q[43];
rz(-pi/2048) q[54];
cx q[54], q[44];
rz(pi/2048) q[44];
cx q[54], q[44];
rz(-pi/2048) q[44];
rz(-pi/1024) q[54];
cx q[54], q[45];
rz(pi/1024) q[45];
cx q[54], q[45];
rz(-pi/1024) q[45];
rz(-pi/512) q[54];
cx q[54], q[46];
rz(pi/512) q[46];
cx q[54], q[46];
rz(-pi/512) q[46];
rz(-pi/256) q[54];
cx q[54], q[47];
rz(pi/256) q[47];
cx q[54], q[47];
rz(-pi/256) q[47];
rz(-pi/128) q[54];
cx q[54], q[48];
rz(pi/128) q[48];
cx q[54], q[48];
rz(-pi/128) q[48];
rz(-pi/64) q[54];
cx q[54], q[49];
rz(pi/64) q[49];
cx q[54], q[49];
rz(-pi/64) q[49];
rz(-pi/32) q[54];
cx q[54], q[50];
rz(pi/32) q[50];
cx q[54], q[50];
rz(-pi/32) q[50];
rz(-pi/16) q[54];
cx q[54], q[51];
rz(pi/16) q[51];
cx q[54], q[51];
rz(-pi/16) q[51];
rz(-pi/8) q[54];
cx q[54], q[52];
rz(pi/8) q[52];
cx q[54], q[52];
rz(-pi/8) q[52];
rz(-pi/4) q[54];
cx q[54], q[53];
rz(pi/4) q[53];
cx q[54], q[53];
rz(-pi/4) q[53];
h q[54];
cx q[55], q[0];
rz(0) q[0];
cx q[55], q[0];
rz(0) q[0];
rz(0) q[55];
cx q[55], q[1];
rz(0) q[1];
cx q[55], q[1];
rz(0) q[1];
rz(0) q[55];
cx q[55], q[2];
rz(0) q[2];
cx q[55], q[2];
rz(0) q[2];
rz(0) q[55];
cx q[55], q[3];
rz(0) q[3];
cx q[55], q[3];
rz(0) q[3];
rz(0) q[55];
cx q[55], q[4];
rz(0) q[4];
cx q[55], q[4];
rz(0) q[4];
rz(0) q[55];
cx q[55], q[5];
rz(0) q[5];
cx q[55], q[5];
rz(0) q[5];
rz(0) q[55];
cx q[55], q[6];
rz(0) q[6];
cx q[55], q[6];
rz(0) q[6];
rz(0) q[55];
cx q[55], q[7];
rz(0) q[7];
cx q[55], q[7];
rz(0) q[7];
rz(0) q[55];
cx q[55], q[8];
rz(0) q[8];
cx q[55], q[8];
rz(0) q[8];
rz(0) q[55];
cx q[55], q[9];
rz(0) q[9];
cx q[55], q[9];
rz(0) q[9];
rz(0) q[55];
cx q[55], q[10];
rz(0) q[10];
cx q[55], q[10];
rz(0) q[10];
rz(0) q[55];
cx q[55], q[11];
rz(0) q[11];
cx q[55], q[11];
rz(0) q[11];
rz(0) q[55];
cx q[55], q[12];
rz(0) q[12];
cx q[55], q[12];
rz(0) q[12];
rz(0) q[55];
cx q[55], q[13];
rz(0) q[13];
cx q[55], q[13];
rz(0) q[13];
rz(0) q[55];
cx q[55], q[14];
rz(0) q[14];
cx q[55], q[14];
rz(0) q[14];
rz(0) q[55];
cx q[55], q[15];
rz(0) q[15];
cx q[55], q[15];
rz(0) q[15];
rz(0) q[55];
cx q[55], q[16];
rz(0) q[16];
cx q[55], q[16];
rz(0) q[16];
rz(0) q[55];
cx q[55], q[17];
rz(0) q[17];
cx q[55], q[17];
rz(0) q[17];
rz(0) q[55];
cx q[55], q[18];
rz(0) q[18];
cx q[55], q[18];
rz(0) q[18];
rz(0) q[55];
cx q[55], q[19];
rz(0) q[19];
cx q[55], q[19];
rz(0) q[19];
rz(0) q[55];
cx q[55], q[20];
rz(0) q[20];
cx q[55], q[20];
rz(0) q[20];
rz(0) q[55];
cx q[55], q[21];
rz(0) q[21];
cx q[55], q[21];
rz(0) q[21];
rz(0) q[55];
cx q[55], q[22];
rz(0) q[22];
cx q[55], q[22];
rz(0) q[22];
rz(0) q[55];
cx q[55], q[23];
rz(0) q[23];
cx q[55], q[23];
rz(0) q[23];
rz(0) q[55];
cx q[55], q[24];
rz(0) q[24];
cx q[55], q[24];
rz(0) q[24];
rz(-pi/2147483648) q[55];
cx q[55], q[25];
rz(pi/2147483648) q[25];
cx q[55], q[25];
rz(-pi/2147483648) q[25];
rz(-pi/1073741824) q[55];
cx q[55], q[26];
rz(pi/1073741824) q[26];
cx q[55], q[26];
rz(-pi/1073741824) q[26];
rz(-pi/536870912) q[55];
cx q[55], q[27];
rz(pi/536870912) q[27];
cx q[55], q[27];
rz(-pi/536870912) q[27];
rz(-pi/268435456) q[55];
cx q[55], q[28];
rz(pi/268435456) q[28];
cx q[55], q[28];
rz(-pi/268435456) q[28];
rz(-pi/134217728) q[55];
cx q[55], q[29];
rz(pi/134217728) q[29];
cx q[55], q[29];
rz(-pi/134217728) q[29];
rz(-pi/67108864) q[55];
cx q[55], q[30];
rz(pi/67108864) q[30];
cx q[55], q[30];
rz(-pi/67108864) q[30];
rz(-pi/33554432) q[55];
cx q[55], q[31];
rz(pi/33554432) q[31];
cx q[55], q[31];
rz(-pi/33554432) q[31];
rz(-pi/16777216) q[55];
cx q[55], q[32];
rz(pi/16777216) q[32];
cx q[55], q[32];
rz(-pi/16777216) q[32];
rz(-pi/8388608) q[55];
cx q[55], q[33];
rz(pi/8388608) q[33];
cx q[55], q[33];
rz(-pi/8388608) q[33];
rz(-pi/4194304) q[55];
cx q[55], q[34];
rz(pi/4194304) q[34];
cx q[55], q[34];
rz(-pi/4194304) q[34];
rz(-pi/2097152) q[55];
cx q[55], q[35];
rz(pi/2097152) q[35];
cx q[55], q[35];
rz(-pi/2097152) q[35];
rz(-pi/1048576) q[55];
cx q[55], q[36];
rz(pi/1048576) q[36];
cx q[55], q[36];
rz(-pi/1048576) q[36];
rz(-pi/524288) q[55];
cx q[55], q[37];
rz(pi/524288) q[37];
cx q[55], q[37];
rz(-pi/524288) q[37];
rz(-pi/262144) q[55];
cx q[55], q[38];
rz(pi/262144) q[38];
cx q[55], q[38];
rz(-pi/262144) q[38];
rz(-pi/131072) q[55];
cx q[55], q[39];
rz(pi/131072) q[39];
cx q[55], q[39];
rz(-pi/131072) q[39];
rz(-pi/65536) q[55];
cx q[55], q[40];
rz(pi/65536) q[40];
cx q[55], q[40];
rz(-pi/65536) q[40];
rz(-pi/32768) q[55];
cx q[55], q[41];
rz(pi/32768) q[41];
cx q[55], q[41];
rz(-pi/32768) q[41];
rz(-pi/16384) q[55];
cx q[55], q[42];
rz(pi/16384) q[42];
cx q[55], q[42];
rz(-pi/16384) q[42];
rz(-pi/8192) q[55];
cx q[55], q[43];
rz(pi/8192) q[43];
cx q[55], q[43];
rz(-pi/8192) q[43];
rz(-pi/4096) q[55];
cx q[55], q[44];
rz(pi/4096) q[44];
cx q[55], q[44];
rz(-pi/4096) q[44];
rz(-pi/2048) q[55];
cx q[55], q[45];
rz(pi/2048) q[45];
cx q[55], q[45];
rz(-pi/2048) q[45];
rz(-pi/1024) q[55];
cx q[55], q[46];
rz(pi/1024) q[46];
cx q[55], q[46];
rz(-pi/1024) q[46];
rz(-pi/512) q[55];
cx q[55], q[47];
rz(pi/512) q[47];
cx q[55], q[47];
rz(-pi/512) q[47];
rz(-pi/256) q[55];
cx q[55], q[48];
rz(pi/256) q[48];
cx q[55], q[48];
rz(-pi/256) q[48];
rz(-pi/128) q[55];
cx q[55], q[49];
rz(pi/128) q[49];
cx q[55], q[49];
rz(-pi/128) q[49];
rz(-pi/64) q[55];
cx q[55], q[50];
rz(pi/64) q[50];
cx q[55], q[50];
rz(-pi/64) q[50];
rz(-pi/32) q[55];
cx q[55], q[51];
rz(pi/32) q[51];
cx q[55], q[51];
rz(-pi/32) q[51];
rz(-pi/16) q[55];
cx q[55], q[52];
rz(pi/16) q[52];
cx q[55], q[52];
rz(-pi/16) q[52];
rz(-pi/8) q[55];
cx q[55], q[53];
rz(pi/8) q[53];
cx q[55], q[53];
rz(-pi/8) q[53];
rz(-pi/4) q[55];
cx q[55], q[54];
rz(pi/4) q[54];
cx q[55], q[54];
rz(-pi/4) q[54];
h q[55];
cx q[56], q[0];
rz(0) q[0];
cx q[56], q[0];
rz(0) q[0];
rz(0) q[56];
cx q[56], q[1];
rz(0) q[1];
cx q[56], q[1];
rz(0) q[1];
rz(0) q[56];
cx q[56], q[2];
rz(0) q[2];
cx q[56], q[2];
rz(0) q[2];
rz(0) q[56];
cx q[56], q[3];
rz(0) q[3];
cx q[56], q[3];
rz(0) q[3];
rz(0) q[56];
cx q[56], q[4];
rz(0) q[4];
cx q[56], q[4];
rz(0) q[4];
rz(0) q[56];
cx q[56], q[5];
rz(0) q[5];
cx q[56], q[5];
rz(0) q[5];
rz(0) q[56];
cx q[56], q[6];
rz(0) q[6];
cx q[56], q[6];
rz(0) q[6];
rz(0) q[56];
cx q[56], q[7];
rz(0) q[7];
cx q[56], q[7];
rz(0) q[7];
rz(0) q[56];
cx q[56], q[8];
rz(0) q[8];
cx q[56], q[8];
rz(0) q[8];
rz(0) q[56];
cx q[56], q[9];
rz(0) q[9];
cx q[56], q[9];
rz(0) q[9];
rz(0) q[56];
cx q[56], q[10];
rz(0) q[10];
cx q[56], q[10];
rz(0) q[10];
rz(0) q[56];
cx q[56], q[11];
rz(0) q[11];
cx q[56], q[11];
rz(0) q[11];
rz(0) q[56];
cx q[56], q[12];
rz(0) q[12];
cx q[56], q[12];
rz(0) q[12];
rz(0) q[56];
cx q[56], q[13];
rz(0) q[13];
cx q[56], q[13];
rz(0) q[13];
rz(0) q[56];
cx q[56], q[14];
rz(0) q[14];
cx q[56], q[14];
rz(0) q[14];
rz(0) q[56];
cx q[56], q[15];
rz(0) q[15];
cx q[56], q[15];
rz(0) q[15];
rz(0) q[56];
cx q[56], q[16];
rz(0) q[16];
cx q[56], q[16];
rz(0) q[16];
rz(0) q[56];
cx q[56], q[17];
rz(0) q[17];
cx q[56], q[17];
rz(0) q[17];
rz(0) q[56];
cx q[56], q[18];
rz(0) q[18];
cx q[56], q[18];
rz(0) q[18];
rz(0) q[56];
cx q[56], q[19];
rz(0) q[19];
cx q[56], q[19];
rz(0) q[19];
rz(0) q[56];
cx q[56], q[20];
rz(0) q[20];
cx q[56], q[20];
rz(0) q[20];
rz(0) q[56];
cx q[56], q[21];
rz(0) q[21];
cx q[56], q[21];
rz(0) q[21];
rz(0) q[56];
cx q[56], q[22];
rz(0) q[22];
cx q[56], q[22];
rz(0) q[22];
rz(0) q[56];
cx q[56], q[23];
rz(0) q[23];
cx q[56], q[23];
rz(0) q[23];
rz(0) q[56];
cx q[56], q[24];
rz(0) q[24];
cx q[56], q[24];
rz(0) q[24];
rz(0) q[56];
cx q[56], q[25];
rz(0) q[25];
cx q[56], q[25];
rz(0) q[25];
rz(-pi/2147483648) q[56];
cx q[56], q[26];
rz(pi/2147483648) q[26];
cx q[56], q[26];
rz(-pi/2147483648) q[26];
rz(-pi/1073741824) q[56];
cx q[56], q[27];
rz(pi/1073741824) q[27];
cx q[56], q[27];
rz(-pi/1073741824) q[27];
rz(-pi/536870912) q[56];
cx q[56], q[28];
rz(pi/536870912) q[28];
cx q[56], q[28];
rz(-pi/536870912) q[28];
rz(-pi/268435456) q[56];
cx q[56], q[29];
rz(pi/268435456) q[29];
cx q[56], q[29];
rz(-pi/268435456) q[29];
rz(-pi/134217728) q[56];
cx q[56], q[30];
rz(pi/134217728) q[30];
cx q[56], q[30];
rz(-pi/134217728) q[30];
rz(-pi/67108864) q[56];
cx q[56], q[31];
rz(pi/67108864) q[31];
cx q[56], q[31];
rz(-pi/67108864) q[31];
rz(-pi/33554432) q[56];
cx q[56], q[32];
rz(pi/33554432) q[32];
cx q[56], q[32];
rz(-pi/33554432) q[32];
rz(-pi/16777216) q[56];
cx q[56], q[33];
rz(pi/16777216) q[33];
cx q[56], q[33];
rz(-pi/16777216) q[33];
rz(-pi/8388608) q[56];
cx q[56], q[34];
rz(pi/8388608) q[34];
cx q[56], q[34];
rz(-pi/8388608) q[34];
rz(-pi/4194304) q[56];
cx q[56], q[35];
rz(pi/4194304) q[35];
cx q[56], q[35];
rz(-pi/4194304) q[35];
rz(-pi/2097152) q[56];
cx q[56], q[36];
rz(pi/2097152) q[36];
cx q[56], q[36];
rz(-pi/2097152) q[36];
rz(-pi/1048576) q[56];
cx q[56], q[37];
rz(pi/1048576) q[37];
cx q[56], q[37];
rz(-pi/1048576) q[37];
rz(-pi/524288) q[56];
cx q[56], q[38];
rz(pi/524288) q[38];
cx q[56], q[38];
rz(-pi/524288) q[38];
rz(-pi/262144) q[56];
cx q[56], q[39];
rz(pi/262144) q[39];
cx q[56], q[39];
rz(-pi/262144) q[39];
rz(-pi/131072) q[56];
cx q[56], q[40];
rz(pi/131072) q[40];
cx q[56], q[40];
rz(-pi/131072) q[40];
rz(-pi/65536) q[56];
cx q[56], q[41];
rz(pi/65536) q[41];
cx q[56], q[41];
rz(-pi/65536) q[41];
rz(-pi/32768) q[56];
cx q[56], q[42];
rz(pi/32768) q[42];
cx q[56], q[42];
rz(-pi/32768) q[42];
rz(-pi/16384) q[56];
cx q[56], q[43];
rz(pi/16384) q[43];
cx q[56], q[43];
rz(-pi/16384) q[43];
rz(-pi/8192) q[56];
cx q[56], q[44];
rz(pi/8192) q[44];
cx q[56], q[44];
rz(-pi/8192) q[44];
rz(-pi/4096) q[56];
cx q[56], q[45];
rz(pi/4096) q[45];
cx q[56], q[45];
rz(-pi/4096) q[45];
rz(-pi/2048) q[56];
cx q[56], q[46];
rz(pi/2048) q[46];
cx q[56], q[46];
rz(-pi/2048) q[46];
rz(-pi/1024) q[56];
cx q[56], q[47];
rz(pi/1024) q[47];
cx q[56], q[47];
rz(-pi/1024) q[47];
rz(-pi/512) q[56];
cx q[56], q[48];
rz(pi/512) q[48];
cx q[56], q[48];
rz(-pi/512) q[48];
rz(-pi/256) q[56];
cx q[56], q[49];
rz(pi/256) q[49];
cx q[56], q[49];
rz(-pi/256) q[49];
rz(-pi/128) q[56];
cx q[56], q[50];
rz(pi/128) q[50];
cx q[56], q[50];
rz(-pi/128) q[50];
rz(-pi/64) q[56];
cx q[56], q[51];
rz(pi/64) q[51];
cx q[56], q[51];
rz(-pi/64) q[51];
rz(-pi/32) q[56];
cx q[56], q[52];
rz(pi/32) q[52];
cx q[56], q[52];
rz(-pi/32) q[52];
rz(-pi/16) q[56];
cx q[56], q[53];
rz(pi/16) q[53];
cx q[56], q[53];
rz(-pi/16) q[53];
rz(-pi/8) q[56];
cx q[56], q[54];
rz(pi/8) q[54];
cx q[56], q[54];
rz(-pi/8) q[54];
rz(-pi/4) q[56];
cx q[56], q[55];
rz(pi/4) q[55];
cx q[56], q[55];
rz(-pi/4) q[55];
h q[56];
cx q[57], q[0];
rz(0) q[0];
cx q[57], q[0];
rz(0) q[0];
rz(0) q[57];
cx q[57], q[1];
rz(0) q[1];
cx q[57], q[1];
rz(0) q[1];
rz(0) q[57];
cx q[57], q[2];
rz(0) q[2];
cx q[57], q[2];
rz(0) q[2];
rz(0) q[57];
cx q[57], q[3];
rz(0) q[3];
cx q[57], q[3];
rz(0) q[3];
rz(0) q[57];
cx q[57], q[4];
rz(0) q[4];
cx q[57], q[4];
rz(0) q[4];
rz(0) q[57];
cx q[57], q[5];
rz(0) q[5];
cx q[57], q[5];
rz(0) q[5];
rz(0) q[57];
cx q[57], q[6];
rz(0) q[6];
cx q[57], q[6];
rz(0) q[6];
rz(0) q[57];
cx q[57], q[7];
rz(0) q[7];
cx q[57], q[7];
rz(0) q[7];
rz(0) q[57];
cx q[57], q[8];
rz(0) q[8];
cx q[57], q[8];
rz(0) q[8];
rz(0) q[57];
cx q[57], q[9];
rz(0) q[9];
cx q[57], q[9];
rz(0) q[9];
rz(0) q[57];
cx q[57], q[10];
rz(0) q[10];
cx q[57], q[10];
rz(0) q[10];
rz(0) q[57];
cx q[57], q[11];
rz(0) q[11];
cx q[57], q[11];
rz(0) q[11];
rz(0) q[57];
cx q[57], q[12];
rz(0) q[12];
cx q[57], q[12];
rz(0) q[12];
rz(0) q[57];
cx q[57], q[13];
rz(0) q[13];
cx q[57], q[13];
rz(0) q[13];
rz(0) q[57];
cx q[57], q[14];
rz(0) q[14];
cx q[57], q[14];
rz(0) q[14];
rz(0) q[57];
cx q[57], q[15];
rz(0) q[15];
cx q[57], q[15];
rz(0) q[15];
rz(0) q[57];
cx q[57], q[16];
rz(0) q[16];
cx q[57], q[16];
rz(0) q[16];
rz(0) q[57];
cx q[57], q[17];
rz(0) q[17];
cx q[57], q[17];
rz(0) q[17];
rz(0) q[57];
cx q[57], q[18];
rz(0) q[18];
cx q[57], q[18];
rz(0) q[18];
rz(0) q[57];
cx q[57], q[19];
rz(0) q[19];
cx q[57], q[19];
rz(0) q[19];
rz(0) q[57];
cx q[57], q[20];
rz(0) q[20];
cx q[57], q[20];
rz(0) q[20];
rz(0) q[57];
cx q[57], q[21];
rz(0) q[21];
cx q[57], q[21];
rz(0) q[21];
rz(0) q[57];
cx q[57], q[22];
rz(0) q[22];
cx q[57], q[22];
rz(0) q[22];
rz(0) q[57];
cx q[57], q[23];
rz(0) q[23];
cx q[57], q[23];
rz(0) q[23];
rz(0) q[57];
cx q[57], q[24];
rz(0) q[24];
cx q[57], q[24];
rz(0) q[24];
rz(0) q[57];
cx q[57], q[25];
rz(0) q[25];
cx q[57], q[25];
rz(0) q[25];
rz(0) q[57];
cx q[57], q[26];
rz(0) q[26];
cx q[57], q[26];
rz(0) q[26];
rz(-pi/2147483648) q[57];
cx q[57], q[27];
rz(pi/2147483648) q[27];
cx q[57], q[27];
rz(-pi/2147483648) q[27];
rz(-pi/1073741824) q[57];
cx q[57], q[28];
rz(pi/1073741824) q[28];
cx q[57], q[28];
rz(-pi/1073741824) q[28];
rz(-pi/536870912) q[57];
cx q[57], q[29];
rz(pi/536870912) q[29];
cx q[57], q[29];
rz(-pi/536870912) q[29];
rz(-pi/268435456) q[57];
cx q[57], q[30];
rz(pi/268435456) q[30];
cx q[57], q[30];
rz(-pi/268435456) q[30];
rz(-pi/134217728) q[57];
cx q[57], q[31];
rz(pi/134217728) q[31];
cx q[57], q[31];
rz(-pi/134217728) q[31];
rz(-pi/67108864) q[57];
cx q[57], q[32];
rz(pi/67108864) q[32];
cx q[57], q[32];
rz(-pi/67108864) q[32];
rz(-pi/33554432) q[57];
cx q[57], q[33];
rz(pi/33554432) q[33];
cx q[57], q[33];
rz(-pi/33554432) q[33];
rz(-pi/16777216) q[57];
cx q[57], q[34];
rz(pi/16777216) q[34];
cx q[57], q[34];
rz(-pi/16777216) q[34];
rz(-pi/8388608) q[57];
cx q[57], q[35];
rz(pi/8388608) q[35];
cx q[57], q[35];
rz(-pi/8388608) q[35];
rz(-pi/4194304) q[57];
cx q[57], q[36];
rz(pi/4194304) q[36];
cx q[57], q[36];
rz(-pi/4194304) q[36];
rz(-pi/2097152) q[57];
cx q[57], q[37];
rz(pi/2097152) q[37];
cx q[57], q[37];
rz(-pi/2097152) q[37];
rz(-pi/1048576) q[57];
cx q[57], q[38];
rz(pi/1048576) q[38];
cx q[57], q[38];
rz(-pi/1048576) q[38];
rz(-pi/524288) q[57];
cx q[57], q[39];
rz(pi/524288) q[39];
cx q[57], q[39];
rz(-pi/524288) q[39];
rz(-pi/262144) q[57];
cx q[57], q[40];
rz(pi/262144) q[40];
cx q[57], q[40];
rz(-pi/262144) q[40];
rz(-pi/131072) q[57];
cx q[57], q[41];
rz(pi/131072) q[41];
cx q[57], q[41];
rz(-pi/131072) q[41];
rz(-pi/65536) q[57];
cx q[57], q[42];
rz(pi/65536) q[42];
cx q[57], q[42];
rz(-pi/65536) q[42];
rz(-pi/32768) q[57];
cx q[57], q[43];
rz(pi/32768) q[43];
cx q[57], q[43];
rz(-pi/32768) q[43];
rz(-pi/16384) q[57];
cx q[57], q[44];
rz(pi/16384) q[44];
cx q[57], q[44];
rz(-pi/16384) q[44];
rz(-pi/8192) q[57];
cx q[57], q[45];
rz(pi/8192) q[45];
cx q[57], q[45];
rz(-pi/8192) q[45];
rz(-pi/4096) q[57];
cx q[57], q[46];
rz(pi/4096) q[46];
cx q[57], q[46];
rz(-pi/4096) q[46];
rz(-pi/2048) q[57];
cx q[57], q[47];
rz(pi/2048) q[47];
cx q[57], q[47];
rz(-pi/2048) q[47];
rz(-pi/1024) q[57];
cx q[57], q[48];
rz(pi/1024) q[48];
cx q[57], q[48];
rz(-pi/1024) q[48];
rz(-pi/512) q[57];
cx q[57], q[49];
rz(pi/512) q[49];
cx q[57], q[49];
rz(-pi/512) q[49];
rz(-pi/256) q[57];
cx q[57], q[50];
rz(pi/256) q[50];
cx q[57], q[50];
rz(-pi/256) q[50];
rz(-pi/128) q[57];
cx q[57], q[51];
rz(pi/128) q[51];
cx q[57], q[51];
rz(-pi/128) q[51];
rz(-pi/64) q[57];
cx q[57], q[52];
rz(pi/64) q[52];
cx q[57], q[52];
rz(-pi/64) q[52];
rz(-pi/32) q[57];
cx q[57], q[53];
rz(pi/32) q[53];
cx q[57], q[53];
rz(-pi/32) q[53];
rz(-pi/16) q[57];
cx q[57], q[54];
rz(pi/16) q[54];
cx q[57], q[54];
rz(-pi/16) q[54];
rz(-pi/8) q[57];
cx q[57], q[55];
rz(pi/8) q[55];
cx q[57], q[55];
rz(-pi/8) q[55];
rz(-pi/4) q[57];
cx q[57], q[56];
rz(pi/4) q[56];
cx q[57], q[56];
rz(-pi/4) q[56];
h q[57];
cx q[58], q[0];
rz(0) q[0];
cx q[58], q[0];
rz(0) q[0];
rz(0) q[58];
cx q[58], q[1];
rz(0) q[1];
cx q[58], q[1];
rz(0) q[1];
rz(0) q[58];
cx q[58], q[2];
rz(0) q[2];
cx q[58], q[2];
rz(0) q[2];
rz(0) q[58];
cx q[58], q[3];
rz(0) q[3];
cx q[58], q[3];
rz(0) q[3];
rz(0) q[58];
cx q[58], q[4];
rz(0) q[4];
cx q[58], q[4];
rz(0) q[4];
rz(0) q[58];
cx q[58], q[5];
rz(0) q[5];
cx q[58], q[5];
rz(0) q[5];
rz(0) q[58];
cx q[58], q[6];
rz(0) q[6];
cx q[58], q[6];
rz(0) q[6];
rz(0) q[58];
cx q[58], q[7];
rz(0) q[7];
cx q[58], q[7];
rz(0) q[7];
rz(0) q[58];
cx q[58], q[8];
rz(0) q[8];
cx q[58], q[8];
rz(0) q[8];
rz(0) q[58];
cx q[58], q[9];
rz(0) q[9];
cx q[58], q[9];
rz(0) q[9];
rz(0) q[58];
cx q[58], q[10];
rz(0) q[10];
cx q[58], q[10];
rz(0) q[10];
rz(0) q[58];
cx q[58], q[11];
rz(0) q[11];
cx q[58], q[11];
rz(0) q[11];
rz(0) q[58];
cx q[58], q[12];
rz(0) q[12];
cx q[58], q[12];
rz(0) q[12];
rz(0) q[58];
cx q[58], q[13];
rz(0) q[13];
cx q[58], q[13];
rz(0) q[13];
rz(0) q[58];
cx q[58], q[14];
rz(0) q[14];
cx q[58], q[14];
rz(0) q[14];
rz(0) q[58];
cx q[58], q[15];
rz(0) q[15];
cx q[58], q[15];
rz(0) q[15];
rz(0) q[58];
cx q[58], q[16];
rz(0) q[16];
cx q[58], q[16];
rz(0) q[16];
rz(0) q[58];
cx q[58], q[17];
rz(0) q[17];
cx q[58], q[17];
rz(0) q[17];
rz(0) q[58];
cx q[58], q[18];
rz(0) q[18];
cx q[58], q[18];
rz(0) q[18];
rz(0) q[58];
cx q[58], q[19];
rz(0) q[19];
cx q[58], q[19];
rz(0) q[19];
rz(0) q[58];
cx q[58], q[20];
rz(0) q[20];
cx q[58], q[20];
rz(0) q[20];
rz(0) q[58];
cx q[58], q[21];
rz(0) q[21];
cx q[58], q[21];
rz(0) q[21];
rz(0) q[58];
cx q[58], q[22];
rz(0) q[22];
cx q[58], q[22];
rz(0) q[22];
rz(0) q[58];
cx q[58], q[23];
rz(0) q[23];
cx q[58], q[23];
rz(0) q[23];
rz(0) q[58];
cx q[58], q[24];
rz(0) q[24];
cx q[58], q[24];
rz(0) q[24];
rz(0) q[58];
cx q[58], q[25];
rz(0) q[25];
cx q[58], q[25];
rz(0) q[25];
rz(0) q[58];
cx q[58], q[26];
rz(0) q[26];
cx q[58], q[26];
rz(0) q[26];
rz(0) q[58];
cx q[58], q[27];
rz(0) q[27];
cx q[58], q[27];
rz(0) q[27];
rz(-pi/2147483648) q[58];
cx q[58], q[28];
rz(pi/2147483648) q[28];
cx q[58], q[28];
rz(-pi/2147483648) q[28];
rz(-pi/1073741824) q[58];
cx q[58], q[29];
rz(pi/1073741824) q[29];
cx q[58], q[29];
rz(-pi/1073741824) q[29];
rz(-pi/536870912) q[58];
cx q[58], q[30];
rz(pi/536870912) q[30];
cx q[58], q[30];
rz(-pi/536870912) q[30];
rz(-pi/268435456) q[58];
cx q[58], q[31];
rz(pi/268435456) q[31];
cx q[58], q[31];
rz(-pi/268435456) q[31];
rz(-pi/134217728) q[58];
cx q[58], q[32];
rz(pi/134217728) q[32];
cx q[58], q[32];
rz(-pi/134217728) q[32];
rz(-pi/67108864) q[58];
cx q[58], q[33];
rz(pi/67108864) q[33];
cx q[58], q[33];
rz(-pi/67108864) q[33];
rz(-pi/33554432) q[58];
cx q[58], q[34];
rz(pi/33554432) q[34];
cx q[58], q[34];
rz(-pi/33554432) q[34];
rz(-pi/16777216) q[58];
cx q[58], q[35];
rz(pi/16777216) q[35];
cx q[58], q[35];
rz(-pi/16777216) q[35];
rz(-pi/8388608) q[58];
cx q[58], q[36];
rz(pi/8388608) q[36];
cx q[58], q[36];
rz(-pi/8388608) q[36];
rz(-pi/4194304) q[58];
cx q[58], q[37];
rz(pi/4194304) q[37];
cx q[58], q[37];
rz(-pi/4194304) q[37];
rz(-pi/2097152) q[58];
cx q[58], q[38];
rz(pi/2097152) q[38];
cx q[58], q[38];
rz(-pi/2097152) q[38];
rz(-pi/1048576) q[58];
cx q[58], q[39];
rz(pi/1048576) q[39];
cx q[58], q[39];
rz(-pi/1048576) q[39];
rz(-pi/524288) q[58];
cx q[58], q[40];
rz(pi/524288) q[40];
cx q[58], q[40];
rz(-pi/524288) q[40];
rz(-pi/262144) q[58];
cx q[58], q[41];
rz(pi/262144) q[41];
cx q[58], q[41];
rz(-pi/262144) q[41];
rz(-pi/131072) q[58];
cx q[58], q[42];
rz(pi/131072) q[42];
cx q[58], q[42];
rz(-pi/131072) q[42];
rz(-pi/65536) q[58];
cx q[58], q[43];
rz(pi/65536) q[43];
cx q[58], q[43];
rz(-pi/65536) q[43];
rz(-pi/32768) q[58];
cx q[58], q[44];
rz(pi/32768) q[44];
cx q[58], q[44];
rz(-pi/32768) q[44];
rz(-pi/16384) q[58];
cx q[58], q[45];
rz(pi/16384) q[45];
cx q[58], q[45];
rz(-pi/16384) q[45];
rz(-pi/8192) q[58];
cx q[58], q[46];
rz(pi/8192) q[46];
cx q[58], q[46];
rz(-pi/8192) q[46];
rz(-pi/4096) q[58];
cx q[58], q[47];
rz(pi/4096) q[47];
cx q[58], q[47];
rz(-pi/4096) q[47];
rz(-pi/2048) q[58];
cx q[58], q[48];
rz(pi/2048) q[48];
cx q[58], q[48];
rz(-pi/2048) q[48];
rz(-pi/1024) q[58];
cx q[58], q[49];
rz(pi/1024) q[49];
cx q[58], q[49];
rz(-pi/1024) q[49];
rz(-pi/512) q[58];
cx q[58], q[50];
rz(pi/512) q[50];
cx q[58], q[50];
rz(-pi/512) q[50];
rz(-pi/256) q[58];
cx q[58], q[51];
rz(pi/256) q[51];
cx q[58], q[51];
rz(-pi/256) q[51];
rz(-pi/128) q[58];
cx q[58], q[52];
rz(pi/128) q[52];
cx q[58], q[52];
rz(-pi/128) q[52];
rz(-pi/64) q[58];
cx q[58], q[53];
rz(pi/64) q[53];
cx q[58], q[53];
rz(-pi/64) q[53];
rz(-pi/32) q[58];
cx q[58], q[54];
rz(pi/32) q[54];
cx q[58], q[54];
rz(-pi/32) q[54];
rz(-pi/16) q[58];
cx q[58], q[55];
rz(pi/16) q[55];
cx q[58], q[55];
rz(-pi/16) q[55];
rz(-pi/8) q[58];
cx q[58], q[56];
rz(pi/8) q[56];
cx q[58], q[56];
rz(-pi/8) q[56];
rz(-pi/4) q[58];
cx q[58], q[57];
rz(pi/4) q[57];
cx q[58], q[57];
rz(-pi/4) q[57];
h q[58];
cx q[59], q[0];
rz(0) q[0];
cx q[59], q[0];
rz(0) q[0];
rz(0) q[59];
cx q[59], q[1];
rz(0) q[1];
cx q[59], q[1];
rz(0) q[1];
rz(0) q[59];
cx q[59], q[2];
rz(0) q[2];
cx q[59], q[2];
rz(0) q[2];
rz(0) q[59];
cx q[59], q[3];
rz(0) q[3];
cx q[59], q[3];
rz(0) q[3];
rz(0) q[59];
cx q[59], q[4];
rz(0) q[4];
cx q[59], q[4];
rz(0) q[4];
rz(0) q[59];
cx q[59], q[5];
rz(0) q[5];
cx q[59], q[5];
rz(0) q[5];
rz(0) q[59];
cx q[59], q[6];
rz(0) q[6];
cx q[59], q[6];
rz(0) q[6];
rz(0) q[59];
cx q[59], q[7];
rz(0) q[7];
cx q[59], q[7];
rz(0) q[7];
rz(0) q[59];
cx q[59], q[8];
rz(0) q[8];
cx q[59], q[8];
rz(0) q[8];
rz(0) q[59];
cx q[59], q[9];
rz(0) q[9];
cx q[59], q[9];
rz(0) q[9];
rz(0) q[59];
cx q[59], q[10];
rz(0) q[10];
cx q[59], q[10];
rz(0) q[10];
rz(0) q[59];
cx q[59], q[11];
rz(0) q[11];
cx q[59], q[11];
rz(0) q[11];
rz(0) q[59];
cx q[59], q[12];
rz(0) q[12];
cx q[59], q[12];
rz(0) q[12];
rz(0) q[59];
cx q[59], q[13];
rz(0) q[13];
cx q[59], q[13];
rz(0) q[13];
rz(0) q[59];
cx q[59], q[14];
rz(0) q[14];
cx q[59], q[14];
rz(0) q[14];
rz(0) q[59];
cx q[59], q[15];
rz(0) q[15];
cx q[59], q[15];
rz(0) q[15];
rz(0) q[59];
cx q[59], q[16];
rz(0) q[16];
cx q[59], q[16];
rz(0) q[16];
rz(0) q[59];
cx q[59], q[17];
rz(0) q[17];
cx q[59], q[17];
rz(0) q[17];
rz(0) q[59];
cx q[59], q[18];
rz(0) q[18];
cx q[59], q[18];
rz(0) q[18];
rz(0) q[59];
cx q[59], q[19];
rz(0) q[19];
cx q[59], q[19];
rz(0) q[19];
rz(0) q[59];
cx q[59], q[20];
rz(0) q[20];
cx q[59], q[20];
rz(0) q[20];
rz(0) q[59];
cx q[59], q[21];
rz(0) q[21];
cx q[59], q[21];
rz(0) q[21];
rz(0) q[59];
cx q[59], q[22];
rz(0) q[22];
cx q[59], q[22];
rz(0) q[22];
rz(0) q[59];
cx q[59], q[23];
rz(0) q[23];
cx q[59], q[23];
rz(0) q[23];
rz(0) q[59];
cx q[59], q[24];
rz(0) q[24];
cx q[59], q[24];
rz(0) q[24];
rz(0) q[59];
cx q[59], q[25];
rz(0) q[25];
cx q[59], q[25];
rz(0) q[25];
rz(0) q[59];
cx q[59], q[26];
rz(0) q[26];
cx q[59], q[26];
rz(0) q[26];
rz(0) q[59];
cx q[59], q[27];
rz(0) q[27];
cx q[59], q[27];
rz(0) q[27];
rz(0) q[59];
cx q[59], q[28];
rz(0) q[28];
cx q[59], q[28];
rz(0) q[28];
rz(-pi/2147483648) q[59];
cx q[59], q[29];
rz(pi/2147483648) q[29];
cx q[59], q[29];
rz(-pi/2147483648) q[29];
rz(-pi/1073741824) q[59];
cx q[59], q[30];
rz(pi/1073741824) q[30];
cx q[59], q[30];
rz(-pi/1073741824) q[30];
rz(-pi/536870912) q[59];
cx q[59], q[31];
rz(pi/536870912) q[31];
cx q[59], q[31];
rz(-pi/536870912) q[31];
rz(-pi/268435456) q[59];
cx q[59], q[32];
rz(pi/268435456) q[32];
cx q[59], q[32];
rz(-pi/268435456) q[32];
rz(-pi/134217728) q[59];
cx q[59], q[33];
rz(pi/134217728) q[33];
cx q[59], q[33];
rz(-pi/134217728) q[33];
rz(-pi/67108864) q[59];
cx q[59], q[34];
rz(pi/67108864) q[34];
cx q[59], q[34];
rz(-pi/67108864) q[34];
rz(-pi/33554432) q[59];
cx q[59], q[35];
rz(pi/33554432) q[35];
cx q[59], q[35];
rz(-pi/33554432) q[35];
rz(-pi/16777216) q[59];
cx q[59], q[36];
rz(pi/16777216) q[36];
cx q[59], q[36];
rz(-pi/16777216) q[36];
rz(-pi/8388608) q[59];
cx q[59], q[37];
rz(pi/8388608) q[37];
cx q[59], q[37];
rz(-pi/8388608) q[37];
rz(-pi/4194304) q[59];
cx q[59], q[38];
rz(pi/4194304) q[38];
cx q[59], q[38];
rz(-pi/4194304) q[38];
rz(-pi/2097152) q[59];
cx q[59], q[39];
rz(pi/2097152) q[39];
cx q[59], q[39];
rz(-pi/2097152) q[39];
rz(-pi/1048576) q[59];
cx q[59], q[40];
rz(pi/1048576) q[40];
cx q[59], q[40];
rz(-pi/1048576) q[40];
rz(-pi/524288) q[59];
cx q[59], q[41];
rz(pi/524288) q[41];
cx q[59], q[41];
rz(-pi/524288) q[41];
rz(-pi/262144) q[59];
cx q[59], q[42];
rz(pi/262144) q[42];
cx q[59], q[42];
rz(-pi/262144) q[42];
rz(-pi/131072) q[59];
cx q[59], q[43];
rz(pi/131072) q[43];
cx q[59], q[43];
rz(-pi/131072) q[43];
rz(-pi/65536) q[59];
cx q[59], q[44];
rz(pi/65536) q[44];
cx q[59], q[44];
rz(-pi/65536) q[44];
rz(-pi/32768) q[59];
cx q[59], q[45];
rz(pi/32768) q[45];
cx q[59], q[45];
rz(-pi/32768) q[45];
rz(-pi/16384) q[59];
cx q[59], q[46];
rz(pi/16384) q[46];
cx q[59], q[46];
rz(-pi/16384) q[46];
rz(-pi/8192) q[59];
cx q[59], q[47];
rz(pi/8192) q[47];
cx q[59], q[47];
rz(-pi/8192) q[47];
rz(-pi/4096) q[59];
cx q[59], q[48];
rz(pi/4096) q[48];
cx q[59], q[48];
rz(-pi/4096) q[48];
rz(-pi/2048) q[59];
cx q[59], q[49];
rz(pi/2048) q[49];
cx q[59], q[49];
rz(-pi/2048) q[49];
rz(-pi/1024) q[59];
cx q[59], q[50];
rz(pi/1024) q[50];
cx q[59], q[50];
rz(-pi/1024) q[50];
rz(-pi/512) q[59];
cx q[59], q[51];
rz(pi/512) q[51];
cx q[59], q[51];
rz(-pi/512) q[51];
rz(-pi/256) q[59];
cx q[59], q[52];
rz(pi/256) q[52];
cx q[59], q[52];
rz(-pi/256) q[52];
rz(-pi/128) q[59];
cx q[59], q[53];
rz(pi/128) q[53];
cx q[59], q[53];
rz(-pi/128) q[53];
rz(-pi/64) q[59];
cx q[59], q[54];
rz(pi/64) q[54];
cx q[59], q[54];
rz(-pi/64) q[54];
rz(-pi/32) q[59];
cx q[59], q[55];
rz(pi/32) q[55];
cx q[59], q[55];
rz(-pi/32) q[55];
rz(-pi/16) q[59];
cx q[59], q[56];
rz(pi/16) q[56];
cx q[59], q[56];
rz(-pi/16) q[56];
rz(-pi/8) q[59];
cx q[59], q[57];
rz(pi/8) q[57];
cx q[59], q[57];
rz(-pi/8) q[57];
rz(-pi/4) q[59];
cx q[59], q[58];
rz(pi/4) q[58];
cx q[59], q[58];
rz(-pi/4) q[58];
h q[59];
cx q[60], q[0];
rz(0) q[0];
cx q[60], q[0];
rz(0) q[0];
rz(0) q[60];
cx q[60], q[1];
rz(0) q[1];
cx q[60], q[1];
rz(0) q[1];
rz(0) q[60];
cx q[60], q[2];
rz(0) q[2];
cx q[60], q[2];
rz(0) q[2];
rz(0) q[60];
cx q[60], q[3];
rz(0) q[3];
cx q[60], q[3];
rz(0) q[3];
rz(0) q[60];
cx q[60], q[4];
rz(0) q[4];
cx q[60], q[4];
rz(0) q[4];
rz(0) q[60];
cx q[60], q[5];
rz(0) q[5];
cx q[60], q[5];
rz(0) q[5];
rz(0) q[60];
cx q[60], q[6];
rz(0) q[6];
cx q[60], q[6];
rz(0) q[6];
rz(0) q[60];
cx q[60], q[7];
rz(0) q[7];
cx q[60], q[7];
rz(0) q[7];
rz(0) q[60];
cx q[60], q[8];
rz(0) q[8];
cx q[60], q[8];
rz(0) q[8];
rz(0) q[60];
cx q[60], q[9];
rz(0) q[9];
cx q[60], q[9];
rz(0) q[9];
rz(0) q[60];
cx q[60], q[10];
rz(0) q[10];
cx q[60], q[10];
rz(0) q[10];
rz(0) q[60];
cx q[60], q[11];
rz(0) q[11];
cx q[60], q[11];
rz(0) q[11];
rz(0) q[60];
cx q[60], q[12];
rz(0) q[12];
cx q[60], q[12];
rz(0) q[12];
rz(0) q[60];
cx q[60], q[13];
rz(0) q[13];
cx q[60], q[13];
rz(0) q[13];
rz(0) q[60];
cx q[60], q[14];
rz(0) q[14];
cx q[60], q[14];
rz(0) q[14];
rz(0) q[60];
cx q[60], q[15];
rz(0) q[15];
cx q[60], q[15];
rz(0) q[15];
rz(0) q[60];
cx q[60], q[16];
rz(0) q[16];
cx q[60], q[16];
rz(0) q[16];
rz(0) q[60];
cx q[60], q[17];
rz(0) q[17];
cx q[60], q[17];
rz(0) q[17];
rz(0) q[60];
cx q[60], q[18];
rz(0) q[18];
cx q[60], q[18];
rz(0) q[18];
rz(0) q[60];
cx q[60], q[19];
rz(0) q[19];
cx q[60], q[19];
rz(0) q[19];
rz(0) q[60];
cx q[60], q[20];
rz(0) q[20];
cx q[60], q[20];
rz(0) q[20];
rz(0) q[60];
cx q[60], q[21];
rz(0) q[21];
cx q[60], q[21];
rz(0) q[21];
rz(0) q[60];
cx q[60], q[22];
rz(0) q[22];
cx q[60], q[22];
rz(0) q[22];
rz(0) q[60];
cx q[60], q[23];
rz(0) q[23];
cx q[60], q[23];
rz(0) q[23];
rz(0) q[60];
cx q[60], q[24];
rz(0) q[24];
cx q[60], q[24];
rz(0) q[24];
rz(0) q[60];
cx q[60], q[25];
rz(0) q[25];
cx q[60], q[25];
rz(0) q[25];
rz(0) q[60];
cx q[60], q[26];
rz(0) q[26];
cx q[60], q[26];
rz(0) q[26];
rz(0) q[60];
cx q[60], q[27];
rz(0) q[27];
cx q[60], q[27];
rz(0) q[27];
rz(0) q[60];
cx q[60], q[28];
rz(0) q[28];
cx q[60], q[28];
rz(0) q[28];
rz(0) q[60];
cx q[60], q[29];
rz(0) q[29];
cx q[60], q[29];
rz(0) q[29];
rz(-pi/2147483648) q[60];
cx q[60], q[30];
rz(pi/2147483648) q[30];
cx q[60], q[30];
rz(-pi/2147483648) q[30];
rz(-pi/1073741824) q[60];
cx q[60], q[31];
rz(pi/1073741824) q[31];
cx q[60], q[31];
rz(-pi/1073741824) q[31];
rz(-pi/536870912) q[60];
cx q[60], q[32];
rz(pi/536870912) q[32];
cx q[60], q[32];
rz(-pi/536870912) q[32];
rz(-pi/268435456) q[60];
cx q[60], q[33];
rz(pi/268435456) q[33];
cx q[60], q[33];
rz(-pi/268435456) q[33];
rz(-pi/134217728) q[60];
cx q[60], q[34];
rz(pi/134217728) q[34];
cx q[60], q[34];
rz(-pi/134217728) q[34];
rz(-pi/67108864) q[60];
cx q[60], q[35];
rz(pi/67108864) q[35];
cx q[60], q[35];
rz(-pi/67108864) q[35];
rz(-pi/33554432) q[60];
cx q[60], q[36];
rz(pi/33554432) q[36];
cx q[60], q[36];
rz(-pi/33554432) q[36];
rz(-pi/16777216) q[60];
cx q[60], q[37];
rz(pi/16777216) q[37];
cx q[60], q[37];
rz(-pi/16777216) q[37];
rz(-pi/8388608) q[60];
cx q[60], q[38];
rz(pi/8388608) q[38];
cx q[60], q[38];
rz(-pi/8388608) q[38];
rz(-pi/4194304) q[60];
cx q[60], q[39];
rz(pi/4194304) q[39];
cx q[60], q[39];
rz(-pi/4194304) q[39];
rz(-pi/2097152) q[60];
cx q[60], q[40];
rz(pi/2097152) q[40];
cx q[60], q[40];
rz(-pi/2097152) q[40];
rz(-pi/1048576) q[60];
cx q[60], q[41];
rz(pi/1048576) q[41];
cx q[60], q[41];
rz(-pi/1048576) q[41];
rz(-pi/524288) q[60];
cx q[60], q[42];
rz(pi/524288) q[42];
cx q[60], q[42];
rz(-pi/524288) q[42];
rz(-pi/262144) q[60];
cx q[60], q[43];
rz(pi/262144) q[43];
cx q[60], q[43];
rz(-pi/262144) q[43];
rz(-pi/131072) q[60];
cx q[60], q[44];
rz(pi/131072) q[44];
cx q[60], q[44];
rz(-pi/131072) q[44];
rz(-pi/65536) q[60];
cx q[60], q[45];
rz(pi/65536) q[45];
cx q[60], q[45];
rz(-pi/65536) q[45];
rz(-pi/32768) q[60];
cx q[60], q[46];
rz(pi/32768) q[46];
cx q[60], q[46];
rz(-pi/32768) q[46];
rz(-pi/16384) q[60];
cx q[60], q[47];
rz(pi/16384) q[47];
cx q[60], q[47];
rz(-pi/16384) q[47];
rz(-pi/8192) q[60];
cx q[60], q[48];
rz(pi/8192) q[48];
cx q[60], q[48];
rz(-pi/8192) q[48];
rz(-pi/4096) q[60];
cx q[60], q[49];
rz(pi/4096) q[49];
cx q[60], q[49];
rz(-pi/4096) q[49];
rz(-pi/2048) q[60];
cx q[60], q[50];
rz(pi/2048) q[50];
cx q[60], q[50];
rz(-pi/2048) q[50];
rz(-pi/1024) q[60];
cx q[60], q[51];
rz(pi/1024) q[51];
cx q[60], q[51];
rz(-pi/1024) q[51];
rz(-pi/512) q[60];
cx q[60], q[52];
rz(pi/512) q[52];
cx q[60], q[52];
rz(-pi/512) q[52];
rz(-pi/256) q[60];
cx q[60], q[53];
rz(pi/256) q[53];
cx q[60], q[53];
rz(-pi/256) q[53];
rz(-pi/128) q[60];
cx q[60], q[54];
rz(pi/128) q[54];
cx q[60], q[54];
rz(-pi/128) q[54];
rz(-pi/64) q[60];
cx q[60], q[55];
rz(pi/64) q[55];
cx q[60], q[55];
rz(-pi/64) q[55];
rz(-pi/32) q[60];
cx q[60], q[56];
rz(pi/32) q[56];
cx q[60], q[56];
rz(-pi/32) q[56];
rz(-pi/16) q[60];
cx q[60], q[57];
rz(pi/16) q[57];
cx q[60], q[57];
rz(-pi/16) q[57];
rz(-pi/8) q[60];
cx q[60], q[58];
rz(pi/8) q[58];
cx q[60], q[58];
rz(-pi/8) q[58];
rz(-pi/4) q[60];
cx q[60], q[59];
rz(pi/4) q[59];
cx q[60], q[59];
rz(-pi/4) q[59];
h q[60];
cx q[61], q[0];
rz(0) q[0];
cx q[61], q[0];
rz(0) q[0];
rz(0) q[61];
cx q[61], q[1];
rz(0) q[1];
cx q[61], q[1];
rz(0) q[1];
rz(0) q[61];
cx q[61], q[2];
rz(0) q[2];
cx q[61], q[2];
rz(0) q[2];
rz(0) q[61];
cx q[61], q[3];
rz(0) q[3];
cx q[61], q[3];
rz(0) q[3];
rz(0) q[61];
cx q[61], q[4];
rz(0) q[4];
cx q[61], q[4];
rz(0) q[4];
rz(0) q[61];
cx q[61], q[5];
rz(0) q[5];
cx q[61], q[5];
rz(0) q[5];
rz(0) q[61];
cx q[61], q[6];
rz(0) q[6];
cx q[61], q[6];
rz(0) q[6];
rz(0) q[61];
cx q[61], q[7];
rz(0) q[7];
cx q[61], q[7];
rz(0) q[7];
rz(0) q[61];
cx q[61], q[8];
rz(0) q[8];
cx q[61], q[8];
rz(0) q[8];
rz(0) q[61];
cx q[61], q[9];
rz(0) q[9];
cx q[61], q[9];
rz(0) q[9];
rz(0) q[61];
cx q[61], q[10];
rz(0) q[10];
cx q[61], q[10];
rz(0) q[10];
rz(0) q[61];
cx q[61], q[11];
rz(0) q[11];
cx q[61], q[11];
rz(0) q[11];
rz(0) q[61];
cx q[61], q[12];
rz(0) q[12];
cx q[61], q[12];
rz(0) q[12];
rz(0) q[61];
cx q[61], q[13];
rz(0) q[13];
cx q[61], q[13];
rz(0) q[13];
rz(0) q[61];
cx q[61], q[14];
rz(0) q[14];
cx q[61], q[14];
rz(0) q[14];
rz(0) q[61];
cx q[61], q[15];
rz(0) q[15];
cx q[61], q[15];
rz(0) q[15];
rz(0) q[61];
cx q[61], q[16];
rz(0) q[16];
cx q[61], q[16];
rz(0) q[16];
rz(0) q[61];
cx q[61], q[17];
rz(0) q[17];
cx q[61], q[17];
rz(0) q[17];
rz(0) q[61];
cx q[61], q[18];
rz(0) q[18];
cx q[61], q[18];
rz(0) q[18];
rz(0) q[61];
cx q[61], q[19];
rz(0) q[19];
cx q[61], q[19];
rz(0) q[19];
rz(0) q[61];
cx q[61], q[20];
rz(0) q[20];
cx q[61], q[20];
rz(0) q[20];
rz(0) q[61];
cx q[61], q[21];
rz(0) q[21];
cx q[61], q[21];
rz(0) q[21];
rz(0) q[61];
cx q[61], q[22];
rz(0) q[22];
cx q[61], q[22];
rz(0) q[22];
rz(0) q[61];
cx q[61], q[23];
rz(0) q[23];
cx q[61], q[23];
rz(0) q[23];
rz(0) q[61];
cx q[61], q[24];
rz(0) q[24];
cx q[61], q[24];
rz(0) q[24];
rz(0) q[61];
cx q[61], q[25];
rz(0) q[25];
cx q[61], q[25];
rz(0) q[25];
rz(0) q[61];
cx q[61], q[26];
rz(0) q[26];
cx q[61], q[26];
rz(0) q[26];
rz(0) q[61];
cx q[61], q[27];
rz(0) q[27];
cx q[61], q[27];
rz(0) q[27];
rz(0) q[61];
cx q[61], q[28];
rz(0) q[28];
cx q[61], q[28];
rz(0) q[28];
rz(0) q[61];
cx q[61], q[29];
rz(0) q[29];
cx q[61], q[29];
rz(0) q[29];
rz(0) q[61];
cx q[61], q[30];
rz(0) q[30];
cx q[61], q[30];
rz(0) q[30];
rz(-pi/2147483648) q[61];
cx q[61], q[31];
rz(pi/2147483648) q[31];
cx q[61], q[31];
rz(-pi/2147483648) q[31];
rz(-pi/1073741824) q[61];
cx q[61], q[32];
rz(pi/1073741824) q[32];
cx q[61], q[32];
rz(-pi/1073741824) q[32];
rz(-pi/536870912) q[61];
cx q[61], q[33];
rz(pi/536870912) q[33];
cx q[61], q[33];
rz(-pi/536870912) q[33];
rz(-pi/268435456) q[61];
cx q[61], q[34];
rz(pi/268435456) q[34];
cx q[61], q[34];
rz(-pi/268435456) q[34];
rz(-pi/134217728) q[61];
cx q[61], q[35];
rz(pi/134217728) q[35];
cx q[61], q[35];
rz(-pi/134217728) q[35];
rz(-pi/67108864) q[61];
cx q[61], q[36];
rz(pi/67108864) q[36];
cx q[61], q[36];
rz(-pi/67108864) q[36];
rz(-pi/33554432) q[61];
cx q[61], q[37];
rz(pi/33554432) q[37];
cx q[61], q[37];
rz(-pi/33554432) q[37];
rz(-pi/16777216) q[61];
cx q[61], q[38];
rz(pi/16777216) q[38];
cx q[61], q[38];
rz(-pi/16777216) q[38];
rz(-pi/8388608) q[61];
cx q[61], q[39];
rz(pi/8388608) q[39];
cx q[61], q[39];
rz(-pi/8388608) q[39];
rz(-pi/4194304) q[61];
cx q[61], q[40];
rz(pi/4194304) q[40];
cx q[61], q[40];
rz(-pi/4194304) q[40];
rz(-pi/2097152) q[61];
cx q[61], q[41];
rz(pi/2097152) q[41];
cx q[61], q[41];
rz(-pi/2097152) q[41];
rz(-pi/1048576) q[61];
cx q[61], q[42];
rz(pi/1048576) q[42];
cx q[61], q[42];
rz(-pi/1048576) q[42];
rz(-pi/524288) q[61];
cx q[61], q[43];
rz(pi/524288) q[43];
cx q[61], q[43];
rz(-pi/524288) q[43];
rz(-pi/262144) q[61];
cx q[61], q[44];
rz(pi/262144) q[44];
cx q[61], q[44];
rz(-pi/262144) q[44];
rz(-pi/131072) q[61];
cx q[61], q[45];
rz(pi/131072) q[45];
cx q[61], q[45];
rz(-pi/131072) q[45];
rz(-pi/65536) q[61];
cx q[61], q[46];
rz(pi/65536) q[46];
cx q[61], q[46];
rz(-pi/65536) q[46];
rz(-pi/32768) q[61];
cx q[61], q[47];
rz(pi/32768) q[47];
cx q[61], q[47];
rz(-pi/32768) q[47];
rz(-pi/16384) q[61];
cx q[61], q[48];
rz(pi/16384) q[48];
cx q[61], q[48];
rz(-pi/16384) q[48];
rz(-pi/8192) q[61];
cx q[61], q[49];
rz(pi/8192) q[49];
cx q[61], q[49];
rz(-pi/8192) q[49];
rz(-pi/4096) q[61];
cx q[61], q[50];
rz(pi/4096) q[50];
cx q[61], q[50];
rz(-pi/4096) q[50];
rz(-pi/2048) q[61];
cx q[61], q[51];
rz(pi/2048) q[51];
cx q[61], q[51];
rz(-pi/2048) q[51];
rz(-pi/1024) q[61];
cx q[61], q[52];
rz(pi/1024) q[52];
cx q[61], q[52];
rz(-pi/1024) q[52];
rz(-pi/512) q[61];
cx q[61], q[53];
rz(pi/512) q[53];
cx q[61], q[53];
rz(-pi/512) q[53];
rz(-pi/256) q[61];
cx q[61], q[54];
rz(pi/256) q[54];
cx q[61], q[54];
rz(-pi/256) q[54];
rz(-pi/128) q[61];
cx q[61], q[55];
rz(pi/128) q[55];
cx q[61], q[55];
rz(-pi/128) q[55];
rz(-pi/64) q[61];
cx q[61], q[56];
rz(pi/64) q[56];
cx q[61], q[56];
rz(-pi/64) q[56];
rz(-pi/32) q[61];
cx q[61], q[57];
rz(pi/32) q[57];
cx q[61], q[57];
rz(-pi/32) q[57];
rz(-pi/16) q[61];
cx q[61], q[58];
rz(pi/16) q[58];
cx q[61], q[58];
rz(-pi/16) q[58];
rz(-pi/8) q[61];
cx q[61], q[59];
rz(pi/8) q[59];
cx q[61], q[59];
rz(-pi/8) q[59];
rz(-pi/4) q[61];
cx q[61], q[60];
rz(pi/4) q[60];
cx q[61], q[60];
rz(-pi/4) q[60];
h q[61];
cx q[62], q[0];
rz(0) q[0];
cx q[62], q[0];
rz(0) q[0];
rz(0) q[62];
cx q[62], q[1];
rz(0) q[1];
cx q[62], q[1];
rz(0) q[1];
rz(0) q[62];
cx q[62], q[2];
rz(0) q[2];
cx q[62], q[2];
rz(0) q[2];
rz(0) q[62];
cx q[62], q[3];
rz(0) q[3];
cx q[62], q[3];
rz(0) q[3];
rz(0) q[62];
cx q[62], q[4];
rz(0) q[4];
cx q[62], q[4];
rz(0) q[4];
rz(0) q[62];
cx q[62], q[5];
rz(0) q[5];
cx q[62], q[5];
rz(0) q[5];
rz(0) q[62];
cx q[62], q[6];
rz(0) q[6];
cx q[62], q[6];
rz(0) q[6];
rz(0) q[62];
cx q[62], q[7];
rz(0) q[7];
cx q[62], q[7];
rz(0) q[7];
rz(0) q[62];
cx q[62], q[8];
rz(0) q[8];
cx q[62], q[8];
rz(0) q[8];
rz(0) q[62];
cx q[62], q[9];
rz(0) q[9];
cx q[62], q[9];
rz(0) q[9];
rz(0) q[62];
cx q[62], q[10];
rz(0) q[10];
cx q[62], q[10];
rz(0) q[10];
rz(0) q[62];
cx q[62], q[11];
rz(0) q[11];
cx q[62], q[11];
rz(0) q[11];
rz(0) q[62];
cx q[62], q[12];
rz(0) q[12];
cx q[62], q[12];
rz(0) q[12];
rz(0) q[62];
cx q[62], q[13];
rz(0) q[13];
cx q[62], q[13];
rz(0) q[13];
rz(0) q[62];
cx q[62], q[14];
rz(0) q[14];
cx q[62], q[14];
rz(0) q[14];
rz(0) q[62];
cx q[62], q[15];
rz(0) q[15];
cx q[62], q[15];
rz(0) q[15];
rz(0) q[62];
cx q[62], q[16];
rz(0) q[16];
cx q[62], q[16];
rz(0) q[16];
rz(0) q[62];
cx q[62], q[17];
rz(0) q[17];
cx q[62], q[17];
rz(0) q[17];
rz(0) q[62];
cx q[62], q[18];
rz(0) q[18];
cx q[62], q[18];
rz(0) q[18];
rz(0) q[62];
cx q[62], q[19];
rz(0) q[19];
cx q[62], q[19];
rz(0) q[19];
rz(0) q[62];
cx q[62], q[20];
rz(0) q[20];
cx q[62], q[20];
rz(0) q[20];
rz(0) q[62];
cx q[62], q[21];
rz(0) q[21];
cx q[62], q[21];
rz(0) q[21];
rz(0) q[62];
cx q[62], q[22];
rz(0) q[22];
cx q[62], q[22];
rz(0) q[22];
rz(0) q[62];
cx q[62], q[23];
rz(0) q[23];
cx q[62], q[23];
rz(0) q[23];
rz(0) q[62];
cx q[62], q[24];
rz(0) q[24];
cx q[62], q[24];
rz(0) q[24];
rz(0) q[62];
cx q[62], q[25];
rz(0) q[25];
cx q[62], q[25];
rz(0) q[25];
rz(0) q[62];
cx q[62], q[26];
rz(0) q[26];
cx q[62], q[26];
rz(0) q[26];
rz(0) q[62];
cx q[62], q[27];
rz(0) q[27];
cx q[62], q[27];
rz(0) q[27];
rz(0) q[62];
cx q[62], q[28];
rz(0) q[28];
cx q[62], q[28];
rz(0) q[28];
rz(0) q[62];
cx q[62], q[29];
rz(0) q[29];
cx q[62], q[29];
rz(0) q[29];
rz(0) q[62];
cx q[62], q[30];
rz(0) q[30];
cx q[62], q[30];
rz(0) q[30];
rz(0) q[62];
cx q[62], q[31];
rz(0) q[31];
cx q[62], q[31];
rz(0) q[31];
rz(-pi/2147483648) q[62];
cx q[62], q[32];
rz(pi/2147483648) q[32];
cx q[62], q[32];
rz(-pi/2147483648) q[32];
rz(-pi/1073741824) q[62];
cx q[62], q[33];
rz(pi/1073741824) q[33];
cx q[62], q[33];
rz(-pi/1073741824) q[33];
rz(-pi/536870912) q[62];
cx q[62], q[34];
rz(pi/536870912) q[34];
cx q[62], q[34];
rz(-pi/536870912) q[34];
rz(-pi/268435456) q[62];
cx q[62], q[35];
rz(pi/268435456) q[35];
cx q[62], q[35];
rz(-pi/268435456) q[35];
rz(-pi/134217728) q[62];
cx q[62], q[36];
rz(pi/134217728) q[36];
cx q[62], q[36];
rz(-pi/134217728) q[36];
rz(-pi/67108864) q[62];
cx q[62], q[37];
rz(pi/67108864) q[37];
cx q[62], q[37];
rz(-pi/67108864) q[37];
rz(-pi/33554432) q[62];
cx q[62], q[38];
rz(pi/33554432) q[38];
cx q[62], q[38];
rz(-pi/33554432) q[38];
rz(-pi/16777216) q[62];
cx q[62], q[39];
rz(pi/16777216) q[39];
cx q[62], q[39];
rz(-pi/16777216) q[39];
rz(-pi/8388608) q[62];
cx q[62], q[40];
rz(pi/8388608) q[40];
cx q[62], q[40];
rz(-pi/8388608) q[40];
rz(-pi/4194304) q[62];
cx q[62], q[41];
rz(pi/4194304) q[41];
cx q[62], q[41];
rz(-pi/4194304) q[41];
rz(-pi/2097152) q[62];
cx q[62], q[42];
rz(pi/2097152) q[42];
cx q[62], q[42];
rz(-pi/2097152) q[42];
rz(-pi/1048576) q[62];
cx q[62], q[43];
rz(pi/1048576) q[43];
cx q[62], q[43];
rz(-pi/1048576) q[43];
rz(-pi/524288) q[62];
cx q[62], q[44];
rz(pi/524288) q[44];
cx q[62], q[44];
rz(-pi/524288) q[44];
rz(-pi/262144) q[62];
cx q[62], q[45];
rz(pi/262144) q[45];
cx q[62], q[45];
rz(-pi/262144) q[45];
rz(-pi/131072) q[62];
cx q[62], q[46];
rz(pi/131072) q[46];
cx q[62], q[46];
rz(-pi/131072) q[46];
rz(-pi/65536) q[62];
cx q[62], q[47];
rz(pi/65536) q[47];
cx q[62], q[47];
rz(-pi/65536) q[47];
rz(-pi/32768) q[62];
cx q[62], q[48];
rz(pi/32768) q[48];
cx q[62], q[48];
rz(-pi/32768) q[48];
rz(-pi/16384) q[62];
cx q[62], q[49];
rz(pi/16384) q[49];
cx q[62], q[49];
rz(-pi/16384) q[49];
rz(-pi/8192) q[62];
cx q[62], q[50];
rz(pi/8192) q[50];
cx q[62], q[50];
rz(-pi/8192) q[50];
rz(-pi/4096) q[62];
cx q[62], q[51];
rz(pi/4096) q[51];
cx q[62], q[51];
rz(-pi/4096) q[51];
rz(-pi/2048) q[62];
cx q[62], q[52];
rz(pi/2048) q[52];
cx q[62], q[52];
rz(-pi/2048) q[52];
rz(-pi/1024) q[62];
cx q[62], q[53];
rz(pi/1024) q[53];
cx q[62], q[53];
rz(-pi/1024) q[53];
rz(-pi/512) q[62];
cx q[62], q[54];
rz(pi/512) q[54];
cx q[62], q[54];
rz(-pi/512) q[54];
rz(-pi/256) q[62];
cx q[62], q[55];
rz(pi/256) q[55];
cx q[62], q[55];
rz(-pi/256) q[55];
rz(-pi/128) q[62];
cx q[62], q[56];
rz(pi/128) q[56];
cx q[62], q[56];
rz(-pi/128) q[56];
rz(-pi/64) q[62];
cx q[62], q[57];
rz(pi/64) q[57];
cx q[62], q[57];
rz(-pi/64) q[57];
rz(-pi/32) q[62];
cx q[62], q[58];
rz(pi/32) q[58];
cx q[62], q[58];
rz(-pi/32) q[58];
rz(-pi/16) q[62];
cx q[62], q[59];
rz(pi/16) q[59];
cx q[62], q[59];
rz(-pi/16) q[59];
rz(-pi/8) q[62];
cx q[62], q[60];
rz(pi/8) q[60];
cx q[62], q[60];
rz(-pi/8) q[60];
rz(-pi/4) q[62];
cx q[62], q[61];
rz(pi/4) q[61];
cx q[62], q[61];
rz(-pi/4) q[61];
h q[62];
cx q[63], q[0];
rz(0) q[0];
cx q[63], q[0];
rz(0) q[0];
rz(0) q[63];
cx q[63], q[1];
rz(0) q[1];
cx q[63], q[1];
rz(0) q[1];
rz(0) q[63];
cx q[63], q[2];
rz(0) q[2];
cx q[63], q[2];
rz(0) q[2];
rz(0) q[63];
cx q[63], q[3];
rz(0) q[3];
cx q[63], q[3];
rz(0) q[3];
rz(0) q[63];
cx q[63], q[4];
rz(0) q[4];
cx q[63], q[4];
rz(0) q[4];
rz(0) q[63];
cx q[63], q[5];
rz(0) q[5];
cx q[63], q[5];
rz(0) q[5];
rz(0) q[63];
cx q[63], q[6];
rz(0) q[6];
cx q[63], q[6];
rz(0) q[6];
rz(0) q[63];
cx q[63], q[7];
rz(0) q[7];
cx q[63], q[7];
rz(0) q[7];
rz(0) q[63];
cx q[63], q[8];
rz(0) q[8];
cx q[63], q[8];
rz(0) q[8];
rz(0) q[63];
cx q[63], q[9];
rz(0) q[9];
cx q[63], q[9];
rz(0) q[9];
rz(0) q[63];
cx q[63], q[10];
rz(0) q[10];
cx q[63], q[10];
rz(0) q[10];
rz(0) q[63];
cx q[63], q[11];
rz(0) q[11];
cx q[63], q[11];
rz(0) q[11];
rz(0) q[63];
cx q[63], q[12];
rz(0) q[12];
cx q[63], q[12];
rz(0) q[12];
rz(0) q[63];
cx q[63], q[13];
rz(0) q[13];
cx q[63], q[13];
rz(0) q[13];
rz(0) q[63];
cx q[63], q[14];
rz(0) q[14];
cx q[63], q[14];
rz(0) q[14];
rz(0) q[63];
cx q[63], q[15];
rz(0) q[15];
cx q[63], q[15];
rz(0) q[15];
rz(0) q[63];
cx q[63], q[16];
rz(0) q[16];
cx q[63], q[16];
rz(0) q[16];
rz(0) q[63];
cx q[63], q[17];
rz(0) q[17];
cx q[63], q[17];
rz(0) q[17];
rz(0) q[63];
cx q[63], q[18];
rz(0) q[18];
cx q[63], q[18];
rz(0) q[18];
rz(0) q[63];
cx q[63], q[19];
rz(0) q[19];
cx q[63], q[19];
rz(0) q[19];
rz(0) q[63];
cx q[63], q[20];
rz(0) q[20];
cx q[63], q[20];
rz(0) q[20];
rz(0) q[63];
cx q[63], q[21];
rz(0) q[21];
cx q[63], q[21];
rz(0) q[21];
rz(0) q[63];
cx q[63], q[22];
rz(0) q[22];
cx q[63], q[22];
rz(0) q[22];
rz(0) q[63];
cx q[63], q[23];
rz(0) q[23];
cx q[63], q[23];
rz(0) q[23];
rz(0) q[63];
cx q[63], q[24];
rz(0) q[24];
cx q[63], q[24];
rz(0) q[24];
rz(0) q[63];
cx q[63], q[25];
rz(0) q[25];
cx q[63], q[25];
rz(0) q[25];
rz(0) q[63];
cx q[63], q[26];
rz(0) q[26];
cx q[63], q[26];
rz(0) q[26];
rz(0) q[63];
cx q[63], q[27];
rz(0) q[27];
cx q[63], q[27];
rz(0) q[27];
rz(0) q[63];
cx q[63], q[28];
rz(0) q[28];
cx q[63], q[28];
rz(0) q[28];
rz(0) q[63];
cx q[63], q[29];
rz(0) q[29];
cx q[63], q[29];
rz(0) q[29];
rz(0) q[63];
cx q[63], q[30];
rz(0) q[30];
cx q[63], q[30];
rz(0) q[30];
rz(0) q[63];
cx q[63], q[31];
rz(0) q[31];
cx q[63], q[31];
rz(0) q[31];
rz(0) q[63];
cx q[63], q[32];
rz(0) q[32];
cx q[63], q[32];
rz(0) q[32];
rz(-pi/2147483648) q[63];
cx q[63], q[33];
rz(pi/2147483648) q[33];
cx q[63], q[33];
rz(-pi/2147483648) q[33];
rz(-pi/1073741824) q[63];
cx q[63], q[34];
rz(pi/1073741824) q[34];
cx q[63], q[34];
rz(-pi/1073741824) q[34];
rz(-pi/536870912) q[63];
cx q[63], q[35];
rz(pi/536870912) q[35];
cx q[63], q[35];
rz(-pi/536870912) q[35];
rz(-pi/268435456) q[63];
cx q[63], q[36];
rz(pi/268435456) q[36];
cx q[63], q[36];
rz(-pi/268435456) q[36];
rz(-pi/134217728) q[63];
cx q[63], q[37];
rz(pi/134217728) q[37];
cx q[63], q[37];
rz(-pi/134217728) q[37];
rz(-pi/67108864) q[63];
cx q[63], q[38];
rz(pi/67108864) q[38];
cx q[63], q[38];
rz(-pi/67108864) q[38];
rz(-pi/33554432) q[63];
cx q[63], q[39];
rz(pi/33554432) q[39];
cx q[63], q[39];
rz(-pi/33554432) q[39];
rz(-pi/16777216) q[63];
cx q[63], q[40];
rz(pi/16777216) q[40];
cx q[63], q[40];
rz(-pi/16777216) q[40];
rz(-pi/8388608) q[63];
cx q[63], q[41];
rz(pi/8388608) q[41];
cx q[63], q[41];
rz(-pi/8388608) q[41];
rz(-pi/4194304) q[63];
cx q[63], q[42];
rz(pi/4194304) q[42];
cx q[63], q[42];
rz(-pi/4194304) q[42];
rz(-pi/2097152) q[63];
cx q[63], q[43];
rz(pi/2097152) q[43];
cx q[63], q[43];
rz(-pi/2097152) q[43];
rz(-pi/1048576) q[63];
cx q[63], q[44];
rz(pi/1048576) q[44];
cx q[63], q[44];
rz(-pi/1048576) q[44];
rz(-pi/524288) q[63];
cx q[63], q[45];
rz(pi/524288) q[45];
cx q[63], q[45];
rz(-pi/524288) q[45];
rz(-pi/262144) q[63];
cx q[63], q[46];
rz(pi/262144) q[46];
cx q[63], q[46];
rz(-pi/262144) q[46];
rz(-pi/131072) q[63];
cx q[63], q[47];
rz(pi/131072) q[47];
cx q[63], q[47];
rz(-pi/131072) q[47];
rz(-pi/65536) q[63];
cx q[63], q[48];
rz(pi/65536) q[48];
cx q[63], q[48];
rz(-pi/65536) q[48];
rz(-pi/32768) q[63];
cx q[63], q[49];
rz(pi/32768) q[49];
cx q[63], q[49];
rz(-pi/32768) q[49];
rz(-pi/16384) q[63];
cx q[63], q[50];
rz(pi/16384) q[50];
cx q[63], q[50];
rz(-pi/16384) q[50];
rz(-pi/8192) q[63];
cx q[63], q[51];
rz(pi/8192) q[51];
cx q[63], q[51];
rz(-pi/8192) q[51];
rz(-pi/4096) q[63];
cx q[63], q[52];
rz(pi/4096) q[52];
cx q[63], q[52];
rz(-pi/4096) q[52];
rz(-pi/2048) q[63];
cx q[63], q[53];
rz(pi/2048) q[53];
cx q[63], q[53];
rz(-pi/2048) q[53];
rz(-pi/1024) q[63];
cx q[63], q[54];
rz(pi/1024) q[54];
cx q[63], q[54];
rz(-pi/1024) q[54];
rz(-pi/512) q[63];
cx q[63], q[55];
rz(pi/512) q[55];
cx q[63], q[55];
rz(-pi/512) q[55];
rz(-pi/256) q[63];
cx q[63], q[56];
rz(pi/256) q[56];
cx q[63], q[56];
rz(-pi/256) q[56];
rz(-pi/128) q[63];
cx q[63], q[57];
rz(pi/128) q[57];
cx q[63], q[57];
rz(-pi/128) q[57];
rz(-pi/64) q[63];
cx q[63], q[58];
rz(pi/64) q[58];
cx q[63], q[58];
rz(-pi/64) q[58];
rz(-pi/32) q[63];
cx q[63], q[59];
rz(pi/32) q[59];
cx q[63], q[59];
rz(-pi/32) q[59];
rz(-pi/16) q[63];
cx q[63], q[60];
rz(pi/16) q[60];
cx q[63], q[60];
rz(-pi/16) q[60];
rz(-pi/8) q[63];
cx q[63], q[61];
rz(pi/8) q[61];
cx q[63], q[61];
rz(-pi/8) q[61];
rz(-pi/4) q[63];
cx q[63], q[62];
rz(pi/4) q[62];
cx q[63], q[62];
rz(-pi/4) q[62];
h q[63];
cx q[64], q[0];
rz(0) q[0];
cx q[64], q[0];
rz(0) q[0];
rz(0) q[64];
cx q[64], q[1];
rz(0) q[1];
cx q[64], q[1];
rz(0) q[1];
rz(0) q[64];
cx q[64], q[2];
rz(0) q[2];
cx q[64], q[2];
rz(0) q[2];
rz(0) q[64];
cx q[64], q[3];
rz(0) q[3];
cx q[64], q[3];
rz(0) q[3];
rz(0) q[64];
cx q[64], q[4];
rz(0) q[4];
cx q[64], q[4];
rz(0) q[4];
rz(0) q[64];
cx q[64], q[5];
rz(0) q[5];
cx q[64], q[5];
rz(0) q[5];
rz(0) q[64];
cx q[64], q[6];
rz(0) q[6];
cx q[64], q[6];
rz(0) q[6];
rz(0) q[64];
cx q[64], q[7];
rz(0) q[7];
cx q[64], q[7];
rz(0) q[7];
rz(0) q[64];
cx q[64], q[8];
rz(0) q[8];
cx q[64], q[8];
rz(0) q[8];
rz(0) q[64];
cx q[64], q[9];
rz(0) q[9];
cx q[64], q[9];
rz(0) q[9];
rz(0) q[64];
cx q[64], q[10];
rz(0) q[10];
cx q[64], q[10];
rz(0) q[10];
rz(0) q[64];
cx q[64], q[11];
rz(0) q[11];
cx q[64], q[11];
rz(0) q[11];
rz(0) q[64];
cx q[64], q[12];
rz(0) q[12];
cx q[64], q[12];
rz(0) q[12];
rz(0) q[64];
cx q[64], q[13];
rz(0) q[13];
cx q[64], q[13];
rz(0) q[13];
rz(0) q[64];
cx q[64], q[14];
rz(0) q[14];
cx q[64], q[14];
rz(0) q[14];
rz(0) q[64];
cx q[64], q[15];
rz(0) q[15];
cx q[64], q[15];
rz(0) q[15];
rz(0) q[64];
cx q[64], q[16];
rz(0) q[16];
cx q[64], q[16];
rz(0) q[16];
rz(0) q[64];
cx q[64], q[17];
rz(0) q[17];
cx q[64], q[17];
rz(0) q[17];
rz(0) q[64];
cx q[64], q[18];
rz(0) q[18];
cx q[64], q[18];
rz(0) q[18];
rz(0) q[64];
cx q[64], q[19];
rz(0) q[19];
cx q[64], q[19];
rz(0) q[19];
rz(0) q[64];
cx q[64], q[20];
rz(0) q[20];
cx q[64], q[20];
rz(0) q[20];
rz(0) q[64];
cx q[64], q[21];
rz(0) q[21];
cx q[64], q[21];
rz(0) q[21];
rz(0) q[64];
cx q[64], q[22];
rz(0) q[22];
cx q[64], q[22];
rz(0) q[22];
rz(0) q[64];
cx q[64], q[23];
rz(0) q[23];
cx q[64], q[23];
rz(0) q[23];
rz(0) q[64];
cx q[64], q[24];
rz(0) q[24];
cx q[64], q[24];
rz(0) q[24];
rz(0) q[64];
cx q[64], q[25];
rz(0) q[25];
cx q[64], q[25];
rz(0) q[25];
rz(0) q[64];
cx q[64], q[26];
rz(0) q[26];
cx q[64], q[26];
rz(0) q[26];
rz(0) q[64];
cx q[64], q[27];
rz(0) q[27];
cx q[64], q[27];
rz(0) q[27];
rz(0) q[64];
cx q[64], q[28];
rz(0) q[28];
cx q[64], q[28];
rz(0) q[28];
rz(0) q[64];
cx q[64], q[29];
rz(0) q[29];
cx q[64], q[29];
rz(0) q[29];
rz(0) q[64];
cx q[64], q[30];
rz(0) q[30];
cx q[64], q[30];
rz(0) q[30];
rz(0) q[64];
cx q[64], q[31];
rz(0) q[31];
cx q[64], q[31];
rz(0) q[31];
rz(0) q[64];
cx q[64], q[32];
rz(0) q[32];
cx q[64], q[32];
rz(0) q[32];
rz(0) q[64];
cx q[64], q[33];
rz(0) q[33];
cx q[64], q[33];
rz(0) q[33];
rz(-pi/2147483648) q[64];
cx q[64], q[34];
rz(pi/2147483648) q[34];
cx q[64], q[34];
rz(-pi/2147483648) q[34];
rz(-pi/1073741824) q[64];
cx q[64], q[35];
rz(pi/1073741824) q[35];
cx q[64], q[35];
rz(-pi/1073741824) q[35];
rz(-pi/536870912) q[64];
cx q[64], q[36];
rz(pi/536870912) q[36];
cx q[64], q[36];
rz(-pi/536870912) q[36];
rz(-pi/268435456) q[64];
cx q[64], q[37];
rz(pi/268435456) q[37];
cx q[64], q[37];
rz(-pi/268435456) q[37];
rz(-pi/134217728) q[64];
cx q[64], q[38];
rz(pi/134217728) q[38];
cx q[64], q[38];
rz(-pi/134217728) q[38];
rz(-pi/67108864) q[64];
cx q[64], q[39];
rz(pi/67108864) q[39];
cx q[64], q[39];
rz(-pi/67108864) q[39];
rz(-pi/33554432) q[64];
cx q[64], q[40];
rz(pi/33554432) q[40];
cx q[64], q[40];
rz(-pi/33554432) q[40];
rz(-pi/16777216) q[64];
cx q[64], q[41];
rz(pi/16777216) q[41];
cx q[64], q[41];
rz(-pi/16777216) q[41];
rz(-pi/8388608) q[64];
cx q[64], q[42];
rz(pi/8388608) q[42];
cx q[64], q[42];
rz(-pi/8388608) q[42];
rz(-pi/4194304) q[64];
cx q[64], q[43];
rz(pi/4194304) q[43];
cx q[64], q[43];
rz(-pi/4194304) q[43];
rz(-pi/2097152) q[64];
cx q[64], q[44];
rz(pi/2097152) q[44];
cx q[64], q[44];
rz(-pi/2097152) q[44];
rz(-pi/1048576) q[64];
cx q[64], q[45];
rz(pi/1048576) q[45];
cx q[64], q[45];
rz(-pi/1048576) q[45];
rz(-pi/524288) q[64];
cx q[64], q[46];
rz(pi/524288) q[46];
cx q[64], q[46];
rz(-pi/524288) q[46];
rz(-pi/262144) q[64];
cx q[64], q[47];
rz(pi/262144) q[47];
cx q[64], q[47];
rz(-pi/262144) q[47];
rz(-pi/131072) q[64];
cx q[64], q[48];
rz(pi/131072) q[48];
cx q[64], q[48];
rz(-pi/131072) q[48];
rz(-pi/65536) q[64];
cx q[64], q[49];
rz(pi/65536) q[49];
cx q[64], q[49];
rz(-pi/65536) q[49];
rz(-pi/32768) q[64];
cx q[64], q[50];
rz(pi/32768) q[50];
cx q[64], q[50];
rz(-pi/32768) q[50];
rz(-pi/16384) q[64];
cx q[64], q[51];
rz(pi/16384) q[51];
cx q[64], q[51];
rz(-pi/16384) q[51];
rz(-pi/8192) q[64];
cx q[64], q[52];
rz(pi/8192) q[52];
cx q[64], q[52];
rz(-pi/8192) q[52];
rz(-pi/4096) q[64];
cx q[64], q[53];
rz(pi/4096) q[53];
cx q[64], q[53];
rz(-pi/4096) q[53];
rz(-pi/2048) q[64];
cx q[64], q[54];
rz(pi/2048) q[54];
cx q[64], q[54];
rz(-pi/2048) q[54];
rz(-pi/1024) q[64];
cx q[64], q[55];
rz(pi/1024) q[55];
cx q[64], q[55];
rz(-pi/1024) q[55];
rz(-pi/512) q[64];
cx q[64], q[56];
rz(pi/512) q[56];
cx q[64], q[56];
rz(-pi/512) q[56];
rz(-pi/256) q[64];
cx q[64], q[57];
rz(pi/256) q[57];
cx q[64], q[57];
rz(-pi/256) q[57];
rz(-pi/128) q[64];
cx q[64], q[58];
rz(pi/128) q[58];
cx q[64], q[58];
rz(-pi/128) q[58];
rz(-pi/64) q[64];
cx q[64], q[59];
rz(pi/64) q[59];
cx q[64], q[59];
rz(-pi/64) q[59];
rz(-pi/32) q[64];
cx q[64], q[60];
rz(pi/32) q[60];
cx q[64], q[60];
rz(-pi/32) q[60];
rz(-pi/16) q[64];
cx q[64], q[61];
rz(pi/16) q[61];
cx q[64], q[61];
rz(-pi/16) q[61];
rz(-pi/8) q[64];
cx q[64], q[62];
rz(pi/8) q[62];
cx q[64], q[62];
rz(-pi/8) q[62];
rz(-pi/4) q[64];
cx q[64], q[63];
rz(pi/4) q[63];
cx q[64], q[63];
rz(-pi/4) q[63];
h q[64];
cx q[65], q[0];
rz(0) q[0];
cx q[65], q[0];
rz(0) q[0];
rz(0) q[65];
cx q[65], q[1];
rz(0) q[1];
cx q[65], q[1];
rz(0) q[1];
rz(0) q[65];
cx q[65], q[2];
rz(0) q[2];
cx q[65], q[2];
rz(0) q[2];
rz(0) q[65];
cx q[65], q[3];
rz(0) q[3];
cx q[65], q[3];
rz(0) q[3];
rz(0) q[65];
cx q[65], q[4];
rz(0) q[4];
cx q[65], q[4];
rz(0) q[4];
rz(0) q[65];
cx q[65], q[5];
rz(0) q[5];
cx q[65], q[5];
rz(0) q[5];
rz(0) q[65];
cx q[65], q[6];
rz(0) q[6];
cx q[65], q[6];
rz(0) q[6];
rz(0) q[65];
cx q[65], q[7];
rz(0) q[7];
cx q[65], q[7];
rz(0) q[7];
rz(0) q[65];
cx q[65], q[8];
rz(0) q[8];
cx q[65], q[8];
rz(0) q[8];
rz(0) q[65];
cx q[65], q[9];
rz(0) q[9];
cx q[65], q[9];
rz(0) q[9];
rz(0) q[65];
cx q[65], q[10];
rz(0) q[10];
cx q[65], q[10];
rz(0) q[10];
rz(0) q[65];
cx q[65], q[11];
rz(0) q[11];
cx q[65], q[11];
rz(0) q[11];
rz(0) q[65];
cx q[65], q[12];
rz(0) q[12];
cx q[65], q[12];
rz(0) q[12];
rz(0) q[65];
cx q[65], q[13];
rz(0) q[13];
cx q[65], q[13];
rz(0) q[13];
rz(0) q[65];
cx q[65], q[14];
rz(0) q[14];
cx q[65], q[14];
rz(0) q[14];
rz(0) q[65];
cx q[65], q[15];
rz(0) q[15];
cx q[65], q[15];
rz(0) q[15];
rz(0) q[65];
cx q[65], q[16];
rz(0) q[16];
cx q[65], q[16];
rz(0) q[16];
rz(0) q[65];
cx q[65], q[17];
rz(0) q[17];
cx q[65], q[17];
rz(0) q[17];
rz(0) q[65];
cx q[65], q[18];
rz(0) q[18];
cx q[65], q[18];
rz(0) q[18];
rz(0) q[65];
cx q[65], q[19];
rz(0) q[19];
cx q[65], q[19];
rz(0) q[19];
rz(0) q[65];
cx q[65], q[20];
rz(0) q[20];
cx q[65], q[20];
rz(0) q[20];
rz(0) q[65];
cx q[65], q[21];
rz(0) q[21];
cx q[65], q[21];
rz(0) q[21];
rz(0) q[65];
cx q[65], q[22];
rz(0) q[22];
cx q[65], q[22];
rz(0) q[22];
rz(0) q[65];
cx q[65], q[23];
rz(0) q[23];
cx q[65], q[23];
rz(0) q[23];
rz(0) q[65];
cx q[65], q[24];
rz(0) q[24];
cx q[65], q[24];
rz(0) q[24];
rz(0) q[65];
cx q[65], q[25];
rz(0) q[25];
cx q[65], q[25];
rz(0) q[25];
rz(0) q[65];
cx q[65], q[26];
rz(0) q[26];
cx q[65], q[26];
rz(0) q[26];
rz(0) q[65];
cx q[65], q[27];
rz(0) q[27];
cx q[65], q[27];
rz(0) q[27];
rz(0) q[65];
cx q[65], q[28];
rz(0) q[28];
cx q[65], q[28];
rz(0) q[28];
rz(0) q[65];
cx q[65], q[29];
rz(0) q[29];
cx q[65], q[29];
rz(0) q[29];
rz(0) q[65];
cx q[65], q[30];
rz(0) q[30];
cx q[65], q[30];
rz(0) q[30];
rz(0) q[65];
cx q[65], q[31];
rz(0) q[31];
cx q[65], q[31];
rz(0) q[31];
rz(0) q[65];
cx q[65], q[32];
rz(0) q[32];
cx q[65], q[32];
rz(0) q[32];
rz(0) q[65];
cx q[65], q[33];
rz(0) q[33];
cx q[65], q[33];
rz(0) q[33];
rz(0) q[65];
cx q[65], q[34];
rz(0) q[34];
cx q[65], q[34];
rz(0) q[34];
rz(-pi/2147483648) q[65];
cx q[65], q[35];
rz(pi/2147483648) q[35];
cx q[65], q[35];
rz(-pi/2147483648) q[35];
rz(-pi/1073741824) q[65];
cx q[65], q[36];
rz(pi/1073741824) q[36];
cx q[65], q[36];
rz(-pi/1073741824) q[36];
rz(-pi/536870912) q[65];
cx q[65], q[37];
rz(pi/536870912) q[37];
cx q[65], q[37];
rz(-pi/536870912) q[37];
rz(-pi/268435456) q[65];
cx q[65], q[38];
rz(pi/268435456) q[38];
cx q[65], q[38];
rz(-pi/268435456) q[38];
rz(-pi/134217728) q[65];
cx q[65], q[39];
rz(pi/134217728) q[39];
cx q[65], q[39];
rz(-pi/134217728) q[39];
rz(-pi/67108864) q[65];
cx q[65], q[40];
rz(pi/67108864) q[40];
cx q[65], q[40];
rz(-pi/67108864) q[40];
rz(-pi/33554432) q[65];
cx q[65], q[41];
rz(pi/33554432) q[41];
cx q[65], q[41];
rz(-pi/33554432) q[41];
rz(-pi/16777216) q[65];
cx q[65], q[42];
rz(pi/16777216) q[42];
cx q[65], q[42];
rz(-pi/16777216) q[42];
rz(-pi/8388608) q[65];
cx q[65], q[43];
rz(pi/8388608) q[43];
cx q[65], q[43];
rz(-pi/8388608) q[43];
rz(-pi/4194304) q[65];
cx q[65], q[44];
rz(pi/4194304) q[44];
cx q[65], q[44];
rz(-pi/4194304) q[44];
rz(-pi/2097152) q[65];
cx q[65], q[45];
rz(pi/2097152) q[45];
cx q[65], q[45];
rz(-pi/2097152) q[45];
rz(-pi/1048576) q[65];
cx q[65], q[46];
rz(pi/1048576) q[46];
cx q[65], q[46];
rz(-pi/1048576) q[46];
rz(-pi/524288) q[65];
cx q[65], q[47];
rz(pi/524288) q[47];
cx q[65], q[47];
rz(-pi/524288) q[47];
rz(-pi/262144) q[65];
cx q[65], q[48];
rz(pi/262144) q[48];
cx q[65], q[48];
rz(-pi/262144) q[48];
rz(-pi/131072) q[65];
cx q[65], q[49];
rz(pi/131072) q[49];
cx q[65], q[49];
rz(-pi/131072) q[49];
rz(-pi/65536) q[65];
cx q[65], q[50];
rz(pi/65536) q[50];
cx q[65], q[50];
rz(-pi/65536) q[50];
rz(-pi/32768) q[65];
cx q[65], q[51];
rz(pi/32768) q[51];
cx q[65], q[51];
rz(-pi/32768) q[51];
rz(-pi/16384) q[65];
cx q[65], q[52];
rz(pi/16384) q[52];
cx q[65], q[52];
rz(-pi/16384) q[52];
rz(-pi/8192) q[65];
cx q[65], q[53];
rz(pi/8192) q[53];
cx q[65], q[53];
rz(-pi/8192) q[53];
rz(-pi/4096) q[65];
cx q[65], q[54];
rz(pi/4096) q[54];
cx q[65], q[54];
rz(-pi/4096) q[54];
rz(-pi/2048) q[65];
cx q[65], q[55];
rz(pi/2048) q[55];
cx q[65], q[55];
rz(-pi/2048) q[55];
rz(-pi/1024) q[65];
cx q[65], q[56];
rz(pi/1024) q[56];
cx q[65], q[56];
rz(-pi/1024) q[56];
rz(-pi/512) q[65];
cx q[65], q[57];
rz(pi/512) q[57];
cx q[65], q[57];
rz(-pi/512) q[57];
rz(-pi/256) q[65];
cx q[65], q[58];
rz(pi/256) q[58];
cx q[65], q[58];
rz(-pi/256) q[58];
rz(-pi/128) q[65];
cx q[65], q[59];
rz(pi/128) q[59];
cx q[65], q[59];
rz(-pi/128) q[59];
rz(-pi/64) q[65];
cx q[65], q[60];
rz(pi/64) q[60];
cx q[65], q[60];
rz(-pi/64) q[60];
rz(-pi/32) q[65];
cx q[65], q[61];
rz(pi/32) q[61];
cx q[65], q[61];
rz(-pi/32) q[61];
rz(-pi/16) q[65];
cx q[65], q[62];
rz(pi/16) q[62];
cx q[65], q[62];
rz(-pi/16) q[62];
rz(-pi/8) q[65];
cx q[65], q[63];
rz(pi/8) q[63];
cx q[65], q[63];
rz(-pi/8) q[63];
rz(-pi/4) q[65];
cx q[65], q[64];
rz(pi/4) q[64];
cx q[65], q[64];
rz(-pi/4) q[64];
h q[65];
cx q[66], q[0];
rz(0) q[0];
cx q[66], q[0];
rz(0) q[0];
rz(0) q[66];
cx q[66], q[1];
rz(0) q[1];
cx q[66], q[1];
rz(0) q[1];
rz(0) q[66];
cx q[66], q[2];
rz(0) q[2];
cx q[66], q[2];
rz(0) q[2];
rz(0) q[66];
cx q[66], q[3];
rz(0) q[3];
cx q[66], q[3];
rz(0) q[3];
rz(0) q[66];
cx q[66], q[4];
rz(0) q[4];
cx q[66], q[4];
rz(0) q[4];
rz(0) q[66];
cx q[66], q[5];
rz(0) q[5];
cx q[66], q[5];
rz(0) q[5];
rz(0) q[66];
cx q[66], q[6];
rz(0) q[6];
cx q[66], q[6];
rz(0) q[6];
rz(0) q[66];
cx q[66], q[7];
rz(0) q[7];
cx q[66], q[7];
rz(0) q[7];
rz(0) q[66];
cx q[66], q[8];
rz(0) q[8];
cx q[66], q[8];
rz(0) q[8];
rz(0) q[66];
cx q[66], q[9];
rz(0) q[9];
cx q[66], q[9];
rz(0) q[9];
rz(0) q[66];
cx q[66], q[10];
rz(0) q[10];
cx q[66], q[10];
rz(0) q[10];
rz(0) q[66];
cx q[66], q[11];
rz(0) q[11];
cx q[66], q[11];
rz(0) q[11];
rz(0) q[66];
cx q[66], q[12];
rz(0) q[12];
cx q[66], q[12];
rz(0) q[12];
rz(0) q[66];
cx q[66], q[13];
rz(0) q[13];
cx q[66], q[13];
rz(0) q[13];
rz(0) q[66];
cx q[66], q[14];
rz(0) q[14];
cx q[66], q[14];
rz(0) q[14];
rz(0) q[66];
cx q[66], q[15];
rz(0) q[15];
cx q[66], q[15];
rz(0) q[15];
rz(0) q[66];
cx q[66], q[16];
rz(0) q[16];
cx q[66], q[16];
rz(0) q[16];
rz(0) q[66];
cx q[66], q[17];
rz(0) q[17];
cx q[66], q[17];
rz(0) q[17];
rz(0) q[66];
cx q[66], q[18];
rz(0) q[18];
cx q[66], q[18];
rz(0) q[18];
rz(0) q[66];
cx q[66], q[19];
rz(0) q[19];
cx q[66], q[19];
rz(0) q[19];
rz(0) q[66];
cx q[66], q[20];
rz(0) q[20];
cx q[66], q[20];
rz(0) q[20];
rz(0) q[66];
cx q[66], q[21];
rz(0) q[21];
cx q[66], q[21];
rz(0) q[21];
rz(0) q[66];
cx q[66], q[22];
rz(0) q[22];
cx q[66], q[22];
rz(0) q[22];
rz(0) q[66];
cx q[66], q[23];
rz(0) q[23];
cx q[66], q[23];
rz(0) q[23];
rz(0) q[66];
cx q[66], q[24];
rz(0) q[24];
cx q[66], q[24];
rz(0) q[24];
rz(0) q[66];
cx q[66], q[25];
rz(0) q[25];
cx q[66], q[25];
rz(0) q[25];
rz(0) q[66];
cx q[66], q[26];
rz(0) q[26];
cx q[66], q[26];
rz(0) q[26];
rz(0) q[66];
cx q[66], q[27];
rz(0) q[27];
cx q[66], q[27];
rz(0) q[27];
rz(0) q[66];
cx q[66], q[28];
rz(0) q[28];
cx q[66], q[28];
rz(0) q[28];
rz(0) q[66];
cx q[66], q[29];
rz(0) q[29];
cx q[66], q[29];
rz(0) q[29];
rz(0) q[66];
cx q[66], q[30];
rz(0) q[30];
cx q[66], q[30];
rz(0) q[30];
rz(0) q[66];
cx q[66], q[31];
rz(0) q[31];
cx q[66], q[31];
rz(0) q[31];
rz(0) q[66];
cx q[66], q[32];
rz(0) q[32];
cx q[66], q[32];
rz(0) q[32];
rz(0) q[66];
cx q[66], q[33];
rz(0) q[33];
cx q[66], q[33];
rz(0) q[33];
rz(0) q[66];
cx q[66], q[34];
rz(0) q[34];
cx q[66], q[34];
rz(0) q[34];
rz(0) q[66];
cx q[66], q[35];
rz(0) q[35];
cx q[66], q[35];
rz(0) q[35];
rz(-pi/2147483648) q[66];
cx q[66], q[36];
rz(pi/2147483648) q[36];
cx q[66], q[36];
rz(-pi/2147483648) q[36];
rz(-pi/1073741824) q[66];
cx q[66], q[37];
rz(pi/1073741824) q[37];
cx q[66], q[37];
rz(-pi/1073741824) q[37];
rz(-pi/536870912) q[66];
cx q[66], q[38];
rz(pi/536870912) q[38];
cx q[66], q[38];
rz(-pi/536870912) q[38];
rz(-pi/268435456) q[66];
cx q[66], q[39];
rz(pi/268435456) q[39];
cx q[66], q[39];
rz(-pi/268435456) q[39];
rz(-pi/134217728) q[66];
cx q[66], q[40];
rz(pi/134217728) q[40];
cx q[66], q[40];
rz(-pi/134217728) q[40];
rz(-pi/67108864) q[66];
cx q[66], q[41];
rz(pi/67108864) q[41];
cx q[66], q[41];
rz(-pi/67108864) q[41];
rz(-pi/33554432) q[66];
cx q[66], q[42];
rz(pi/33554432) q[42];
cx q[66], q[42];
rz(-pi/33554432) q[42];
rz(-pi/16777216) q[66];
cx q[66], q[43];
rz(pi/16777216) q[43];
cx q[66], q[43];
rz(-pi/16777216) q[43];
rz(-pi/8388608) q[66];
cx q[66], q[44];
rz(pi/8388608) q[44];
cx q[66], q[44];
rz(-pi/8388608) q[44];
rz(-pi/4194304) q[66];
cx q[66], q[45];
rz(pi/4194304) q[45];
cx q[66], q[45];
rz(-pi/4194304) q[45];
rz(-pi/2097152) q[66];
cx q[66], q[46];
rz(pi/2097152) q[46];
cx q[66], q[46];
rz(-pi/2097152) q[46];
rz(-pi/1048576) q[66];
cx q[66], q[47];
rz(pi/1048576) q[47];
cx q[66], q[47];
rz(-pi/1048576) q[47];
rz(-pi/524288) q[66];
cx q[66], q[48];
rz(pi/524288) q[48];
cx q[66], q[48];
rz(-pi/524288) q[48];
rz(-pi/262144) q[66];
cx q[66], q[49];
rz(pi/262144) q[49];
cx q[66], q[49];
rz(-pi/262144) q[49];
rz(-pi/131072) q[66];
cx q[66], q[50];
rz(pi/131072) q[50];
cx q[66], q[50];
rz(-pi/131072) q[50];
rz(-pi/65536) q[66];
cx q[66], q[51];
rz(pi/65536) q[51];
cx q[66], q[51];
rz(-pi/65536) q[51];
rz(-pi/32768) q[66];
cx q[66], q[52];
rz(pi/32768) q[52];
cx q[66], q[52];
rz(-pi/32768) q[52];
rz(-pi/16384) q[66];
cx q[66], q[53];
rz(pi/16384) q[53];
cx q[66], q[53];
rz(-pi/16384) q[53];
rz(-pi/8192) q[66];
cx q[66], q[54];
rz(pi/8192) q[54];
cx q[66], q[54];
rz(-pi/8192) q[54];
rz(-pi/4096) q[66];
cx q[66], q[55];
rz(pi/4096) q[55];
cx q[66], q[55];
rz(-pi/4096) q[55];
rz(-pi/2048) q[66];
cx q[66], q[56];
rz(pi/2048) q[56];
cx q[66], q[56];
rz(-pi/2048) q[56];
rz(-pi/1024) q[66];
cx q[66], q[57];
rz(pi/1024) q[57];
cx q[66], q[57];
rz(-pi/1024) q[57];
rz(-pi/512) q[66];
cx q[66], q[58];
rz(pi/512) q[58];
cx q[66], q[58];
rz(-pi/512) q[58];
rz(-pi/256) q[66];
cx q[66], q[59];
rz(pi/256) q[59];
cx q[66], q[59];
rz(-pi/256) q[59];
rz(-pi/128) q[66];
cx q[66], q[60];
rz(pi/128) q[60];
cx q[66], q[60];
rz(-pi/128) q[60];
rz(-pi/64) q[66];
cx q[66], q[61];
rz(pi/64) q[61];
cx q[66], q[61];
rz(-pi/64) q[61];
rz(-pi/32) q[66];
cx q[66], q[62];
rz(pi/32) q[62];
cx q[66], q[62];
rz(-pi/32) q[62];
rz(-pi/16) q[66];
cx q[66], q[63];
rz(pi/16) q[63];
cx q[66], q[63];
rz(-pi/16) q[63];
rz(-pi/8) q[66];
cx q[66], q[64];
rz(pi/8) q[64];
cx q[66], q[64];
rz(-pi/8) q[64];
rz(-pi/4) q[66];
cx q[66], q[65];
rz(pi/4) q[65];
cx q[66], q[65];
rz(-pi/4) q[65];
h q[66];
cx q[67], q[0];
rz(0) q[0];
cx q[67], q[0];
rz(0) q[0];
rz(0) q[67];
cx q[67], q[1];
rz(0) q[1];
cx q[67], q[1];
rz(0) q[1];
rz(0) q[67];
cx q[67], q[2];
rz(0) q[2];
cx q[67], q[2];
rz(0) q[2];
rz(0) q[67];
cx q[67], q[3];
rz(0) q[3];
cx q[67], q[3];
rz(0) q[3];
rz(0) q[67];
cx q[67], q[4];
rz(0) q[4];
cx q[67], q[4];
rz(0) q[4];
rz(0) q[67];
cx q[67], q[5];
rz(0) q[5];
cx q[67], q[5];
rz(0) q[5];
rz(0) q[67];
cx q[67], q[6];
rz(0) q[6];
cx q[67], q[6];
rz(0) q[6];
rz(0) q[67];
cx q[67], q[7];
rz(0) q[7];
cx q[67], q[7];
rz(0) q[7];
rz(0) q[67];
cx q[67], q[8];
rz(0) q[8];
cx q[67], q[8];
rz(0) q[8];
rz(0) q[67];
cx q[67], q[9];
rz(0) q[9];
cx q[67], q[9];
rz(0) q[9];
rz(0) q[67];
cx q[67], q[10];
rz(0) q[10];
cx q[67], q[10];
rz(0) q[10];
rz(0) q[67];
cx q[67], q[11];
rz(0) q[11];
cx q[67], q[11];
rz(0) q[11];
rz(0) q[67];
cx q[67], q[12];
rz(0) q[12];
cx q[67], q[12];
rz(0) q[12];
rz(0) q[67];
cx q[67], q[13];
rz(0) q[13];
cx q[67], q[13];
rz(0) q[13];
rz(0) q[67];
cx q[67], q[14];
rz(0) q[14];
cx q[67], q[14];
rz(0) q[14];
rz(0) q[67];
cx q[67], q[15];
rz(0) q[15];
cx q[67], q[15];
rz(0) q[15];
rz(0) q[67];
cx q[67], q[16];
rz(0) q[16];
cx q[67], q[16];
rz(0) q[16];
rz(0) q[67];
cx q[67], q[17];
rz(0) q[17];
cx q[67], q[17];
rz(0) q[17];
rz(0) q[67];
cx q[67], q[18];
rz(0) q[18];
cx q[67], q[18];
rz(0) q[18];
rz(0) q[67];
cx q[67], q[19];
rz(0) q[19];
cx q[67], q[19];
rz(0) q[19];
rz(0) q[67];
cx q[67], q[20];
rz(0) q[20];
cx q[67], q[20];
rz(0) q[20];
rz(0) q[67];
cx q[67], q[21];
rz(0) q[21];
cx q[67], q[21];
rz(0) q[21];
rz(0) q[67];
cx q[67], q[22];
rz(0) q[22];
cx q[67], q[22];
rz(0) q[22];
rz(0) q[67];
cx q[67], q[23];
rz(0) q[23];
cx q[67], q[23];
rz(0) q[23];
rz(0) q[67];
cx q[67], q[24];
rz(0) q[24];
cx q[67], q[24];
rz(0) q[24];
rz(0) q[67];
cx q[67], q[25];
rz(0) q[25];
cx q[67], q[25];
rz(0) q[25];
rz(0) q[67];
cx q[67], q[26];
rz(0) q[26];
cx q[67], q[26];
rz(0) q[26];
rz(0) q[67];
cx q[67], q[27];
rz(0) q[27];
cx q[67], q[27];
rz(0) q[27];
rz(0) q[67];
cx q[67], q[28];
rz(0) q[28];
cx q[67], q[28];
rz(0) q[28];
rz(0) q[67];
cx q[67], q[29];
rz(0) q[29];
cx q[67], q[29];
rz(0) q[29];
rz(0) q[67];
cx q[67], q[30];
rz(0) q[30];
cx q[67], q[30];
rz(0) q[30];
rz(0) q[67];
cx q[67], q[31];
rz(0) q[31];
cx q[67], q[31];
rz(0) q[31];
rz(0) q[67];
cx q[67], q[32];
rz(0) q[32];
cx q[67], q[32];
rz(0) q[32];
rz(0) q[67];
cx q[67], q[33];
rz(0) q[33];
cx q[67], q[33];
rz(0) q[33];
rz(0) q[67];
cx q[67], q[34];
rz(0) q[34];
cx q[67], q[34];
rz(0) q[34];
rz(0) q[67];
cx q[67], q[35];
rz(0) q[35];
cx q[67], q[35];
rz(0) q[35];
rz(0) q[67];
cx q[67], q[36];
rz(0) q[36];
cx q[67], q[36];
rz(0) q[36];
rz(-pi/2147483648) q[67];
cx q[67], q[37];
rz(pi/2147483648) q[37];
cx q[67], q[37];
rz(-pi/2147483648) q[37];
rz(-pi/1073741824) q[67];
cx q[67], q[38];
rz(pi/1073741824) q[38];
cx q[67], q[38];
rz(-pi/1073741824) q[38];
rz(-pi/536870912) q[67];
cx q[67], q[39];
rz(pi/536870912) q[39];
cx q[67], q[39];
rz(-pi/536870912) q[39];
rz(-pi/268435456) q[67];
cx q[67], q[40];
rz(pi/268435456) q[40];
cx q[67], q[40];
rz(-pi/268435456) q[40];
rz(-pi/134217728) q[67];
cx q[67], q[41];
rz(pi/134217728) q[41];
cx q[67], q[41];
rz(-pi/134217728) q[41];
rz(-pi/67108864) q[67];
cx q[67], q[42];
rz(pi/67108864) q[42];
cx q[67], q[42];
rz(-pi/67108864) q[42];
rz(-pi/33554432) q[67];
cx q[67], q[43];
rz(pi/33554432) q[43];
cx q[67], q[43];
rz(-pi/33554432) q[43];
rz(-pi/16777216) q[67];
cx q[67], q[44];
rz(pi/16777216) q[44];
cx q[67], q[44];
rz(-pi/16777216) q[44];
rz(-pi/8388608) q[67];
cx q[67], q[45];
rz(pi/8388608) q[45];
cx q[67], q[45];
rz(-pi/8388608) q[45];
rz(-pi/4194304) q[67];
cx q[67], q[46];
rz(pi/4194304) q[46];
cx q[67], q[46];
rz(-pi/4194304) q[46];
rz(-pi/2097152) q[67];
cx q[67], q[47];
rz(pi/2097152) q[47];
cx q[67], q[47];
rz(-pi/2097152) q[47];
rz(-pi/1048576) q[67];
cx q[67], q[48];
rz(pi/1048576) q[48];
cx q[67], q[48];
rz(-pi/1048576) q[48];
rz(-pi/524288) q[67];
cx q[67], q[49];
rz(pi/524288) q[49];
cx q[67], q[49];
rz(-pi/524288) q[49];
rz(-pi/262144) q[67];
cx q[67], q[50];
rz(pi/262144) q[50];
cx q[67], q[50];
rz(-pi/262144) q[50];
rz(-pi/131072) q[67];
cx q[67], q[51];
rz(pi/131072) q[51];
cx q[67], q[51];
rz(-pi/131072) q[51];
rz(-pi/65536) q[67];
cx q[67], q[52];
rz(pi/65536) q[52];
cx q[67], q[52];
rz(-pi/65536) q[52];
rz(-pi/32768) q[67];
cx q[67], q[53];
rz(pi/32768) q[53];
cx q[67], q[53];
rz(-pi/32768) q[53];
rz(-pi/16384) q[67];
cx q[67], q[54];
rz(pi/16384) q[54];
cx q[67], q[54];
rz(-pi/16384) q[54];
rz(-pi/8192) q[67];
cx q[67], q[55];
rz(pi/8192) q[55];
cx q[67], q[55];
rz(-pi/8192) q[55];
rz(-pi/4096) q[67];
cx q[67], q[56];
rz(pi/4096) q[56];
cx q[67], q[56];
rz(-pi/4096) q[56];
rz(-pi/2048) q[67];
cx q[67], q[57];
rz(pi/2048) q[57];
cx q[67], q[57];
rz(-pi/2048) q[57];
rz(-pi/1024) q[67];
cx q[67], q[58];
rz(pi/1024) q[58];
cx q[67], q[58];
rz(-pi/1024) q[58];
rz(-pi/512) q[67];
cx q[67], q[59];
rz(pi/512) q[59];
cx q[67], q[59];
rz(-pi/512) q[59];
rz(-pi/256) q[67];
cx q[67], q[60];
rz(pi/256) q[60];
cx q[67], q[60];
rz(-pi/256) q[60];
rz(-pi/128) q[67];
cx q[67], q[61];
rz(pi/128) q[61];
cx q[67], q[61];
rz(-pi/128) q[61];
rz(-pi/64) q[67];
cx q[67], q[62];
rz(pi/64) q[62];
cx q[67], q[62];
rz(-pi/64) q[62];
rz(-pi/32) q[67];
cx q[67], q[63];
rz(pi/32) q[63];
cx q[67], q[63];
rz(-pi/32) q[63];
rz(-pi/16) q[67];
cx q[67], q[64];
rz(pi/16) q[64];
cx q[67], q[64];
rz(-pi/16) q[64];
rz(-pi/8) q[67];
cx q[67], q[65];
rz(pi/8) q[65];
cx q[67], q[65];
rz(-pi/8) q[65];
rz(-pi/4) q[67];
cx q[67], q[66];
rz(pi/4) q[66];
cx q[67], q[66];
rz(-pi/4) q[66];
h q[67];
cx q[68], q[0];
rz(0) q[0];
cx q[68], q[0];
rz(0) q[0];
rz(0) q[68];
cx q[68], q[1];
rz(0) q[1];
cx q[68], q[1];
rz(0) q[1];
rz(0) q[68];
cx q[68], q[2];
rz(0) q[2];
cx q[68], q[2];
rz(0) q[2];
rz(0) q[68];
cx q[68], q[3];
rz(0) q[3];
cx q[68], q[3];
rz(0) q[3];
rz(0) q[68];
cx q[68], q[4];
rz(0) q[4];
cx q[68], q[4];
rz(0) q[4];
rz(0) q[68];
cx q[68], q[5];
rz(0) q[5];
cx q[68], q[5];
rz(0) q[5];
rz(0) q[68];
cx q[68], q[6];
rz(0) q[6];
cx q[68], q[6];
rz(0) q[6];
rz(0) q[68];
cx q[68], q[7];
rz(0) q[7];
cx q[68], q[7];
rz(0) q[7];
rz(0) q[68];
cx q[68], q[8];
rz(0) q[8];
cx q[68], q[8];
rz(0) q[8];
rz(0) q[68];
cx q[68], q[9];
rz(0) q[9];
cx q[68], q[9];
rz(0) q[9];
rz(0) q[68];
cx q[68], q[10];
rz(0) q[10];
cx q[68], q[10];
rz(0) q[10];
rz(0) q[68];
cx q[68], q[11];
rz(0) q[11];
cx q[68], q[11];
rz(0) q[11];
rz(0) q[68];
cx q[68], q[12];
rz(0) q[12];
cx q[68], q[12];
rz(0) q[12];
rz(0) q[68];
cx q[68], q[13];
rz(0) q[13];
cx q[68], q[13];
rz(0) q[13];
rz(0) q[68];
cx q[68], q[14];
rz(0) q[14];
cx q[68], q[14];
rz(0) q[14];
rz(0) q[68];
cx q[68], q[15];
rz(0) q[15];
cx q[68], q[15];
rz(0) q[15];
rz(0) q[68];
cx q[68], q[16];
rz(0) q[16];
cx q[68], q[16];
rz(0) q[16];
rz(0) q[68];
cx q[68], q[17];
rz(0) q[17];
cx q[68], q[17];
rz(0) q[17];
rz(0) q[68];
cx q[68], q[18];
rz(0) q[18];
cx q[68], q[18];
rz(0) q[18];
rz(0) q[68];
cx q[68], q[19];
rz(0) q[19];
cx q[68], q[19];
rz(0) q[19];
rz(0) q[68];
cx q[68], q[20];
rz(0) q[20];
cx q[68], q[20];
rz(0) q[20];
rz(0) q[68];
cx q[68], q[21];
rz(0) q[21];
cx q[68], q[21];
rz(0) q[21];
rz(0) q[68];
cx q[68], q[22];
rz(0) q[22];
cx q[68], q[22];
rz(0) q[22];
rz(0) q[68];
cx q[68], q[23];
rz(0) q[23];
cx q[68], q[23];
rz(0) q[23];
rz(0) q[68];
cx q[68], q[24];
rz(0) q[24];
cx q[68], q[24];
rz(0) q[24];
rz(0) q[68];
cx q[68], q[25];
rz(0) q[25];
cx q[68], q[25];
rz(0) q[25];
rz(0) q[68];
cx q[68], q[26];
rz(0) q[26];
cx q[68], q[26];
rz(0) q[26];
rz(0) q[68];
cx q[68], q[27];
rz(0) q[27];
cx q[68], q[27];
rz(0) q[27];
rz(0) q[68];
cx q[68], q[28];
rz(0) q[28];
cx q[68], q[28];
rz(0) q[28];
rz(0) q[68];
cx q[68], q[29];
rz(0) q[29];
cx q[68], q[29];
rz(0) q[29];
rz(0) q[68];
cx q[68], q[30];
rz(0) q[30];
cx q[68], q[30];
rz(0) q[30];
rz(0) q[68];
cx q[68], q[31];
rz(0) q[31];
cx q[68], q[31];
rz(0) q[31];
rz(0) q[68];
cx q[68], q[32];
rz(0) q[32];
cx q[68], q[32];
rz(0) q[32];
rz(0) q[68];
cx q[68], q[33];
rz(0) q[33];
cx q[68], q[33];
rz(0) q[33];
rz(0) q[68];
cx q[68], q[34];
rz(0) q[34];
cx q[68], q[34];
rz(0) q[34];
rz(0) q[68];
cx q[68], q[35];
rz(0) q[35];
cx q[68], q[35];
rz(0) q[35];
rz(0) q[68];
cx q[68], q[36];
rz(0) q[36];
cx q[68], q[36];
rz(0) q[36];
rz(0) q[68];
cx q[68], q[37];
rz(0) q[37];
cx q[68], q[37];
rz(0) q[37];
rz(-pi/2147483648) q[68];
cx q[68], q[38];
rz(pi/2147483648) q[38];
cx q[68], q[38];
rz(-pi/2147483648) q[38];
rz(-pi/1073741824) q[68];
cx q[68], q[39];
rz(pi/1073741824) q[39];
cx q[68], q[39];
rz(-pi/1073741824) q[39];
rz(-pi/536870912) q[68];
cx q[68], q[40];
rz(pi/536870912) q[40];
cx q[68], q[40];
rz(-pi/536870912) q[40];
rz(-pi/268435456) q[68];
cx q[68], q[41];
rz(pi/268435456) q[41];
cx q[68], q[41];
rz(-pi/268435456) q[41];
rz(-pi/134217728) q[68];
cx q[68], q[42];
rz(pi/134217728) q[42];
cx q[68], q[42];
rz(-pi/134217728) q[42];
rz(-pi/67108864) q[68];
cx q[68], q[43];
rz(pi/67108864) q[43];
cx q[68], q[43];
rz(-pi/67108864) q[43];
rz(-pi/33554432) q[68];
cx q[68], q[44];
rz(pi/33554432) q[44];
cx q[68], q[44];
rz(-pi/33554432) q[44];
rz(-pi/16777216) q[68];
cx q[68], q[45];
rz(pi/16777216) q[45];
cx q[68], q[45];
rz(-pi/16777216) q[45];
rz(-pi/8388608) q[68];
cx q[68], q[46];
rz(pi/8388608) q[46];
cx q[68], q[46];
rz(-pi/8388608) q[46];
rz(-pi/4194304) q[68];
cx q[68], q[47];
rz(pi/4194304) q[47];
cx q[68], q[47];
rz(-pi/4194304) q[47];
rz(-pi/2097152) q[68];
cx q[68], q[48];
rz(pi/2097152) q[48];
cx q[68], q[48];
rz(-pi/2097152) q[48];
rz(-pi/1048576) q[68];
cx q[68], q[49];
rz(pi/1048576) q[49];
cx q[68], q[49];
rz(-pi/1048576) q[49];
rz(-pi/524288) q[68];
cx q[68], q[50];
rz(pi/524288) q[50];
cx q[68], q[50];
rz(-pi/524288) q[50];
rz(-pi/262144) q[68];
cx q[68], q[51];
rz(pi/262144) q[51];
cx q[68], q[51];
rz(-pi/262144) q[51];
rz(-pi/131072) q[68];
cx q[68], q[52];
rz(pi/131072) q[52];
cx q[68], q[52];
rz(-pi/131072) q[52];
rz(-pi/65536) q[68];
cx q[68], q[53];
rz(pi/65536) q[53];
cx q[68], q[53];
rz(-pi/65536) q[53];
rz(-pi/32768) q[68];
cx q[68], q[54];
rz(pi/32768) q[54];
cx q[68], q[54];
rz(-pi/32768) q[54];
rz(-pi/16384) q[68];
cx q[68], q[55];
rz(pi/16384) q[55];
cx q[68], q[55];
rz(-pi/16384) q[55];
rz(-pi/8192) q[68];
cx q[68], q[56];
rz(pi/8192) q[56];
cx q[68], q[56];
rz(-pi/8192) q[56];
rz(-pi/4096) q[68];
cx q[68], q[57];
rz(pi/4096) q[57];
cx q[68], q[57];
rz(-pi/4096) q[57];
rz(-pi/2048) q[68];
cx q[68], q[58];
rz(pi/2048) q[58];
cx q[68], q[58];
rz(-pi/2048) q[58];
rz(-pi/1024) q[68];
cx q[68], q[59];
rz(pi/1024) q[59];
cx q[68], q[59];
rz(-pi/1024) q[59];
rz(-pi/512) q[68];
cx q[68], q[60];
rz(pi/512) q[60];
cx q[68], q[60];
rz(-pi/512) q[60];
rz(-pi/256) q[68];
cx q[68], q[61];
rz(pi/256) q[61];
cx q[68], q[61];
rz(-pi/256) q[61];
rz(-pi/128) q[68];
cx q[68], q[62];
rz(pi/128) q[62];
cx q[68], q[62];
rz(-pi/128) q[62];
rz(-pi/64) q[68];
cx q[68], q[63];
rz(pi/64) q[63];
cx q[68], q[63];
rz(-pi/64) q[63];
rz(-pi/32) q[68];
cx q[68], q[64];
rz(pi/32) q[64];
cx q[68], q[64];
rz(-pi/32) q[64];
rz(-pi/16) q[68];
cx q[68], q[65];
rz(pi/16) q[65];
cx q[68], q[65];
rz(-pi/16) q[65];
rz(-pi/8) q[68];
cx q[68], q[66];
rz(pi/8) q[66];
cx q[68], q[66];
rz(-pi/8) q[66];
rz(-pi/4) q[68];
cx q[68], q[67];
rz(pi/4) q[67];
cx q[68], q[67];
rz(-pi/4) q[67];
h q[68];
cx q[69], q[0];
rz(0) q[0];
cx q[69], q[0];
rz(0) q[0];
rz(0) q[69];
cx q[69], q[1];
rz(0) q[1];
cx q[69], q[1];
rz(0) q[1];
rz(0) q[69];
cx q[69], q[2];
rz(0) q[2];
cx q[69], q[2];
rz(0) q[2];
rz(0) q[69];
cx q[69], q[3];
rz(0) q[3];
cx q[69], q[3];
rz(0) q[3];
rz(0) q[69];
cx q[69], q[4];
rz(0) q[4];
cx q[69], q[4];
rz(0) q[4];
rz(0) q[69];
cx q[69], q[5];
rz(0) q[5];
cx q[69], q[5];
rz(0) q[5];
rz(0) q[69];
cx q[69], q[6];
rz(0) q[6];
cx q[69], q[6];
rz(0) q[6];
rz(0) q[69];
cx q[69], q[7];
rz(0) q[7];
cx q[69], q[7];
rz(0) q[7];
rz(0) q[69];
cx q[69], q[8];
rz(0) q[8];
cx q[69], q[8];
rz(0) q[8];
rz(0) q[69];
cx q[69], q[9];
rz(0) q[9];
cx q[69], q[9];
rz(0) q[9];
rz(0) q[69];
cx q[69], q[10];
rz(0) q[10];
cx q[69], q[10];
rz(0) q[10];
rz(0) q[69];
cx q[69], q[11];
rz(0) q[11];
cx q[69], q[11];
rz(0) q[11];
rz(0) q[69];
cx q[69], q[12];
rz(0) q[12];
cx q[69], q[12];
rz(0) q[12];
rz(0) q[69];
cx q[69], q[13];
rz(0) q[13];
cx q[69], q[13];
rz(0) q[13];
rz(0) q[69];
cx q[69], q[14];
rz(0) q[14];
cx q[69], q[14];
rz(0) q[14];
rz(0) q[69];
cx q[69], q[15];
rz(0) q[15];
cx q[69], q[15];
rz(0) q[15];
rz(0) q[69];
cx q[69], q[16];
rz(0) q[16];
cx q[69], q[16];
rz(0) q[16];
rz(0) q[69];
cx q[69], q[17];
rz(0) q[17];
cx q[69], q[17];
rz(0) q[17];
rz(0) q[69];
cx q[69], q[18];
rz(0) q[18];
cx q[69], q[18];
rz(0) q[18];
rz(0) q[69];
cx q[69], q[19];
rz(0) q[19];
cx q[69], q[19];
rz(0) q[19];
rz(0) q[69];
cx q[69], q[20];
rz(0) q[20];
cx q[69], q[20];
rz(0) q[20];
rz(0) q[69];
cx q[69], q[21];
rz(0) q[21];
cx q[69], q[21];
rz(0) q[21];
rz(0) q[69];
cx q[69], q[22];
rz(0) q[22];
cx q[69], q[22];
rz(0) q[22];
rz(0) q[69];
cx q[69], q[23];
rz(0) q[23];
cx q[69], q[23];
rz(0) q[23];
rz(0) q[69];
cx q[69], q[24];
rz(0) q[24];
cx q[69], q[24];
rz(0) q[24];
rz(0) q[69];
cx q[69], q[25];
rz(0) q[25];
cx q[69], q[25];
rz(0) q[25];
rz(0) q[69];
cx q[69], q[26];
rz(0) q[26];
cx q[69], q[26];
rz(0) q[26];
rz(0) q[69];
cx q[69], q[27];
rz(0) q[27];
cx q[69], q[27];
rz(0) q[27];
rz(0) q[69];
cx q[69], q[28];
rz(0) q[28];
cx q[69], q[28];
rz(0) q[28];
rz(0) q[69];
cx q[69], q[29];
rz(0) q[29];
cx q[69], q[29];
rz(0) q[29];
rz(0) q[69];
cx q[69], q[30];
rz(0) q[30];
cx q[69], q[30];
rz(0) q[30];
rz(0) q[69];
cx q[69], q[31];
rz(0) q[31];
cx q[69], q[31];
rz(0) q[31];
rz(0) q[69];
cx q[69], q[32];
rz(0) q[32];
cx q[69], q[32];
rz(0) q[32];
rz(0) q[69];
cx q[69], q[33];
rz(0) q[33];
cx q[69], q[33];
rz(0) q[33];
rz(0) q[69];
cx q[69], q[34];
rz(0) q[34];
cx q[69], q[34];
rz(0) q[34];
rz(0) q[69];
cx q[69], q[35];
rz(0) q[35];
cx q[69], q[35];
rz(0) q[35];
rz(0) q[69];
cx q[69], q[36];
rz(0) q[36];
cx q[69], q[36];
rz(0) q[36];
rz(0) q[69];
cx q[69], q[37];
rz(0) q[37];
cx q[69], q[37];
rz(0) q[37];
rz(0) q[69];
cx q[69], q[38];
rz(0) q[38];
cx q[69], q[38];
rz(0) q[38];
rz(-pi/2147483648) q[69];
cx q[69], q[39];
rz(pi/2147483648) q[39];
cx q[69], q[39];
rz(-pi/2147483648) q[39];
rz(-pi/1073741824) q[69];
cx q[69], q[40];
rz(pi/1073741824) q[40];
cx q[69], q[40];
rz(-pi/1073741824) q[40];
rz(-pi/536870912) q[69];
cx q[69], q[41];
rz(pi/536870912) q[41];
cx q[69], q[41];
rz(-pi/536870912) q[41];
rz(-pi/268435456) q[69];
cx q[69], q[42];
rz(pi/268435456) q[42];
cx q[69], q[42];
rz(-pi/268435456) q[42];
rz(-pi/134217728) q[69];
cx q[69], q[43];
rz(pi/134217728) q[43];
cx q[69], q[43];
rz(-pi/134217728) q[43];
rz(-pi/67108864) q[69];
cx q[69], q[44];
rz(pi/67108864) q[44];
cx q[69], q[44];
rz(-pi/67108864) q[44];
rz(-pi/33554432) q[69];
cx q[69], q[45];
rz(pi/33554432) q[45];
cx q[69], q[45];
rz(-pi/33554432) q[45];
rz(-pi/16777216) q[69];
cx q[69], q[46];
rz(pi/16777216) q[46];
cx q[69], q[46];
rz(-pi/16777216) q[46];
rz(-pi/8388608) q[69];
cx q[69], q[47];
rz(pi/8388608) q[47];
cx q[69], q[47];
rz(-pi/8388608) q[47];
rz(-pi/4194304) q[69];
cx q[69], q[48];
rz(pi/4194304) q[48];
cx q[69], q[48];
rz(-pi/4194304) q[48];
rz(-pi/2097152) q[69];
cx q[69], q[49];
rz(pi/2097152) q[49];
cx q[69], q[49];
rz(-pi/2097152) q[49];
rz(-pi/1048576) q[69];
cx q[69], q[50];
rz(pi/1048576) q[50];
cx q[69], q[50];
rz(-pi/1048576) q[50];
rz(-pi/524288) q[69];
cx q[69], q[51];
rz(pi/524288) q[51];
cx q[69], q[51];
rz(-pi/524288) q[51];
rz(-pi/262144) q[69];
cx q[69], q[52];
rz(pi/262144) q[52];
cx q[69], q[52];
rz(-pi/262144) q[52];
rz(-pi/131072) q[69];
cx q[69], q[53];
rz(pi/131072) q[53];
cx q[69], q[53];
rz(-pi/131072) q[53];
rz(-pi/65536) q[69];
cx q[69], q[54];
rz(pi/65536) q[54];
cx q[69], q[54];
rz(-pi/65536) q[54];
rz(-pi/32768) q[69];
cx q[69], q[55];
rz(pi/32768) q[55];
cx q[69], q[55];
rz(-pi/32768) q[55];
rz(-pi/16384) q[69];
cx q[69], q[56];
rz(pi/16384) q[56];
cx q[69], q[56];
rz(-pi/16384) q[56];
rz(-pi/8192) q[69];
cx q[69], q[57];
rz(pi/8192) q[57];
cx q[69], q[57];
rz(-pi/8192) q[57];
rz(-pi/4096) q[69];
cx q[69], q[58];
rz(pi/4096) q[58];
cx q[69], q[58];
rz(-pi/4096) q[58];
rz(-pi/2048) q[69];
cx q[69], q[59];
rz(pi/2048) q[59];
cx q[69], q[59];
rz(-pi/2048) q[59];
rz(-pi/1024) q[69];
cx q[69], q[60];
rz(pi/1024) q[60];
cx q[69], q[60];
rz(-pi/1024) q[60];
rz(-pi/512) q[69];
cx q[69], q[61];
rz(pi/512) q[61];
cx q[69], q[61];
rz(-pi/512) q[61];
rz(-pi/256) q[69];
cx q[69], q[62];
rz(pi/256) q[62];
cx q[69], q[62];
rz(-pi/256) q[62];
rz(-pi/128) q[69];
cx q[69], q[63];
rz(pi/128) q[63];
cx q[69], q[63];
rz(-pi/128) q[63];
rz(-pi/64) q[69];
cx q[69], q[64];
rz(pi/64) q[64];
cx q[69], q[64];
rz(-pi/64) q[64];
rz(-pi/32) q[69];
cx q[69], q[65];
rz(pi/32) q[65];
cx q[69], q[65];
rz(-pi/32) q[65];
rz(-pi/16) q[69];
cx q[69], q[66];
rz(pi/16) q[66];
cx q[69], q[66];
rz(-pi/16) q[66];
rz(-pi/8) q[69];
cx q[69], q[67];
rz(pi/8) q[67];
cx q[69], q[67];
rz(-pi/8) q[67];
rz(-pi/4) q[69];
cx q[69], q[68];
rz(pi/4) q[68];
cx q[69], q[68];
rz(-pi/4) q[68];
h q[69];
cx q[70], q[0];
rz(0) q[0];
cx q[70], q[0];
rz(0) q[0];
rz(0) q[70];
cx q[70], q[1];
rz(0) q[1];
cx q[70], q[1];
rz(0) q[1];
rz(0) q[70];
cx q[70], q[2];
rz(0) q[2];
cx q[70], q[2];
rz(0) q[2];
rz(0) q[70];
cx q[70], q[3];
rz(0) q[3];
cx q[70], q[3];
rz(0) q[3];
rz(0) q[70];
cx q[70], q[4];
rz(0) q[4];
cx q[70], q[4];
rz(0) q[4];
rz(0) q[70];
cx q[70], q[5];
rz(0) q[5];
cx q[70], q[5];
rz(0) q[5];
rz(0) q[70];
cx q[70], q[6];
rz(0) q[6];
cx q[70], q[6];
rz(0) q[6];
rz(0) q[70];
cx q[70], q[7];
rz(0) q[7];
cx q[70], q[7];
rz(0) q[7];
rz(0) q[70];
cx q[70], q[8];
rz(0) q[8];
cx q[70], q[8];
rz(0) q[8];
rz(0) q[70];
cx q[70], q[9];
rz(0) q[9];
cx q[70], q[9];
rz(0) q[9];
rz(0) q[70];
cx q[70], q[10];
rz(0) q[10];
cx q[70], q[10];
rz(0) q[10];
rz(0) q[70];
cx q[70], q[11];
rz(0) q[11];
cx q[70], q[11];
rz(0) q[11];
rz(0) q[70];
cx q[70], q[12];
rz(0) q[12];
cx q[70], q[12];
rz(0) q[12];
rz(0) q[70];
cx q[70], q[13];
rz(0) q[13];
cx q[70], q[13];
rz(0) q[13];
rz(0) q[70];
cx q[70], q[14];
rz(0) q[14];
cx q[70], q[14];
rz(0) q[14];
rz(0) q[70];
cx q[70], q[15];
rz(0) q[15];
cx q[70], q[15];
rz(0) q[15];
rz(0) q[70];
cx q[70], q[16];
rz(0) q[16];
cx q[70], q[16];
rz(0) q[16];
rz(0) q[70];
cx q[70], q[17];
rz(0) q[17];
cx q[70], q[17];
rz(0) q[17];
rz(0) q[70];
cx q[70], q[18];
rz(0) q[18];
cx q[70], q[18];
rz(0) q[18];
rz(0) q[70];
cx q[70], q[19];
rz(0) q[19];
cx q[70], q[19];
rz(0) q[19];
rz(0) q[70];
cx q[70], q[20];
rz(0) q[20];
cx q[70], q[20];
rz(0) q[20];
rz(0) q[70];
cx q[70], q[21];
rz(0) q[21];
cx q[70], q[21];
rz(0) q[21];
rz(0) q[70];
cx q[70], q[22];
rz(0) q[22];
cx q[70], q[22];
rz(0) q[22];
rz(0) q[70];
cx q[70], q[23];
rz(0) q[23];
cx q[70], q[23];
rz(0) q[23];
rz(0) q[70];
cx q[70], q[24];
rz(0) q[24];
cx q[70], q[24];
rz(0) q[24];
rz(0) q[70];
cx q[70], q[25];
rz(0) q[25];
cx q[70], q[25];
rz(0) q[25];
rz(0) q[70];
cx q[70], q[26];
rz(0) q[26];
cx q[70], q[26];
rz(0) q[26];
rz(0) q[70];
cx q[70], q[27];
rz(0) q[27];
cx q[70], q[27];
rz(0) q[27];
rz(0) q[70];
cx q[70], q[28];
rz(0) q[28];
cx q[70], q[28];
rz(0) q[28];
rz(0) q[70];
cx q[70], q[29];
rz(0) q[29];
cx q[70], q[29];
rz(0) q[29];
rz(0) q[70];
cx q[70], q[30];
rz(0) q[30];
cx q[70], q[30];
rz(0) q[30];
rz(0) q[70];
cx q[70], q[31];
rz(0) q[31];
cx q[70], q[31];
rz(0) q[31];
rz(0) q[70];
cx q[70], q[32];
rz(0) q[32];
cx q[70], q[32];
rz(0) q[32];
rz(0) q[70];
cx q[70], q[33];
rz(0) q[33];
cx q[70], q[33];
rz(0) q[33];
rz(0) q[70];
cx q[70], q[34];
rz(0) q[34];
cx q[70], q[34];
rz(0) q[34];
rz(0) q[70];
cx q[70], q[35];
rz(0) q[35];
cx q[70], q[35];
rz(0) q[35];
rz(0) q[70];
cx q[70], q[36];
rz(0) q[36];
cx q[70], q[36];
rz(0) q[36];
rz(0) q[70];
cx q[70], q[37];
rz(0) q[37];
cx q[70], q[37];
rz(0) q[37];
rz(0) q[70];
cx q[70], q[38];
rz(0) q[38];
cx q[70], q[38];
rz(0) q[38];
rz(0) q[70];
cx q[70], q[39];
rz(0) q[39];
cx q[70], q[39];
rz(0) q[39];
rz(-pi/2147483648) q[70];
cx q[70], q[40];
rz(pi/2147483648) q[40];
cx q[70], q[40];
rz(-pi/2147483648) q[40];
rz(-pi/1073741824) q[70];
cx q[70], q[41];
rz(pi/1073741824) q[41];
cx q[70], q[41];
rz(-pi/1073741824) q[41];
rz(-pi/536870912) q[70];
cx q[70], q[42];
rz(pi/536870912) q[42];
cx q[70], q[42];
rz(-pi/536870912) q[42];
rz(-pi/268435456) q[70];
cx q[70], q[43];
rz(pi/268435456) q[43];
cx q[70], q[43];
rz(-pi/268435456) q[43];
rz(-pi/134217728) q[70];
cx q[70], q[44];
rz(pi/134217728) q[44];
cx q[70], q[44];
rz(-pi/134217728) q[44];
rz(-pi/67108864) q[70];
cx q[70], q[45];
rz(pi/67108864) q[45];
cx q[70], q[45];
rz(-pi/67108864) q[45];
rz(-pi/33554432) q[70];
cx q[70], q[46];
rz(pi/33554432) q[46];
cx q[70], q[46];
rz(-pi/33554432) q[46];
rz(-pi/16777216) q[70];
cx q[70], q[47];
rz(pi/16777216) q[47];
cx q[70], q[47];
rz(-pi/16777216) q[47];
rz(-pi/8388608) q[70];
cx q[70], q[48];
rz(pi/8388608) q[48];
cx q[70], q[48];
rz(-pi/8388608) q[48];
rz(-pi/4194304) q[70];
cx q[70], q[49];
rz(pi/4194304) q[49];
cx q[70], q[49];
rz(-pi/4194304) q[49];
rz(-pi/2097152) q[70];
cx q[70], q[50];
rz(pi/2097152) q[50];
cx q[70], q[50];
rz(-pi/2097152) q[50];
rz(-pi/1048576) q[70];
cx q[70], q[51];
rz(pi/1048576) q[51];
cx q[70], q[51];
rz(-pi/1048576) q[51];
rz(-pi/524288) q[70];
cx q[70], q[52];
rz(pi/524288) q[52];
cx q[70], q[52];
rz(-pi/524288) q[52];
rz(-pi/262144) q[70];
cx q[70], q[53];
rz(pi/262144) q[53];
cx q[70], q[53];
rz(-pi/262144) q[53];
rz(-pi/131072) q[70];
cx q[70], q[54];
rz(pi/131072) q[54];
cx q[70], q[54];
rz(-pi/131072) q[54];
rz(-pi/65536) q[70];
cx q[70], q[55];
rz(pi/65536) q[55];
cx q[70], q[55];
rz(-pi/65536) q[55];
rz(-pi/32768) q[70];
cx q[70], q[56];
rz(pi/32768) q[56];
cx q[70], q[56];
rz(-pi/32768) q[56];
rz(-pi/16384) q[70];
cx q[70], q[57];
rz(pi/16384) q[57];
cx q[70], q[57];
rz(-pi/16384) q[57];
rz(-pi/8192) q[70];
cx q[70], q[58];
rz(pi/8192) q[58];
cx q[70], q[58];
rz(-pi/8192) q[58];
rz(-pi/4096) q[70];
cx q[70], q[59];
rz(pi/4096) q[59];
cx q[70], q[59];
rz(-pi/4096) q[59];
rz(-pi/2048) q[70];
cx q[70], q[60];
rz(pi/2048) q[60];
cx q[70], q[60];
rz(-pi/2048) q[60];
rz(-pi/1024) q[70];
cx q[70], q[61];
rz(pi/1024) q[61];
cx q[70], q[61];
rz(-pi/1024) q[61];
rz(-pi/512) q[70];
cx q[70], q[62];
rz(pi/512) q[62];
cx q[70], q[62];
rz(-pi/512) q[62];
rz(-pi/256) q[70];
cx q[70], q[63];
rz(pi/256) q[63];
cx q[70], q[63];
rz(-pi/256) q[63];
rz(-pi/128) q[70];
cx q[70], q[64];
rz(pi/128) q[64];
cx q[70], q[64];
rz(-pi/128) q[64];
rz(-pi/64) q[70];
cx q[70], q[65];
rz(pi/64) q[65];
cx q[70], q[65];
rz(-pi/64) q[65];
rz(-pi/32) q[70];
cx q[70], q[66];
rz(pi/32) q[66];
cx q[70], q[66];
rz(-pi/32) q[66];
rz(-pi/16) q[70];
cx q[70], q[67];
rz(pi/16) q[67];
cx q[70], q[67];
rz(-pi/16) q[67];
rz(-pi/8) q[70];
cx q[70], q[68];
rz(pi/8) q[68];
cx q[70], q[68];
rz(-pi/8) q[68];
rz(-pi/4) q[70];
cx q[70], q[69];
rz(pi/4) q[69];
cx q[70], q[69];
rz(-pi/4) q[69];
h q[70];
cx q[71], q[0];
rz(0) q[0];
cx q[71], q[0];
rz(0) q[0];
rz(0) q[71];
cx q[71], q[1];
rz(0) q[1];
cx q[71], q[1];
rz(0) q[1];
rz(0) q[71];
cx q[71], q[2];
rz(0) q[2];
cx q[71], q[2];
rz(0) q[2];
rz(0) q[71];
cx q[71], q[3];
rz(0) q[3];
cx q[71], q[3];
rz(0) q[3];
rz(0) q[71];
cx q[71], q[4];
rz(0) q[4];
cx q[71], q[4];
rz(0) q[4];
rz(0) q[71];
cx q[71], q[5];
rz(0) q[5];
cx q[71], q[5];
rz(0) q[5];
rz(0) q[71];
cx q[71], q[6];
rz(0) q[6];
cx q[71], q[6];
rz(0) q[6];
rz(0) q[71];
cx q[71], q[7];
rz(0) q[7];
cx q[71], q[7];
rz(0) q[7];
rz(0) q[71];
cx q[71], q[8];
rz(0) q[8];
cx q[71], q[8];
rz(0) q[8];
rz(0) q[71];
cx q[71], q[9];
rz(0) q[9];
cx q[71], q[9];
rz(0) q[9];
rz(0) q[71];
cx q[71], q[10];
rz(0) q[10];
cx q[71], q[10];
rz(0) q[10];
rz(0) q[71];
cx q[71], q[11];
rz(0) q[11];
cx q[71], q[11];
rz(0) q[11];
rz(0) q[71];
cx q[71], q[12];
rz(0) q[12];
cx q[71], q[12];
rz(0) q[12];
rz(0) q[71];
cx q[71], q[13];
rz(0) q[13];
cx q[71], q[13];
rz(0) q[13];
rz(0) q[71];
cx q[71], q[14];
rz(0) q[14];
cx q[71], q[14];
rz(0) q[14];
rz(0) q[71];
cx q[71], q[15];
rz(0) q[15];
cx q[71], q[15];
rz(0) q[15];
rz(0) q[71];
cx q[71], q[16];
rz(0) q[16];
cx q[71], q[16];
rz(0) q[16];
rz(0) q[71];
cx q[71], q[17];
rz(0) q[17];
cx q[71], q[17];
rz(0) q[17];
rz(0) q[71];
cx q[71], q[18];
rz(0) q[18];
cx q[71], q[18];
rz(0) q[18];
rz(0) q[71];
cx q[71], q[19];
rz(0) q[19];
cx q[71], q[19];
rz(0) q[19];
rz(0) q[71];
cx q[71], q[20];
rz(0) q[20];
cx q[71], q[20];
rz(0) q[20];
rz(0) q[71];
cx q[71], q[21];
rz(0) q[21];
cx q[71], q[21];
rz(0) q[21];
rz(0) q[71];
cx q[71], q[22];
rz(0) q[22];
cx q[71], q[22];
rz(0) q[22];
rz(0) q[71];
cx q[71], q[23];
rz(0) q[23];
cx q[71], q[23];
rz(0) q[23];
rz(0) q[71];
cx q[71], q[24];
rz(0) q[24];
cx q[71], q[24];
rz(0) q[24];
rz(0) q[71];
cx q[71], q[25];
rz(0) q[25];
cx q[71], q[25];
rz(0) q[25];
rz(0) q[71];
cx q[71], q[26];
rz(0) q[26];
cx q[71], q[26];
rz(0) q[26];
rz(0) q[71];
cx q[71], q[27];
rz(0) q[27];
cx q[71], q[27];
rz(0) q[27];
rz(0) q[71];
cx q[71], q[28];
rz(0) q[28];
cx q[71], q[28];
rz(0) q[28];
rz(0) q[71];
cx q[71], q[29];
rz(0) q[29];
cx q[71], q[29];
rz(0) q[29];
rz(0) q[71];
cx q[71], q[30];
rz(0) q[30];
cx q[71], q[30];
rz(0) q[30];
rz(0) q[71];
cx q[71], q[31];
rz(0) q[31];
cx q[71], q[31];
rz(0) q[31];
rz(0) q[71];
cx q[71], q[32];
rz(0) q[32];
cx q[71], q[32];
rz(0) q[32];
rz(0) q[71];
cx q[71], q[33];
rz(0) q[33];
cx q[71], q[33];
rz(0) q[33];
rz(0) q[71];
cx q[71], q[34];
rz(0) q[34];
cx q[71], q[34];
rz(0) q[34];
rz(0) q[71];
cx q[71], q[35];
rz(0) q[35];
cx q[71], q[35];
rz(0) q[35];
rz(0) q[71];
cx q[71], q[36];
rz(0) q[36];
cx q[71], q[36];
rz(0) q[36];
rz(0) q[71];
cx q[71], q[37];
rz(0) q[37];
cx q[71], q[37];
rz(0) q[37];
rz(0) q[71];
cx q[71], q[38];
rz(0) q[38];
cx q[71], q[38];
rz(0) q[38];
rz(0) q[71];
cx q[71], q[39];
rz(0) q[39];
cx q[71], q[39];
rz(0) q[39];
rz(0) q[71];
cx q[71], q[40];
rz(0) q[40];
cx q[71], q[40];
rz(0) q[40];
rz(-pi/2147483648) q[71];
cx q[71], q[41];
rz(pi/2147483648) q[41];
cx q[71], q[41];
rz(-pi/2147483648) q[41];
rz(-pi/1073741824) q[71];
cx q[71], q[42];
rz(pi/1073741824) q[42];
cx q[71], q[42];
rz(-pi/1073741824) q[42];
rz(-pi/536870912) q[71];
cx q[71], q[43];
rz(pi/536870912) q[43];
cx q[71], q[43];
rz(-pi/536870912) q[43];
rz(-pi/268435456) q[71];
cx q[71], q[44];
rz(pi/268435456) q[44];
cx q[71], q[44];
rz(-pi/268435456) q[44];
rz(-pi/134217728) q[71];
cx q[71], q[45];
rz(pi/134217728) q[45];
cx q[71], q[45];
rz(-pi/134217728) q[45];
rz(-pi/67108864) q[71];
cx q[71], q[46];
rz(pi/67108864) q[46];
cx q[71], q[46];
rz(-pi/67108864) q[46];
rz(-pi/33554432) q[71];
cx q[71], q[47];
rz(pi/33554432) q[47];
cx q[71], q[47];
rz(-pi/33554432) q[47];
rz(-pi/16777216) q[71];
cx q[71], q[48];
rz(pi/16777216) q[48];
cx q[71], q[48];
rz(-pi/16777216) q[48];
rz(-pi/8388608) q[71];
cx q[71], q[49];
rz(pi/8388608) q[49];
cx q[71], q[49];
rz(-pi/8388608) q[49];
rz(-pi/4194304) q[71];
cx q[71], q[50];
rz(pi/4194304) q[50];
cx q[71], q[50];
rz(-pi/4194304) q[50];
rz(-pi/2097152) q[71];
cx q[71], q[51];
rz(pi/2097152) q[51];
cx q[71], q[51];
rz(-pi/2097152) q[51];
rz(-pi/1048576) q[71];
cx q[71], q[52];
rz(pi/1048576) q[52];
cx q[71], q[52];
rz(-pi/1048576) q[52];
rz(-pi/524288) q[71];
cx q[71], q[53];
rz(pi/524288) q[53];
cx q[71], q[53];
rz(-pi/524288) q[53];
rz(-pi/262144) q[71];
cx q[71], q[54];
rz(pi/262144) q[54];
cx q[71], q[54];
rz(-pi/262144) q[54];
rz(-pi/131072) q[71];
cx q[71], q[55];
rz(pi/131072) q[55];
cx q[71], q[55];
rz(-pi/131072) q[55];
rz(-pi/65536) q[71];
cx q[71], q[56];
rz(pi/65536) q[56];
cx q[71], q[56];
rz(-pi/65536) q[56];
rz(-pi/32768) q[71];
cx q[71], q[57];
rz(pi/32768) q[57];
cx q[71], q[57];
rz(-pi/32768) q[57];
rz(-pi/16384) q[71];
cx q[71], q[58];
rz(pi/16384) q[58];
cx q[71], q[58];
rz(-pi/16384) q[58];
rz(-pi/8192) q[71];
cx q[71], q[59];
rz(pi/8192) q[59];
cx q[71], q[59];
rz(-pi/8192) q[59];
rz(-pi/4096) q[71];
cx q[71], q[60];
rz(pi/4096) q[60];
cx q[71], q[60];
rz(-pi/4096) q[60];
rz(-pi/2048) q[71];
cx q[71], q[61];
rz(pi/2048) q[61];
cx q[71], q[61];
rz(-pi/2048) q[61];
rz(-pi/1024) q[71];
cx q[71], q[62];
rz(pi/1024) q[62];
cx q[71], q[62];
rz(-pi/1024) q[62];
rz(-pi/512) q[71];
cx q[71], q[63];
rz(pi/512) q[63];
cx q[71], q[63];
rz(-pi/512) q[63];
rz(-pi/256) q[71];
cx q[71], q[64];
rz(pi/256) q[64];
cx q[71], q[64];
rz(-pi/256) q[64];
rz(-pi/128) q[71];
cx q[71], q[65];
rz(pi/128) q[65];
cx q[71], q[65];
rz(-pi/128) q[65];
rz(-pi/64) q[71];
cx q[71], q[66];
rz(pi/64) q[66];
cx q[71], q[66];
rz(-pi/64) q[66];
rz(-pi/32) q[71];
cx q[71], q[67];
rz(pi/32) q[67];
cx q[71], q[67];
rz(-pi/32) q[67];
rz(-pi/16) q[71];
cx q[71], q[68];
rz(pi/16) q[68];
cx q[71], q[68];
rz(-pi/16) q[68];
rz(-pi/8) q[71];
cx q[71], q[69];
rz(pi/8) q[69];
cx q[71], q[69];
rz(-pi/8) q[69];
rz(-pi/4) q[71];
cx q[71], q[70];
rz(pi/4) q[70];
cx q[71], q[70];
rz(-pi/4) q[70];
h q[71];
cx q[72], q[0];
rz(0) q[0];
cx q[72], q[0];
rz(0) q[0];
rz(0) q[72];
cx q[72], q[1];
rz(0) q[1];
cx q[72], q[1];
rz(0) q[1];
rz(0) q[72];
cx q[72], q[2];
rz(0) q[2];
cx q[72], q[2];
rz(0) q[2];
rz(0) q[72];
cx q[72], q[3];
rz(0) q[3];
cx q[72], q[3];
rz(0) q[3];
rz(0) q[72];
cx q[72], q[4];
rz(0) q[4];
cx q[72], q[4];
rz(0) q[4];
rz(0) q[72];
cx q[72], q[5];
rz(0) q[5];
cx q[72], q[5];
rz(0) q[5];
rz(0) q[72];
cx q[72], q[6];
rz(0) q[6];
cx q[72], q[6];
rz(0) q[6];
rz(0) q[72];
cx q[72], q[7];
rz(0) q[7];
cx q[72], q[7];
rz(0) q[7];
rz(0) q[72];
cx q[72], q[8];
rz(0) q[8];
cx q[72], q[8];
rz(0) q[8];
rz(0) q[72];
cx q[72], q[9];
rz(0) q[9];
cx q[72], q[9];
rz(0) q[9];
rz(0) q[72];
cx q[72], q[10];
rz(0) q[10];
cx q[72], q[10];
rz(0) q[10];
rz(0) q[72];
cx q[72], q[11];
rz(0) q[11];
cx q[72], q[11];
rz(0) q[11];
rz(0) q[72];
cx q[72], q[12];
rz(0) q[12];
cx q[72], q[12];
rz(0) q[12];
rz(0) q[72];
cx q[72], q[13];
rz(0) q[13];
cx q[72], q[13];
rz(0) q[13];
rz(0) q[72];
cx q[72], q[14];
rz(0) q[14];
cx q[72], q[14];
rz(0) q[14];
rz(0) q[72];
cx q[72], q[15];
rz(0) q[15];
cx q[72], q[15];
rz(0) q[15];
rz(0) q[72];
cx q[72], q[16];
rz(0) q[16];
cx q[72], q[16];
rz(0) q[16];
rz(0) q[72];
cx q[72], q[17];
rz(0) q[17];
cx q[72], q[17];
rz(0) q[17];
rz(0) q[72];
cx q[72], q[18];
rz(0) q[18];
cx q[72], q[18];
rz(0) q[18];
rz(0) q[72];
cx q[72], q[19];
rz(0) q[19];
cx q[72], q[19];
rz(0) q[19];
rz(0) q[72];
cx q[72], q[20];
rz(0) q[20];
cx q[72], q[20];
rz(0) q[20];
rz(0) q[72];
cx q[72], q[21];
rz(0) q[21];
cx q[72], q[21];
rz(0) q[21];
rz(0) q[72];
cx q[72], q[22];
rz(0) q[22];
cx q[72], q[22];
rz(0) q[22];
rz(0) q[72];
cx q[72], q[23];
rz(0) q[23];
cx q[72], q[23];
rz(0) q[23];
rz(0) q[72];
cx q[72], q[24];
rz(0) q[24];
cx q[72], q[24];
rz(0) q[24];
rz(0) q[72];
cx q[72], q[25];
rz(0) q[25];
cx q[72], q[25];
rz(0) q[25];
rz(0) q[72];
cx q[72], q[26];
rz(0) q[26];
cx q[72], q[26];
rz(0) q[26];
rz(0) q[72];
cx q[72], q[27];
rz(0) q[27];
cx q[72], q[27];
rz(0) q[27];
rz(0) q[72];
cx q[72], q[28];
rz(0) q[28];
cx q[72], q[28];
rz(0) q[28];
rz(0) q[72];
cx q[72], q[29];
rz(0) q[29];
cx q[72], q[29];
rz(0) q[29];
rz(0) q[72];
cx q[72], q[30];
rz(0) q[30];
cx q[72], q[30];
rz(0) q[30];
rz(0) q[72];
cx q[72], q[31];
rz(0) q[31];
cx q[72], q[31];
rz(0) q[31];
rz(0) q[72];
cx q[72], q[32];
rz(0) q[32];
cx q[72], q[32];
rz(0) q[32];
rz(0) q[72];
cx q[72], q[33];
rz(0) q[33];
cx q[72], q[33];
rz(0) q[33];
rz(0) q[72];
cx q[72], q[34];
rz(0) q[34];
cx q[72], q[34];
rz(0) q[34];
rz(0) q[72];
cx q[72], q[35];
rz(0) q[35];
cx q[72], q[35];
rz(0) q[35];
rz(0) q[72];
cx q[72], q[36];
rz(0) q[36];
cx q[72], q[36];
rz(0) q[36];
rz(0) q[72];
cx q[72], q[37];
rz(0) q[37];
cx q[72], q[37];
rz(0) q[37];
rz(0) q[72];
cx q[72], q[38];
rz(0) q[38];
cx q[72], q[38];
rz(0) q[38];
rz(0) q[72];
cx q[72], q[39];
rz(0) q[39];
cx q[72], q[39];
rz(0) q[39];
rz(0) q[72];
cx q[72], q[40];
rz(0) q[40];
cx q[72], q[40];
rz(0) q[40];
rz(0) q[72];
cx q[72], q[41];
rz(0) q[41];
cx q[72], q[41];
rz(0) q[41];
rz(-pi/2147483648) q[72];
cx q[72], q[42];
rz(pi/2147483648) q[42];
cx q[72], q[42];
rz(-pi/2147483648) q[42];
rz(-pi/1073741824) q[72];
cx q[72], q[43];
rz(pi/1073741824) q[43];
cx q[72], q[43];
rz(-pi/1073741824) q[43];
rz(-pi/536870912) q[72];
cx q[72], q[44];
rz(pi/536870912) q[44];
cx q[72], q[44];
rz(-pi/536870912) q[44];
rz(-pi/268435456) q[72];
cx q[72], q[45];
rz(pi/268435456) q[45];
cx q[72], q[45];
rz(-pi/268435456) q[45];
rz(-pi/134217728) q[72];
cx q[72], q[46];
rz(pi/134217728) q[46];
cx q[72], q[46];
rz(-pi/134217728) q[46];
rz(-pi/67108864) q[72];
cx q[72], q[47];
rz(pi/67108864) q[47];
cx q[72], q[47];
rz(-pi/67108864) q[47];
rz(-pi/33554432) q[72];
cx q[72], q[48];
rz(pi/33554432) q[48];
cx q[72], q[48];
rz(-pi/33554432) q[48];
rz(-pi/16777216) q[72];
cx q[72], q[49];
rz(pi/16777216) q[49];
cx q[72], q[49];
rz(-pi/16777216) q[49];
rz(-pi/8388608) q[72];
cx q[72], q[50];
rz(pi/8388608) q[50];
cx q[72], q[50];
rz(-pi/8388608) q[50];
rz(-pi/4194304) q[72];
cx q[72], q[51];
rz(pi/4194304) q[51];
cx q[72], q[51];
rz(-pi/4194304) q[51];
rz(-pi/2097152) q[72];
cx q[72], q[52];
rz(pi/2097152) q[52];
cx q[72], q[52];
rz(-pi/2097152) q[52];
rz(-pi/1048576) q[72];
cx q[72], q[53];
rz(pi/1048576) q[53];
cx q[72], q[53];
rz(-pi/1048576) q[53];
rz(-pi/524288) q[72];
cx q[72], q[54];
rz(pi/524288) q[54];
cx q[72], q[54];
rz(-pi/524288) q[54];
rz(-pi/262144) q[72];
cx q[72], q[55];
rz(pi/262144) q[55];
cx q[72], q[55];
rz(-pi/262144) q[55];
rz(-pi/131072) q[72];
cx q[72], q[56];
rz(pi/131072) q[56];
cx q[72], q[56];
rz(-pi/131072) q[56];
rz(-pi/65536) q[72];
cx q[72], q[57];
rz(pi/65536) q[57];
cx q[72], q[57];
rz(-pi/65536) q[57];
rz(-pi/32768) q[72];
cx q[72], q[58];
rz(pi/32768) q[58];
cx q[72], q[58];
rz(-pi/32768) q[58];
rz(-pi/16384) q[72];
cx q[72], q[59];
rz(pi/16384) q[59];
cx q[72], q[59];
rz(-pi/16384) q[59];
rz(-pi/8192) q[72];
cx q[72], q[60];
rz(pi/8192) q[60];
cx q[72], q[60];
rz(-pi/8192) q[60];
rz(-pi/4096) q[72];
cx q[72], q[61];
rz(pi/4096) q[61];
cx q[72], q[61];
rz(-pi/4096) q[61];
rz(-pi/2048) q[72];
cx q[72], q[62];
rz(pi/2048) q[62];
cx q[72], q[62];
rz(-pi/2048) q[62];
rz(-pi/1024) q[72];
cx q[72], q[63];
rz(pi/1024) q[63];
cx q[72], q[63];
rz(-pi/1024) q[63];
rz(-pi/512) q[72];
cx q[72], q[64];
rz(pi/512) q[64];
cx q[72], q[64];
rz(-pi/512) q[64];
rz(-pi/256) q[72];
cx q[72], q[65];
rz(pi/256) q[65];
cx q[72], q[65];
rz(-pi/256) q[65];
rz(-pi/128) q[72];
cx q[72], q[66];
rz(pi/128) q[66];
cx q[72], q[66];
rz(-pi/128) q[66];
rz(-pi/64) q[72];
cx q[72], q[67];
rz(pi/64) q[67];
cx q[72], q[67];
rz(-pi/64) q[67];
rz(-pi/32) q[72];
cx q[72], q[68];
rz(pi/32) q[68];
cx q[72], q[68];
rz(-pi/32) q[68];
rz(-pi/16) q[72];
cx q[72], q[69];
rz(pi/16) q[69];
cx q[72], q[69];
rz(-pi/16) q[69];
rz(-pi/8) q[72];
cx q[72], q[70];
rz(pi/8) q[70];
cx q[72], q[70];
rz(-pi/8) q[70];
rz(-pi/4) q[72];
cx q[72], q[71];
rz(pi/4) q[71];
cx q[72], q[71];
rz(-pi/4) q[71];
h q[72];
cx q[73], q[0];
rz(0) q[0];
cx q[73], q[0];
rz(0) q[0];
rz(0) q[73];
cx q[73], q[1];
rz(0) q[1];
cx q[73], q[1];
rz(0) q[1];
rz(0) q[73];
cx q[73], q[2];
rz(0) q[2];
cx q[73], q[2];
rz(0) q[2];
rz(0) q[73];
cx q[73], q[3];
rz(0) q[3];
cx q[73], q[3];
rz(0) q[3];
rz(0) q[73];
cx q[73], q[4];
rz(0) q[4];
cx q[73], q[4];
rz(0) q[4];
rz(0) q[73];
cx q[73], q[5];
rz(0) q[5];
cx q[73], q[5];
rz(0) q[5];
rz(0) q[73];
cx q[73], q[6];
rz(0) q[6];
cx q[73], q[6];
rz(0) q[6];
rz(0) q[73];
cx q[73], q[7];
rz(0) q[7];
cx q[73], q[7];
rz(0) q[7];
rz(0) q[73];
cx q[73], q[8];
rz(0) q[8];
cx q[73], q[8];
rz(0) q[8];
rz(0) q[73];
cx q[73], q[9];
rz(0) q[9];
cx q[73], q[9];
rz(0) q[9];
rz(0) q[73];
cx q[73], q[10];
rz(0) q[10];
cx q[73], q[10];
rz(0) q[10];
rz(0) q[73];
cx q[73], q[11];
rz(0) q[11];
cx q[73], q[11];
rz(0) q[11];
rz(0) q[73];
cx q[73], q[12];
rz(0) q[12];
cx q[73], q[12];
rz(0) q[12];
rz(0) q[73];
cx q[73], q[13];
rz(0) q[13];
cx q[73], q[13];
rz(0) q[13];
rz(0) q[73];
cx q[73], q[14];
rz(0) q[14];
cx q[73], q[14];
rz(0) q[14];
rz(0) q[73];
cx q[73], q[15];
rz(0) q[15];
cx q[73], q[15];
rz(0) q[15];
rz(0) q[73];
cx q[73], q[16];
rz(0) q[16];
cx q[73], q[16];
rz(0) q[16];
rz(0) q[73];
cx q[73], q[17];
rz(0) q[17];
cx q[73], q[17];
rz(0) q[17];
rz(0) q[73];
cx q[73], q[18];
rz(0) q[18];
cx q[73], q[18];
rz(0) q[18];
rz(0) q[73];
cx q[73], q[19];
rz(0) q[19];
cx q[73], q[19];
rz(0) q[19];
rz(0) q[73];
cx q[73], q[20];
rz(0) q[20];
cx q[73], q[20];
rz(0) q[20];
rz(0) q[73];
cx q[73], q[21];
rz(0) q[21];
cx q[73], q[21];
rz(0) q[21];
rz(0) q[73];
cx q[73], q[22];
rz(0) q[22];
cx q[73], q[22];
rz(0) q[22];
rz(0) q[73];
cx q[73], q[23];
rz(0) q[23];
cx q[73], q[23];
rz(0) q[23];
rz(0) q[73];
cx q[73], q[24];
rz(0) q[24];
cx q[73], q[24];
rz(0) q[24];
rz(0) q[73];
cx q[73], q[25];
rz(0) q[25];
cx q[73], q[25];
rz(0) q[25];
rz(0) q[73];
cx q[73], q[26];
rz(0) q[26];
cx q[73], q[26];
rz(0) q[26];
rz(0) q[73];
cx q[73], q[27];
rz(0) q[27];
cx q[73], q[27];
rz(0) q[27];
rz(0) q[73];
cx q[73], q[28];
rz(0) q[28];
cx q[73], q[28];
rz(0) q[28];
rz(0) q[73];
cx q[73], q[29];
rz(0) q[29];
cx q[73], q[29];
rz(0) q[29];
rz(0) q[73];
cx q[73], q[30];
rz(0) q[30];
cx q[73], q[30];
rz(0) q[30];
rz(0) q[73];
cx q[73], q[31];
rz(0) q[31];
cx q[73], q[31];
rz(0) q[31];
rz(0) q[73];
cx q[73], q[32];
rz(0) q[32];
cx q[73], q[32];
rz(0) q[32];
rz(0) q[73];
cx q[73], q[33];
rz(0) q[33];
cx q[73], q[33];
rz(0) q[33];
rz(0) q[73];
cx q[73], q[34];
rz(0) q[34];
cx q[73], q[34];
rz(0) q[34];
rz(0) q[73];
cx q[73], q[35];
rz(0) q[35];
cx q[73], q[35];
rz(0) q[35];
rz(0) q[73];
cx q[73], q[36];
rz(0) q[36];
cx q[73], q[36];
rz(0) q[36];
rz(0) q[73];
cx q[73], q[37];
rz(0) q[37];
cx q[73], q[37];
rz(0) q[37];
rz(0) q[73];
cx q[73], q[38];
rz(0) q[38];
cx q[73], q[38];
rz(0) q[38];
rz(0) q[73];
cx q[73], q[39];
rz(0) q[39];
cx q[73], q[39];
rz(0) q[39];
rz(0) q[73];
cx q[73], q[40];
rz(0) q[40];
cx q[73], q[40];
rz(0) q[40];
rz(0) q[73];
cx q[73], q[41];
rz(0) q[41];
cx q[73], q[41];
rz(0) q[41];
rz(0) q[73];
cx q[73], q[42];
rz(0) q[42];
cx q[73], q[42];
rz(0) q[42];
rz(-pi/2147483648) q[73];
cx q[73], q[43];
rz(pi/2147483648) q[43];
cx q[73], q[43];
rz(-pi/2147483648) q[43];
rz(-pi/1073741824) q[73];
cx q[73], q[44];
rz(pi/1073741824) q[44];
cx q[73], q[44];
rz(-pi/1073741824) q[44];
rz(-pi/536870912) q[73];
cx q[73], q[45];
rz(pi/536870912) q[45];
cx q[73], q[45];
rz(-pi/536870912) q[45];
rz(-pi/268435456) q[73];
cx q[73], q[46];
rz(pi/268435456) q[46];
cx q[73], q[46];
rz(-pi/268435456) q[46];
rz(-pi/134217728) q[73];
cx q[73], q[47];
rz(pi/134217728) q[47];
cx q[73], q[47];
rz(-pi/134217728) q[47];
rz(-pi/67108864) q[73];
cx q[73], q[48];
rz(pi/67108864) q[48];
cx q[73], q[48];
rz(-pi/67108864) q[48];
rz(-pi/33554432) q[73];
cx q[73], q[49];
rz(pi/33554432) q[49];
cx q[73], q[49];
rz(-pi/33554432) q[49];
rz(-pi/16777216) q[73];
cx q[73], q[50];
rz(pi/16777216) q[50];
cx q[73], q[50];
rz(-pi/16777216) q[50];
rz(-pi/8388608) q[73];
cx q[73], q[51];
rz(pi/8388608) q[51];
cx q[73], q[51];
rz(-pi/8388608) q[51];
rz(-pi/4194304) q[73];
cx q[73], q[52];
rz(pi/4194304) q[52];
cx q[73], q[52];
rz(-pi/4194304) q[52];
rz(-pi/2097152) q[73];
cx q[73], q[53];
rz(pi/2097152) q[53];
cx q[73], q[53];
rz(-pi/2097152) q[53];
rz(-pi/1048576) q[73];
cx q[73], q[54];
rz(pi/1048576) q[54];
cx q[73], q[54];
rz(-pi/1048576) q[54];
rz(-pi/524288) q[73];
cx q[73], q[55];
rz(pi/524288) q[55];
cx q[73], q[55];
rz(-pi/524288) q[55];
rz(-pi/262144) q[73];
cx q[73], q[56];
rz(pi/262144) q[56];
cx q[73], q[56];
rz(-pi/262144) q[56];
rz(-pi/131072) q[73];
cx q[73], q[57];
rz(pi/131072) q[57];
cx q[73], q[57];
rz(-pi/131072) q[57];
rz(-pi/65536) q[73];
cx q[73], q[58];
rz(pi/65536) q[58];
cx q[73], q[58];
rz(-pi/65536) q[58];
rz(-pi/32768) q[73];
cx q[73], q[59];
rz(pi/32768) q[59];
cx q[73], q[59];
rz(-pi/32768) q[59];
rz(-pi/16384) q[73];
cx q[73], q[60];
rz(pi/16384) q[60];
cx q[73], q[60];
rz(-pi/16384) q[60];
rz(-pi/8192) q[73];
cx q[73], q[61];
rz(pi/8192) q[61];
cx q[73], q[61];
rz(-pi/8192) q[61];
rz(-pi/4096) q[73];
cx q[73], q[62];
rz(pi/4096) q[62];
cx q[73], q[62];
rz(-pi/4096) q[62];
rz(-pi/2048) q[73];
cx q[73], q[63];
rz(pi/2048) q[63];
cx q[73], q[63];
rz(-pi/2048) q[63];
rz(-pi/1024) q[73];
cx q[73], q[64];
rz(pi/1024) q[64];
cx q[73], q[64];
rz(-pi/1024) q[64];
rz(-pi/512) q[73];
cx q[73], q[65];
rz(pi/512) q[65];
cx q[73], q[65];
rz(-pi/512) q[65];
rz(-pi/256) q[73];
cx q[73], q[66];
rz(pi/256) q[66];
cx q[73], q[66];
rz(-pi/256) q[66];
rz(-pi/128) q[73];
cx q[73], q[67];
rz(pi/128) q[67];
cx q[73], q[67];
rz(-pi/128) q[67];
rz(-pi/64) q[73];
cx q[73], q[68];
rz(pi/64) q[68];
cx q[73], q[68];
rz(-pi/64) q[68];
rz(-pi/32) q[73];
cx q[73], q[69];
rz(pi/32) q[69];
cx q[73], q[69];
rz(-pi/32) q[69];
rz(-pi/16) q[73];
cx q[73], q[70];
rz(pi/16) q[70];
cx q[73], q[70];
rz(-pi/16) q[70];
rz(-pi/8) q[73];
cx q[73], q[71];
rz(pi/8) q[71];
cx q[73], q[71];
rz(-pi/8) q[71];
rz(-pi/4) q[73];
cx q[73], q[72];
rz(pi/4) q[72];
cx q[73], q[72];
rz(-pi/4) q[72];
h q[73];
cx q[74], q[0];
rz(0) q[0];
cx q[74], q[0];
rz(0) q[0];
rz(0) q[74];
cx q[74], q[1];
rz(0) q[1];
cx q[74], q[1];
rz(0) q[1];
rz(0) q[74];
cx q[74], q[2];
rz(0) q[2];
cx q[74], q[2];
rz(0) q[2];
rz(0) q[74];
cx q[74], q[3];
rz(0) q[3];
cx q[74], q[3];
rz(0) q[3];
rz(0) q[74];
cx q[74], q[4];
rz(0) q[4];
cx q[74], q[4];
rz(0) q[4];
rz(0) q[74];
cx q[74], q[5];
rz(0) q[5];
cx q[74], q[5];
rz(0) q[5];
rz(0) q[74];
cx q[74], q[6];
rz(0) q[6];
cx q[74], q[6];
rz(0) q[6];
rz(0) q[74];
cx q[74], q[7];
rz(0) q[7];
cx q[74], q[7];
rz(0) q[7];
rz(0) q[74];
cx q[74], q[8];
rz(0) q[8];
cx q[74], q[8];
rz(0) q[8];
rz(0) q[74];
cx q[74], q[9];
rz(0) q[9];
cx q[74], q[9];
rz(0) q[9];
rz(0) q[74];
cx q[74], q[10];
rz(0) q[10];
cx q[74], q[10];
rz(0) q[10];
rz(0) q[74];
cx q[74], q[11];
rz(0) q[11];
cx q[74], q[11];
rz(0) q[11];
rz(0) q[74];
cx q[74], q[12];
rz(0) q[12];
cx q[74], q[12];
rz(0) q[12];
rz(0) q[74];
cx q[74], q[13];
rz(0) q[13];
cx q[74], q[13];
rz(0) q[13];
rz(0) q[74];
cx q[74], q[14];
rz(0) q[14];
cx q[74], q[14];
rz(0) q[14];
rz(0) q[74];
cx q[74], q[15];
rz(0) q[15];
cx q[74], q[15];
rz(0) q[15];
rz(0) q[74];
cx q[74], q[16];
rz(0) q[16];
cx q[74], q[16];
rz(0) q[16];
rz(0) q[74];
cx q[74], q[17];
rz(0) q[17];
cx q[74], q[17];
rz(0) q[17];
rz(0) q[74];
cx q[74], q[18];
rz(0) q[18];
cx q[74], q[18];
rz(0) q[18];
rz(0) q[74];
cx q[74], q[19];
rz(0) q[19];
cx q[74], q[19];
rz(0) q[19];
rz(0) q[74];
cx q[74], q[20];
rz(0) q[20];
cx q[74], q[20];
rz(0) q[20];
rz(0) q[74];
cx q[74], q[21];
rz(0) q[21];
cx q[74], q[21];
rz(0) q[21];
rz(0) q[74];
cx q[74], q[22];
rz(0) q[22];
cx q[74], q[22];
rz(0) q[22];
rz(0) q[74];
cx q[74], q[23];
rz(0) q[23];
cx q[74], q[23];
rz(0) q[23];
rz(0) q[74];
cx q[74], q[24];
rz(0) q[24];
cx q[74], q[24];
rz(0) q[24];
rz(0) q[74];
cx q[74], q[25];
rz(0) q[25];
cx q[74], q[25];
rz(0) q[25];
rz(0) q[74];
cx q[74], q[26];
rz(0) q[26];
cx q[74], q[26];
rz(0) q[26];
rz(0) q[74];
cx q[74], q[27];
rz(0) q[27];
cx q[74], q[27];
rz(0) q[27];
rz(0) q[74];
cx q[74], q[28];
rz(0) q[28];
cx q[74], q[28];
rz(0) q[28];
rz(0) q[74];
cx q[74], q[29];
rz(0) q[29];
cx q[74], q[29];
rz(0) q[29];
rz(0) q[74];
cx q[74], q[30];
rz(0) q[30];
cx q[74], q[30];
rz(0) q[30];
rz(0) q[74];
cx q[74], q[31];
rz(0) q[31];
cx q[74], q[31];
rz(0) q[31];
rz(0) q[74];
cx q[74], q[32];
rz(0) q[32];
cx q[74], q[32];
rz(0) q[32];
rz(0) q[74];
cx q[74], q[33];
rz(0) q[33];
cx q[74], q[33];
rz(0) q[33];
rz(0) q[74];
cx q[74], q[34];
rz(0) q[34];
cx q[74], q[34];
rz(0) q[34];
rz(0) q[74];
cx q[74], q[35];
rz(0) q[35];
cx q[74], q[35];
rz(0) q[35];
rz(0) q[74];
cx q[74], q[36];
rz(0) q[36];
cx q[74], q[36];
rz(0) q[36];
rz(0) q[74];
cx q[74], q[37];
rz(0) q[37];
cx q[74], q[37];
rz(0) q[37];
rz(0) q[74];
cx q[74], q[38];
rz(0) q[38];
cx q[74], q[38];
rz(0) q[38];
rz(0) q[74];
cx q[74], q[39];
rz(0) q[39];
cx q[74], q[39];
rz(0) q[39];
rz(0) q[74];
cx q[74], q[40];
rz(0) q[40];
cx q[74], q[40];
rz(0) q[40];
rz(0) q[74];
cx q[74], q[41];
rz(0) q[41];
cx q[74], q[41];
rz(0) q[41];
rz(0) q[74];
cx q[74], q[42];
rz(0) q[42];
cx q[74], q[42];
rz(0) q[42];
rz(0) q[74];
cx q[74], q[43];
rz(0) q[43];
cx q[74], q[43];
rz(0) q[43];
rz(-pi/2147483648) q[74];
cx q[74], q[44];
rz(pi/2147483648) q[44];
cx q[74], q[44];
rz(-pi/2147483648) q[44];
rz(-pi/1073741824) q[74];
cx q[74], q[45];
rz(pi/1073741824) q[45];
cx q[74], q[45];
rz(-pi/1073741824) q[45];
rz(-pi/536870912) q[74];
cx q[74], q[46];
rz(pi/536870912) q[46];
cx q[74], q[46];
rz(-pi/536870912) q[46];
rz(-pi/268435456) q[74];
cx q[74], q[47];
rz(pi/268435456) q[47];
cx q[74], q[47];
rz(-pi/268435456) q[47];
rz(-pi/134217728) q[74];
cx q[74], q[48];
rz(pi/134217728) q[48];
cx q[74], q[48];
rz(-pi/134217728) q[48];
rz(-pi/67108864) q[74];
cx q[74], q[49];
rz(pi/67108864) q[49];
cx q[74], q[49];
rz(-pi/67108864) q[49];
rz(-pi/33554432) q[74];
cx q[74], q[50];
rz(pi/33554432) q[50];
cx q[74], q[50];
rz(-pi/33554432) q[50];
rz(-pi/16777216) q[74];
cx q[74], q[51];
rz(pi/16777216) q[51];
cx q[74], q[51];
rz(-pi/16777216) q[51];
rz(-pi/8388608) q[74];
cx q[74], q[52];
rz(pi/8388608) q[52];
cx q[74], q[52];
rz(-pi/8388608) q[52];
rz(-pi/4194304) q[74];
cx q[74], q[53];
rz(pi/4194304) q[53];
cx q[74], q[53];
rz(-pi/4194304) q[53];
rz(-pi/2097152) q[74];
cx q[74], q[54];
rz(pi/2097152) q[54];
cx q[74], q[54];
rz(-pi/2097152) q[54];
rz(-pi/1048576) q[74];
cx q[74], q[55];
rz(pi/1048576) q[55];
cx q[74], q[55];
rz(-pi/1048576) q[55];
rz(-pi/524288) q[74];
cx q[74], q[56];
rz(pi/524288) q[56];
cx q[74], q[56];
rz(-pi/524288) q[56];
rz(-pi/262144) q[74];
cx q[74], q[57];
rz(pi/262144) q[57];
cx q[74], q[57];
rz(-pi/262144) q[57];
rz(-pi/131072) q[74];
cx q[74], q[58];
rz(pi/131072) q[58];
cx q[74], q[58];
rz(-pi/131072) q[58];
rz(-pi/65536) q[74];
cx q[74], q[59];
rz(pi/65536) q[59];
cx q[74], q[59];
rz(-pi/65536) q[59];
rz(-pi/32768) q[74];
cx q[74], q[60];
rz(pi/32768) q[60];
cx q[74], q[60];
rz(-pi/32768) q[60];
rz(-pi/16384) q[74];
cx q[74], q[61];
rz(pi/16384) q[61];
cx q[74], q[61];
rz(-pi/16384) q[61];
rz(-pi/8192) q[74];
cx q[74], q[62];
rz(pi/8192) q[62];
cx q[74], q[62];
rz(-pi/8192) q[62];
rz(-pi/4096) q[74];
cx q[74], q[63];
rz(pi/4096) q[63];
cx q[74], q[63];
rz(-pi/4096) q[63];
rz(-pi/2048) q[74];
cx q[74], q[64];
rz(pi/2048) q[64];
cx q[74], q[64];
rz(-pi/2048) q[64];
rz(-pi/1024) q[74];
cx q[74], q[65];
rz(pi/1024) q[65];
cx q[74], q[65];
rz(-pi/1024) q[65];
rz(-pi/512) q[74];
cx q[74], q[66];
rz(pi/512) q[66];
cx q[74], q[66];
rz(-pi/512) q[66];
rz(-pi/256) q[74];
cx q[74], q[67];
rz(pi/256) q[67];
cx q[74], q[67];
rz(-pi/256) q[67];
rz(-pi/128) q[74];
cx q[74], q[68];
rz(pi/128) q[68];
cx q[74], q[68];
rz(-pi/128) q[68];
rz(-pi/64) q[74];
cx q[74], q[69];
rz(pi/64) q[69];
cx q[74], q[69];
rz(-pi/64) q[69];
rz(-pi/32) q[74];
cx q[74], q[70];
rz(pi/32) q[70];
cx q[74], q[70];
rz(-pi/32) q[70];
rz(-pi/16) q[74];
cx q[74], q[71];
rz(pi/16) q[71];
cx q[74], q[71];
rz(-pi/16) q[71];
rz(-pi/8) q[74];
cx q[74], q[72];
rz(pi/8) q[72];
cx q[74], q[72];
rz(-pi/8) q[72];
rz(-pi/4) q[74];
cx q[74], q[73];
rz(pi/4) q[73];
cx q[74], q[73];
rz(-pi/4) q[73];
h q[74];
cx q[75], q[0];
rz(0) q[0];
cx q[75], q[0];
rz(0) q[0];
rz(0) q[75];
cx q[75], q[1];
rz(0) q[1];
cx q[75], q[1];
rz(0) q[1];
rz(0) q[75];
cx q[75], q[2];
rz(0) q[2];
cx q[75], q[2];
rz(0) q[2];
rz(0) q[75];
cx q[75], q[3];
rz(0) q[3];
cx q[75], q[3];
rz(0) q[3];
rz(0) q[75];
cx q[75], q[4];
rz(0) q[4];
cx q[75], q[4];
rz(0) q[4];
rz(0) q[75];
cx q[75], q[5];
rz(0) q[5];
cx q[75], q[5];
rz(0) q[5];
rz(0) q[75];
cx q[75], q[6];
rz(0) q[6];
cx q[75], q[6];
rz(0) q[6];
rz(0) q[75];
cx q[75], q[7];
rz(0) q[7];
cx q[75], q[7];
rz(0) q[7];
rz(0) q[75];
cx q[75], q[8];
rz(0) q[8];
cx q[75], q[8];
rz(0) q[8];
rz(0) q[75];
cx q[75], q[9];
rz(0) q[9];
cx q[75], q[9];
rz(0) q[9];
rz(0) q[75];
cx q[75], q[10];
rz(0) q[10];
cx q[75], q[10];
rz(0) q[10];
rz(0) q[75];
cx q[75], q[11];
rz(0) q[11];
cx q[75], q[11];
rz(0) q[11];
rz(0) q[75];
cx q[75], q[12];
rz(0) q[12];
cx q[75], q[12];
rz(0) q[12];
rz(0) q[75];
cx q[75], q[13];
rz(0) q[13];
cx q[75], q[13];
rz(0) q[13];
rz(0) q[75];
cx q[75], q[14];
rz(0) q[14];
cx q[75], q[14];
rz(0) q[14];
rz(0) q[75];
cx q[75], q[15];
rz(0) q[15];
cx q[75], q[15];
rz(0) q[15];
rz(0) q[75];
cx q[75], q[16];
rz(0) q[16];
cx q[75], q[16];
rz(0) q[16];
rz(0) q[75];
cx q[75], q[17];
rz(0) q[17];
cx q[75], q[17];
rz(0) q[17];
rz(0) q[75];
cx q[75], q[18];
rz(0) q[18];
cx q[75], q[18];
rz(0) q[18];
rz(0) q[75];
cx q[75], q[19];
rz(0) q[19];
cx q[75], q[19];
rz(0) q[19];
rz(0) q[75];
cx q[75], q[20];
rz(0) q[20];
cx q[75], q[20];
rz(0) q[20];
rz(0) q[75];
cx q[75], q[21];
rz(0) q[21];
cx q[75], q[21];
rz(0) q[21];
rz(0) q[75];
cx q[75], q[22];
rz(0) q[22];
cx q[75], q[22];
rz(0) q[22];
rz(0) q[75];
cx q[75], q[23];
rz(0) q[23];
cx q[75], q[23];
rz(0) q[23];
rz(0) q[75];
cx q[75], q[24];
rz(0) q[24];
cx q[75], q[24];
rz(0) q[24];
rz(0) q[75];
cx q[75], q[25];
rz(0) q[25];
cx q[75], q[25];
rz(0) q[25];
rz(0) q[75];
cx q[75], q[26];
rz(0) q[26];
cx q[75], q[26];
rz(0) q[26];
rz(0) q[75];
cx q[75], q[27];
rz(0) q[27];
cx q[75], q[27];
rz(0) q[27];
rz(0) q[75];
cx q[75], q[28];
rz(0) q[28];
cx q[75], q[28];
rz(0) q[28];
rz(0) q[75];
cx q[75], q[29];
rz(0) q[29];
cx q[75], q[29];
rz(0) q[29];
rz(0) q[75];
cx q[75], q[30];
rz(0) q[30];
cx q[75], q[30];
rz(0) q[30];
rz(0) q[75];
cx q[75], q[31];
rz(0) q[31];
cx q[75], q[31];
rz(0) q[31];
rz(0) q[75];
cx q[75], q[32];
rz(0) q[32];
cx q[75], q[32];
rz(0) q[32];
rz(0) q[75];
cx q[75], q[33];
rz(0) q[33];
cx q[75], q[33];
rz(0) q[33];
rz(0) q[75];
cx q[75], q[34];
rz(0) q[34];
cx q[75], q[34];
rz(0) q[34];
rz(0) q[75];
cx q[75], q[35];
rz(0) q[35];
cx q[75], q[35];
rz(0) q[35];
rz(0) q[75];
cx q[75], q[36];
rz(0) q[36];
cx q[75], q[36];
rz(0) q[36];
rz(0) q[75];
cx q[75], q[37];
rz(0) q[37];
cx q[75], q[37];
rz(0) q[37];
rz(0) q[75];
cx q[75], q[38];
rz(0) q[38];
cx q[75], q[38];
rz(0) q[38];
rz(0) q[75];
cx q[75], q[39];
rz(0) q[39];
cx q[75], q[39];
rz(0) q[39];
rz(0) q[75];
cx q[75], q[40];
rz(0) q[40];
cx q[75], q[40];
rz(0) q[40];
rz(0) q[75];
cx q[75], q[41];
rz(0) q[41];
cx q[75], q[41];
rz(0) q[41];
rz(0) q[75];
cx q[75], q[42];
rz(0) q[42];
cx q[75], q[42];
rz(0) q[42];
rz(0) q[75];
cx q[75], q[43];
rz(0) q[43];
cx q[75], q[43];
rz(0) q[43];
rz(0) q[75];
cx q[75], q[44];
rz(0) q[44];
cx q[75], q[44];
rz(0) q[44];
rz(-pi/2147483648) q[75];
cx q[75], q[45];
rz(pi/2147483648) q[45];
cx q[75], q[45];
rz(-pi/2147483648) q[45];
rz(-pi/1073741824) q[75];
cx q[75], q[46];
rz(pi/1073741824) q[46];
cx q[75], q[46];
rz(-pi/1073741824) q[46];
rz(-pi/536870912) q[75];
cx q[75], q[47];
rz(pi/536870912) q[47];
cx q[75], q[47];
rz(-pi/536870912) q[47];
rz(-pi/268435456) q[75];
cx q[75], q[48];
rz(pi/268435456) q[48];
cx q[75], q[48];
rz(-pi/268435456) q[48];
rz(-pi/134217728) q[75];
cx q[75], q[49];
rz(pi/134217728) q[49];
cx q[75], q[49];
rz(-pi/134217728) q[49];
rz(-pi/67108864) q[75];
cx q[75], q[50];
rz(pi/67108864) q[50];
cx q[75], q[50];
rz(-pi/67108864) q[50];
rz(-pi/33554432) q[75];
cx q[75], q[51];
rz(pi/33554432) q[51];
cx q[75], q[51];
rz(-pi/33554432) q[51];
rz(-pi/16777216) q[75];
cx q[75], q[52];
rz(pi/16777216) q[52];
cx q[75], q[52];
rz(-pi/16777216) q[52];
rz(-pi/8388608) q[75];
cx q[75], q[53];
rz(pi/8388608) q[53];
cx q[75], q[53];
rz(-pi/8388608) q[53];
rz(-pi/4194304) q[75];
cx q[75], q[54];
rz(pi/4194304) q[54];
cx q[75], q[54];
rz(-pi/4194304) q[54];
rz(-pi/2097152) q[75];
cx q[75], q[55];
rz(pi/2097152) q[55];
cx q[75], q[55];
rz(-pi/2097152) q[55];
rz(-pi/1048576) q[75];
cx q[75], q[56];
rz(pi/1048576) q[56];
cx q[75], q[56];
rz(-pi/1048576) q[56];
rz(-pi/524288) q[75];
cx q[75], q[57];
rz(pi/524288) q[57];
cx q[75], q[57];
rz(-pi/524288) q[57];
rz(-pi/262144) q[75];
cx q[75], q[58];
rz(pi/262144) q[58];
cx q[75], q[58];
rz(-pi/262144) q[58];
rz(-pi/131072) q[75];
cx q[75], q[59];
rz(pi/131072) q[59];
cx q[75], q[59];
rz(-pi/131072) q[59];
rz(-pi/65536) q[75];
cx q[75], q[60];
rz(pi/65536) q[60];
cx q[75], q[60];
rz(-pi/65536) q[60];
rz(-pi/32768) q[75];
cx q[75], q[61];
rz(pi/32768) q[61];
cx q[75], q[61];
rz(-pi/32768) q[61];
rz(-pi/16384) q[75];
cx q[75], q[62];
rz(pi/16384) q[62];
cx q[75], q[62];
rz(-pi/16384) q[62];
rz(-pi/8192) q[75];
cx q[75], q[63];
rz(pi/8192) q[63];
cx q[75], q[63];
rz(-pi/8192) q[63];
rz(-pi/4096) q[75];
cx q[75], q[64];
rz(pi/4096) q[64];
cx q[75], q[64];
rz(-pi/4096) q[64];
rz(-pi/2048) q[75];
cx q[75], q[65];
rz(pi/2048) q[65];
cx q[75], q[65];
rz(-pi/2048) q[65];
rz(-pi/1024) q[75];
cx q[75], q[66];
rz(pi/1024) q[66];
cx q[75], q[66];
rz(-pi/1024) q[66];
rz(-pi/512) q[75];
cx q[75], q[67];
rz(pi/512) q[67];
cx q[75], q[67];
rz(-pi/512) q[67];
rz(-pi/256) q[75];
cx q[75], q[68];
rz(pi/256) q[68];
cx q[75], q[68];
rz(-pi/256) q[68];
rz(-pi/128) q[75];
cx q[75], q[69];
rz(pi/128) q[69];
cx q[75], q[69];
rz(-pi/128) q[69];
rz(-pi/64) q[75];
cx q[75], q[70];
rz(pi/64) q[70];
cx q[75], q[70];
rz(-pi/64) q[70];
rz(-pi/32) q[75];
cx q[75], q[71];
rz(pi/32) q[71];
cx q[75], q[71];
rz(-pi/32) q[71];
rz(-pi/16) q[75];
cx q[75], q[72];
rz(pi/16) q[72];
cx q[75], q[72];
rz(-pi/16) q[72];
rz(-pi/8) q[75];
cx q[75], q[73];
rz(pi/8) q[73];
cx q[75], q[73];
rz(-pi/8) q[73];
rz(-pi/4) q[75];
cx q[75], q[74];
rz(pi/4) q[74];
cx q[75], q[74];
rz(-pi/4) q[74];
h q[75];
cx q[76], q[0];
rz(0) q[0];
cx q[76], q[0];
rz(0) q[0];
rz(0) q[76];
cx q[76], q[1];
rz(0) q[1];
cx q[76], q[1];
rz(0) q[1];
rz(0) q[76];
cx q[76], q[2];
rz(0) q[2];
cx q[76], q[2];
rz(0) q[2];
rz(0) q[76];
cx q[76], q[3];
rz(0) q[3];
cx q[76], q[3];
rz(0) q[3];
rz(0) q[76];
cx q[76], q[4];
rz(0) q[4];
cx q[76], q[4];
rz(0) q[4];
rz(0) q[76];
cx q[76], q[5];
rz(0) q[5];
cx q[76], q[5];
rz(0) q[5];
rz(0) q[76];
cx q[76], q[6];
rz(0) q[6];
cx q[76], q[6];
rz(0) q[6];
rz(0) q[76];
cx q[76], q[7];
rz(0) q[7];
cx q[76], q[7];
rz(0) q[7];
rz(0) q[76];
cx q[76], q[8];
rz(0) q[8];
cx q[76], q[8];
rz(0) q[8];
rz(0) q[76];
cx q[76], q[9];
rz(0) q[9];
cx q[76], q[9];
rz(0) q[9];
rz(0) q[76];
cx q[76], q[10];
rz(0) q[10];
cx q[76], q[10];
rz(0) q[10];
rz(0) q[76];
cx q[76], q[11];
rz(0) q[11];
cx q[76], q[11];
rz(0) q[11];
rz(0) q[76];
cx q[76], q[12];
rz(0) q[12];
cx q[76], q[12];
rz(0) q[12];
rz(0) q[76];
cx q[76], q[13];
rz(0) q[13];
cx q[76], q[13];
rz(0) q[13];
rz(0) q[76];
cx q[76], q[14];
rz(0) q[14];
cx q[76], q[14];
rz(0) q[14];
rz(0) q[76];
cx q[76], q[15];
rz(0) q[15];
cx q[76], q[15];
rz(0) q[15];
rz(0) q[76];
cx q[76], q[16];
rz(0) q[16];
cx q[76], q[16];
rz(0) q[16];
rz(0) q[76];
cx q[76], q[17];
rz(0) q[17];
cx q[76], q[17];
rz(0) q[17];
rz(0) q[76];
cx q[76], q[18];
rz(0) q[18];
cx q[76], q[18];
rz(0) q[18];
rz(0) q[76];
cx q[76], q[19];
rz(0) q[19];
cx q[76], q[19];
rz(0) q[19];
rz(0) q[76];
cx q[76], q[20];
rz(0) q[20];
cx q[76], q[20];
rz(0) q[20];
rz(0) q[76];
cx q[76], q[21];
rz(0) q[21];
cx q[76], q[21];
rz(0) q[21];
rz(0) q[76];
cx q[76], q[22];
rz(0) q[22];
cx q[76], q[22];
rz(0) q[22];
rz(0) q[76];
cx q[76], q[23];
rz(0) q[23];
cx q[76], q[23];
rz(0) q[23];
rz(0) q[76];
cx q[76], q[24];
rz(0) q[24];
cx q[76], q[24];
rz(0) q[24];
rz(0) q[76];
cx q[76], q[25];
rz(0) q[25];
cx q[76], q[25];
rz(0) q[25];
rz(0) q[76];
cx q[76], q[26];
rz(0) q[26];
cx q[76], q[26];
rz(0) q[26];
rz(0) q[76];
cx q[76], q[27];
rz(0) q[27];
cx q[76], q[27];
rz(0) q[27];
rz(0) q[76];
cx q[76], q[28];
rz(0) q[28];
cx q[76], q[28];
rz(0) q[28];
rz(0) q[76];
cx q[76], q[29];
rz(0) q[29];
cx q[76], q[29];
rz(0) q[29];
rz(0) q[76];
cx q[76], q[30];
rz(0) q[30];
cx q[76], q[30];
rz(0) q[30];
rz(0) q[76];
cx q[76], q[31];
rz(0) q[31];
cx q[76], q[31];
rz(0) q[31];
rz(0) q[76];
cx q[76], q[32];
rz(0) q[32];
cx q[76], q[32];
rz(0) q[32];
rz(0) q[76];
cx q[76], q[33];
rz(0) q[33];
cx q[76], q[33];
rz(0) q[33];
rz(0) q[76];
cx q[76], q[34];
rz(0) q[34];
cx q[76], q[34];
rz(0) q[34];
rz(0) q[76];
cx q[76], q[35];
rz(0) q[35];
cx q[76], q[35];
rz(0) q[35];
rz(0) q[76];
cx q[76], q[36];
rz(0) q[36];
cx q[76], q[36];
rz(0) q[36];
rz(0) q[76];
cx q[76], q[37];
rz(0) q[37];
cx q[76], q[37];
rz(0) q[37];
rz(0) q[76];
cx q[76], q[38];
rz(0) q[38];
cx q[76], q[38];
rz(0) q[38];
rz(0) q[76];
cx q[76], q[39];
rz(0) q[39];
cx q[76], q[39];
rz(0) q[39];
rz(0) q[76];
cx q[76], q[40];
rz(0) q[40];
cx q[76], q[40];
rz(0) q[40];
rz(0) q[76];
cx q[76], q[41];
rz(0) q[41];
cx q[76], q[41];
rz(0) q[41];
rz(0) q[76];
cx q[76], q[42];
rz(0) q[42];
cx q[76], q[42];
rz(0) q[42];
rz(0) q[76];
cx q[76], q[43];
rz(0) q[43];
cx q[76], q[43];
rz(0) q[43];
rz(0) q[76];
cx q[76], q[44];
rz(0) q[44];
cx q[76], q[44];
rz(0) q[44];
rz(0) q[76];
cx q[76], q[45];
rz(0) q[45];
cx q[76], q[45];
rz(0) q[45];
rz(-pi/2147483648) q[76];
cx q[76], q[46];
rz(pi/2147483648) q[46];
cx q[76], q[46];
rz(-pi/2147483648) q[46];
rz(-pi/1073741824) q[76];
cx q[76], q[47];
rz(pi/1073741824) q[47];
cx q[76], q[47];
rz(-pi/1073741824) q[47];
rz(-pi/536870912) q[76];
cx q[76], q[48];
rz(pi/536870912) q[48];
cx q[76], q[48];
rz(-pi/536870912) q[48];
rz(-pi/268435456) q[76];
cx q[76], q[49];
rz(pi/268435456) q[49];
cx q[76], q[49];
rz(-pi/268435456) q[49];
rz(-pi/134217728) q[76];
cx q[76], q[50];
rz(pi/134217728) q[50];
cx q[76], q[50];
rz(-pi/134217728) q[50];
rz(-pi/67108864) q[76];
cx q[76], q[51];
rz(pi/67108864) q[51];
cx q[76], q[51];
rz(-pi/67108864) q[51];
rz(-pi/33554432) q[76];
cx q[76], q[52];
rz(pi/33554432) q[52];
cx q[76], q[52];
rz(-pi/33554432) q[52];
rz(-pi/16777216) q[76];
cx q[76], q[53];
rz(pi/16777216) q[53];
cx q[76], q[53];
rz(-pi/16777216) q[53];
rz(-pi/8388608) q[76];
cx q[76], q[54];
rz(pi/8388608) q[54];
cx q[76], q[54];
rz(-pi/8388608) q[54];
rz(-pi/4194304) q[76];
cx q[76], q[55];
rz(pi/4194304) q[55];
cx q[76], q[55];
rz(-pi/4194304) q[55];
rz(-pi/2097152) q[76];
cx q[76], q[56];
rz(pi/2097152) q[56];
cx q[76], q[56];
rz(-pi/2097152) q[56];
rz(-pi/1048576) q[76];
cx q[76], q[57];
rz(pi/1048576) q[57];
cx q[76], q[57];
rz(-pi/1048576) q[57];
rz(-pi/524288) q[76];
cx q[76], q[58];
rz(pi/524288) q[58];
cx q[76], q[58];
rz(-pi/524288) q[58];
rz(-pi/262144) q[76];
cx q[76], q[59];
rz(pi/262144) q[59];
cx q[76], q[59];
rz(-pi/262144) q[59];
rz(-pi/131072) q[76];
cx q[76], q[60];
rz(pi/131072) q[60];
cx q[76], q[60];
rz(-pi/131072) q[60];
rz(-pi/65536) q[76];
cx q[76], q[61];
rz(pi/65536) q[61];
cx q[76], q[61];
rz(-pi/65536) q[61];
rz(-pi/32768) q[76];
cx q[76], q[62];
rz(pi/32768) q[62];
cx q[76], q[62];
rz(-pi/32768) q[62];
rz(-pi/16384) q[76];
cx q[76], q[63];
rz(pi/16384) q[63];
cx q[76], q[63];
rz(-pi/16384) q[63];
rz(-pi/8192) q[76];
cx q[76], q[64];
rz(pi/8192) q[64];
cx q[76], q[64];
rz(-pi/8192) q[64];
rz(-pi/4096) q[76];
cx q[76], q[65];
rz(pi/4096) q[65];
cx q[76], q[65];
rz(-pi/4096) q[65];
rz(-pi/2048) q[76];
cx q[76], q[66];
rz(pi/2048) q[66];
cx q[76], q[66];
rz(-pi/2048) q[66];
rz(-pi/1024) q[76];
cx q[76], q[67];
rz(pi/1024) q[67];
cx q[76], q[67];
rz(-pi/1024) q[67];
rz(-pi/512) q[76];
cx q[76], q[68];
rz(pi/512) q[68];
cx q[76], q[68];
rz(-pi/512) q[68];
rz(-pi/256) q[76];
cx q[76], q[69];
rz(pi/256) q[69];
cx q[76], q[69];
rz(-pi/256) q[69];
rz(-pi/128) q[76];
cx q[76], q[70];
rz(pi/128) q[70];
cx q[76], q[70];
rz(-pi/128) q[70];
rz(-pi/64) q[76];
cx q[76], q[71];
rz(pi/64) q[71];
cx q[76], q[71];
rz(-pi/64) q[71];
rz(-pi/32) q[76];
cx q[76], q[72];
rz(pi/32) q[72];
cx q[76], q[72];
rz(-pi/32) q[72];
rz(-pi/16) q[76];
cx q[76], q[73];
rz(pi/16) q[73];
cx q[76], q[73];
rz(-pi/16) q[73];
rz(-pi/8) q[76];
cx q[76], q[74];
rz(pi/8) q[74];
cx q[76], q[74];
rz(-pi/8) q[74];
rz(-pi/4) q[76];
cx q[76], q[75];
rz(pi/4) q[75];
cx q[76], q[75];
rz(-pi/4) q[75];
h q[76];
cx q[77], q[0];
rz(0) q[0];
cx q[77], q[0];
rz(0) q[0];
rz(0) q[77];
cx q[77], q[1];
rz(0) q[1];
cx q[77], q[1];
rz(0) q[1];
rz(0) q[77];
cx q[77], q[2];
rz(0) q[2];
cx q[77], q[2];
rz(0) q[2];
rz(0) q[77];
cx q[77], q[3];
rz(0) q[3];
cx q[77], q[3];
rz(0) q[3];
rz(0) q[77];
cx q[77], q[4];
rz(0) q[4];
cx q[77], q[4];
rz(0) q[4];
rz(0) q[77];
cx q[77], q[5];
rz(0) q[5];
cx q[77], q[5];
rz(0) q[5];
rz(0) q[77];
cx q[77], q[6];
rz(0) q[6];
cx q[77], q[6];
rz(0) q[6];
rz(0) q[77];
cx q[77], q[7];
rz(0) q[7];
cx q[77], q[7];
rz(0) q[7];
rz(0) q[77];
cx q[77], q[8];
rz(0) q[8];
cx q[77], q[8];
rz(0) q[8];
rz(0) q[77];
cx q[77], q[9];
rz(0) q[9];
cx q[77], q[9];
rz(0) q[9];
rz(0) q[77];
cx q[77], q[10];
rz(0) q[10];
cx q[77], q[10];
rz(0) q[10];
rz(0) q[77];
cx q[77], q[11];
rz(0) q[11];
cx q[77], q[11];
rz(0) q[11];
rz(0) q[77];
cx q[77], q[12];
rz(0) q[12];
cx q[77], q[12];
rz(0) q[12];
rz(0) q[77];
cx q[77], q[13];
rz(0) q[13];
cx q[77], q[13];
rz(0) q[13];
rz(0) q[77];
cx q[77], q[14];
rz(0) q[14];
cx q[77], q[14];
rz(0) q[14];
rz(0) q[77];
cx q[77], q[15];
rz(0) q[15];
cx q[77], q[15];
rz(0) q[15];
rz(0) q[77];
cx q[77], q[16];
rz(0) q[16];
cx q[77], q[16];
rz(0) q[16];
rz(0) q[77];
cx q[77], q[17];
rz(0) q[17];
cx q[77], q[17];
rz(0) q[17];
rz(0) q[77];
cx q[77], q[18];
rz(0) q[18];
cx q[77], q[18];
rz(0) q[18];
rz(0) q[77];
cx q[77], q[19];
rz(0) q[19];
cx q[77], q[19];
rz(0) q[19];
rz(0) q[77];
cx q[77], q[20];
rz(0) q[20];
cx q[77], q[20];
rz(0) q[20];
rz(0) q[77];
cx q[77], q[21];
rz(0) q[21];
cx q[77], q[21];
rz(0) q[21];
rz(0) q[77];
cx q[77], q[22];
rz(0) q[22];
cx q[77], q[22];
rz(0) q[22];
rz(0) q[77];
cx q[77], q[23];
rz(0) q[23];
cx q[77], q[23];
rz(0) q[23];
rz(0) q[77];
cx q[77], q[24];
rz(0) q[24];
cx q[77], q[24];
rz(0) q[24];
rz(0) q[77];
cx q[77], q[25];
rz(0) q[25];
cx q[77], q[25];
rz(0) q[25];
rz(0) q[77];
cx q[77], q[26];
rz(0) q[26];
cx q[77], q[26];
rz(0) q[26];
rz(0) q[77];
cx q[77], q[27];
rz(0) q[27];
cx q[77], q[27];
rz(0) q[27];
rz(0) q[77];
cx q[77], q[28];
rz(0) q[28];
cx q[77], q[28];
rz(0) q[28];
rz(0) q[77];
cx q[77], q[29];
rz(0) q[29];
cx q[77], q[29];
rz(0) q[29];
rz(0) q[77];
cx q[77], q[30];
rz(0) q[30];
cx q[77], q[30];
rz(0) q[30];
rz(0) q[77];
cx q[77], q[31];
rz(0) q[31];
cx q[77], q[31];
rz(0) q[31];
rz(0) q[77];
cx q[77], q[32];
rz(0) q[32];
cx q[77], q[32];
rz(0) q[32];
rz(0) q[77];
cx q[77], q[33];
rz(0) q[33];
cx q[77], q[33];
rz(0) q[33];
rz(0) q[77];
cx q[77], q[34];
rz(0) q[34];
cx q[77], q[34];
rz(0) q[34];
rz(0) q[77];
cx q[77], q[35];
rz(0) q[35];
cx q[77], q[35];
rz(0) q[35];
rz(0) q[77];
cx q[77], q[36];
rz(0) q[36];
cx q[77], q[36];
rz(0) q[36];
rz(0) q[77];
cx q[77], q[37];
rz(0) q[37];
cx q[77], q[37];
rz(0) q[37];
rz(0) q[77];
cx q[77], q[38];
rz(0) q[38];
cx q[77], q[38];
rz(0) q[38];
rz(0) q[77];
cx q[77], q[39];
rz(0) q[39];
cx q[77], q[39];
rz(0) q[39];
rz(0) q[77];
cx q[77], q[40];
rz(0) q[40];
cx q[77], q[40];
rz(0) q[40];
rz(0) q[77];
cx q[77], q[41];
rz(0) q[41];
cx q[77], q[41];
rz(0) q[41];
rz(0) q[77];
cx q[77], q[42];
rz(0) q[42];
cx q[77], q[42];
rz(0) q[42];
rz(0) q[77];
cx q[77], q[43];
rz(0) q[43];
cx q[77], q[43];
rz(0) q[43];
rz(0) q[77];
cx q[77], q[44];
rz(0) q[44];
cx q[77], q[44];
rz(0) q[44];
rz(0) q[77];
cx q[77], q[45];
rz(0) q[45];
cx q[77], q[45];
rz(0) q[45];
rz(0) q[77];
cx q[77], q[46];
rz(0) q[46];
cx q[77], q[46];
rz(0) q[46];
rz(-pi/2147483648) q[77];
cx q[77], q[47];
rz(pi/2147483648) q[47];
cx q[77], q[47];
rz(-pi/2147483648) q[47];
rz(-pi/1073741824) q[77];
cx q[77], q[48];
rz(pi/1073741824) q[48];
cx q[77], q[48];
rz(-pi/1073741824) q[48];
rz(-pi/536870912) q[77];
cx q[77], q[49];
rz(pi/536870912) q[49];
cx q[77], q[49];
rz(-pi/536870912) q[49];
rz(-pi/268435456) q[77];
cx q[77], q[50];
rz(pi/268435456) q[50];
cx q[77], q[50];
rz(-pi/268435456) q[50];
rz(-pi/134217728) q[77];
cx q[77], q[51];
rz(pi/134217728) q[51];
cx q[77], q[51];
rz(-pi/134217728) q[51];
rz(-pi/67108864) q[77];
cx q[77], q[52];
rz(pi/67108864) q[52];
cx q[77], q[52];
rz(-pi/67108864) q[52];
rz(-pi/33554432) q[77];
cx q[77], q[53];
rz(pi/33554432) q[53];
cx q[77], q[53];
rz(-pi/33554432) q[53];
rz(-pi/16777216) q[77];
cx q[77], q[54];
rz(pi/16777216) q[54];
cx q[77], q[54];
rz(-pi/16777216) q[54];
rz(-pi/8388608) q[77];
cx q[77], q[55];
rz(pi/8388608) q[55];
cx q[77], q[55];
rz(-pi/8388608) q[55];
rz(-pi/4194304) q[77];
cx q[77], q[56];
rz(pi/4194304) q[56];
cx q[77], q[56];
rz(-pi/4194304) q[56];
rz(-pi/2097152) q[77];
cx q[77], q[57];
rz(pi/2097152) q[57];
cx q[77], q[57];
rz(-pi/2097152) q[57];
rz(-pi/1048576) q[77];
cx q[77], q[58];
rz(pi/1048576) q[58];
cx q[77], q[58];
rz(-pi/1048576) q[58];
rz(-pi/524288) q[77];
cx q[77], q[59];
rz(pi/524288) q[59];
cx q[77], q[59];
rz(-pi/524288) q[59];
rz(-pi/262144) q[77];
cx q[77], q[60];
rz(pi/262144) q[60];
cx q[77], q[60];
rz(-pi/262144) q[60];
rz(-pi/131072) q[77];
cx q[77], q[61];
rz(pi/131072) q[61];
cx q[77], q[61];
rz(-pi/131072) q[61];
rz(-pi/65536) q[77];
cx q[77], q[62];
rz(pi/65536) q[62];
cx q[77], q[62];
rz(-pi/65536) q[62];
rz(-pi/32768) q[77];
cx q[77], q[63];
rz(pi/32768) q[63];
cx q[77], q[63];
rz(-pi/32768) q[63];
rz(-pi/16384) q[77];
cx q[77], q[64];
rz(pi/16384) q[64];
cx q[77], q[64];
rz(-pi/16384) q[64];
rz(-pi/8192) q[77];
cx q[77], q[65];
rz(pi/8192) q[65];
cx q[77], q[65];
rz(-pi/8192) q[65];
rz(-pi/4096) q[77];
cx q[77], q[66];
rz(pi/4096) q[66];
cx q[77], q[66];
rz(-pi/4096) q[66];
rz(-pi/2048) q[77];
cx q[77], q[67];
rz(pi/2048) q[67];
cx q[77], q[67];
rz(-pi/2048) q[67];
rz(-pi/1024) q[77];
cx q[77], q[68];
rz(pi/1024) q[68];
cx q[77], q[68];
rz(-pi/1024) q[68];
rz(-pi/512) q[77];
cx q[77], q[69];
rz(pi/512) q[69];
cx q[77], q[69];
rz(-pi/512) q[69];
rz(-pi/256) q[77];
cx q[77], q[70];
rz(pi/256) q[70];
cx q[77], q[70];
rz(-pi/256) q[70];
rz(-pi/128) q[77];
cx q[77], q[71];
rz(pi/128) q[71];
cx q[77], q[71];
rz(-pi/128) q[71];
rz(-pi/64) q[77];
cx q[77], q[72];
rz(pi/64) q[72];
cx q[77], q[72];
rz(-pi/64) q[72];
rz(-pi/32) q[77];
cx q[77], q[73];
rz(pi/32) q[73];
cx q[77], q[73];
rz(-pi/32) q[73];
rz(-pi/16) q[77];
cx q[77], q[74];
rz(pi/16) q[74];
cx q[77], q[74];
rz(-pi/16) q[74];
rz(-pi/8) q[77];
cx q[77], q[75];
rz(pi/8) q[75];
cx q[77], q[75];
rz(-pi/8) q[75];
rz(-pi/4) q[77];
cx q[77], q[76];
rz(pi/4) q[76];
cx q[77], q[76];
rz(-pi/4) q[76];
h q[77];
cx q[78], q[0];
rz(0) q[0];
cx q[78], q[0];
rz(0) q[0];
rz(0) q[78];
cx q[78], q[1];
rz(0) q[1];
cx q[78], q[1];
rz(0) q[1];
rz(0) q[78];
cx q[78], q[2];
rz(0) q[2];
cx q[78], q[2];
rz(0) q[2];
rz(0) q[78];
cx q[78], q[3];
rz(0) q[3];
cx q[78], q[3];
rz(0) q[3];
rz(0) q[78];
cx q[78], q[4];
rz(0) q[4];
cx q[78], q[4];
rz(0) q[4];
rz(0) q[78];
cx q[78], q[5];
rz(0) q[5];
cx q[78], q[5];
rz(0) q[5];
rz(0) q[78];
cx q[78], q[6];
rz(0) q[6];
cx q[78], q[6];
rz(0) q[6];
rz(0) q[78];
cx q[78], q[7];
rz(0) q[7];
cx q[78], q[7];
rz(0) q[7];
rz(0) q[78];
cx q[78], q[8];
rz(0) q[8];
cx q[78], q[8];
rz(0) q[8];
rz(0) q[78];
cx q[78], q[9];
rz(0) q[9];
cx q[78], q[9];
rz(0) q[9];
rz(0) q[78];
cx q[78], q[10];
rz(0) q[10];
cx q[78], q[10];
rz(0) q[10];
rz(0) q[78];
cx q[78], q[11];
rz(0) q[11];
cx q[78], q[11];
rz(0) q[11];
rz(0) q[78];
cx q[78], q[12];
rz(0) q[12];
cx q[78], q[12];
rz(0) q[12];
rz(0) q[78];
cx q[78], q[13];
rz(0) q[13];
cx q[78], q[13];
rz(0) q[13];
rz(0) q[78];
cx q[78], q[14];
rz(0) q[14];
cx q[78], q[14];
rz(0) q[14];
rz(0) q[78];
cx q[78], q[15];
rz(0) q[15];
cx q[78], q[15];
rz(0) q[15];
rz(0) q[78];
cx q[78], q[16];
rz(0) q[16];
cx q[78], q[16];
rz(0) q[16];
rz(0) q[78];
cx q[78], q[17];
rz(0) q[17];
cx q[78], q[17];
rz(0) q[17];
rz(0) q[78];
cx q[78], q[18];
rz(0) q[18];
cx q[78], q[18];
rz(0) q[18];
rz(0) q[78];
cx q[78], q[19];
rz(0) q[19];
cx q[78], q[19];
rz(0) q[19];
rz(0) q[78];
cx q[78], q[20];
rz(0) q[20];
cx q[78], q[20];
rz(0) q[20];
rz(0) q[78];
cx q[78], q[21];
rz(0) q[21];
cx q[78], q[21];
rz(0) q[21];
rz(0) q[78];
cx q[78], q[22];
rz(0) q[22];
cx q[78], q[22];
rz(0) q[22];
rz(0) q[78];
cx q[78], q[23];
rz(0) q[23];
cx q[78], q[23];
rz(0) q[23];
rz(0) q[78];
cx q[78], q[24];
rz(0) q[24];
cx q[78], q[24];
rz(0) q[24];
rz(0) q[78];
cx q[78], q[25];
rz(0) q[25];
cx q[78], q[25];
rz(0) q[25];
rz(0) q[78];
cx q[78], q[26];
rz(0) q[26];
cx q[78], q[26];
rz(0) q[26];
rz(0) q[78];
cx q[78], q[27];
rz(0) q[27];
cx q[78], q[27];
rz(0) q[27];
rz(0) q[78];
cx q[78], q[28];
rz(0) q[28];
cx q[78], q[28];
rz(0) q[28];
rz(0) q[78];
cx q[78], q[29];
rz(0) q[29];
cx q[78], q[29];
rz(0) q[29];
rz(0) q[78];
cx q[78], q[30];
rz(0) q[30];
cx q[78], q[30];
rz(0) q[30];
rz(0) q[78];
cx q[78], q[31];
rz(0) q[31];
cx q[78], q[31];
rz(0) q[31];
rz(0) q[78];
cx q[78], q[32];
rz(0) q[32];
cx q[78], q[32];
rz(0) q[32];
rz(0) q[78];
cx q[78], q[33];
rz(0) q[33];
cx q[78], q[33];
rz(0) q[33];
rz(0) q[78];
cx q[78], q[34];
rz(0) q[34];
cx q[78], q[34];
rz(0) q[34];
rz(0) q[78];
cx q[78], q[35];
rz(0) q[35];
cx q[78], q[35];
rz(0) q[35];
rz(0) q[78];
cx q[78], q[36];
rz(0) q[36];
cx q[78], q[36];
rz(0) q[36];
rz(0) q[78];
cx q[78], q[37];
rz(0) q[37];
cx q[78], q[37];
rz(0) q[37];
rz(0) q[78];
cx q[78], q[38];
rz(0) q[38];
cx q[78], q[38];
rz(0) q[38];
rz(0) q[78];
cx q[78], q[39];
rz(0) q[39];
cx q[78], q[39];
rz(0) q[39];
rz(0) q[78];
cx q[78], q[40];
rz(0) q[40];
cx q[78], q[40];
rz(0) q[40];
rz(0) q[78];
cx q[78], q[41];
rz(0) q[41];
cx q[78], q[41];
rz(0) q[41];
rz(0) q[78];
cx q[78], q[42];
rz(0) q[42];
cx q[78], q[42];
rz(0) q[42];
rz(0) q[78];
cx q[78], q[43];
rz(0) q[43];
cx q[78], q[43];
rz(0) q[43];
rz(0) q[78];
cx q[78], q[44];
rz(0) q[44];
cx q[78], q[44];
rz(0) q[44];
rz(0) q[78];
cx q[78], q[45];
rz(0) q[45];
cx q[78], q[45];
rz(0) q[45];
rz(0) q[78];
cx q[78], q[46];
rz(0) q[46];
cx q[78], q[46];
rz(0) q[46];
rz(0) q[78];
cx q[78], q[47];
rz(0) q[47];
cx q[78], q[47];
rz(0) q[47];
rz(-pi/2147483648) q[78];
cx q[78], q[48];
rz(pi/2147483648) q[48];
cx q[78], q[48];
rz(-pi/2147483648) q[48];
rz(-pi/1073741824) q[78];
cx q[78], q[49];
rz(pi/1073741824) q[49];
cx q[78], q[49];
rz(-pi/1073741824) q[49];
rz(-pi/536870912) q[78];
cx q[78], q[50];
rz(pi/536870912) q[50];
cx q[78], q[50];
rz(-pi/536870912) q[50];
rz(-pi/268435456) q[78];
cx q[78], q[51];
rz(pi/268435456) q[51];
cx q[78], q[51];
rz(-pi/268435456) q[51];
rz(-pi/134217728) q[78];
cx q[78], q[52];
rz(pi/134217728) q[52];
cx q[78], q[52];
rz(-pi/134217728) q[52];
rz(-pi/67108864) q[78];
cx q[78], q[53];
rz(pi/67108864) q[53];
cx q[78], q[53];
rz(-pi/67108864) q[53];
rz(-pi/33554432) q[78];
cx q[78], q[54];
rz(pi/33554432) q[54];
cx q[78], q[54];
rz(-pi/33554432) q[54];
rz(-pi/16777216) q[78];
cx q[78], q[55];
rz(pi/16777216) q[55];
cx q[78], q[55];
rz(-pi/16777216) q[55];
rz(-pi/8388608) q[78];
cx q[78], q[56];
rz(pi/8388608) q[56];
cx q[78], q[56];
rz(-pi/8388608) q[56];
rz(-pi/4194304) q[78];
cx q[78], q[57];
rz(pi/4194304) q[57];
cx q[78], q[57];
rz(-pi/4194304) q[57];
rz(-pi/2097152) q[78];
cx q[78], q[58];
rz(pi/2097152) q[58];
cx q[78], q[58];
rz(-pi/2097152) q[58];
rz(-pi/1048576) q[78];
cx q[78], q[59];
rz(pi/1048576) q[59];
cx q[78], q[59];
rz(-pi/1048576) q[59];
rz(-pi/524288) q[78];
cx q[78], q[60];
rz(pi/524288) q[60];
cx q[78], q[60];
rz(-pi/524288) q[60];
rz(-pi/262144) q[78];
cx q[78], q[61];
rz(pi/262144) q[61];
cx q[78], q[61];
rz(-pi/262144) q[61];
rz(-pi/131072) q[78];
cx q[78], q[62];
rz(pi/131072) q[62];
cx q[78], q[62];
rz(-pi/131072) q[62];
rz(-pi/65536) q[78];
cx q[78], q[63];
rz(pi/65536) q[63];
cx q[78], q[63];
rz(-pi/65536) q[63];
rz(-pi/32768) q[78];
cx q[78], q[64];
rz(pi/32768) q[64];
cx q[78], q[64];
rz(-pi/32768) q[64];
rz(-pi/16384) q[78];
cx q[78], q[65];
rz(pi/16384) q[65];
cx q[78], q[65];
rz(-pi/16384) q[65];
rz(-pi/8192) q[78];
cx q[78], q[66];
rz(pi/8192) q[66];
cx q[78], q[66];
rz(-pi/8192) q[66];
rz(-pi/4096) q[78];
cx q[78], q[67];
rz(pi/4096) q[67];
cx q[78], q[67];
rz(-pi/4096) q[67];
rz(-pi/2048) q[78];
cx q[78], q[68];
rz(pi/2048) q[68];
cx q[78], q[68];
rz(-pi/2048) q[68];
rz(-pi/1024) q[78];
cx q[78], q[69];
rz(pi/1024) q[69];
cx q[78], q[69];
rz(-pi/1024) q[69];
rz(-pi/512) q[78];
cx q[78], q[70];
rz(pi/512) q[70];
cx q[78], q[70];
rz(-pi/512) q[70];
rz(-pi/256) q[78];
cx q[78], q[71];
rz(pi/256) q[71];
cx q[78], q[71];
rz(-pi/256) q[71];
rz(-pi/128) q[78];
cx q[78], q[72];
rz(pi/128) q[72];
cx q[78], q[72];
rz(-pi/128) q[72];
rz(-pi/64) q[78];
cx q[78], q[73];
rz(pi/64) q[73];
cx q[78], q[73];
rz(-pi/64) q[73];
rz(-pi/32) q[78];
cx q[78], q[74];
rz(pi/32) q[74];
cx q[78], q[74];
rz(-pi/32) q[74];
rz(-pi/16) q[78];
cx q[78], q[75];
rz(pi/16) q[75];
cx q[78], q[75];
rz(-pi/16) q[75];
rz(-pi/8) q[78];
cx q[78], q[76];
rz(pi/8) q[76];
cx q[78], q[76];
rz(-pi/8) q[76];
rz(-pi/4) q[78];
cx q[78], q[77];
rz(pi/4) q[77];
cx q[78], q[77];
rz(-pi/4) q[77];
h q[78];
cx q[79], q[0];
rz(0) q[0];
cx q[79], q[0];
rz(0) q[0];
rz(0) q[79];
cx q[79], q[1];
rz(0) q[1];
cx q[79], q[1];
rz(0) q[1];
rz(0) q[79];
cx q[79], q[2];
rz(0) q[2];
cx q[79], q[2];
rz(0) q[2];
rz(0) q[79];
cx q[79], q[3];
rz(0) q[3];
cx q[79], q[3];
rz(0) q[3];
rz(0) q[79];
cx q[79], q[4];
rz(0) q[4];
cx q[79], q[4];
rz(0) q[4];
rz(0) q[79];
cx q[79], q[5];
rz(0) q[5];
cx q[79], q[5];
rz(0) q[5];
rz(0) q[79];
cx q[79], q[6];
rz(0) q[6];
cx q[79], q[6];
rz(0) q[6];
rz(0) q[79];
cx q[79], q[7];
rz(0) q[7];
cx q[79], q[7];
rz(0) q[7];
rz(0) q[79];
cx q[79], q[8];
rz(0) q[8];
cx q[79], q[8];
rz(0) q[8];
rz(0) q[79];
cx q[79], q[9];
rz(0) q[9];
cx q[79], q[9];
rz(0) q[9];
rz(0) q[79];
cx q[79], q[10];
rz(0) q[10];
cx q[79], q[10];
rz(0) q[10];
rz(0) q[79];
cx q[79], q[11];
rz(0) q[11];
cx q[79], q[11];
rz(0) q[11];
rz(0) q[79];
cx q[79], q[12];
rz(0) q[12];
cx q[79], q[12];
rz(0) q[12];
rz(0) q[79];
cx q[79], q[13];
rz(0) q[13];
cx q[79], q[13];
rz(0) q[13];
rz(0) q[79];
cx q[79], q[14];
rz(0) q[14];
cx q[79], q[14];
rz(0) q[14];
rz(0) q[79];
cx q[79], q[15];
rz(0) q[15];
cx q[79], q[15];
rz(0) q[15];
rz(0) q[79];
cx q[79], q[16];
rz(0) q[16];
cx q[79], q[16];
rz(0) q[16];
rz(0) q[79];
cx q[79], q[17];
rz(0) q[17];
cx q[79], q[17];
rz(0) q[17];
rz(0) q[79];
cx q[79], q[18];
rz(0) q[18];
cx q[79], q[18];
rz(0) q[18];
rz(0) q[79];
cx q[79], q[19];
rz(0) q[19];
cx q[79], q[19];
rz(0) q[19];
rz(0) q[79];
cx q[79], q[20];
rz(0) q[20];
cx q[79], q[20];
rz(0) q[20];
rz(0) q[79];
cx q[79], q[21];
rz(0) q[21];
cx q[79], q[21];
rz(0) q[21];
rz(0) q[79];
cx q[79], q[22];
rz(0) q[22];
cx q[79], q[22];
rz(0) q[22];
rz(0) q[79];
cx q[79], q[23];
rz(0) q[23];
cx q[79], q[23];
rz(0) q[23];
rz(0) q[79];
cx q[79], q[24];
rz(0) q[24];
cx q[79], q[24];
rz(0) q[24];
rz(0) q[79];
cx q[79], q[25];
rz(0) q[25];
cx q[79], q[25];
rz(0) q[25];
rz(0) q[79];
cx q[79], q[26];
rz(0) q[26];
cx q[79], q[26];
rz(0) q[26];
rz(0) q[79];
cx q[79], q[27];
rz(0) q[27];
cx q[79], q[27];
rz(0) q[27];
rz(0) q[79];
cx q[79], q[28];
rz(0) q[28];
cx q[79], q[28];
rz(0) q[28];
rz(0) q[79];
cx q[79], q[29];
rz(0) q[29];
cx q[79], q[29];
rz(0) q[29];
rz(0) q[79];
cx q[79], q[30];
rz(0) q[30];
cx q[79], q[30];
rz(0) q[30];
rz(0) q[79];
cx q[79], q[31];
rz(0) q[31];
cx q[79], q[31];
rz(0) q[31];
rz(0) q[79];
cx q[79], q[32];
rz(0) q[32];
cx q[79], q[32];
rz(0) q[32];
rz(0) q[79];
cx q[79], q[33];
rz(0) q[33];
cx q[79], q[33];
rz(0) q[33];
rz(0) q[79];
cx q[79], q[34];
rz(0) q[34];
cx q[79], q[34];
rz(0) q[34];
rz(0) q[79];
cx q[79], q[35];
rz(0) q[35];
cx q[79], q[35];
rz(0) q[35];
rz(0) q[79];
cx q[79], q[36];
rz(0) q[36];
cx q[79], q[36];
rz(0) q[36];
rz(0) q[79];
cx q[79], q[37];
rz(0) q[37];
cx q[79], q[37];
rz(0) q[37];
rz(0) q[79];
cx q[79], q[38];
rz(0) q[38];
cx q[79], q[38];
rz(0) q[38];
rz(0) q[79];
cx q[79], q[39];
rz(0) q[39];
cx q[79], q[39];
rz(0) q[39];
rz(0) q[79];
cx q[79], q[40];
rz(0) q[40];
cx q[79], q[40];
rz(0) q[40];
rz(0) q[79];
cx q[79], q[41];
rz(0) q[41];
cx q[79], q[41];
rz(0) q[41];
rz(0) q[79];
cx q[79], q[42];
rz(0) q[42];
cx q[79], q[42];
rz(0) q[42];
rz(0) q[79];
cx q[79], q[43];
rz(0) q[43];
cx q[79], q[43];
rz(0) q[43];
rz(0) q[79];
cx q[79], q[44];
rz(0) q[44];
cx q[79], q[44];
rz(0) q[44];
rz(0) q[79];
cx q[79], q[45];
rz(0) q[45];
cx q[79], q[45];
rz(0) q[45];
rz(0) q[79];
cx q[79], q[46];
rz(0) q[46];
cx q[79], q[46];
rz(0) q[46];
rz(0) q[79];
cx q[79], q[47];
rz(0) q[47];
cx q[79], q[47];
rz(0) q[47];
rz(0) q[79];
cx q[79], q[48];
rz(0) q[48];
cx q[79], q[48];
rz(0) q[48];
rz(-pi/2147483648) q[79];
cx q[79], q[49];
rz(pi/2147483648) q[49];
cx q[79], q[49];
rz(-pi/2147483648) q[49];
rz(-pi/1073741824) q[79];
cx q[79], q[50];
rz(pi/1073741824) q[50];
cx q[79], q[50];
rz(-pi/1073741824) q[50];
rz(-pi/536870912) q[79];
cx q[79], q[51];
rz(pi/536870912) q[51];
cx q[79], q[51];
rz(-pi/536870912) q[51];
rz(-pi/268435456) q[79];
cx q[79], q[52];
rz(pi/268435456) q[52];
cx q[79], q[52];
rz(-pi/268435456) q[52];
rz(-pi/134217728) q[79];
cx q[79], q[53];
rz(pi/134217728) q[53];
cx q[79], q[53];
rz(-pi/134217728) q[53];
rz(-pi/67108864) q[79];
cx q[79], q[54];
rz(pi/67108864) q[54];
cx q[79], q[54];
rz(-pi/67108864) q[54];
rz(-pi/33554432) q[79];
cx q[79], q[55];
rz(pi/33554432) q[55];
cx q[79], q[55];
rz(-pi/33554432) q[55];
rz(-pi/16777216) q[79];
cx q[79], q[56];
rz(pi/16777216) q[56];
cx q[79], q[56];
rz(-pi/16777216) q[56];
rz(-pi/8388608) q[79];
cx q[79], q[57];
rz(pi/8388608) q[57];
cx q[79], q[57];
rz(-pi/8388608) q[57];
rz(-pi/4194304) q[79];
cx q[79], q[58];
rz(pi/4194304) q[58];
cx q[79], q[58];
rz(-pi/4194304) q[58];
rz(-pi/2097152) q[79];
cx q[79], q[59];
rz(pi/2097152) q[59];
cx q[79], q[59];
rz(-pi/2097152) q[59];
rz(-pi/1048576) q[79];
cx q[79], q[60];
rz(pi/1048576) q[60];
cx q[79], q[60];
rz(-pi/1048576) q[60];
rz(-pi/524288) q[79];
cx q[79], q[61];
rz(pi/524288) q[61];
cx q[79], q[61];
rz(-pi/524288) q[61];
rz(-pi/262144) q[79];
cx q[79], q[62];
rz(pi/262144) q[62];
cx q[79], q[62];
rz(-pi/262144) q[62];
rz(-pi/131072) q[79];
cx q[79], q[63];
rz(pi/131072) q[63];
cx q[79], q[63];
rz(-pi/131072) q[63];
rz(-pi/65536) q[79];
cx q[79], q[64];
rz(pi/65536) q[64];
cx q[79], q[64];
rz(-pi/65536) q[64];
rz(-pi/32768) q[79];
cx q[79], q[65];
rz(pi/32768) q[65];
cx q[79], q[65];
rz(-pi/32768) q[65];
rz(-pi/16384) q[79];
cx q[79], q[66];
rz(pi/16384) q[66];
cx q[79], q[66];
rz(-pi/16384) q[66];
rz(-pi/8192) q[79];
cx q[79], q[67];
rz(pi/8192) q[67];
cx q[79], q[67];
rz(-pi/8192) q[67];
rz(-pi/4096) q[79];
cx q[79], q[68];
rz(pi/4096) q[68];
cx q[79], q[68];
rz(-pi/4096) q[68];
rz(-pi/2048) q[79];
cx q[79], q[69];
rz(pi/2048) q[69];
cx q[79], q[69];
rz(-pi/2048) q[69];
rz(-pi/1024) q[79];
cx q[79], q[70];
rz(pi/1024) q[70];
cx q[79], q[70];
rz(-pi/1024) q[70];
rz(-pi/512) q[79];
cx q[79], q[71];
rz(pi/512) q[71];
cx q[79], q[71];
rz(-pi/512) q[71];
rz(-pi/256) q[79];
cx q[79], q[72];
rz(pi/256) q[72];
cx q[79], q[72];
rz(-pi/256) q[72];
rz(-pi/128) q[79];
cx q[79], q[73];
rz(pi/128) q[73];
cx q[79], q[73];
rz(-pi/128) q[73];
rz(-pi/64) q[79];
cx q[79], q[74];
rz(pi/64) q[74];
cx q[79], q[74];
rz(-pi/64) q[74];
rz(-pi/32) q[79];
cx q[79], q[75];
rz(pi/32) q[75];
cx q[79], q[75];
rz(-pi/32) q[75];
rz(-pi/16) q[79];
cx q[79], q[76];
rz(pi/16) q[76];
cx q[79], q[76];
rz(-pi/16) q[76];
rz(-pi/8) q[79];
cx q[79], q[77];
rz(pi/8) q[77];
cx q[79], q[77];
rz(-pi/8) q[77];
rz(-pi/4) q[79];
cx q[79], q[78];
rz(pi/4) q[78];
cx q[79], q[78];
rz(-pi/4) q[78];
h q[79];
rz(0) q[80];
cx q[80], q[0];
rz(0) q[0];
cx q[80], q[0];
rz(0) q[0];
rz(0) q[80];
cx q[80], q[1];
rz(0) q[1];
cx q[80], q[1];
rz(0) q[1];
rz(0) q[80];
cx q[80], q[2];
rz(0) q[2];
cx q[80], q[2];
rz(0) q[2];
rz(0) q[80];
cx q[80], q[3];
rz(0) q[3];
cx q[80], q[3];
rz(0) q[3];
rz(0) q[80];
cx q[80], q[4];
rz(0) q[4];
cx q[80], q[4];
rz(0) q[4];
rz(0) q[80];
cx q[80], q[5];
rz(0) q[5];
cx q[80], q[5];
rz(0) q[5];
rz(0) q[80];
cx q[80], q[6];
rz(0) q[6];
cx q[80], q[6];
rz(0) q[6];
rz(0) q[80];
cx q[80], q[7];
rz(0) q[7];
cx q[80], q[7];
rz(0) q[7];
rz(0) q[80];
cx q[80], q[8];
rz(0) q[8];
cx q[80], q[8];
rz(0) q[8];
rz(0) q[80];
cx q[80], q[9];
rz(0) q[9];
cx q[80], q[9];
rz(0) q[9];
rz(0) q[80];
cx q[80], q[10];
rz(0) q[10];
cx q[80], q[10];
rz(0) q[10];
rz(0) q[80];
cx q[80], q[11];
rz(0) q[11];
cx q[80], q[11];
rz(0) q[11];
rz(0) q[80];
cx q[80], q[12];
rz(0) q[12];
cx q[80], q[12];
rz(0) q[12];
rz(0) q[80];
cx q[80], q[13];
rz(0) q[13];
cx q[80], q[13];
rz(0) q[13];
rz(0) q[80];
cx q[80], q[14];
rz(0) q[14];
cx q[80], q[14];
rz(0) q[14];
rz(0) q[80];
cx q[80], q[15];
rz(0) q[15];
cx q[80], q[15];
rz(0) q[15];
rz(0) q[80];
cx q[80], q[16];
rz(0) q[16];
cx q[80], q[16];
rz(0) q[16];
rz(0) q[80];
cx q[80], q[17];
rz(0) q[17];
cx q[80], q[17];
rz(0) q[17];
rz(0) q[80];
cx q[80], q[18];
rz(0) q[18];
cx q[80], q[18];
rz(0) q[18];
rz(0) q[80];
cx q[80], q[19];
rz(0) q[19];
cx q[80], q[19];
rz(0) q[19];
rz(0) q[80];
cx q[80], q[20];
rz(0) q[20];
cx q[80], q[20];
rz(0) q[20];
rz(0) q[80];
cx q[80], q[21];
rz(0) q[21];
cx q[80], q[21];
rz(0) q[21];
rz(0) q[80];
cx q[80], q[22];
rz(0) q[22];
cx q[80], q[22];
rz(0) q[22];
rz(0) q[80];
cx q[80], q[23];
rz(0) q[23];
cx q[80], q[23];
rz(0) q[23];
rz(0) q[80];
cx q[80], q[24];
rz(0) q[24];
cx q[80], q[24];
rz(0) q[24];
rz(0) q[80];
cx q[80], q[25];
rz(0) q[25];
cx q[80], q[25];
rz(0) q[25];
rz(0) q[80];
cx q[80], q[26];
rz(0) q[26];
cx q[80], q[26];
rz(0) q[26];
rz(0) q[80];
cx q[80], q[27];
rz(0) q[27];
cx q[80], q[27];
rz(0) q[27];
rz(0) q[80];
cx q[80], q[28];
rz(0) q[28];
cx q[80], q[28];
rz(0) q[28];
rz(0) q[80];
cx q[80], q[29];
rz(0) q[29];
cx q[80], q[29];
rz(0) q[29];
rz(0) q[80];
cx q[80], q[30];
rz(0) q[30];
cx q[80], q[30];
rz(0) q[30];
rz(0) q[80];
cx q[80], q[31];
rz(0) q[31];
cx q[80], q[31];
rz(0) q[31];
rz(0) q[80];
cx q[80], q[32];
rz(0) q[32];
cx q[80], q[32];
rz(0) q[32];
rz(0) q[80];
cx q[80], q[33];
rz(0) q[33];
cx q[80], q[33];
rz(0) q[33];
rz(0) q[80];
cx q[80], q[34];
rz(0) q[34];
cx q[80], q[34];
rz(0) q[34];
rz(0) q[80];
cx q[80], q[35];
rz(0) q[35];
cx q[80], q[35];
rz(0) q[35];
rz(0) q[80];
cx q[80], q[36];
rz(0) q[36];
cx q[80], q[36];
rz(0) q[36];
rz(0) q[80];
cx q[80], q[37];
rz(0) q[37];
cx q[80], q[37];
rz(0) q[37];
rz(0) q[80];
cx q[80], q[38];
rz(0) q[38];
cx q[80], q[38];
rz(0) q[38];
rz(0) q[80];
cx q[80], q[39];
rz(0) q[39];
cx q[80], q[39];
rz(0) q[39];
rz(0) q[80];
cx q[80], q[40];
rz(0) q[40];
cx q[80], q[40];
rz(0) q[40];
rz(0) q[80];
cx q[80], q[41];
rz(0) q[41];
cx q[80], q[41];
rz(0) q[41];
rz(0) q[80];
cx q[80], q[42];
rz(0) q[42];
cx q[80], q[42];
rz(0) q[42];
rz(0) q[80];
cx q[80], q[43];
rz(0) q[43];
cx q[80], q[43];
rz(0) q[43];
rz(0) q[80];
cx q[80], q[44];
rz(0) q[44];
cx q[80], q[44];
rz(0) q[44];
rz(0) q[80];
cx q[80], q[45];
rz(0) q[45];
cx q[80], q[45];
rz(0) q[45];
rz(0) q[80];
cx q[80], q[46];
rz(0) q[46];
cx q[80], q[46];
rz(0) q[46];
rz(0) q[80];
cx q[80], q[47];
rz(0) q[47];
cx q[80], q[47];
rz(0) q[47];
rz(0) q[80];
cx q[80], q[48];
rz(0) q[48];
cx q[80], q[48];
rz(0) q[48];
rz(0) q[80];
cx q[80], q[49];
rz(0) q[49];
cx q[80], q[49];
rz(0) q[49];
rz(-pi/2147483648) q[80];
cx q[80], q[50];
rz(pi/2147483648) q[50];
cx q[80], q[50];
rz(-pi/2147483648) q[50];
rz(-pi/1073741824) q[80];
cx q[80], q[51];
rz(pi/1073741824) q[51];
cx q[80], q[51];
rz(-pi/1073741824) q[51];
rz(-pi/536870912) q[80];
cx q[80], q[52];
rz(pi/536870912) q[52];
cx q[80], q[52];
rz(-pi/536870912) q[52];
rz(-pi/268435456) q[80];
cx q[80], q[53];
rz(pi/268435456) q[53];
cx q[80], q[53];
rz(-pi/268435456) q[53];
rz(-pi/134217728) q[80];
cx q[80], q[54];
rz(pi/134217728) q[54];
cx q[80], q[54];
rz(-pi/134217728) q[54];
rz(-pi/67108864) q[80];
cx q[80], q[55];
rz(pi/67108864) q[55];
cx q[80], q[55];
rz(-pi/67108864) q[55];
rz(-pi/33554432) q[80];
cx q[80], q[56];
rz(pi/33554432) q[56];
cx q[80], q[56];
rz(-pi/33554432) q[56];
rz(-pi/16777216) q[80];
cx q[80], q[57];
rz(pi/16777216) q[57];
cx q[80], q[57];
rz(-pi/16777216) q[57];
rz(-pi/8388608) q[80];
cx q[80], q[58];
rz(pi/8388608) q[58];
cx q[80], q[58];
rz(-pi/8388608) q[58];
rz(-pi/4194304) q[80];
cx q[80], q[59];
rz(pi/4194304) q[59];
cx q[80], q[59];
rz(-pi/4194304) q[59];
rz(-pi/2097152) q[80];
cx q[80], q[60];
rz(pi/2097152) q[60];
cx q[80], q[60];
rz(-pi/2097152) q[60];
rz(-pi/1048576) q[80];
cx q[80], q[61];
rz(pi/1048576) q[61];
cx q[80], q[61];
rz(-pi/1048576) q[61];
rz(-pi/524288) q[80];
cx q[80], q[62];
rz(pi/524288) q[62];
cx q[80], q[62];
rz(-pi/524288) q[62];
rz(-pi/262144) q[80];
cx q[80], q[63];
rz(pi/262144) q[63];
cx q[80], q[63];
rz(-pi/262144) q[63];
rz(-pi/131072) q[80];
cx q[80], q[64];
rz(pi/131072) q[64];
cx q[80], q[64];
rz(-pi/131072) q[64];
rz(-pi/65536) q[80];
cx q[80], q[65];
rz(pi/65536) q[65];
cx q[80], q[65];
rz(-pi/65536) q[65];
rz(-pi/32768) q[80];
cx q[80], q[66];
rz(pi/32768) q[66];
cx q[80], q[66];
rz(-pi/32768) q[66];
rz(-pi/16384) q[80];
cx q[80], q[67];
rz(pi/16384) q[67];
cx q[80], q[67];
rz(-pi/16384) q[67];
rz(-pi/8192) q[80];
cx q[80], q[68];
rz(pi/8192) q[68];
cx q[80], q[68];
rz(-pi/8192) q[68];
rz(-pi/4096) q[80];
cx q[80], q[69];
rz(pi/4096) q[69];
cx q[80], q[69];
rz(-pi/4096) q[69];
rz(-pi/2048) q[80];
cx q[80], q[70];
rz(pi/2048) q[70];
cx q[80], q[70];
rz(-pi/2048) q[70];
rz(-pi/1024) q[80];
cx q[80], q[71];
rz(pi/1024) q[71];
cx q[80], q[71];
rz(-pi/1024) q[71];
rz(-pi/512) q[80];
cx q[80], q[72];
rz(pi/512) q[72];
cx q[80], q[72];
rz(-pi/512) q[72];
rz(-pi/256) q[80];
cx q[80], q[73];
rz(pi/256) q[73];
cx q[80], q[73];
rz(-pi/256) q[73];
rz(-pi/128) q[80];
cx q[80], q[74];
rz(pi/128) q[74];
cx q[80], q[74];
rz(-pi/128) q[74];
rz(-pi/64) q[80];
cx q[80], q[75];
rz(pi/64) q[75];
cx q[80], q[75];
rz(-pi/64) q[75];
rz(-pi/32) q[80];
cx q[80], q[76];
rz(pi/32) q[76];
cx q[80], q[76];
rz(-pi/32) q[76];
rz(-pi/16) q[80];
cx q[80], q[77];
rz(pi/16) q[77];
cx q[80], q[77];
rz(-pi/16) q[77];
rz(-pi/8) q[80];
cx q[80], q[78];
rz(pi/8) q[78];
cx q[80], q[78];
rz(-pi/8) q[78];
rz(-pi/4) q[80];
cx q[80], q[79];
rz(pi/4) q[79];
cx q[80], q[79];
rz(-pi/4) q[79];
h q[80];
barrier q[0], q[1], q[2], q[3], q[4], q[5], q[6], q[7], q[8], q[9], q[10], q[11], q[12], q[13], q[14], q[15], q[16], q[17], q[18], q[19], q[20], q[21], q[22], q[23], q[24], q[25], q[26], q[27], q[28], q[29], q[30], q[31], q[32], q[33], q[34], q[35], q[36], q[37], q[38], q[39], q[40], q[41], q[42], q[43], q[44], q[45], q[46], q[47], q[48], q[49], q[50], q[51], q[52], q[53], q[54], q[55], q[56], q[57], q[58], q[59], q[60], q[61], q[62], q[63], q[64], q[65], q[66], q[67], q[68], q[69], q[70], q[71], q[72], q[73], q[74], q[75], q[76], q[77], q[78], q[79], q[80], psi[0];
c[0] = measure q[0];
c[1] = measure q[1];
c[2] = measure q[2];
c[3] = measure q[3];
c[4] = measure q[4];
c[5] = measure q[5];
c[6] = measure q[6];
c[7] = measure q[7];
c[8] = measure q[8];
c[9] = measure q[9];
c[10] = measure q[10];
c[11] = measure q[11];
c[12] = measure q[12];
c[13] = measure q[13];
c[14] = measure q[14];
c[15] = measure q[15];
c[16] = measure q[16];
c[17] = measure q[17];
c[18] = measure q[18];
c[19] = measure q[19];
c[20] = measure q[20];
c[21] = measure q[21];
c[22] = measure q[22];
c[23] = measure q[23];
c[24] = measure q[24];
c[25] = measure q[25];
c[26] = measure q[26];
c[27] = measure q[27];
c[28] = measure q[28];
c[29] = measure q[29];
c[30] = measure q[30];
c[31] = measure q[31];
c[32] = measure q[32];
c[33] = measure q[33];
c[34] = measure q[34];
c[35] = measure q[35];
c[36] = measure q[36];
c[37] = measure q[37];
c[38] = measure q[38];
c[39] = measure q[39];
c[40] = measure q[40];
c[41] = measure q[41];
c[42] = measure q[42];
c[43] = measure q[43];
c[44] = measure q[44];
c[45] = measure q[45];
c[46] = measure q[46];
c[47] = measure q[47];
c[48] = measure q[48];
c[49] = measure q[49];
c[50] = measure q[50];
c[51] = measure q[51];
c[52] = measure q[52];
c[53] = measure q[53];
c[54] = measure q[54];
c[55] = measure q[55];
c[56] = measure q[56];
c[57] = measure q[57];
c[58] = measure q[58];
c[59] = measure q[59];
c[60] = measure q[60];
c[61] = measure q[61];
c[62] = measure q[62];
c[63] = measure q[63];
c[64] = measure q[64];
c[65] = measure q[65];
c[66] = measure q[66];
c[67] = measure q[67];
c[68] = measure q[68];
c[69] = measure q[69];
c[70] = measure q[70];
c[71] = measure q[71];
c[72] = measure q[72];
c[73] = measure q[73];
c[74] = measure q[74];
c[75] = measure q[75];
c[76] = measure q[76];
c[77] = measure q[77];
c[78] = measure q[78];
c[79] = measure q[79];
c[80] = measure q[80];
