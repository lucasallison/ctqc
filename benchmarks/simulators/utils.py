...
def exec_subprocess_with_memory_limit(command, mb_mem_limit=1, check_interval=0.1):
    ...
    try:
        ...
        while process.poll() is None:
            try:
                # Get the memory usage of the main process and its children
                ...
                if total_memory_rss > mb_mem_limit * 1024 * 1024:  # Convert MB to bytes
                    ...
                    # Ensure all processes are killed if termination doesn't work
                    for p in all_procs:
                        if p.is_running():
                            p.kill()
